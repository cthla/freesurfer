...
/*--------------------------------------------------*/
int gdfPrintStdout(FSGD *gd) { return gdfPrintHeader(stdout, gd); }
...
static int gdfPrintV1(FILE *fp, FSGD *gd)
{
  ...
  for (n = 0; n < gd->nvarsfromfile; n++)
    fprintf(fp, "# VariableFromFile %s %s %d %d\n", gd->tablefile[n], gd->varfield[n], gd->fieldcol[n], gd->datacol[n]);
...
static FSGD *gdfReadV1(char *gdfname)
{
  ...
  while (1)
  {
    ...
    if (!strcasecmp(tag, "Class"))
    {
      ...
      if (r == 1)
        sscanf(tmpstr, "%s", gd->classmarker[gd->nclasses]);
      if (r == 2)
        sscanf(tmpstr, "%s %s", gd->classmarker[gd->nclasses], gd->classcolor[gd->nclasses]);
      gd->nclasses++;
      continue;
    }
    ...
    if (!strcasecmp(tag, "Variables") || !strcasecmp(tag, "Variable"))
    {
      ...
      for (m = 0; m < r; m++)
      {
        if (fscanf(fp, "%s", gd->varlabel[m]) != 1)
        {
          printf("ERROR while reading file");
          return (NULL);
        }
    ...
    if (!strcasecmp(tag, "VariableFromFile"))
    {
      ...
      if (fscanf(fp, "%d %d", &gd->fieldcol[m], &gd->datacol[m]) != 2)
      {
        printf("ERROR while reading file");
        return (NULL);
      }
    ...
    if (!strcasecmp(tag, "Input"))
    {
      ...
      for (m = 0; m < gd->nvariables; m++)
      {
        if (fscanf(fp, "%f", &gd->varvals[n][m]) != 1)
        {
          printf("ERROR while reading file");
          return (NULL);
        }
    ...
    if (tag[0] != '#')
      fprintf(stderr, "INFO: ignoring tag %s \n", tag);
    if (!fgets(tmpstr, 1000, fp))
    {
      printf("ERROR while reading file");
      return (NULL);
    ...
    for (m = 0; m < gd->FContrastNSub[n]; m++)
    {
      ...
      for (k = 0; k < gd->nContrasts; k++)
      {
        if (!strcmp(gd->FContrastSub[n][m], gd->ContrastName[k]))
        {
          ...
          {
            printf(
                "ERROR: FContrast %s references another FContrast %s\n", gd->ContrastName[n], gd->FContrastSub[n][m]);
            goto formaterror;
          }
          if (ncols == -1)
            ncols = gd->C[k]->cols;
  ...
  else
  {
    ...
    {
      if (gd->DeMean)
        printf("INFO: demeaning continuous variables\n");
      else
        printf("INFO: NOT demeaning continuous variables\n");
    }
  }
...
int gdfClassNo(FSGD *gd, char *class_number)
{
  ...
  for (code = 0; code < gd->nclasses; code++)
    if (!strcmp(gd->classlabel[code], class_number))
      return (code);
  return (-1);
}
...
static int gdfCheckSubjRep(FSGD *gd)
{
  ...
  if (getenv("FSGDF_ALLOW_SUBJ_REP") != NULL)
    return (-1);
...
int gdfVarMeans(FSGD *gd)
{
  ...
  for (vno = 0; vno < gd->nvariables; vno++)
  {
    ...
    if (gd->VarStds[vno] < FLT_MIN)
      gd->VarStds[vno] = FLT_MIN;
  }
...
{
  if (strcmp(gd2mtx_method, "doss") == 0 || strcmp(gd2mtx_method, "dods") == 0 || strcmp(gd2mtx_method, "none") == 0)
    return (0);
...
/*------------------------------------------------------------
  ...
  ------------------------------------------------------------*/
int gdfOffsetSlope(FSGD *gd, int classno, int varno, int c, int r, int s, float *offset, float *slope)
{
  MATRIX *y, *b;
  int n, nf;
  int nslope = 0;

  if (strlen(gd->DesignMatMethod) == 0 || strcmp(gd->DesignMatMethod, "none") == 0)
  {
  ...
  if (c < 0 || c >= gd->data->width || r < 0 || r >= gd->data->height || s < 0 || s >= gd->data->depth)
  {
  ...
  for (n = 0; n < nf; n++)
    y->rptr[n + 1][1] = MRIFseq_vox(gd->data, c, r, s, n);
...
int gdfGetNthVariableLabel(FSGD *gd, int nvariable, char *label)
{
  if (NULL == gd)
    return (-1);
  if (nvariable < 0 || (gd->nvariables != 0 && nvariable >= gd->nvariables))
    return (-1);
...
/*------------------------------------------------------------
  ...
  subject index.
  ------------------------------------------------------------*/
int gdfGetNthSubjectNthValue(FSGD *gd, int nsubject, int nvariable, float *value)
{
  if (NULL == gd)
    return (-1);
  if (nsubject < 0 || nsubject >= gd->ninputs)
...
/*------------------------------------------------------------
  ...
  ------------------------------------------------------------*/
int gdfGetNthSubjectMeasurement(FSGD *gd, int nsubject, int x, int y, int z, float *value)
{
  float v;
  int errs = 0;
...
/*-------------------------------------------------------
  ...
  -------------------------------------------------------*/
FSGD *gdfSubSet(FSGD *infsgd, int nClasses, char **ClassList, int nVars, char **VarList)
{
  FSGD *fsgd;
  ...
  {
    if (nClasses > 0)
      ic = gdfClassNo(infsgd, ClassList[c]);
    else
      ic = c;
  ...
  {
    if (nVars > 0)
      iv = gdfGetVarLabelNo(infsgd, VarList[v]);
    else
      iv = v;
...
/*-------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------*/
char *gdfGetSDataFromTable(char *tablefile, char *field, int fieldcol, int datacol)
{
  FILE *fp;
  int ncols;
  ...
  while (1)
  {
    ...
    if (line[0] == '#')
    {
      if (strcmp(field, "IntraCranialVol") != 0)
        continue;
...
/*-------------------------------------------------------------------------
  ...
  returns 0.
  ------------------------------------------------------------------------*/
int gdfGetDDataFromTable(char *tablefile, char *field, int fieldcol, int datacol, double *data)
{
  char *s;
