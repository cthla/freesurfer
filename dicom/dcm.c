 ...
 */
static CONDITION newElementItem(DCM_ELEMENT *src, CTNBOOLEAN allocateData, PRV_ELEMENT_ITEM **dst);
static CONDITION findCreateGroup(PRIVATE_OBJECT **object, unsigned short group, PRV_GROUP_ITEM **groupPtr);
static CONDITION insertNewElement(PRIVATE_OBJECT **object, DCM_ELEMENT *element);
static CONDITION updateObjectType(PRIVATE_OBJECT **object, DCM_ELEMENT *element);
static CONDITION updateSpecialElements(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *item);
static void exportFixedFields(
    DCM_ELEMENT *element, unsigned char *b, U32 length, int byteOrder, CTNBOOLEAN explicitVR, U32 *rtnLength);
static CONDITION exportData(PRIVATE_OBJECT **object,
...
static CONDITION writeFile(void *buffer, U32 length, int flag, void /* int */ *fd);
static CONDITION countBytes(void *buffer, U32 length, int flag, void /* unsigned long */ *sizePtr);
static CONDITION exportStream(DCM_OBJECT **callerObject,
                              unsigned long opt,
                              void *buffer,
                              U32 bufferlength,
                              CONDITION (*callback)(),
...
static CONDITION verifyFormat(PRV_ELEMENT_ITEM *item);
#if 0
static CONDITION
...
#endif
static CONDITION readFile1(const char *name,
...
static CONDITION copyData(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *item, DCM_ELEMENT *to, U32 *rtnLength);
static CONDITION readLengthToEnd(int fd, const char *fileName, unsigned long opt, U32 *lengthToEnd);
#ifdef LITTLE_ENDIAN_ARCHITECTURE
static void swapATGroupElement(DCM_ELEMENT *e);
#endif
static void dumpBinaryData(void *d, DCM_VALUEREPRESENTATION vr, long vm, long vmLimit);
static void compareGroup(PRV_GROUP_ITEM *g1,
                         PRV_GROUP_ITEM *g2,
...
DCM_OpenFile(const char *name, unsigned long opt, DCM_OBJECT **callerObject)
{
  ...
  if ((opt & (DCM_ORDERMASK | DCM_FILEFORMATMASK)) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION), "Byte order", "DCM_OpenFile");
  ...
  if (fd < 0)
  {
    return COND_PushCondition(DCM_FILEOPENFAILED, DCM_Message(DCM_FILEOPENFAILED), name, "DCM_OpenFile");
  }
  size = fileSize(fd);
  if (size <= 0)
  ...
  {
    cond = readLengthToEnd(fd, name, opt & (~DCM_LENGTHTOENDMASK), &lengthToEnd);
    if (cond != DCM_NORMAL)
    {
      (void)close(fd);
      return COND_PushCondition(DCM_FILEOPENFAILED, DCM_Message(DCM_FILEOPENFAILED), name, "DCM_OpenFile");
...
#ifdef OLDSMM
  cond = readFile(name, NULL, fd, size, 0, 0, opt, callerObject, NULL, &remainFileOpen, NULL, NULL, NULL);
#endif
  cond = readFile1(
      name, NULL, fd, size, &fileOffset, 0, opt, NULL, callerObject, NULL, &remainFileOpen, NULL, NULL, NULL);
...
CONDITION
DCM_ReadStream(DCM_OBJECT **callerObject,
               unsigned long opt,
               long size,
               void *ctx,
               CONDITION (*rd)(void *ctx, void *buf, int toRead, int *bytesRead),
               CONDITION (*sk)(void *ctx, int offset, int flag))
{
  ...
  if ((opt & (DCM_ORDERMASK | DCM_FILEFORMATMASK)) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION), "Byte order", "DCM_ReadStream");

  cond = readFile1("", NULL, fd, size, &fileOffset, 0, opt, NULL, callerObject, NULL, &remainFileOpen, ctx, rd, sk);
  if (cond != DCM_NORMAL)
    return COND_PushCondition(DCM_READSTREAMFAILED, DCM_Message(DCM_READSTREAMFAILED), "DCM_ReadStream");
  else
    return DCM_NORMAL;
}
...
DCM_CreateObject(DCM_OBJECT **object, unsigned long opt)
{
  ...
  *obj;
  ...
  if (object == NULL)
  {
    (void)COND_PushCondition(DCM_NULLADDRESS, DCM_Message(DCM_NULLADDRESS), "DCM_CreateObject");
    return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_CreateObject");
  }
  obj = (PRIVATE_OBJECT *)CTN_MALLOC(sizeof(PRIVATE_OBJECT));
  if (obj == NULL)
  {
    (void)COND_PushCondition(
        DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(PRIVATE_OBJECT), "DCM_CreateObject");
    *object = NULL;
    return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_CreateObject");
  }
  ...
  if (obj->groupList == NULL)
  {
    CTN_FREE(obj);
    *object = NULL;
    return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_CreateObject");
  }
...
DCM_CloseObject(DCM_OBJECT **callerObject)
{
  ...
  *sequenceItem;
  ...
  while ((group = LST_Pop(&(*object)->groupList)) != NULL)
  {
    ...
    if (cond != LST_NORMAL)
      return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_CloseObject");
    CTN_FREE(group);
  }
...
DCM_AddElement(DCM_OBJECT **callerObject, DCM_ELEMENT *element)
{
  ...
  *groupItem;
  ...
  {
    if (localElement.representation == DCM_OT || localElement.representation == DCM_CTX)
      localElement.representation = element->representation;
    if (element->representation != DCM_UN && element->representation != localElement.representation)
    {
  ...
  *elementItem, *groupLengthItem;
  ...
  if (groupItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), group, element, "DCM_RemoveElement");
  ...
  if (elementItem->element.representation == DCM_OW || elementItem->element.representation == DCM_OB
...
CONDITION
DCM_GetElementValue(DCM_OBJECT **callerObject, DCM_ELEMENT *element, U32 *rtnLength, void **ctx)
{
  PRIVATE_OBJECT
  **object;
  PRV_GROUP_ITEM
  ...
  while (elementItem != NULL)
  {
    if (elementItem->element.tag == element->tag)
    {
      ...
      if (element->representation == DCM_SQ)
        return COND_PushCondition(
            DCM_CANNOTGETSEQUENCEVALUE, DCM_Message(DCM_CANNOTGETSEQUENCEVALUE), element->tag, "DCM_GetElementValue");

      p = *ctx;
      if ((long)p > elementItem->element.length)
        return COND_PushCondition(DCM_ILLEGALCONTEXT, DCM_Message(DCM_ILLEGALCONTEXT), "DCM_GetElementValue");
      ...
      *rtnLength = l;
      {
        if (elementItem->element.d.ot == NULL)
        {
          if ((*object)->fd != -1)
          {
            (void)lseek((*object)->fd, elementItem->dataOffset + (long)p, SEEK_SET);
            nBytes = read((*object)->fd, element->d.ot, (int)l);
          }
          else
          {
            (*object)->sk((*object)->userCtx, (long)(elementItem->dataOffset + (long)p), SEEK_SET);
            cond = (*object)->rd((*object)->userCtx, element->d.ot, l, &nBytes);
          }
          if ((unsigned)nBytes != l)
          {
            return COND_PushCondition(
                DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "DCM_GetElementValue");
          }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
        ...
        {
          unsigned char *q;
          q = (unsigned char *)elementItem->element.d.ot + (long)p;
...
CONDITION
DCM_GetElementValueOffset(DCM_OBJECT **callerObject, DCM_ELEMENT *element, unsigned long offset)
{
  PRIVATE_OBJECT **object;
...

  {
    ...
    }
    {
      if (elementItem->element.d.ot == NULL)
      {
        ...
        {
          return COND_PushCondition(
              DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "DCM_GetElementValueValue");
        }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
        if (elementItem->element.representation == DCM_AT)
        {
...
CONDITION
DCM_GetElementSize(DCM_OBJECT **callerObject, DCM_TAG tag, U32 *rtnLength)
{
  PRIVATE_OBJECT
  **object;
  PRV_GROUP_ITEM
  ...
  *elementItem;
  ...
  if (groupItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), group, element, "DCM_GetElementSize");
...
CONDITION
DCM_ScanParseObject(DCM_OBJECT **callerObject,
                    void *buf,
                    size_t bufferSize,
...
CONDITION
DCM_ImportStream(unsigned char *buf, unsigned long length, unsigned long opt, DCM_OBJECT **callerObject)
{
#ifdef DEBUG
  if (debug)
  ...
  if ((opt & DCM_ORDERMASK) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION), "Byte order", "DCM_ImportStream");

  return readFile1("", buf, -1, length, 0, 0, opt, NULL, callerObject, NULL, NULL, NULL, NULL, NULL);
}
...
CONDITION
DCM_ExportStream(DCM_OBJECT **callerObject,
                 unsigned long opt,
                 void *buffer,
                 ...
                 void *ctx)
{
  ...
  **object;
  ...
  return exportStream(callerObject, opt, buffer, bufferlength, callback, ctx, 0);
}
...
DCM_DumpElements(DCM_OBJECT **callerObject, long vm)
{
  ...
  *sq;
  ...
  while (groupItem != NULL)
  {
    printf("Group: %04x, Length: %8ld\n", groupItem->group, groupItem->baseLength);
    elementItem = LST_Head(&groupItem->elementList);
    if (elementItem != NULL)
      (void)LST_Position(&groupItem->elementList, elementItem);
    while (elementItem != NULL)
    {
      ...
      else
      {
        ...
        case DCM_UT:
          ...
          break;
        case DCM_SL:
          (void)printf("%8x %d\n", *elementItem->element.d.sl, *elementItem->element.d.sl);
          if (vm > 1)
            dumpBinaryData(elementItem->element.d.ot,
                           elementItem->element.representation,
                           elementItem->element.length / sizeof(U32),
                           vm);
          break;
        case DCM_SS:
          (void)printf("%4x %d\n", *elementItem->element.d.ss, *elementItem->element.d.ss);
          if (vm > 1)
            dumpBinaryData(elementItem->element.d.ot,
                           elementItem->element.representation,
                           elementItem->element.length / sizeof(short),
                           vm);
          break;
        case DCM_SQ:
        ...
        case DCM_AT:
        case DCM_UL:
          (void)printf("%8x %d\n", *elementItem->element.d.ul, *elementItem->element.d.ul);
          if (vm > 1)
            dumpBinaryData(elementItem->element.d.ot,
                           elementItem->element.representation,
                           elementItem->element.length / sizeof(U32),
          ...
          break;
        case DCM_US:
          (void)printf("%4x %d\n", *elementItem->element.d.us, *elementItem->element.d.us);
          if (vm > 1)
            dumpBinaryData(elementItem->element.d.ot,
                           elementItem->element.representation,
                           elementItem->element.length / sizeof(unsigned short),
                           vm);
          break;
        case DCM_OB:
        case DCM_UN:
          dumpBinaryData(
              elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length, 8);
          break;
...
DCM_FormatElements(DCM_OBJECT **callerObject, long vm, const char *prefix)
{
  ...
  *sq;
  ...
  while (groupItem != NULL)
  {
    printf("%sGroup: %04x, Length: %8lu\n", prefix, groupItem->group, groupItem->baseLength);
    elementItem = LST_Head(&groupItem->elementList);
    if (elementItem != NULL)
      (void)LST_Position(&groupItem->elementList, elementItem);
    while (elementItem != NULL)
    {
      ...
      else
      {
        ...
        case DCM_UT:
          ...
          break;
        case DCM_SL:
          (void)printf("%8x %d\n", (unsigned int)*elementItem->element.d.sl, *elementItem->element.d.sl);
          if (vm > 1)
            dumpBinaryData(elementItem->element.d.ot,
                           elementItem->element.representation,
                           elementItem->element.length / sizeof(U32),
        ...
        case DCM_AT:
        case DCM_UL:
          (void)printf("%8x %u\n", (unsigned int)*elementItem->element.d.ul, *elementItem->element.d.ul);
          if (vm > 1)
            dumpBinaryData(elementItem->element.d.ot,
                           elementItem->element.representation,
                           elementItem->element.length / sizeof(U32),
...
void DCM_Debug(CTNBOOLEAN flag) { debug = flag; }
...
DCM_WriteFile(DCM_OBJECT **callerObject, unsigned long opt, const char *file)
{
  ...
  **object;
...
#elif _MSC_VER
  fd = _open(file, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, _S_IREAD | _S_IWRITE);
#else
  ...
  {
    return COND_PushCondition(
        DCM_FILECREATEFAILED, DCM_Message(DCM_FILECREATEFAILED), file, strerror(errno), "DCM_WriteFile");
  }
  cond = DCM_ExportStream(callerObject, opt, buf, (unsigned long)sizeof(buf), writeFile, &fd);
  if (cond != DCM_NORMAL)
    return cond;
...
*/
CONDITION
DCM_ParseObject(DCM_OBJECT **callerObject,
                DCM_ELEMENT *vector,
                int count,
                DCM_FLAGGED_ELEMENT *flaggedVector,
                int flagCount,
...
DCM_RemoveGroup(DCM_OBJECT **callerObject, unsigned short group)
{
  ...
  *elementItem;
  ...
  groupItem = LST_Head(&(*object)->groupList);
  if (groupItem == NULL)
    return COND_PushCondition(DCM_GROUPNOTFOUND, DCM_Message(DCM_GROUPNOTFOUND), (int)group, "DCM_RemoveGroup");
  ...
  if (cond != LST_NORMAL)
    return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_RemoveGroup");
  CTN_FREE(groupItem);
  return DCM_NORMAL;
}
...
DCM_GetSequenceElement(DCM_OBJECT **object, DCM_TAG top, DCM_ELEMENT *e)
{
  ...
  {
    return COND_PushCondition(
        DCM_UNEXPECTEDREPRESENTATION, DCM_Message(DCM_UNEXPECTEDREPRESENTATION), "DCM_GetSequenceElement", "sequence");
  }
...
*/
CONDITION
DCM_GetElementValueList(DCM_OBJECT **object, DCM_TAG tag, size_t structureSize, long stringOffset, LST_HEAD **list)
{
  PRIVATE_OBJECT
  **obj;
  CONDITION
  ...
  *elementItem;
  ...
  {
    return COND_PushCondition(
        DCM_UNEXPECTEDREPRESENTATION, DCM_Message(DCM_UNEXPECTEDREPRESENTATION), "DCM_GetElementValueList", "string");
  }
  ...
  while (l > 0)
  {
    ...
    if (l != 0)
    {
      ...
      if (p == NULL)
        return COND_PushCondition(
            DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), structureSize, "DCM_GetElementValueList");
      dst = p + stringOffset;
      ...
      *dst = '\0';
      ...
      if (cond != LST_NORMAL)
        return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_GetElementValueList");
    }
  }
  return DCM_NORMAL;
}
...
*/
CONDITION
DCM_AddElementList(DCM_OBJECT **callerObject, DCM_ELEMENT *element, LST_HEAD *list, long offset)
{
  DCM_ELEMENT
...
CONDITION
DCM_ComputeExportLength(DCM_OBJECT **callerObject, unsigned long opt, unsigned long *length)
{
  PRIVATE_OBJECT
  **object;
  unsigned char buf[2048];
  ...
  *length = 0;
  cond = DCM_ExportStream(callerObject, opt, buf, (unsigned long)sizeof(buf), countBytes, length);
  if (cond != DCM_NORMAL)
    return cond;
...
CONDITION
DCM_CompareAttributes(DCM_OBJECT **o1,
                      DCM_OBJECT **o2,
                      void (*callback)(const DCM_ELEMENT *e1, const DCM_ELEMENT *e2, void *ctx),
                      void *ctx)
{
  PRIVATE_OBJECT **object1, **object2;
  PRV_GROUP_ITEM *groupItem1, *groupItem2;
  CONDITION cond;
...
static CONDITION newElementItem(DCM_ELEMENT *src, CTNBOOLEAN allocateData, PRV_ELEMENT_ITEM **dst)
{
  ...
  if (debug)
  {
    fprintf(stderr, "newElementItem: CTN_MALLOC %8d %8d ", (int)l, (int)(sizeof(PRV_ELEMENT_ITEM) + l));
  }
...
static CONDITION findCreateGroup(PRIVATE_OBJECT **object, unsigned short group, PRV_GROUP_ITEM **groupItem)
{
  PRV_GROUP_ITEM
  *item;
  CONDITION
...

  {
    ...
    *groupItem = newGroupItem;
    ...
    if (newGroupItem->elementList == NULL)
      return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "findCreateGroup");
    ...
    if ((*object)->groupLengthFlag)
    {
      ...
      if (LST_Insert(&newGroupItem->elementList, groupLengthItem, LST_K_AFTER) != LST_NORMAL)
        return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "findCreateGroup");
...
static CONDITION insertNewElement(PRIVATE_OBJECT **object, DCM_ELEMENT *element)
{
  ...
  *groupItem;
  ...
  if (groupItem == NULL)
    return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "insertNewElement");

  if (groupItem->baseLength != DCM_UNSPECIFIEDLENGTH)
    groupItem->baseLength += 2 + 2 + 4 + newItem->paddedDataLength;

  if (newItem->element.representation == DCM_OW || newItem->element.representation == DCM_OB
  ...
  if (DCM_TAG_ELEMENT(nextItem->element.tag) == 0x0000)
    (void)memcpy(nextItem->element.d.ot, &groupItem->baseLength, sizeof(groupItem->baseLength));
  ...
  */
  ...
  while (nextItem != NULL)
  {
    if (DCM_TAG_GROUP(element->tag) != DCM_TAG_GROUP(nextItem->element.tag))
    {
    ...
    else if (DCM_TAG_ELEMENT(element->tag) < DCM_TAG_ELEMENT(nextItem->element.tag))
    {
...
static CONDITION insertThisElementItem(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *newItem)
{
  ...
  cond = findCreateGroup(object, DCM_TAG_GROUP(newItem->element.tag), &groupItem);

  if (groupItem == NULL)
    return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "insertThisElementItem");
  ...
  */
  ...
  while (nextItem != NULL)
  {
    if (DCM_TAG_GROUP(newItem->element.tag) != DCM_TAG_GROUP(nextItem->element.tag))
    {
      return COND_PushCondition(DCM_BADELEMENTINGROUP,
                                DCM_Message(DCM_BADELEMENTINGROUP),
                                DCM_TAG_GROUP(nextItem->element.tag),
    ...
    else if (DCM_TAG_ELEMENT(newItem->element.tag) < DCM_TAG_ELEMENT(nextItem->element.tag))
    {
      cond = LST_Insert(&groupItem->elementList, newItem, LST_K_BEFORE);
      if (cond != LST_NORMAL)
        return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "insertThisElementItem");
...
// printf("(*object)->metaHeaderLength: %d",(*object)->metaHeaderLength);
  ...
  case DCM_METATRANSFERSYNTAX:
    ...
    { /* Must be an encapsulated transfer syntax */
      (*object)->dataOptions = DCM_EXPLICITLITTLEENDIAN;
    }
    break;
  case DCM_MAKETAG(0x003a, 0x0103):
    strncpy((*object)->waveformDataVR, item->element.d.string, item->element.length);
...
static VRMAP *lookupVRCode(const char *code)
{
  int i;
...
static void mapVRtoASCII(DCM_VALUEREPRESENTATION vr, char *s)
{
  int i;
...
static void exportVRLength(DCM_ELEMENT *e, unsigned char *b, int byteOrder, U32 *rtnLength)
{
  int i;
  char *c = "xx";
  unsigned char *p;
...
static CONDITION exportPreamble(PRIVATE_OBJECT **obj, unsigned char *dst, U32 bufferLength, U32 *rtnLength)
{
  *rtnLength = 0;
  if (bufferLength < (DCM_PREAMBLELENGTH + 4))
    return COND_PushCondition(
        DCM_EXPORTBUFFERTOOSMALL, DCM_Message(DCM_EXPORTBUFFERTOOSMALL), (int)bufferLength, "exportPreamble");
...
static void
exportFixedFields(DCM_ELEMENT *e, unsigned char *b, U32 length, int byteOrder, CTNBOOLEAN explicitVR, U32 *rtnLength)
{
  unsigned char *p;
  unsigned short group, element;
  U32 minimumLength;
  ...
  *rtnLength = 0;
  if (element->d.ot == NULL)
  {
    ...
    else
    {
      ...
      // cond =
      (*object)->rd((*object)->userCtx, b, (long)length, &nBytes);
    }
    if ((U32)nBytes != length)
    {
      char b[512];
      sprintf(b, "byte count: %d %d, errno: %d", nBytes, length, errno);
      (void)COND_PushCondition(DCM_GENERALWARNING, DCM_Message(DCM_GENERALWARNING), "exportData", b);
      return COND_PushCondition(
          DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "exportData");
    }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
...
static CONDITION exportEncapsulatedPixels(PRIVATE_OBJECT **object,
                                          PRV_ELEMENT_ITEM *item,
                                          ...
                                          void *ctx)
{
  ...
  if (element->d.ot == NULL)
  {
    ...
    while (toExport > 0)
    {
      ...
      if ((U32)nBytes != length)
      {
        char b[512];
        sprintf(b, "byte count: %d %d, errno: %d", nBytes, length, errno);
        (void)COND_PushCondition(DCM_GENERALWARNING, DCM_Message(DCM_GENERALWARNING), "exportEncapsualtedPixels", b);
        return COND_PushCondition(
            DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "exportEncapsualtedPixels");
      }
      cond = callback(buffer, length, 0, ctx);
      if (cond != DCM_NORMAL)
      {
        return COND_PushCondition(DCM_CALLBACKABORTED, DCM_Message(DCM_CALLBACKABORTED), "exportStream");
      }
      toExport -= length;
    }
  }
  else
  {
    ...
    e.length = 0xffffffff;
    exportFixedFields(&e, buffer, bufferlength, LITTLE_ORDER /*byteOrder*/, 1 /* explicitV*/, &rtnLength);
    toExport = rtnLength;
    e.tag = 0xfffee000;
    e.length = 0;
    e.representation = DCM_DLM;
    e.d.ot = 0;
    exportFixedFields(&e, buffer + toExport, bufferlength, LITTLE_ORDER /*byteOrder*/, 1 /* explicitV*/, &rtnLength);
    toExport += rtnLength;

    cond = callback(buffer, toExport, 0, ctx);
    if (cond != DCM_NORMAL)
    {
      return COND_PushCondition(DCM_CALLBACKABORTED, DCM_Message(DCM_CALLBACKABORTED), "exportEncapsulatedPixels");
    }
...
static CONDITION exportPixels(PRIVATE_OBJECT **object,
                              PRV_ELEMENT_ITEM *item,
                              ...
                              int explicitVR)
{
  ...
  {
    return exportEncapsulatedPixels(object, item, buffer, bufferlength, callback, ctx);
  }
  ...
  dst = buffer;
  c = bufferlength;
  exportFixedFields(element, dst, bufferlength, byteOrder, explicitVR, &rtnLength);
  dst += rtnLength;
  c -= rtnLength;
  ...
  while (remainingData > 0)
  {
    ...
    if (element->d.ot != NULL)
    {
      remainingData = element->length - (src - ((unsigned char *)element->d.ot));
    }
    else
    {
      remainingData = element->length - (item->currentOffset - item->dataOffset);
    }

    exportLength = (remainingData < c) ? remainingData : c;
    cond = exportData(object, item, src, dst, exportLength, byteOrder, &rtnLength);
    if (cond != DCM_NORMAL)
      return cond;
    ...
    if (c <= 20)
    {
      ...
      if (cond != DCM_NORMAL)
      {
        return COND_PushCondition(DCM_CALLBACKABORTED, DCM_Message(DCM_CALLBACKABORTED), "exportPixels");
      }
      bytesExported = 0;
      c = bufferlength;
...
static void swapInPlace(PRIVATE_OBJECT **object, DCM_ELEMENT *e)
{
  ...
  length = e->length;
  p1 = e->d.ot;
  if (e->representation == DCM_US || e->representation == DCM_SS || e->representation == DCM_OW
      || e->representation == DCM_AT)
  {
    if (e->tag == DCM_PXLPIXELDATA && (*object)->pixelBitsAllocated != 16)
      return;
...
static CONDITION writeFile(void *buffer, U32 length, int flag, void /* int */ *fdPtr)
{
  int bytesWritten;
  int *fd;
  ...
  if (bytesWritten != (int)length)
    return COND_PushCondition(DCM_FILEIOERROR, DCM_Message(DCM_FILEIOERROR), "", strerror(errno), "writeFile");
  else
    return DCM_NORMAL;
}
...
static CONDITION setFileOptions(DCM_OBJECT **obj, unsigned long *opt)
{
  CONDITION cond;
  char xferSyntax[DICOM_UI_LENGTH + 1];
  DCM_ELEMENT e = { DCM_METATRANSFERSYNTAX, DCM_UI, "", 1, sizeof(xferSyntax), { NULL } };
  ...
  *opt = 0;
  ...
  { /* Must be an encapsulated xfer syntax */
...
static CONDITION extractFileOptions(
    unsigned long opt, CTNBOOLEAN *part10File, CTNBOOLEAN *explicitVR, int *byteOrder, CTNBOOLEAN *encapsulatedPixels)
{
  *part10File = *explicitVR = FALSE;
  ...
  if ((opt & DCM_ORDERMASK) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION), "Byte order", "extractFileOptions");
...
static U32 computeGroupLength(PRV_GROUP_ITEM *groupItem, CTNBOOLEAN explicitVR)
{
  return (explicitVR) ? groupItem->baseLength + 4 * groupItem->longVRAttributes : groupItem->baseLength;
}
                              ...
                              int sequenceLevel)
{
  ...
  *elementItem;
  ...
  U32 c, bytesExported = 0, rtnLength, remainingData, exportLength;
  CONDITION
  cond;
  DCM_SEQUENCE_ITEM
  *sequenceItem;
  DCM_ELEMENT
  itemMarker = { DCM_DLMITEM, DCM_DLM, "", 1, DCM_UNSPECIFIEDLENGTH, { NULL } },
  itemDelimiter = { DCM_DLMITEMDELIMITATIONITEM, DCM_DLM, "", 1, 0, { NULL } },
  sequenceDelimiter = { DCM_DLMSEQUENCEDELIMITATIONITEM, DCM_DLM, "", 1, 0, { NULL } };
  CTNBOOLEAN
  unspecifiedSQLength = FALSE, explicitVR = FALSE, part10File = FALSE, encapsulatedPixels = FALSE;
  ...
  if ((opt & DCM_ORDERMASK) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION), "Byte order", "exportStream");
  ...
  if (sequenceLevel != 0)
  {
    ...
    exportFixedFields(&itemMarker, dst, bufferlength, byteOrder, explicitVR, &rtnLength);
    dst += rtnLength;
    c -= rtnLength;
    bytesExported += rtnLength;
  }
...
/*  Take this code out to allow empty groups. */
  ...
  while (groupItem != NULL)
  {
    if (part10File && groupItem->group != DCM_GROUPFILEMETA)
    {
      ...
      {
        opt = fileOptions;
        cond = extractFileOptions(opt, &part10File, &explicitVR, &byteOrder, &encapsulatedPixels);
        if (cond != DCM_NORMAL)
          return cond;
      }
    }
    ...
    while (elementItem != NULL)
    {
      ...
      if (element.tag == DCM_PXLPIXELDATA)
      {
        ...
        cond = exportPixels(
            object, elementItem, encapsulatedPixels, buffer, bufferlength, callback, ctx, byteOrder, explicitVR);
        if (cond != DCM_NORMAL)
          return cond;
      ...
      else if (element.representation == DCM_SQ)
      {
        ...
        if (element.d.sq != NULL)
        {
          ...
          {
            cond = exportStream(&sequenceItem->object, opt, buffer, bufferlength, callback, ctx, sequenceLevel + 1);
            if (cond != DCM_NORMAL)
              return cond;
        ...
        if (element.length == DCM_UNSPECIFIEDLENGTH)
        {
          ...
          exportFixedFields(&sequenceDelimiter, dst, bufferlength, byteOrder, explicitVR, &rtnLength);
          dst += rtnLength;
          c -= rtnLength;
          bytesExported += rtnLength;
        }
      ...
      else
      {
        while (remainingData > 0)
        {
          ...
          if (element.d.ot != NULL)
            ...
            remainingData = element.length - (elementItem->currentOffset - elementItem->dataOffset);

          exportLength = (remainingData < c) ? remainingData : c;
          cond = exportData(object, elementItem, src, dst, exportLength, byteOrder, &rtnLength);
          if (cond != DCM_NORMAL)
            return cond;
  ...
  if ((sequenceLevel != 0) && unspecifiedSQLength)
  {
    ...
    exportFixedFields(&itemDelimiter, dst, bufferlength, byteOrder, explicitVR, &rtnLength);
    dst += rtnLength;
    c -= rtnLength;
    bytesExported += rtnLength;
  }
...
static CONDITION readPreamble(const char *name,
                              unsigned char **ptr,
                              int fd,
                              U32 *size,
                              off_t *fileOffset,
                              ...
                              U32 *scannedLength)
{
  ...
  if ((*size < DCM_PREAMBLELENGTH + 4) && knownLength)
  {
    ...
    return COND_PushCondition(DCM_ILLEGALSTREAMLENGTH, DCM_Message(DCM_ILLEGALSTREAMLENGTH), *size, "readPreamble");
  }
  if (*ptr == NULL)
  {
    if (fd != -1)
    ...
    {
      // cond =
      (*object)->rd((*object)->userCtx, (*object)->preamble, DCM_PREAMBLELENGTH, &nBytes);
      // cond =
      (*object)->rd((*object)->userCtx, label, sizeof(label), &tmp);
      nBytes += tmp;
    }

    if (nBytes != DCM_PREAMBLELENGTH + sizeof(label))
      return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), name, "readPreamble");
  }
  else
  {
...
static CONDITION readGroupElement(const char *name,
                                  unsigned char **ptr,
                                  int fd,
                                  U32 *size,
                                  off_t *fileOffset,
                                  ...
                                  DCM_ELEMENT *e)
{
  ...
  if ((*size < 4) && knownLength)
  {
    if (debug)
      (void)DCM_DumpElements((DCM_OBJECT **)object, 0);
    (void)DCM_CloseObject((DCM_OBJECT **)object);
    return COND_PushCondition(DCM_ILLEGALSTREAMLENGTH, DCM_Message(DCM_ILLEGALSTREAMLENGTH), *size, "readFile");
  }
  if (*ptr == NULL)
  {
    if (fd != -1)
    ...
    if (nBytes != 4)
    {
      ...
      return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), name, "readGroupElement");
    }
    localPtr = buf;
  }
  else
...
static CONDITION readVRLength(const char *name,
                              unsigned char **ptr,
                              int fd,
                              U32 *size,
                              off_t *fileOffset,
                              ...
                              DCM_ELEMENT *e)
{
  ...
  if ((*size < 4) && knownLength)
  {
    if (debug)
      (void)DCM_DumpElements((DCM_OBJECT **)object, 0);
    (void)DCM_CloseObject((DCM_OBJECT **)object);
    return COND_PushCondition(DCM_ILLEGALSTREAMLENGTH, DCM_Message(DCM_ILLEGALSTREAMLENGTH), *size, "readVRLength");
  }
  if (*ptr == NULL)
  {
    if (fd != -1)
    ...
    {
      // cond =
      (*object)->rd((*object)->userCtx, buf, 4, &nBytes);
    }

    if (nBytes != 4)
      return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), name, "readVRLength");
    localPtr = buf;
  }
  else
  {
  ...
  if (explicitVR)
  {
    ...
    if (vrPtr == NULL)
      return COND_PushCondition(DCM_UNRECOGNIZEDVRCODE, DCM_Message(DCM_UNRECOGNIZEDVRCODE), vrCode, "readVRLength");
    ...
    if (vrPtr->representation != e->representation)
    {
      ...
      else if (e->representation == DCM_UN || e->representation == DCM_CTX || e->representation == DCM_RET
      ...
      else
      {
        if (e->tag != DCM_PXLPIXELDATA)
          return COND_PushCondition(DCM_VRMISMATCH, DCM_Message(DCM_VRMISMATCH), vrCode, e->tag);
      }
    }
    if (vrPtr->representation != DCM_OW && vrPtr->representation != DCM_OB && vrPtr->representation != DCM_UN
  ...
  if (debug)
  {
    ...
    fprintf(stderr, "%2s %6d %06x %s\n", localVR, e->length, (unsigned int)*fileOffset, e->description);
  }
  if (((e->length & 1) != 0) && (e->length != DCM_UNSPECIFIEDLENGTH))
  {
    if (debug)
...
static CONDITION readSequence(const char *name,
                              unsigned char **ptr,
                              int fd,
                              U32 *size,
                              off_t *fileOffset,
                              ...
                              PRV_ELEMENT_ITEM **elementItem)
{
  ...
  *sequenceObject;
  ...
  (*elementItem)->element.d.sq = LST_Create();
  if ((*elementItem)->element.d.sq == NULL)
    return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "readSequence");
  ...
  {
    if (debug)
      fprintf(stderr, "Sequence Length: %d %x\n", localLength, (unsigned int)localLength);
    ...
    if (debug)
      fprintf(stderr,
              "Sequence item: %4x %4x %d (%x)\n",
              DCM_TAG_GROUP(tagE.tag),
              DCM_TAG_ELEMENT(tagE.tag),
              tagE.length,
    ...
    if (tagE.tag == DCM_DLMITEM)
    {
      ...
      if (cond == DCM_NORMAL)
      {
        ...
        if (sequenceItem == NULL)
          return COND_PushCondition(
              DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(*sequenceItem), "readFile");

        ((PRIVATE_OBJECT *)sequenceObject)->offset = itemTagOffset;
        sequenceItem->object = sequenceObject;
        cond = LST_Enqueue(&(*elementItem)->element.d.sq, sequenceItem);
        if (cond != LST_NORMAL)
          return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "readFile");
        if (*size != (long)DCM_UNSPECIFIEDLENGTH)
          *size -= sequenceLength;
        if (scannedLength != NULL)
          *scannedLength += sequenceLength;
...
static CONDITION scanCompressedPixels(char *name,
                                      unsigned char **ptr,
                                      int fd,
                                      U32 *size,
                                      off_t *fileOffset,
...
static CONDITION readData(const char *name,
                          unsigned char **ptr,
                          int fd,
                          U32 *size,
                          off_t *fileOffset,
                          ...
                          PRV_ELEMENT_ITEM **elementItem)
{
  ...
  if (pixelFlag && fileFlag)
  {
    ...
    *remainOpenFlag = TRUE;
    ...
    if (fileFlag)
    {
      ...
      {
        (*object)->sk((*object)->userCtx, (*elementItem)->element.length, SEEK_CUR);
      }
      (*object)->fd = fd;
    }
  }
  else
  {
    if (fileFlag)
    {
      if (fd != -1)
      {
        nBytes = read(fd, (*elementItem)->element.d.ot, (int)(*elementItem)->element.length);
      }
      else
      {
        cond = (*object)->rd(
            (*object)->userCtx, (*elementItem)->element.d.ot, (long)(*elementItem)->element.length, &nBytes);
      }
      if (nBytes != (int)(*elementItem)->element.length)
      {
        (void)DCM_CloseObject((DCM_OBJECT **)object);
        return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), name, "readFile");
      }
    }
    else
    {
      (void)memcpy((*elementItem)->element.d.ot, *ptr, (*elementItem)->element.length);
      *ptr += (*elementItem)->originalDataLength;
    }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
    if ((*elementItem)->element.representation == DCM_AT)
  ...
  if (((*elementItem)->paddedDataLength != DCM_UNSPECIFIEDLENGTH) && ((*elementItem)->paddedDataLength & 1))
    (*elementItem)->paddedDataLength += 1;
  (*object)->objectSize += (*elementItem)->paddedDataLength;
...
static CONDITION checkAttributeOrder(DCM_ELEMENT *e, long *lastGroup, long *lastElement, CTNBOOLEAN allowRepeatElements)
{
  ...
  if ((long)group == *lastGroup)
  {
    ...
    if ((long)element <= *lastElement)
      return COND_PushCondition(
          DCM_ELEMENTOUTOFORDER, DCM_Message(DCM_ELEMENTOUTOFORDER), group, element, "checkAttributeOrder");
  }
...
static CONDITION handleGroupItem(PRIVATE_OBJECT **obj, PRV_GROUP_ITEM **groupItem, unsigned short group)
{
  ...
  if (createGroupFlag == TRUE)
  {
    *groupItem = CTN_MALLOC(sizeof(**groupItem));
    ...
    {
      (void)DCM_CloseObject((DCM_OBJECT **)obj);
      return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "handleGroupItem");
    }
                           ...
                           CONDITION (*sk)(void *ctx, int offset, int flag))
{
  ...
  DCM_ELEMENT e;
  CTNBOOLEAN convertFlag = FALSE, done = FALSE, knownLength = TRUE, explicitVR = FALSE, acceptVRMismatch = FALSE,
             part10Flag = FALSE;
  ...
  {
    flag = readPreamble(name, &ptr, fd, &size, fileOffset, knownLength, object, scannedLength);
    if (flag != DCM_NORMAL)
      goto abort;
  }
  while (!done)
  {
    ...
    if ((e.representation == DCM_UN) && (e.length == DCM_UNSPECIFIEDLENGTH))
    {
      e.representation = DCM_SQ;
    }
#ifndef SMM
    if ((e.tag == DCM_DLMITEMDELIMITATIONITEM) || (e.tag == DCM_DLMSEQUENCEDELIMITATIONITEM))
    {
      return DCM_NORMAL;
    }
#else
    ...
    {
      groupItem->baseLength += 8 + elementItem->paddedDataLength;
      if (elementItem->element.representation == DCM_OB || elementItem->element.representation == DCM_OW
          || elementItem->element.representation == DCM_SQ)
      {
        groupItem->longVRAttributes++;
        (*object)->longVRAttributes++;
...
static CONDITION copyData(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *from, DCM_ELEMENT *to, U32 *rtnLength)
{
  ...
  if (from->element.d.ot == NULL)
  {
    if ((*object)->fd != -1)
    {
      (void)lseek((*object)->fd, from->dataOffset + (long)p, SEEK_SET);
      nBytes = read((*object)->fd, to->d.ot, (int)l);
    }
    else
    {
      (*object)->sk((*object)->userCtx, (long)(from->dataOffset + (long)p), SEEK_SET);
      // cond =
      (*object)->rd((*object)->userCtx, to->d.ot, (long)l, &nBytes);
    }
    if (nBytes != (int)l)
    {
      return COND_PushCondition(
          DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "copyData (DCM internal)");
    }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
  ...
  {
    unsigned char *q;
    q = (unsigned char *)from->element.d.ot + (long)p;
...
static CONDITION readLengthToEnd(int fd, const char *fileName, unsigned long opt, U32 *lengthToEnd)
{
  ...
  if (read(fd, buf, 24) != 24)
    return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), fileName, "(DCM)readLengthToEnd");
...
static void dumpSS(short *ss, long vm)
{
  long index = 0;
  while (index < vm)
  {
...
static void dumpSL(S32 *sl, long vm)
{
  long index = 0;
  while (index < vm)
  {
...
static void dumpUS(unsigned short *us, long vm)
{
  long index = 0;
  while (index < vm)
  {
...
static void dumpUL(U32 *ul, long vm)
{
  long index = 0;
  while (index < vm)
  {
...
static void compareGroup(PRV_GROUP_ITEM *g1,
                         ...
                         void *ctx)
{
  PRV_ELEMENT_ITEM *e1 = NULL, *e2 = NULL;
...
static void copySequence(PRIVATE_OBJECT **dstObj, DCM_ELEMENT *e)
{
  LST_HEAD *lst;
...
CONDITION
DCM_GetCompressedValue(
    DCM_OBJECT **callerObject, DCM_TAG tag, void *buf, size_t bufSize, DCM_GET_COMPRESSED_CALLBACK *callback, void *ctx)
{
  PRIVATE_OBJECT
  **object;
  PRV_ELEMENT_ITEM
  ...
  while (elementLength != 0)
  {
    ...
    else
    {
      ...
      while (l != 0)
      {
        ...
        callback(buf, toRead, index, (index != lastIndex) ? 1 : 0, 0, startOfFragment, ctx);
        l -= toRead;
...
DCM_PrintSequenceList(DCM_OBJECT **object, DCM_TAG tag)
{
  ...
  while (sqItem != NULL)
  {
    ...
    printf(
        "size: %6ld offset: %6ld, pixel offset: %6ld\n", sqObject->objectSize, sqObject->offset, sqObject->pixelOffset);
    sqItem = LST_Next(&lst);
  }
  return cond;
}
...
CONDITION
DCM_GetSequenceByOffset(DCM_OBJECT **object, DCM_TAG tag, unsigned long offset, DCM_OBJECT **rtnObject)
{
  PRIVATE_OBJECT **obj, *sqObject;
  CONDITION cond;
  PRV_ELEMENT_ITEM *elementItem;
...
DCM_CopyObject(DCM_OBJECT **src, DCM_OBJECT **dst)
{
  ...
  if (src == NULL)
  {
    (void)COND_PushCondition(DCM_NULLADDRESS, DCM_Message(DCM_NULLADDRESS), "DCM_CopyObject");
    return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_CopyObject");
  }
  dstObj = (PRIVATE_OBJECT *)CTN_MALLOC(sizeof(PRIVATE_OBJECT));
  if (dstObj == NULL)
  {
    (void)COND_PushCondition(
        DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(PRIVATE_OBJECT), "DCM_CopyObject");
    *dst = NULL;
    return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_CopyObject");
  }
...
DCM_MergeObject(DCM_OBJECT **src, DCM_OBJECT **dst)
{
  ...
  if (src == NULL)
  {
    (void)COND_PushCondition(DCM_NULLADDRESS, DCM_Message(DCM_NULLADDRESS), "DCM_MergeObject");
    return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_MergeObject");
  }
  dstObj = *((PRIVATE_OBJECT **)dst);
  if (dstObj == NULL)
  {
    (void)COND_PushCondition(
        DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(PRIVATE_OBJECT), "DCM_MergeObject");
    *dst = NULL;
    return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_MergeObject");
  }
  srcObj = (PRIVATE_OBJECT **)src;
...
DCM_AddFragment(DCM_OBJECT **callerObject, void *fragment, U32 fragmentLength)
{
  ...
  {
    return COND_PushCondition(
        DCM_UNEVENFRAGMENTLENGTH, DCM_Message(DCM_UNEVENFRAGMENTLENGTH), fragmentLength, "DCM_AddFragment");
  }
  ...
  if (cond != DCM_NORMAL)
    return COND_PushCondition(DCM_INSERTFAILED, DCM_Message(DCM_INSERTFAILED), 0x7fe0, 0x0010, "DCM_AddFragment");
  ...
  if (elementItem == NULL)
  {
    ...
    {
      return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_AddFragment");
    }
  ...
  {
    return COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), mallocLength, "DCM_AddFragment");
  }

  fragmentItem->fragment = (unsigned char *)fragmentItem + (long)sizeof(DCM_FRAGMENT_ITEM);
