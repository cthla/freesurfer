function transformedTemplateFileName = samseg_registerAtlas(...
    imageFileName,...
    templateFileName,...
...
if useInitialTransformForMeshRestPosition
...
[ reducedAlphas, ~ ] = kvlReduceAlphas( originalAlphas, compressionLookupTableFileName, sameGaussianParameters );
size( reducedAlphas )
...
while true
   ...
   [ ~, maximalDeformation ] = kvlStepOptimizer( optimizer );
...
%         extraImageToImageTransformMatrix = Y * inv( originalY );
        extraImageToImageTransformMatrix = Y / originalY;
...
worldToWorldTransformMatrix = (imageToWorldTransformMatrix * imageToImageTransformMatrix) / templateImageToWorldTransformMatrix; %#ok
...
disp(desiredTemplateImageToWorldTransformMatrix);
