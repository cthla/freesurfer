function transformedTemplateFileName = samseg_registerToAtlas(...
    imageFileName,...
    templateFileName,...
...
%
% [CTHLA] this file has (at some point) been modified to add rician noise to the fixed image, which is not apparent from description.
%      further modified to be function based, to properly scope dependent variable names, thereby avoiding calling functions which modify
...
imageBuffer( perfectlyZeroMask(:) ) = 3 * RicianNoise( perfectlyZeroMask(:) );
kvlSetImageBuffer( imageHandle, imageBuffer );
imageFileNoise = sprintf('%s/%s_noise.mgz',savePath,filename);
kvlWriteImage( imageHandle, imageFileNoise );

transformedTemplateFileName = samseg_coregisterAtlas_SPM(...
    imageFileNoise, ...
    templateFileName, ...
    meshCollectionFileName, ...
    compressionLookupTableFileName, ...
