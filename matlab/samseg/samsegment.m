function samsegment(imageFileNames, outDir, atlasDir, varargin)
%   Call: samsegment(imageFileName, outDir, atlasDir, OPTIONAL)
%
%   TODO (header):
%       - fill out parameter descriptions
%       - add more input error catching, as desired
%
%     TODO:
...
if nargin < 3
    error('Call: samsegment(imageFileNames, outDir, atlasDir, OPTIONAL)')
end
...
% =============================================================
parser = inputParser;
addRequired(parser, 'imageFileNames');
addRequired(parser, 'outDir');
addRequired(parser, 'atlasDir');

defaultMultiResolutionSpecification = struct( [] );
defaultMultiResolutionSpecification{ 1 }.meshSmoothingSigma = 2.0; % In mm
defaultMultiResolutionSpecification{ 1 }.targetDownsampledVoxelSpacing = 2.0; % In mm
defaultMultiResolutionSpecification{ 1 }.maximumNumberOfIterations = 100;
defaultMultiResolutionSpecification{ 2 }.meshSmoothingSigma = 0.0; % In mm
defaultMultiResolutionSpecification{ 2 }.targetDownsampledVoxelSpacing = 1.0; % In mm
defaultMultiResolutionSpecification{ 2 }.maximumNumberOfIterations = 100;
addParameter(parser, 'multiResolutionSpecification', defaultMultiResolutionSpecification);

addParameter(parser, 'maximumNumberOfDeformationIterations', 20);
addParameter(parser, 'absoluteCostPerVoxelDecreaseStopCriterion', 1e-4);
addParameter(parser, 'maximalDeformationStopCriterion', 1e-3);
addParameter(parser, 'lineSearchMaximalDeformationIntervalStopCriterion', 1e-3); %idem
addParameter(parser, 'maximalDeformationAppliedStopCriterion', 0);
addParameter(parser, 'BFGSMaximumMemoryLength', 12);
addParameter(parser, 'K', 1e-1);
addParameter(parser, 'brainMaskingSmoothingSigma', 3);
addParameter(parser, 'brainMaskingThreshold', 1e-2);

addParameter(parser, 'nThreads', 1, @isnumeric);

addParameter(parser, 'verbose', 0, @isnumeric);
addParameter(parser, 'showFigures', 0);

% =============================================================
%
% Now parse, and catch errors
%
% =============================================================
parse(parser, imageFileNames, outDir, atlasDir, varargin{:});

multiResolutionSpecification = parser.Results.multiResolutionSpecification;
maximumNumberOfDeformationIterations = parser.Results.maximumNumberOfDeformationIterations;
absoluteCostPerVoxelDecreaseStopCriterion = parser.Results.absoluteCostPerVoxelDecreaseStopCriterion;
maximalDeformationStopCriterion = parser.Results.maximalDeformationStopCriterion;
lineSearchMaximalDeformationIntervalStopCriterion = parser.Results.lineSearchMaximalDeformationIntervalStopCriterion;
maximalDeformationAppliedStopCriterion = parser.Results.maximalDeformationAppliedStopCriterion;
BFGSMaximumMemoryLength = parser.Results.BFGSMaximumMemoryLength;
K = parser.Results.K;
brainMaskingSmoothingSigma = parser.Results.brainMaskingSmoothingSigma;
brainMaskingThreshold = parser.Results.brainMaskingThreshold;
nThreads = parser.Results.nThreads;
...
fprintf( '==========================\n\n' );
fprintf( 'Running Samseg with the following parameters:\n\n' );
fprintf( '==========================\n\n' );
fprintf( 'first input image %s\n', imageFileNames{1} );
...
meshCollectionFileName = sprintf('%s/CurrentMeshCollection30New.txt.gz', atlasDir);
% This is bascially an LUT
compressionLookupTableFileName = sprintf('%s/namedCompressionLookupTable.txt', atlasDir);
...
for contrastNumber = 1 : numberOfContrasts
    ...
    [ images( contrastNumber ), transform ] = kvlReadCroppedImage( imageFileNames{ contrastNumber }, transformedTemplateFileName ); %#ok
    imageBuffers( :, :, :, contrastNumber ) = kvlGetImageBuffer( images( contrastNumber ) ); %#ok % Get the actual imageBuffer
end
...
for contrastNumber = 1 : numberOfContrasts
    ...
    imageBuffers( :, :, :, contrastNumber ) = imageBuffer; %#ok
...
for contrastNumber = 1 : numberOfContrasts
    ...
    buffer = buffer .* mask;
    imageBuffers( :, :, :, contrastNumber ) = buffer; %#ok
    % kvlSetImageBuffer( images( contrastNumber ), imageBuffers( :, :, :, contrastNumber ) );
end
...
% [cthla] build a vector which indexes each gaussian to it's proper class. Used in optimized GEMS2
...
for multiResolutionLevel = 1 : numberOfMultiResolutionLevels
    ...
    for iterationNumber = 1 : maximumNumberOfIterations
            ...
            if ( numberOfContrasts > 1 )
                ...
                for classNumber = 1:numberOfClasses
                    for i = 1:floor(numberOfGaussiansPerClass(classNumber)/2)
                        meansSpread(i) = (floor(numberOfGaussiansPerClass(classNumber)/2))+(i-1); %#ok
                    end
        ...
        % [cthla] No longer necessary after optimized handling of classes in image likelihood filter
        ...
        for contrastNumber = 1 : numberOfContrasts
            downSampledBiasCorrectedImages( contrastNumber ) = ...
                kvlCreateImage( single( downSampledBiasCorrectedImageBuffers( :, :, :, contrastNumber ) ) ); %#ok
        end
...
% if verbose
    ...
    eval( [ 'save ' outDir '/history.mat history -v7.3' ] );
% end
...
writeOutFreeSurferSeg( imageFileNames{1}, transformedTemplateFileName, labeling, outDir, 'segSubSpace.mgz' );


% write out the bias field and the bias corrected image:
for contrastNumber = 1 : numberOfContrasts
    [ ~, scanName, ~ ] = fileparts( imageFileNames{ contrastNumber } );
    outputfile = [ scanName '_biasField.mgz' ];
    writeOutFreeSurferSeg( imageFileNames{ contrastNumber }, transformedTemplateFileName, ...
        exp( biasFields( :, :, :, contrastNumber ) / 1000 ) .* mask, outDir, outputfile );
    
    outputfile = [ scanName '_biasCorrected.mgz' ];
    writeOutFreeSurferSeg( imageFileNames{ contrastNumber }, transformedTemplateFileName, ...
        exp( biasCorrectedImageBuffers( :, :, :, contrastNumber ) / 1000 ), outDir, outputfile );
end
