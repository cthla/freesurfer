function worldToWorldTransformMatrix = registerAtlas(imageFileName, outDir, AvgDataDir, verbose, showFigures)
...
% Specify which atlas mesh to use. These will be generated by createAtlasMeshForAffineRegistration.m
meshSize = 30 * ones( 1, 3 );
meshType = 1;  % 1 = SPM12 with 6 classes; 2 = SPM12 with 4 classes; 3 = own with 4 classes


kvlClear; % Clear all the wrapped C++ stuff

% Read in image, and figure out where the mesh is located
meshTypeNames = char( 'SPM12_6classes', 'SPM12_4classes', 'Own_4classes' );
nameBase = [ deblank( meshTypeNames( meshType, : ) ) '_' ...
    num2str( meshSize( 1 ) ) 'x' num2str( meshSize( 2 ) ) 'x' num2str( meshSize( 3 ) ) ];
meshCollectionFileName = fullfile( AvgDataDir, [ nameBase '_meshCollection.txt' ] );
...
calculator = kvlGetCostAndGradientCalculator( 'MutualInformation', ...
    image, 'Affine' );
[ cost, ~ ] = kvlEvaluateMeshPosition( calculator, mesh );
if true
    %
    ...
    [ trialCost, ~ ] = kvlEvaluateMeshPosition( calculator, mesh );
    if ( trialCost >= cost )
...
worldToWorldTransformMatrix = ( imageToWorldTransformMatrix * imageToImageTransformMatrix ) / templateImageToWorldTransformMatrix; 

[ ~, templateFileNameBase, templateFileNameExtension ] = fileparts( templateFileName );
transformationMatricesFileName = fullfile( outDir, ...
...
transformedTemplateFileName = fullfile( outDir, ...
...
qcFigureFileName = fullfile( outDir, ...
