...
#define ALPHA(art, j) (1.0 / (art->beta + (double)art->ninputs)) - (.00001 * ((double)j + 1.0))
#ifndef MIN
...
#define Mzj(artmap, i, j) (artmap->zj + (((long)j) * (long)artmap->ninputs) + (long)i)
#define Mwj(artmap, j, k) (artmap->w + (((long)j) * (long)artmap->noutputs) + (long)k)
...
ARTMAP *ArtmapAlloc(int ninputs, int noutputs, double rho_bar, int max_f2)
{
  ...
  artmap->zj = (double huge *)InsHalloc((long)ninputs * (long)max_f2, sizeof(double));
  artmap->w = (int huge *)InsHalloc((long)noutputs * (long)max_f2, sizeof(int));
...
int ArtmapWrite(ARTMAP *artmap, char *fname)
{
  ...
  fprintf(fp, "%d %d %d %f %f\n", artmap->ninputs, artmap->noutputs, artmap->f2nodes, artmap->beta, artmap->rho_bar);
