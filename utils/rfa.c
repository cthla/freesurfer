...
RFA *RFAalloc(RFA_PARMS *parms, int alloc_trees)
{
  ...
  if (rfa->nodes == NULL)
    ErrorExit(ERROR_NOMEMORY, "RFAalloc: could not allocate node X");

  for (x = 0; x < rfa->vg.width; x++)
  {
    rfa->nodes[x] = (RF_NODE **)calloc(rfa->vg.height, sizeof(RF_NODE *));
    if (rfa->nodes[x] == NULL)
      ErrorExit(ERROR_NOMEMORY, "RFAalloc: could not allocate node Y");
    for (y = 0; y < rfa->vg.height; y++)
    {
      rfa->nodes[x][y] = (RF_NODE *)calloc(rfa->vg.height, sizeof(RF_NODE));
      if (rfa->nodes[x][y] == NULL)
        ErrorExit(ERROR_NOMEMORY, "RFAalloc: could not allocate node Z");
      for (z = 0; z < rfa->vg.depth; z++)
      {
        ...
        if (node->label_priors == NULL)
          ErrorExit(ERROR_NOMEMORY, "RFAalloc: could not allocate node label_priors");
        node->labels = (unsigned short *)calloc(MAX_LABELS_PER_NODE, sizeof(node->labels[0]));
        if (node->labels == NULL)
          ErrorExit(ERROR_NOMEMORY, "RFAalloc: could not allocate node labels");
...
int RFAupdateTraining(RFA *rfa, RFA_PARMS *parms)
{
  ...
  {
    if ((xn % 10) == 0)
      printf("training node series %d of %d\n", xn, rfa->width);
    for (yn = 0; yn < rfa->height; yn++)
      for (zn = 0; zn < rfa->depth; zn++)
      {
            ...
            for (yk = 0; yk < rfa->spacing; yk++)
            {
              ...
              for (zk = 0; zk < rfa->spacing; zk++)
              {
                ...
                if (xn == Ggca_x && yn == Ggca_y && zn == Ggca_z && (Ggca_label < 0 || Ggca_label == label))
                {
                  int j;
                  printf("tdata[%d]: ", tno);
                  for (j = 0; j < rfa->nfeatures; j++)
                    printf("%.0f ", training_data[tno][j]);
        ...
        for (max_count = wmsa_count = n = 0; n < node->nlabels; n++)
        {
          ...
          if (node->label_priors[n] > max_count)
            max_count = node->label_priors[n];
...
int RFAwrite(RFA *rfa, char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "RFAwrite(%s): could not open file", fname));
  fprintf(fp,
          "%d %d %d %d %d %d %d %2.1f\n",
          rfa->total_training,
          rfa->wsize,
    ...
    for (yn = 0; yn < rfa->height; yn++)
      for (zn = 0; zn < rfa->depth; zn++)
      {
        ...
        for (n = 0; n < node->nlabels; n++)
          fprintf(fp, "%d ", node->labels[n]);
        fprintf(fp, "\n");
        for (n = 0; n < node->nlabels; n++)
          fprintf(fp, "%2.3f ", node->label_priors[n]);
        fprintf(fp, "\n");
        if (node->nlabels > 1)
          RFwriteInto(node->rf, fp);
      }
...
RFA *RFAread(char *fname)
{
  ...
  if (fscanf(fp,
      ...
      != 8)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "RFAwrite: could not read file"));
  }
    ...
    for (y = 0; y < mri_in->height; y++)
      for (z = 0; z < mri_in->depth; z++)
      {
        ...
        if (x == Gx && y == Gy && z == Gz)
        {
          ...
          for (j = 0; j < node->nlabels; j++)
            printf("%d (%.2f) ", node->labels[j], node->label_priors[j]);
          printf("\nfeature = ");
          for (j = 0; j < rfa->nfeatures; j++)
            printf("%.0f ", feature[j]);
          printf("\n");
