...
extern "C" Geodesics *computeGeodesics(MRIS *surf, float maxdist)
{
  ...
  for (int vertexID = 0; vertexID < surf->nvertices; vertexID++)
  {
    ...
    if (vertexID % 1000 == 0)
      progressBar((float)vertexID / surf->nvertices);
  }
  progressBar(1.0);
  std::cout << std::endl;
  msec = TimerStop(&mytimer);
...
extern "C" Geodesics *geodesicsRead(char *fname, int *pnvertices)
{
  ...
  for (nthvtx = 0; nthvtx < *pnvertices; nthvtx++)
  {
    ...
    if (fread(&geo[nthvtx].vnum, sizeof(int), 1, fp) != 1)
    {
      fclose(fp);
      printf("ERROR (%s): could not read file\n", fname);
      return (NULL);
    }
    if (fread(geo[nthvtx].v, sizeof(int), geo[nthvtx].vnum, fp) != 1)
    {
      fclose(fp);
      printf("ERROR (%s): could not read file\n", fname);
      return (NULL);
    }
    if (fread(geo[nthvtx].dist, sizeof(float), geo[nthvtx].vnum, fp) != 1)
    {
      fclose(fp);
      printf("ERROR (%s): could not read file\n", fname);
      return (NULL);
