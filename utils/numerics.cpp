...
static void
ConvertFromFloatToVNLDouble(const float *iaSource, vnl_vector< double > &oDestination, const int icSourceAndDestination)
{
...
static void
ConvertFromVNLDoubleToFloat(const vnl_vector< double > iSource, float *oaDestination, const int icSourceAndDestination)
{
  // indexing p at 1 because of NR legacy
...
static void
ConvertFromFloatToVNLDouble(float **iaSource, vnl_matrix< double > &oDestination, const int icSourceAndDestination)
{
  // indexing p at 1 because of NR legacy
...
static void
ConvertFromVNLDoubleToFloat(const vnl_matrix< double > iSource, float **oaDestination, const int icSourceAndDestination)
{
  // indexing p at 1 because of NR legacy
  for (int row = 0; row < icSourceAndDestination; row++)
  {
...
static double gamma_log(double x)
{
  ...
  int i;
  double p1[8] = { 4.945235359296727046734888,     2.018112620856775083915565E+02, 2.290838373831346393026739E+03,
  ...
  double p2[8] = { 4.974607845568932035012064,     5.424138599891070494101986E+02, 1.550693864978364947665077E+04,
  ...
  double p4[8] = { 1.474502166059939948905062E+04,  2.426813369486704502836312E+06,  1.214755574045093227939592E+08,
  ...
  double q1[8] = { 6.748212550303777196073036E+01, 1.113332393857199323513008E+03, 7.738757056935398733233834E+03,
  ...
  double q2[8] = { 1.830328399370592604055942E+02, 7.765049321445005871323047E+03, 1.331903827966074194402448E+05,
  ...
  double q4[8] = { 2.690530175870899333379843E+03,  6.393885654300092398984238E+05,  4.135599930241388052042842E+07,
  ...
  else if (fabs(x) <= 12.7)
  {
    ...
    q = exp(-y) * b0
        / (fabs(x) - b1
           + b2
                 / (fabs(x) + b3
                    + b4 / (fabs(x) - b5 + b6 / (fabs(x) + b7 - b8 / (fabs(x) + b9 + b10 / (fabs(x) + b11))))));
    //
    //  12.7 < |X|
    //
  }
 ...
 */
extern "C" void
OpenSplint(float iaX[], float iaY[], float iaYSecondDerivatives[], int icYSecondDerivatives, float iX, float *oY)
{
...
float SplineCubicValue(int n, float t[], float tval, float y[], float ypp[], float *ypval, float *yppval)
{
  ...
  else
  {
    ...
    yval = y[ival]
           + dt
                 * ((y[ival + 1] - y[ival]) / h - (ypp[ival + 1] / 6.0 + ypp[ival] / 3.0) * h
...
/* allocate a float vector with subscript range v[nl..nh] */
{
  ...
  if (!v)
    ErrorExit(ERROR_NOMEMORY, "could not allocate vector(%ld, %ld)", nl, nh);
  return v - nl + NR_END;
}
...
/* allocate a float matrix with subscript range m[nrl..nrh][ncl..nch] */
{
  ...
  if (!m)
    ErrorExit(ERROR_NOMEMORY, "could not allocate matrix(%ld, %ld)", nrow, ncol);
  m += NR_END;
  m -= nrl;
  ...
  if (!m[nrl])
    ErrorExit(ERROR_NOMEMORY, "could not allocate matrix(%ld, %ld) array", nrow, ncol);
  m[nrl] += NR_END;
  m[nrl] -= ncl;
