  ...
  if (names)
    for (i = 0; i < noutputs; i++)
    {
      ...
      if (!rbf->class_names[i])
        ErrorExit(ERROR_NO_MEMORY, "RBFinit: could not allocate class name %d", i);
...
int RBFwrite(RBF *rbf, char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "RBFwrite(%s): could not open file", fname));
...
RBF *RBFread(char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NO_FILE, "RBFread(%s): could not open file", fname));
    ...
    RBF *rbf, int (*get_observation_func)(VECTOR *v_obs, int no, void *parm, int same_class, int *pclass), void *parm)
{
  ...
  for (obs_no = 0; obs_no < rbf->nobs; obs_no++)
  {
    ...
    for (i = 1; i <= rbf->nhidden + 1; i++)
    {
      ...
      for (k = 1; k <= rbf->nhidden + 1; k++)
        m_Gt_G->rptr[i][k] += RVECTOR_ELT(v_hidden, i) * RVECTOR_ELT(v_hidden, k);
    }
  }
  ...
  if (!m_Gt_G_inv)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "rbfCalculateOutputWeights: matrix is singular"));
...
static int rbfAllocateTrainingParameters(RBF *rbf)
{
  ...
  if (!rbf->observed)
    ErrorExit(ERROR_NO_MEMORY, "rbfAllocateTrainingParms: could not allocate observed array");
  rbf->m_delta_sigma_inv = (MATRIX **)calloc(rbf->nhidden, sizeof(MATRIX *));
  if (!rbf->m_delta_sigma_inv)
    ErrorExit(ERROR_NO_MEMORY, "rbfAllocateTrainingParms: could not allocate delta sigma");
  rbf->v_delta_means = (VECTOR **)calloc(rbf->nhidden, sizeof(VECTOR *));
  if (!rbf->v_delta_means)
    ErrorExit(ERROR_NO_MEMORY, "rbfAllocateTrainingParms: could not allocate delta means");
  for (i = 0; i < rbf->nhidden; i++)
  {
  ...
  if (!rbf->m_delta_wij)
    ErrorExit(ERROR_NO_MEMORY, "rbfAllocateTrainingParms: could not allocate m_delta_wij");
