/**
 * @file  selxavgio.c
 * @brief REPLACE_WITH_ONE_LINE_SHORT_DESCRIPTION
 *
 * REPLACE_WITH_LONG_DESCRIPTION_OR_REFERENCE
 */
/*
 * Original Author: REPLACE_WITH_FULL_NAME_OF_CREATING_AUTHOR
 * CVS Revision Info:
 *    $Author: nicks $
 *    $Date: 2011/03/02 00:04:55 $
 *    $Revision: 1.10 $
 *
 * Copyright © 2011 The General Hospital Corporation (Boston, MA) "MGH"
 *
 * Terms and conditions for use, reproduction, distribution and contribution
 * are found in the 'FreeSurfer Software License Agreement' contained
 * in the file 'LICENSE' found in the FreeSurfer distribution, and here:
 *
 * https://surfer.nmr.mgh.harvard.edu/fswiki/FreeSurferSoftwareLicense
 *
 * Reporting: freesurfer@nmr.mgh.harvard.edu
 *
 */

/***************************************************************
  Name:    selxavgio.c
  $Id: selxavgio.c,v 1.10 2011/03/02 00:04:55 nicks Exp $
  Author:  Douglas Greve
  Purpose: Routines for handling header files for data created by
  selxavg or selavg (selectively averaged).
 ****************************************************************/

#include <errno.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <strings.h>

#include "mri_identify.h"
#include "selxavgio.h"
#include <string.h>

extern int errno;

/* ----------------------------------------------------
   sv_sxadat_by_stem() - save selxavg.dat structure using
   volid as base. volid can be either a stem or stem.ext
   ---------------------------------------------------- */
int sv_sxadat_by_stem(SXADAT *sxadat, const char *volid)
{
  char tmpstr[1000];
  int err;
  char *stem;

  stem = IDstemFromName(volid);
  if (stem == NULL)
    sprintf(tmpstr, "%s.dat", volid);
  else
  {
    sprintf(tmpstr, "%s.dat", stem);
    free(stem);
  }
  err = sv_sxadat(sxadat, tmpstr);
  return (err);
}
/* ---------------------------------------------------- */
SXADAT *ld_sxadat_from_stem(const char *volid)
{
  char tmpstr[1000];
  char *stem;
  SXADAT *sxa;

  if (!is_sxa_volume(volid))
    return (NULL);

  stem = IDstemFromName(volid);
  if (stem == NULL)
    sprintf(tmpstr, "%s.dat", volid);
  else
  {
    sprintf(tmpstr, "%s.dat", stem);
    free(stem);
  }

  sxa = ld_sxadat(tmpstr);
  return (sxa);
}

/* ---------------------------------------------------- */
float *sxa_framepower(SXADAT *sxa, int *nframes)
{
  int frame;
  int h, statid, condition;
  float *framepower;

  *nframes = 2 * sxa->Nc * sxa->Nh;
  framepower = (float *)calloc(*nframes, sizeof(float));

  frame = 0;
  for (condition = 0; condition < sxa->Nc; condition++)
  {
    for (statid = 0; statid < 2; statid++)
    {
      for (h = 0; h < sxa->Nh; h++)
      {
        if (statid == 0)
          framepower[frame] = 1.0; /* avereage */
        if (statid == 1)
          framepower[frame] = 2.0; /* std/var */
        frame++;
      }
    }
  }
  return (framepower);
}

/* ---------------------------------------------------- */
int is_sxa_volume(const char *volid)
{
  char tmpstr[1000];
  FILE *fp;
  char *stem;

  if (volid == NULL)
    return (0);
  stem = IDstemFromName(volid);
  if (stem == NULL)
    sprintf(tmpstr, "%s.dat", volid);
  else
  {
    sprintf(tmpstr, "%s.dat", stem);
    free(stem);
  }

  fp = fopen(tmpstr, "r");
  if (fp == NULL)
    return (0);
  fclose(fp);

  return (1);
}

/* ---------------------------------------------------- */
SXADAT *ld_sxadat(const char *sxadatfile)
{
  FILE *fp;
  SXADAT *sxa;
  int n, r, c, Nch;

  fp = fopen(sxadatfile, "r");
  if (fp == NULL)
  {
    perror("ldsxdat():");
    fprintf(stderr, "Could not open %s\n", sxadatfile);
    return (NULL);
  }

  sxa = (SXADAT *)calloc(1, sizeof(SXADAT));
  if (sxa == NULL)
  {
    fprintf(stderr, "Could not alloc SXADAT\n");
    fclose(fp);
    return (NULL);
  }

  if (fscanf(fp, "%*s %f", &sxa->TR) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %f", &sxa->TimeWindow) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %f", &sxa->TPreStim) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->Nc) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->Nh) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  sxa->Nnnc = sxa->Nc - 1;
  n = fscanf(fp, "%*s %d", &sxa->version);
  if (n == 0)
  {
    sxa->version = 0;
    return (sxa);
  }
  if (fscanf(fp, "%*s %f", &sxa->TER) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %f", &sxa->DOF) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }

  if (fscanf(fp, "%*s") != 0)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  sxa->npercond = (int *)calloc(sxa->Nc, sizeof(int));
  for (n = 0; n < sxa->Nc; n++)
  {
    if (fscanf(fp, "%d", &sxa->npercond[n]) != 1)
    {
      fprintf(stderr, "Could not read file\n");
      fclose(fp);
      return (NULL);
    }
  }
  if (fscanf(fp, "%*s %d", &sxa->nruns) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->ntp) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->nrows) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->ncols) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->nskip) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->DTOrder) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %f", &sxa->RescaleFactor) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %f", &sxa->HanningRadius) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->nNoiseAC) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->BrainAirSeg) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  if (fscanf(fp, "%*s %d", &sxa->GammaFit) == 1)
  {
    if (sxa->GammaFit)
    {
      if (fscanf(fp, "%*s") != 0)
      {
        fprintf(stderr, "Could not read file\n");
        fclose(fp);
        return (NULL);
      }
      for (n = 0; n < sxa->GammaFit; n++)
      {
        if (fscanf(fp, "%f", &sxa->gfDelta[n]) != 1)
        {
          fprintf(stderr, "Could not read file\n");
          fclose(fp);
          return (NULL);
        }
      }
      if (fscanf(fp, "%*s") != 0)
      {
        fprintf(stderr, "Could not read file\n");
        fclose(fp);
        return (NULL);
      }
      for (n = 0; n < sxa->GammaFit; n++)
      {
        if (fscanf(fp, "%f", &sxa->gfTau[n]) != 1)
        {
          fprintf(stderr, "Could not read file\n");
          fclose(fp);
          return (NULL);
        }
      }
    }
  }

  if (fscanf(fp, "%*s %d", &sxa->NullCondId) != 1)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }

  Nch = sxa->Nh * sxa->Nnnc;

  if (fscanf(fp, "%*s") != 0)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  sxa->SumXtX = (float *)calloc(Nch * Nch, sizeof(float));
  n = 0;
  for (r = 0; r < Nch; r++)
  {
    for (c = 0; c < Nch; c++)
    {
      if (fscanf(fp, "%f", &sxa->SumXtX[n]) != 1)
      {
        fprintf(stderr, "Could not read file\n");
        fclose(fp);
        return (NULL);
      }
      n++;
    }
  }

  if (sxa->version == 1)
    return (sxa);

  if (fscanf(fp, "%*s") != 0)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  sxa->hCovMtx = (float *)calloc(Nch * Nch, sizeof(float));
  n = 0;
  for (r = 0; r < Nch; r++)
  {
    for (c = 0; c < Nch; c++)
    {
      if (fscanf(fp, "%f", &sxa->hCovMtx[n]) != 1)
      {
        fprintf(stderr, "Could not read file\n");
        fclose(fp);
        return (NULL);
      }
      n++;
    }
  }

  if (fscanf(fp, "%*s") != 0)
  {
    fprintf(stderr, "Could not read file\n");
    fclose(fp);
    return (NULL);
  }
  sxa->CondIdMap = (int *)calloc(sxa->Nc, sizeof(int));
  for (n = 0; n < sxa->Nc; n++)
  {
    if (fscanf(fp, "%d", &sxa->CondIdMap[n]) != 1)
    {
      fprintf(stderr, "Could not read file\n");
      fclose(fp);
      return (NULL);
    }
  }
  fclose(fp);

  return (sxa);
}

/* ---------------------------------------------------- */
int sv_sxadat(SXADAT *sxa, const char *sxadatfile)
{
  FILE *fp;
  int n, r, c, Nch;

  fp = fopen(sxadatfile, "w");
  if (fp == NULL)
  {
    perror("sv_sxdat():");
    fprintf(stderr, "Could not open %s\n", sxadatfile);
    return (1);
  }

  fprintf(fp, "TR         %f\n", sxa->TR);
  fprintf(fp, "TimeWindow %f\n", sxa->TimeWindow);
  fprintf(fp, "TPreStim   %f\n", sxa->TPreStim);
  fprintf(fp, "nCond      %d\n", sxa->Nc);
  fprintf(fp, "Nh         %d\n", sxa->Nh);
  if (sxa->version == 0)
  {
    fclose(fp);
    return (0);
  }
  fprintf(fp, "version %d\n", sxa->version);
  fprintf(fp, "TER %f\n", sxa->TER);
  fprintf(fp, "DOF %f\n", sxa->DOF);

  fprintf(fp, "NPerCond ");
  for (n = 0; n < sxa->Nc; n++)
    fprintf(fp, "%d ", sxa->npercond[n]);
  fprintf(fp, "\n");

  fprintf(fp, "nruns %d\n", sxa->nruns);
  fprintf(fp, "ntp %d\n", sxa->ntp);
  fprintf(fp, "nrows %d\n", sxa->nrows);
  fprintf(fp, "ncols %d\n", sxa->ncols);
  fprintf(fp, "nskip %d\n", sxa->nskip);
  fprintf(fp, "DTOrder %d\n", sxa->DTOrder);
  fprintf(fp, "RescaleFactor %f\n", sxa->RescaleFactor);
  fprintf(fp, "HanningRadius %f\n", sxa->HanningRadius);
  fprintf(fp, "nNoiseAC %d\n", sxa->nNoiseAC);
  fprintf(fp, "BrainAirSeg %d\n", sxa->BrainAirSeg);
  fprintf(fp, "GammaFit %d\n", sxa->GammaFit);
  if (sxa->GammaFit)
  {
    fprintf(fp, "gfDelta ");
    for (n = 0; n < sxa->GammaFit; n++)
      fprintf(fp, "%f ", sxa->gfDelta[n]);
    fprintf(fp, "\n");
    fprintf(fp, "gfTau ");
    for (n = 0; n < sxa->GammaFit; n++)
      fprintf(fp, "%f ", sxa->gfTau[n]);
    fprintf(fp, "\n");
  }
  fprintf(fp, "NullCondId %d\n", sxa->NullCondId);

  fprintf(fp, "SumXtX\n");
  Nch = sxa->Nh * sxa->Nnnc;

  n = 0;
  for (r = 0; r < Nch; r++)
  {
    for (c = 0; c < Nch; c++)
    {
      fprintf(fp, "%f \n", sxa->SumXtX[n]);
      n++;
    }
  }

  if (sxa->version == 1)
    return (0);

  fprintf(fp, "hCovMtx\n");
  n = 0;
  for (r = 0; r < Nch; r++)
  {
    for (c = 0; c < Nch; c++)
    {
      fprintf(fp, "%f \n", sxa->hCovMtx[n]);
      n++;
    }
  }

  fprintf(fp, "CondIdMap ");
  for (n = 0; n < sxa->Nc; n++)
    fprintf(fp, "%d ", sxa->CondIdMap[n]);
  fprintf(fp, "\n");

  fclose(fp);
  return (0);
}
