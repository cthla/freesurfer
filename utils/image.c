...
int ImageUpdateHeader(IMAGE *I, const char *fname)
{
  ...
  if (ecode != HIPS_OK)
    ErrorReturn(-1, (ERROR_NO_FILE, "ImageUpdateHeader: fwrite_header failed (%d)\n", ecode));
...
IMAGE *ImageMul(IMAGE *Isrc1, IMAGE *Isrc2, IMAGE *Idst)
{
  ...
  if (!Idst)
    Idst = ImageAlloc(Isrc1->rows, Isrc1->cols, Isrc1->pixel_format, Isrc1->num_frame);
...
IMAGE *ImageResize(IMAGE *Isrc, IMAGE *Idst, int drows, int dcols)
{
  ...
  if (FEQUAL(x_scale, 1.0f))
    ...
    case PFBYTE:
      ...
      if (ecode != HIPS_OK)
        ErrorExit(ecode, "ImageResize: h_%s(%2.3f, %2.3f) returned %d\n", x_scale > 1.0f ? "enlarge" : "reduce", ecode);
#endif
      break;
    default:
      if (x_scale > 1.0f)
...
IMAGE *ImageNormalizePix(IMAGE *Isrc, IMAGE *Idst)
{
  ...
  if (ecode != HIPS_OK)
    ErrorReturn(NULL, (ecode, "ImageNormalize: h_linscale failed (%d)\n", ecode));
...
int ImageScaleDown(IMAGE *inImage, IMAGE *outImage, float scale)
{
  ...
  for (frame = 0; frame < inImage->num_frame; frame++)
  {
    ...
    case PFBYTE:
      ...
      default:
        ErrorReturn(
            -1, (ERROR_UNSUPPORTED, "ImageScaleDown: unsupported output pixel format %d\n", outImage->pixel_format));
        break;
      }
      break;
...
int ImageScaleUp(IMAGE *inImage, IMAGE *outImage, float scale)
{
  ...
  for (frame = 0; frame < inImage->num_frame; frame++)
  {
    ...
    case PFBYTE:
      ...
      default:
        ErrorReturn(-1,
                    (ERROR_UNSUPPORTED, "ImageScaleUp: unsupported output pixel format %d\n", outImage->pixel_format));
        break;
      }
      break;
    case PFFLOAT:
      ...
      for (inRow = 0; inRow < inRows; inRow++)
        for (inCol = 0; inCol < inCols; inCol++, inIPix++)
        {
          ...
          for (outRow = nint((float)inRow * scale); outRow < endRow; outRow++)
          {
            ...
            for (outCol = nint((float)inCol * scale); outCol < endCol; outCol++, outIPix++)
              *outIPix = *inIPix;
          }
        }
      break;
    default:
      ErrorReturn(-2, (ERROR_UNSUPPORTED, "ImageScaleUp: unsupported input pixel format %d\n", inImage->pixel_format));
      break;
    }
    inImage->image += inImage->sizeimage;
    inImage->firstpix += inImage->sizeimage;
  ...
  for (frame = 0; frame < Isrc->num_frame; frame++)
  {
    ...
    case PFFLOAT: /* float --> byte */
      ...
      default:
        ErrorReturn(
            ERROR_UNSUPPORTED,
            (ERROR_UNSUPPORTED, "ImageDifferentialScaleDown: unsupported output pixel format %d", Iout->pixel_format));
        break;
      }
      break;
    case PFINT:
  ...
  for (frame = 0; frame < Isrc->num_frame; frame++)
  {
    ...
    case PFBYTE:
      ...
      default:
        ErrorReturn(
            -1,
            (ERROR_UNSUPPORTED, "ImageDifferentialScaleUp: unsupported output pixel format %d\n", Iout->pixel_format));
        break;
      }
      break;
    case PFFLOAT:
      ...
      for (inRow = 0; inRow < inRows; inRow++)
        for (inCol = 0; inCol < inCols; inCol++, inIPix++)
        {
          ...
          for (outRow = nint((float)inRow * yscale); outRow < endRow; outRow++)
          {
            ...
            for (outCol = nint((float)inCol * xscale); outCol < endCol; outCol++, outIPix++)
              *outIPix = *inIPix;
          }
        }
      break;
...
int ImageAddSpeckleNoise(IMAGE *inImage, IMAGE *outImage, float amp)
{
  ...
  case PFBYTE:
    ...
    break;
  default:
    ErrorReturn(-1, (ERROR_UNSUPPORTED, "ImageAddSpeckleNoise: unsupported input format %d\n", inImage->pixel_format));
...
IMAGE *ImageCatSeq(IMAGE *Isrc1, IMAGE *Isrc2, IMAGE *Idst)
{
  ...
  if (!Idst)
    Idst = ImageAlloc(Isrc1->rows, Isrc1->cols, Isrc1->pixel_format, Isrc1->num_frame + Isrc2->num_frame);
...
IMAGE *ImageExtractInto(IMAGE *Isrc, IMAGE *Idst, int x0, int y0, int dx, int dy, int xdst, int ydst)
{
  ...
  if ((dx <= 0) || (dy <= 0))
    ErrorReturn(NULL, (ERROR_BADPARM, "ImageExtractInto: invalid dx or dy (%d, %d)", dx, dy));
...
IMAGE *ImageExtract(IMAGE *Isrc, IMAGE *Idst, int x0, int y0, int dx, int dy)
{
  ...
  if ((dx <= 0) || (dy <= 0))
    ErrorReturn(NULL, (ERROR_BADPARM, "ImageExtract: invalid dx or dy (%d, %d)", dx, dy));
