...
MRI *signaRead(char *fname, int read_volume_flag)
{
  ...
  for (slice = 0, i = header.imnr0; i <= header.imnr1; i++, slice++)
  {
    ...
    fp = fopen(fname, "rb");
    if (!fp)
      ErrorReturn(0, (ERROR_NOFILE, "read_signa(%s): could not open file", fname));

    fseek(fp, HLENGTH, SEEK_SET);
    int sizeToRead = mri->width * mri->height;
    if (header.num_echoes > 1)
    {
      ...
      if (fread(&MRISseq_vox(mri, 0, 0, slice, frame), sizeof(short), sizeToRead, fp) != sizeToRead)
      {
        ErrorReturn(0, (ERROR_BAD_FILE, "read_signa: could not read file"));
      }
      orderShortBuffer(&MRISseq_vox(mri, 0, 0, slice, frame), sizeToRead);
      if (frame == 0)
        slice--; // same slice, next echo
    }
    else
    {
      if (fread(&MRISvox(mri, 0, 0, slice), sizeof(short), sizeToRead, fp) != sizeToRead)
      {
        ErrorReturn(0, (ERROR_BAD_FILE, "read_signa: could not read file"));
      }
      orderShortBuffer(&MRISvox(mri, 0, 0, slice), sizeToRead);
    }
    fclose(fp);
    if (odd_only || even_only)
      i++;
