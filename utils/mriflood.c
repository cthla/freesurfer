      ...
      for (z = k - halfside; z <= k + halfside; z++)
      {
        if ((x > 0) && (y > 0) && (z > 0) && (x < width) && (y < height) && (z < depth))
        {
          ...
          if ((vox == Right_Cerebral_Cortex) || (vox == Right_Cerebral_White_Matter) || (vox == Right_Cerebral_Exterior)
              ...
              || (vox == Right_F3orb) || (vox == Right_lOg) || (vox == Right_aOg) || (vox == Right_mOg)
              || (vox == Right_pOg) || (vox == Right_Stellate) || (vox == Right_Porg) || (vox == Right_Aorg))
        ...
        for (i = 0; i < width; i++)
        {
          ...
          if ((vox == Left_Cerebral_Cortex) || (vox == Right_Cerebral_Cortex))
          {
            if (IllegalCorticalNeighbour(mri_cma, mri_white, i, j, k))
            {
              ...
              { /* if the voxel needs to be eroded,
  ...
  for (fno = 0; fno < mris->nfaces; fno++)
  {
    ...
    for (v = 0; v <= numv; v++)
    {
      ...
      for (u = 0; u <= numu; u++)
      {
        ...
        if (vox2sras == NULL)
          vox2sras = MatrixInverse(mris->m_sras2vox, NULL);
        col = nint(fcol);
        row = nint(frow);
        slc = nint(fslc);
