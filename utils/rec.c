...
REC *RecRead(char *fname, int iop_neeg, int iop_nmeg)
{
  ...
  fp = fopen(fname, "r");
  if (fp == NULL)
    ErrorReturn(NULL, (ERROR_BADFILE, "RecRead: could not open file %s", fname));
  ...
  if (fscanf(fp, "%*s") != 0)
  {
    ErrorReturn(NULL, (ERROR_BADFILE, "RecRead: could not read parameter"));
  }
  if (fscanf(fp, "%d %d %d", &rec->ntimepts, &rec->nmeg_channels, &rec->neeg_channels) != 3)
  {
    ErrorReturn(NULL, (ERROR_BADFILE, "RecRead: could not read parameter"));
  }
  ...
  if (!rec->latencies)
    ErrorExit(ERROR_NOMEMORY, "RecRead(%s): could not allocate latency vector", fname);
  ...
  printf("rec file read, sample period %2.4f, starting latency %2.4f\n",
         rec->latencies[1] - rec->latencies[0],
         rec->latencies[0]);
