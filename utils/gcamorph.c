...
#define MAX_ERROR 1000
        ...
        if (x == Gx && y == Gy && z == Gz)
        {
          printf("ll_like: node(%d,%d,%d)-->vox(%2.0f,%2.0f,%2.0F): dI=(%2.1f,%2.1f,%2.1f), "
                 "grad=(%2.2f,%2.2f,%2.2f), "
...
int gcamJacobianTerm(GCA_MORPH *gcam, const MRI *mri, double l_jacobian, double ratio_thresh)
{
  ...
  num = 0;
#ifdef HAVE_OPENMP
#pragma omp parallel for reduction(+ : num) firstprivate(j, k, gcamn, ratio, orig_area, ratio_thresh) shared(gcam)     \
                                                             schedule(static, 1)
#endif
    ...
    GCA_MORPH *gcam, const MRI *mri, double l_jacobian, int i, int j, int k, double *pdx, double *pdy, double *pdz)
{
  ...
  static VECTOR *v_i[_MAX_FS_THREADS] = { NULL }, *v_j[_MAX_FS_THREADS], *v_k[_MAX_FS_THREADS],
                *v_j_x_k[_MAX_FS_THREADS], *v_i_x_j[_MAX_FS_THREADS], *v_k_x_i[_MAX_FS_THREADS],
                *v_grad[_MAX_FS_THREADS], *v_tmp[_MAX_FS_THREADS];
    ...
    GCA_MORPH *gcam, MRI *mri, double l_area, int i, int j, int k, double *pdx, double *pdy, double *pdz)
{
  ...
  double del_v_scale, uk, image_val, error; // area, orig_area;
...
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(tid, j, k, gcamn, neg, num, gcamni, gcamnj, gcamnk, area1, area2) shared(        \
    gcam, Gx, Gy, Gz, v_i, v_j, v_k, gcam_neg_counter, Ginvalid_counter) schedule(static, 1)
#endif
  for (i = 0; i < width; i++)
  {
#ifdef HAVE_OPENMP
...
double gcamJacobianEnergy(const GCA_MORPH *gcam, MRI *mri)
{
  ...
  sse = 0.0f;
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(j, k, gcamn, ratio, exponent, delta)                                             \
                                          shared(width, height, depth, gcam) reduction(+ : sse) schedule(static, 1)
#endif
  for (i = 0; i < width; i++)
  {
    for (j = 0; j < height; j++)
      ...
      for (y = 0; y < height; y++)
      {
        for (z = 0; z < depth; z++)
        {
          ...
          case MRI_FLOAT:
            ...
            break;
          default:
            ErrorReturn(NULL, (ERROR_UNSUPPORTED, "GCAMmorphFromAtlas: unsupported volume type %d", mri_morphed->type));
            break;
          }
        }
      }
...
int GCAMmorphPlistFromAtlas(int N, float *points_in, GCA_MORPH *gcam, float *points_out)
{
  ...
  if (!sample_type) // NN interpolation
  {
    ...
    for (counter = 0; counter < N; counter++)
    {
      ...
      if (GCAsourceFloatVoxelToPriorReal(gcam->gca, NULL, transform, x, y, z, &xr, &yr, &zr) != NO_ERROR)
        continue;
...
MRI *GCAMmorphToAtlas(MRI *mri_src, GCA_MORPH *gcam, MRI *mri_morphed, int frame, int sample_type)
{
  ...
  if (mri_morphed)
  {
    if ((mri_src->xsize != mri_src->ysize) || (mri_src->xsize != mri_src->zsize) || (mri_src->ysize != mri_src->zsize))
    {
...
#else
  int n, nsmall, done = 0, which = GCAM_INTEGRATE_OPTIMAL, max_small, increasing, good_step, good_step_ever; // reduced;
...
// reduced = 1; /* don't reset parms->dt until we switch integration types */
    ...
    if ((pct_change < tol) && !increasing)
    {
      ...
      if (good_step_ever && parms->regrid > 1
          && ((compressed = GCAMcountCompressedNodes(gcam, parms->ratio_thresh)) > 0)
          && 0)
      {
...
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(                                                                                 \
    y, z, vx, vy, vz, vnx, vny, vnz, error, node_sse, dx, dy, dz, xk, yk, zk, xn, yn, zn, num, gcamn, gcamn_nbr)       \
        reduction(+ : sse) shared(gcam, Gx, Gy, Gz) schedule(static, 1)
#endif
...
int gcamSpringTerm(GCA_MORPH *gcam, double l_spring, double ratio_thresh)
{
  ...
  int x, y, z, xk, yk, zk, xn, yn, zn, num; // width, height, depth;
  GCA_MORPH_NODE *gcamn, *gcamn_nbr;
...
MRI *GCAMbuildMostLikelyVolume(GCA_MORPH *gcam, MRI *mri)
{
  ...
  // error check
  if (!mri)
    ErrorExit(ERROR_BADPARM, "GCAMbuildMostLikelyVolume called with null MRI.\n");
      ...
      for (y = 0; y < height; y++)
      {
        for (x = 0; x < width; x++)
        {
          ...
          for (n = 0; n < gcam->ninputs; n++)
          {
            ...
            {
            default:
              ErrorReturn(NULL, (ERROR_UNSUPPORTED, "GCAMbuildMostLikelyVolume: unsupported image type %d", mri->type));
              break;
            case MRI_SHORT:
              MRISseq_vox(mri, x, y, z, n) = nint(val);
              break;
...
MRI *GCAMbuildVolume(GCA_MORPH *gcam, MRI *mri)
{
  ...
  // error check
  if (!mri)
    ErrorExit(ERROR_BADPARM, "GCAMbuildVolume called with null MRI.\n");
  if (mri->width != gcam->atlas.width || mri->height != gcam->atlas.height || mri->depth != gcam->atlas.depth)
    ErrorExit(ERROR_BADPARM,
...
double gcamFindOptimalTimeStep(GCA_MORPH *gcam, GCA_MORPH_PARMS *parms, MRI *mri)
{
  ...
  // start_rms, pct_change;
  VECTOR *vY;
  int N, i, Gxs, Gys, Gzs, suppressed = 0, prev_neg; // bad;
  long diag;
#endif
  ...
  for (x = 0; x < gcam->width; x++)
  {
    for (y = 0; y < gcam->height; y++)
    {
      for (z = 0; z < gcam->depth; z++)
      {
        ...
        */
        ...
        gcamn_inf = &gcam->nodes[x][y + 1][z];
        gcamn_sup = &gcam->nodes[x][y - 1][z];
        if (((IS_HIPPO(gcamn->label) && IS_WM(gcamn_inf->label)) || (IS_WM(gcamn->label) && IS_HIPPO(gcamn_sup->label)))
            == 0)
	...
	load_vals(mri, gcamn->x, gcamn->y, gcamn->z, vals, gcam->ninputs);
	  ...
	  continue ;  // current label already explains intensities quite well
        ...
        {
          if ((gcan->labels[n] == Left_VentralDC || gcan->labels[n] == Right_VentralDC || gcan->labels[n] == Brain_Stem)
...
#pragma omp parallel for firstprivate(tid, i, y, z, gcap, n, norm, dx, dy, dz, gc, node_prob, prob)                    \
                                          shared(gcam, Gx, Gy, Gz, mri_smooth, l_map) schedule(static, 1)
#endif
  for (x = 0; x < gcam->width; x++)
    for (y = 0; y < gcam->height; y++)
...
int GCAMremoveNegativeNodes(GCA_MORPH *gcam, MRI *mri, GCA_MORPH_PARMS *parms)
{
  ...
  parms->noneg = 0;
  parms->l_distance = parms->l_log_likelihood = parms->l_binary = parms->l_multiscale = parms->l_spring = parms->l_area
      = parms->l_elastic = parms->l_smoothness = parms->l_label = 0.0;
  parms->navgs = 0;
  parms->l_area = 0.0;
  parms->l_jacobian = 1;
  parms->dt = 0.1;
  ...
  for (x = 0; x < gcam->width; x++)
    for (y = 0; y < gcam->height; y++)
      for (z = 0; z < gcam->depth; z++)
      {
        ...
        if (x == Gx && y == Gy && z == Gz)
        {
          printf("node(%d,%d,%d) --> (%2.2f, %2.2f, %2.2f), dI=(%2.3f, %2.3f, %2.3f), label %d, binary %2.3f, "
                 "grad=(%2.4f,%2.4f,%2.4f)\n",
                 x,
                 y,
                 z,
...
int gcamShowCompressed(GCA_MORPH *gcam, FILE *fp)
{
  ...
  if (n1 || n2 || n3 || n4)
    fprintf(fp, "%d nodes compressed > 0.5, %d > 0.25, %d > .1, %d > 0.025\n", n1, n2, n3, n4);
  return (n1 + n2 + n3 + n4);
}
...
int GCAMdeformVentricles(GCA_MORPH *gcam, MRI *mri, GCA_MORPH_PARMS *parms)
{
  ...
  for (navgs = parms->navgs, level = parms->levels; level >= 0; level--)
  {
    do
    {
      ...
      if (Gdiag & DIAG_SHOW)
      {
        printf("%04d: dt=%2.6f, rms=%2.3f (%2.3f%%), "
...
MRI *GCAMbuildLabelVolume(GCA_MORPH *gcam, MRI *mri)
{
  ...
  // error check
  if (!mri)
    ErrorExit(ERROR_BADPARM, "GCAbuildLabelVolume called with null MRI.\n");
  if (mri->width != gcam->atlas.width || mri->height != gcam->atlas.height || mri->depth != gcam->atlas.depth)
    ErrorExit(ERROR_BADPARM,
...
#endif
double
GCAMMSgibbsImageLogPosterior(GCAM_MS *gcam_ms, MRI *mri_labels, MRI *mri_inputs, TRANSFORM *transform, MRI *mri_s_index)
{
    ...
    for (y = 0; y < mri_warp->height; y++)
      for (z = 0; z < mri_warp->depth; z++)
      {
            ...
            (gcam->nodes[x][y][z].invalid > 0))            // and it's invalid area or position
        {
          ...
          for (xk = -1; xk <= 1; xk++)
          {
            for (yk = -1; yk <= 1; yk++)
            {
              for (zk = -1; zk <= 1; zk++)
              {
                ...
                num++;
                for (f = 0; f < 3; f++)
                  means[f] += MRIgetVoxVal(mri_warp, xi, yi, zi, f);
              }
            }
          }
          ...
          {
            for (f = 0; f < 3; f++)
              MRIsetVoxVal(mri_warp_dst, x, y, z, f, means[f] / num);
          }
        }
      }
  }
  ...
  for (x = 0; x < gcam->width; x++)
    for (y = 0; y < gcam->height; y++)
      for (z = 0; z < gcam->depth; z++)
      {
        ...
        else if (IS_UNKNOWN(gcamn->label) && (different_neighbor_labels(gcam, x, y, z, 1) > 0))
        { // use a ring of unknowns outside brain to prevent surface of brain from moving too far  inward
#if 0
...
int GCAMregisterVentricles(GCA_MORPH *gcam, MRI *mri, GCA_MORPH_PARMS *parms)
{
  ...
  printf("using threshold %2.1f from atlas ventricle (%2.1f +- %2.1f) and caudate (%2.1f +- %2.1f) distributions for "
    ...
    for (y = 0; y < gcam->height; y++)
      for (z = 0; z < gcam->depth; z++)
      {
        if (x == Gx && y == Gy && z == Gz)
          DiagBreak();
        gcamn = &gcam->nodes[x][y][z];
        if (nint(gcamn->x) == Gvx && nint(gcamn->y) == Gvy && nint(gcamn->z) == Gvz)
          DiagBreak();
        MRIsampleVolume(mri, gcamn->x, gcamn->y, gcamn->z, &val);
        if (val > thresh)
          continue;
  ...
  if (cmean < vmean + 2 * vsigma) // caudate mean must be wrong - don't use it in threshold
  {
    ...
    printf("caudate distribution could not be robustly estimated - using atlas caudate (%2.1f) and ventricle density "
  ...
  */
  ...
  do
  {
    ...
    if (parms->log_fp)
      fprintf(parms->log_fp, "ventricle registration, pass %d of ventricular alignment\n", passno);
    start_rms = GCAMcomputeRMS(gcam, mri, parms);
