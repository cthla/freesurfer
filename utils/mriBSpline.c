...
static void getZLine(const MRI *mri, int x, int y, int f, double Line[])
{
  ...
  for (z = 0; z < mri->depth; z++)
    Line[z] = MRIgetVoxVal(mri, x, y, z, f);
}
...
static void getYLine(const MRI *mri, int x, int z, int f, double Line[])
{
  ...
  for (y = 0; y < mri->height; y++)
    Line[y] = MRIgetVoxVal(mri, x, y, z, f);
}
...
static void getXLine(const MRI *mri, int y, int z, int f, double Line[])
{
  ...
  for (x = 0; x < mri->width; x++)
    Line[x] = MRIgetVoxVal(mri, x, y, z, f);
}
...
static void BsplineWeights(int SplineDegree, const double *val, const int *index, double *weights)
{
  ...
  case 7:
    ...
    weights[2]
        = (397.0 / 7.0 - w * (245.0 / 3.0 + w * (-15.0 + w * (-95.0 / 3.0 + w * (15.0 + w * (5.0 + w * (-5.0 + w)))))))
          / 240.0;
    weights[3] = (2416.0 / 35.0 + w2 * (-48.0 + w2 * (16.0 + w2 * (-4.0 + w)))) / 144.0;
    weights[4] = (1191.0 / 35.0 - w * (-49.0 + w * (-9.0 + w * (19.0 + w * (-3.0 + w) * (-3.0 + w2))))) / 144.0;
    weights[5] = (40.0 / 7.0 + w * (56.0 / 3.0 + w * (24.0 + w * (40.0 / 3.0 + w2 * (-4.0 + w * (-2.0 + w)))))) / 240.0;
    weights[7] = w2;
  ...
  case 9:
    ...
    weights[1]
        = (502.0 / 9.0
           + w * (-246.0
                  + w * (472.0
                         + w * (-504.0 + w * (308.0 + w * (-84.0 + w * (-56.0 / 3.0 + w * (24.0 + w * (-8.0 + w)))))))))
          / 40320.0;
    weights[2] = (3652.0 / 9.0
                  - w * (2023.0 / 2.0
                         + w * (-952.0
                                + w * (938.0 / 3.0
...
static void ExpandCentered_1D(double In[], long NxIn, double Out[], double h[], long nh)
{
  ...
  for (i = 0L; i < NxIn; i++)
  {
    ...
    for (k = 2L; k < nh; k = k + 2L)
    {
      ...
      i2 = i + k / 2L;
      if (i1 < 0L)
      {                              /* Provide the correct border conditions. */
...

{
  ...
  {
    xIndex[k] = (Width == 1) ? (0) : ((xIndex[k] < 0) ? (-xIndex[k] - Width2 * ((-xIndex[k]) / Width2))
                                                      : (xIndex[k] - Width2 * (xIndex[k] / Width2)));
    if (Width <= xIndex[k])
    {
      xIndex[k] = Width2 - xIndex[k];
    }
    yIndex[k] = (Height == 1) ? (0) : ((yIndex[k] < 0) ? (-yIndex[k] - Height2 * ((-yIndex[k]) / Height2))
                                                       : (yIndex[k] - Height2 * (yIndex[k] / Height2)));
    if (Height <= yIndex[k])
    {
      yIndex[k] = Height2 - yIndex[k];
    }
    zIndex[k] = (Depth == 1) ? (0) : ((zIndex[k] < 0) ? (-zIndex[k] - Depth2 * ((-zIndex[k]) / Depth2))
...
#pragma omp parallel
  ...
  if (!mAinv)
    ErrorReturn(NULL, (ERROR_BADPARM, "MRIlinearTransformSpline: xform is singular"));
