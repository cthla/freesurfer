...
SVM *SVMalloc(int ninputs, char *c1_name, char *c2_name)
{
  ...
  if (NULL == svm)
    ErrorExit(ERROR_NOMEMORY, "SVMalloc(%d, %s, %s) - calloc failed", ninputs, c1_name, c2_name);
...
int SVMwrite(SVM *svm, char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "SVMwrite(%s): could not open file", fname));
...
SVM *SVMread(char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_BADPARM, "SVMread(%s): could not open file", fname));

  if (fscanf(fp, "NINPUTS\t%d\n", &ninputs) != 1)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "SVMread could not read file"));
  }
  if (fscanf(fp, "NTRAINING\t%d\n", &ntraining) != 1)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "SVMread could not read file"));
  }
  if (fscanf(fp, "CLASS1\t%s\n", class1_name) != 1)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "SVMread could not read file"));
  }
  if (fscanf(fp, "CLASS2\t%s\n", class2_name) != 1)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "SVMread could not read file"));
  }
  ...
  if (!svm)
    ErrorReturn(NULL, (ERROR_BADPARM, "SVMread(%s): could not create svm", fname));
  ...
  while (NULL != cp)
  {
    ...
    {
      sscanf(cp, "%*s\t%lf", &svm->C);
    }
    else if (stricmp(token, "SIGMA") == 0)
    {
      sscanf(cp, "%*s\t%lf", &svm->sigma);
    }
    else if (stricmp(token, "NTRAINING") == 0)
    {
      sscanf(cp, "%*s\t%d", &svm->ntraining);
    }
    else if (stricmp(token, "TYPE") == 0)
    {
      sscanf(cp, "%*s\t%d", &svm->type);
    }
    else if (stricmp(token, "ALPHA") == 0)
    {
      svm->alpha = (double *)calloc(svm->ntraining, sizeof(double));
      if (svm->alpha == NULL)
        ErrorExit(ERROR_NOMEMORY, "SVMread(%s) - %d calloc failed", fname, svm->ntraining);
    ...
    {
      sscanf(cp, "%*s\t%d", &svm->extra_args);
      svm->args = (char **)calloc(svm->extra_args, sizeof(char *));
      if (!svm->args)
        ErrorExit(ERROR_BADPARM, "SVMread(%s): could not allocate %d extra args", svm->extra_args);
      for (i = 0; i < svm->extra_args; i++)
      {
        cp = fgetl(line, STRLEN - 1, fp);
        if (!cp)
          ErrorExit(ERROR_BADFILE, "SVMread(%s): couldn't read %dth extra arg from file", fname, i);
    ...
    {
      sscanf(cp, "%*s\t%le", &svm->threshold);
    }
    else if (stricmp(token, "NSUPPORT") == 0)
    {
      sscanf(cp, "%*s\t%d", &svm->nsupport);
      svm->asupport = (double *)calloc(svm->nsupport, sizeof(double));
      ...
      for (i = 0; i < svm->nsupport; i++)
      {
        if (i == Gdiag_no)
          DiagBreak();
        if (fscanf(fp, "%le %f\n", &svm->asupport[i], &svm->ysupport[i]) != 2)
        {
          ErrorReturn(NULL, (ERROR_BAD_FILE, "SVMread could not read file"));
        }
