...
/*---------------------------------------------------------
  ...
  unrecoginzed.
  ---------------------------------------------------------*/
int interpolation_code(char *interpolation_string) { return (MRIinterpCode(interpolation_string)); }
/*---------------------------------------------------------
...
{
  if (!strcasecmp(float2int_string, "round") || !strcasecmp(float2int_string, "rint"))
    return (FLT2INT_ROUND);

  if (!strcasecmp(float2int_string, "floor"))
    return (FLT2INT_FLOOR);

  if (!strncasecmp(float2int_string, "tkregister", 5))
    return (FLT2INT_TKREG);
...
MATRIX *ComputeQFWD(MATRIX *Q, MATRIX *F, MATRIX *W, MATRIX *D, MATRIX *QFWD)
{
  ...
  if (Q != NULL)
    MatrixMultiply(QFWDtmp, Q, QFWDtmp);
  if (F != NULL)
    MatrixMultiply(QFWDtmp, F, QFWDtmp);
  if (W != NULL)
    MatrixMultiply(QFWDtmp, W, QFWDtmp);
  if (D != NULL)
    MatrixMultiply(QFWDtmp, D, QFWDtmp);
  ...
  if (ReverseMapFlag)
  {
    ...
    for (svtx = 0; svtx < SrcSurfReg->nvertices; svtx++)
    {
      ...
      for (n = 0; n < npairs; n++)
      {
        ...
        if (UseHash)
          tvtx = MHTfindClosestVertexNo(Hash[kT], SurfReg[kT], v, &dmin);
        if (!UseHash || tvtx < 0)
        {
          if (tvtx < 0)
  ...
  *SrcDist = MRIallocSequence(SrcSurfReg->nvertices, 1, 1, MRI_FLOAT, 1);
  ...
  for (tvtx = 0; tvtx < TrgSurfReg->nvertices; tvtx++)
  {
    ...
    if (UseHash)
      ...
      svtx = MRISfindClosestVertex(SrcSurfReg, v->x, v->y, v->z, &dmin);
    ...
    for (svtx = 0; svtx < SrcSurfReg->nvertices; svtx++)
    {
      if (MRIFseq_vox((*SrcHits), svtx, 0, 0, 0) == 0)
      {
        ...
        if (UseHash)
          ...
          tvtx = MRISfindClosestVertex(TrgSurfReg, v->x, v->y, v->z, &dmin);
        /* Hash table failed, so use brute force */
        if (tvtx < 0)
          tvtx = MRISfindClosestVertex(TrgSurfReg, v->x, v->y, v->z, &dmin);
  ...
  for (c = 0; c < volsurf->width; c++)
  {
    for (r = 0; r < volsurf->height; r++)
    {
      for (s = 0; s < volsurf->depth; s++)
      {
        ...
        if (cR < 0 || cR >= ribbon->width)
          continue;
        if (rR < 0 || rR >= ribbon->height)
          continue;
        if (sR < 0 || sR >= ribbon->depth)
          continue;
        ribval = MRIgetVoxVal(ribbon, cR, rR, sR, 0);
        if (ribval != 3 && ribval != 42)
        {
          for (f = 0; f < nframes; f++)
            MRIsetVoxVal(volsurf, c, r, s, f, 0);
          continue;
        }
        ...
        for (n = 0; n < nsurfs; n++)
        {
          if (surfs[n]->hemisphere == LEFT_HEMISPHERE && ribval != 3)
            continue;
          if (surfs[n]->hemisphere == RIGHT_HEMISPHERE && ribval != 42)
            continue;
    ...
    MRI *seg, LTA *seg2vol, double resmm, int *segidlist, int nsegs, MRI *mask, int ReInit, COLOR_TABLE *ct, MRI *out)
{
  ...
  if (resmm == 0)
    resmm = MIN(MIN(seg->xsize, seg->ysize), seg->zsize);
  ...
  if (ReInit)
  {
    ...
    segidmax = 0;
    for (f = 0; f < nsegs; f++)
      if (segidmax < segidlist[f])
        segidmax = segidlist[f];
    seg2frame = (int *)calloc(sizeof(int), segidmax + 1);
    for (f = 0; f < nsegs; f++)
      seg2frame[segidlist[f]] = f;
  ...
  for (f = 0; f < nthreads; f++)
    nperfth[f] = (float *)calloc(sizeof(float), nsegs);
  ...
  for (nthvox = 0; nthvox < nvox; nthvox++)
  {
    ...
    if (DoSeg == 0)
    {
      for (f = 0; f < nsegs; f++) // dont compute pvf outside, becomes very slow
        if (nperf[f] > 0)
          MRIsetVoxVal(out, c, r, s, f, (float)nperf[f] / npervox);
    }
    else
    {
      // Get seg with highest PVF
      for (f = 0; f < nsegs; f++)
        nperf[f] = (float)nperf[f] / npervox;
  ...
  for (c = 0; c < segpvf->width; c++)
  {
    for (r = 0; r < segpvf->height; r++)
    {
      for (s = 0; s < segpvf->depth; s++)
      {
        ...
        for (tt = 0; tt < nTT; tt++)
          MRIsetVoxVal(pvf, c, r, s, tt, vtt[tt]);
      }
    }
  }
  return (pvf);
...
MRI *MRIaseg2vol(MRI *aseg, MATRIX *tkR, MRI *voltemp, double fthresh, MRI **pvolhit, COLOR_TABLE *ct)
{
  ...
  *pvolhit = volhit;
  ...
  while (n < Na)
  {
    ...
    nhits = 0;
    while ((n + nhits < Na) && (indv == avind[n + nhits].volindex))
      nhits++;
    ...
    if ((double)nhitsmost / nhits < fthresh)
      segid = 0;
...
static int MostHitsInVolVox(ASEGVOLINDEX *avindsorted, int N, int *segidmost, COLOR_TABLE *ct)
{
  ...
  while (n < N)
  {
    ...
    nhits = 0;
    while ((n + nhits < N) && (segid == avindsorted[n + nhits].segid))
      nhits++;
    if (n == 0)
    {
      // first segid
  ...
  if (ct && (float)nhits / N < 0.5)
  {
    ...
    *segidmost = segidlist[0];
    ...
    for (n = 0; n < nsegs; n++)
    {
      if (ttypelist[n] != TTypeMax)
        continue;
      if (nmost > nhitslist[n])
        continue;
      nmost = nhitslist[n];
...
static int CompareAVIndices(const void *i1, const void *i2)
{
  ...
  if (avind1->volindex > avind2->volindex)
    return (+1);
  if (avind1->volindex < avind2->volindex)
    return (-1);
