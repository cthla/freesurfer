...
static char *class_names[NCLASSES] = { "BACKGROUND", "GREY MATTER", "WHITE MATTER", "BRIGHT MATTER" };
...
/*-----------------------------------------------------
                    GLOBAL FUNCTIONS
-------------------------------------------------------*/
int GCarrayUpdate(GCARRAY *gcarray, MRI *mri_src, MRI *mri_norm, MRI *mri_target);
...
GCARRAY *GCarrayAlloc(MRI *mri_template, int scale, int nvars)
{
  ...
  if (!gcarray)
    ErrorReturn(NULL, (ERROR_NO_MEMORY, "GCarrayAlloc: could not alloc struct"));
  ...
  for (z = 0; z < depth; z++)
  {
    ...
    if (!gcarray->gcs[z])
      ErrorExit(ERROR_NO_MEMORY, "GCarrayAlloc: could not allocate gcs[%d]", z);
    ...
    for (y = 0; y < height; y++)
    {
      ...
      if (!gcarray->gcs[z][y])
        ErrorExit(
            ERROR_NO_MEMORY, "GCalloc(%d,%d,%d,%d): could not allocate gc[%d][%d]", width, height, depth, scale, y, z);
...
GCARRAY *GCarrayTrainAll(GCARRAY *gcarray, char *training_file_name, int scale, int ninputs)
{
  ...
  if (!fp)
    ErrorReturn(NULL, (ERROR_NO_FILE, "GCarrayTrainAll(%s): could not open file", training_file_name));
...
/*-----------------------------------------------------
        ...
        Description
          ...
          region.
------------------------------------------------------*/
int GCarrayUpdate(GCARRAY *gcarray, MRI *mri_src, MRI *mri_norm, MRI *mri_target) { return (NO_ERROR); }
/*-----------------------------------------------------
        Parameters:
        ...
        Description
          ...
          region.
------------------------------------------------------*/
int GCarrayFinish(GCARRAY *gcarray) { return (NO_ERROR); }
/*-----------------------------------------------------
        Parameters:
...
int GCarrayTrain(GCARRAY *gcarray, MRI *mri_src, MRI *mri_zscore, MRI *mri_target)
{
  ...
  int x, y, z, x0, y0, z0, x1, y1, z1, xm, ym, zm, xv, yv, zv, width, depth, height, scale, classno, nclasses,
      nobs[NCLASSES], swidth, sheight, sdepth, overlap, ninputs;
  BUFTYPE src, target;
        ...
        for (zm = z0; zm <= z1; zm++)
        {
          for (ym = y0; ym <= y1; ym++)
          {
            for (xm = x0; xm <= x1; xm++)
            {
              ...
              m_inputs[classno]->rptr[nobs[classno] + 1][2] = MRIFvox(mri_zscore, xv, yv, zv);
              if (gcarray->nvars > 2)
              {
  ...
  for (z = 0; z < sdepth; z++)
  {
    for (y = 0; y < sheight; y++)
    {
      ...
      for (x = 0; x < swidth; x++)
      {
        ...
        MRIvoxelToTalairach(mri_src, (double)x, (double)y, (double)z, &xt, &yt, &zt);
...
GCARRAY *GCarrayRead(char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NO_FILE, "GCarrayRead(%s): could not open file", fname));
  ...
  {
    fclose(fp);
    ErrorReturn(NULL, (ERROR_BADFILE, "GCarrayRead(%s): could scanf parms from file", fname));
  }
  ...
  {
    fclose(fp);
    ErrorReturn(NULL, (ERROR_BADPARM, "GCarrayRead(%s): gcarray allocation failed", fname));
  }
...
int GCarrayWrite(GCARRAY *gcarray, char *fname)
{
  ...
  fp = fopen(fname, "wb");
  if (!fp)
    ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "GCarrayWrite(%s): could not open file", fname));

  fprintf(fp,
          "%d %d %d %d %d %d %d %d %2.3f %2.3f %2.3f\n",
          gcarray->scale,
          gcarray->width,
          gcarray->height,
          gcarray->depth,
