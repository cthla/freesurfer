...
int MRIgtmSeg(GTMSEG *gtmseg)
{
  ...
  if (nlist > 0)
    MRIunsegmentWM(aseg, gtmseg->lhw, gtmseg->rhw, list, nlist, NULL, aseg);
...
COLOR_TABLE *GTMSEGctab(GTMSEG *gtmseg, COLOR_TABLE *ctSubCort)
{
  ...
  for (n = 0; n < ct0->nentries; n++)
  {
    ...
    if (hit == 0)
    {
      /* It does not have to be there. Eg, CC will be removed. Also
         ...
         clusterXX. */
      if (Gdiag_no > 1)
        printf("INFO: GTMSEGctab(): lhp n=%d, segid=%d, %s not in segidlist\n", n, segid, cte0->name);
      continue;
    }
  ...
  for (n = 0; n < ct0->nentries; n++)
  {
    ...
    {
      if (Gdiag_no > 1)
        printf("INFO: GTMSEGctab(): rhp n=%d, segid=%d, %s not in segidlist\n", n, segid, cte0->name);
      continue;
    }
    segidhit[m]++;
    ct->entries[segid] = (CTE *)calloc(1, sizeof(COLOR_TABLE_ENTRY));
  ...
  if (gtmseg->SubSegWM)
  {
    ...
    for (n = 0; n < ct0->nentries; n++)
    {
      ...
      {
        if (Gdiag_no > 1)
          printf("INFO: GTMSEGctab(): lhw n=%d, segid=%d, %s not in segidlist\n", n, segid, cte0->name);
        continue;
      }
      segidhit[m]++;
      ct->entries[segid] = (CTE *)calloc(1, sizeof(COLOR_TABLE_ENTRY));
...
int WriteVRFStats(char *fname, GTM *gtm)
{
  ...
  for (n = 0; n < gtm->iXtX->rows; n++)
  {
    ...
    if (gtm->segrvar)
      fprintf(fp, "   %10.4f", sqrt(gtm->segrvar->rptr[n + 1][1]));
    fprintf(fp, "\n");
  }
  fclose(fp);
  fflush(stdout);
...
int GTMsolve(GTM *gtm)
{
  ...
  if (gtm->rvarUnscaled == NULL)
    gtm->rvarUnscaled = MatrixAlloc(1, gtm->res->cols, MATRIX_REAL);
  for (f = 0; f < gtm->res->cols; f++)
  {
    sum = 0;
    for (n = 0; n < gtm->res->rows; n++)
    ...
    for (c = 0; c < gtm->yvol->width; c++)
    {
      for (r = 0; r < gtm->yvol->height; r++)
      {
        ...
        for (f = 0; f < gtm->beta->cols; f++)
        {
          ...
          if (segid != 0)
            gtm->segrvar->rptr[nthseg + 1][f + 1] += v * v;
        }
        k++;
      } // r
    }   // c
...
int GTMrbv(GTM *gtm)
{
  ...
  if (gtm->mask_rbv_to_brain)
  {
    ...
    {
      if (gtm->ctGTMSeg->entries[n] == NULL)
        continue;
    ...
    { // crs order not important
      for (r = 0; r < gtm->rbvseg->height; r++)
      {
        for (s = 0; s < gtm->rbvseg->depth; s++)
        {
          ...
          if (gtm->mask_rbv_to_brain)
          {
            if (c < region->x || c >= region->x + region->dx)
              continue;
            if (r < region->y || r >= region->y + region->dy)
              continue;
            if (s < region->z || s >= region->z + region->dz)
              continue;
          }
...
int GTMmgxpvc(GTM *gtm, int Target)
{
  ...
  for (r = 0; r < gtm->X->rows; r++)
  {
    ...
    if (sum < gtm->mgx_gmthresh)
      for (f = 0; f < gtm->nframes; f++)
        ydiff->rptr[r + 1][f + 1] = 0;
    else
      for (f = 0; f < gtm->nframes; f++)
        ydiff->rptr[r + 1][f + 1] /= sum;
  }

  if (Target == 1)
    gtm->mgx_ctx = GTMmat2vol(gtm, ydiff, NULL);
  if (Target == 2)
    gtm->mgx_subctx = GTMmat2vol(gtm, ydiff, NULL);
  if (Target == 3)
    gtm->mgx_gm = GTMmat2vol(gtm, ydiff, NULL);
...
int GTMmeltzerpvc(GTM *gtm)
{
  ...
  if (gtm->MeltzerBinThresh > 0.0)
  {
    ...
    if (gtm->MeltzerNDil > 0)
    {
      ...
      for (k = 0; k < gtm->MeltzerNDil; k++)
        MRIdilate(gmwmpvf, gmwmpvf);
    }
  }
  ...
  for (k = 0; k < gtm->nsegs; k++)
  {
    ...
    if (nhits == 0)
      continue; // prob neither GM or WM
    for (f = 0; f < gtm->yvol->nframes; f++)
    {
...
MRI *GTMsegSynth(GTM *gtm, int frame, MRI *synth)
{
  ...
  for (c = 0; c < gtm->rbvseg->width; c++)
  { // crs order does not matter here
    for (r = 0; r < gtm->rbvseg->height; r++)
    {
      for (s = 0; s < gtm->rbvseg->depth; s++)
      {
...
int GTMrvarGM(GTM *gtm)
{
  ...
  if (gtm->rvarbrain == NULL)
    gtm->rvarbrain = MatrixAlloc(1, gtm->res->cols, MATRIX_REAL);
  ct = gtm->ctGTMSeg;
  ...
  for (c = 0; c < gtm->yvol->width; c++)
  {
    ...
    for (r = 0; r < gtm->yvol->height; r++)
    {
      for (s = 0; s < gtm->yvol->depth; s++)
      {
        ...
        nth = 0; // row of y or X
        for (f = 0; f < gtm->yvol->nframes; f++)
          y->rptr[nth + 1][f + 1] = MRIgetVoxVal(gtm->yvol, c, r, s, f);
        for (tt = 0; tt < pvfpsf->nframes; tt++)
          X->rptr[nth + 1][tt + 1] = MRIgetVoxVal(pvfpsf, c, r, s, tt);
        nth++;
        for (dc = -gtm->lgtm->nrad; dc <= gtm->lgtm->nrad; dc++)
        {
          if (c + dc < 0 || c + dc >= gtm->yvol->width)
            continue;
          for (dr = -gtm->lgtm->nrad; dr <= gtm->lgtm->nrad; dr++)
          {
            if (r + dr < 0 || r + dr >= gtm->yvol->height)
              continue;
            for (ds = -gtm->lgtm->nrad; ds <= gtm->lgtm->nrad; ds++)
            {
              if (s + ds < 0 || s + ds >= gtm->yvol->depth)
                continue;
              if (dc == 0 && dr == 0 && ds == 0)
                continue;
              for (f = 0; f < gtm->yvol->nframes; f++)
        ...
        if (nkeep != pvfpsf->nframes || nv != nvmax)
        {
          ...
          {
            for (f = 0; f < gtm->yvol->nframes; f++)
              ytmp->rptr[nth + 1][f + 1] = y->rptr[nth + 1][f + 1];
            nkeep = 0;
            for (tt = 0; tt < pvfpsf->nframes; tt++)
            {
              if (Xsum->rptr[1][tt + 1] > gtm->lgtm->Xthresh)
...
int GTMttPercent(GTM *gtm)
{
  ...
  for (nthseg = 0; nthseg < gtm->nsegs; nthseg++)
  {
    sum = 0;
    for (tt = 0; tt < nTT; tt++)
      sum += gtm->ttpct->rptr[nthseg + 1][tt + 1]; // yes, tt+1
    for (tt = 0; tt < nTT; tt++)
