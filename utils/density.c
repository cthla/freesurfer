...
DENSITY *DensityHistogramEstimate(MRI *mri1, MRI *mri2, int nbins, float sigma, int *valid1, int *valid2)
{
  ...
  if (!pdf)
    ErrorExit(ERROR_NOMEMORY, "DensityHistogramEstimate(%d): could not allocate pdf", nbins);
  pdf->sigma = sigma;
  ...
  for (nvox = 0, x = 0; x < mri1->width; x++)
  {
    for (y = 0; y < mri1->height; y++)
    {
      ...
      if (valid1 && (valid1[nint(val1 - pdf->min_val1)] == 0))
        continue;
      val2 = MRIgetVoxVal(mri2, x, y, 0, 0);
      if (valid2 && (valid2[nint(val2 - pdf->min_val2)] == 0))
        continue;
      nvox++;
  ...
  if (sigma > 0)
  {
    ...
    pdf->Ipdf = Ismooth;
    if ((Gdiag & DIAG_WRITE) && DIAG_VERBOSE_ON)
      ImageWrite(pdf->Ipdf, "Ipdf_smooth.tif");
  }
...
int DensityWrite(DENSITY *pdf, char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (fp == NULL)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "DensityWrite(%s): could not open file", fname));
  ...
  for (i = 0; i < n; i++)
    fprintf(fp, "%d ", pdf->valid1[i]);
  fprintf(fp, "\n");
  fprintf(fp, ":valid2 ");
  n = ceil(pdf->max_val2 - pdf->min_val2 + 1);
  for (i = 0; i < n; i++)
    fprintf(fp, "%d ", pdf->valid2[i]);
  fprintf(fp, "\n");
...
DENSITY *DensityRead(char *fname)
{
  ...
  pdf = (DENSITY *)calloc(1, sizeof(DENSITY));
  if (!pdf)
    ErrorExit(ERROR_NOMEMORY, "DensityRead(%s): could not density", fname);

  fp = fopen(fname, "r");
  if (fp == NULL)
    ErrorReturn(NULL, (ERROR_BADPARM, "DensityRead(%s): could not open file", fname));

  cp = fgetl(line, MAX_LINE_LEN, fp);
  i = sscanf(cp, "%*s %d\n", &nbins);
  pdf->Ipdf = ImageAlloc(nbins, nbins, PFFLOAT, 1);
  if (pdf->Ipdf == NULL)
    ErrorExit(ERROR_NOMEMORY, "DensityRead(%d): could not allocate density image", nbins);
  if (fscanf(fp,
             ":vol1 %f %f\n:vol2 %f %f\n:sigma %f\n:dof %d\n:mri1 %s\n:mri2 %s\n:valid1 ",
             &pdf->min_val1,
             &pdf->max_val1,
      ...
      != 8)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "DensityRead(%s): could not read parameter(s)", fname));
  }

  n = ceil(pdf->max_val1 - pdf->min_val1 + 1);
  pdf->valid1 = (int *)calloc(n, sizeof(int));
  if (!pdf->valid1)
    ErrorExit(ERROR_NOMEMORY, "DensityHistogramEstimate: could not allocate lookup tables (%d)\n", n);
  for (i = 0; i < n; i++)
  {
  ...
  if (fscanf(fp, "\n:valid2 ") == EOF)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "DensityRead(%s): reached end of file", fname));
  }
  n = ceil(pdf->max_val2 - pdf->min_val2 + 1);
...
MRI *DensityLikelihoodImage(MRI *mri1, MRI *mri2, MRI *mri_ll, MATRIX *m, DENSITY *pdf, MRI *mri_seg, int inverse)
{
  ...
  for (x = 0; x < mri2->width; x++)
  {
    ...
    for (y = 0; y < mri2->height; y++)
    {
      if (x == Gx && y == Gy)
        DiagBreak();
      if (mri_seg && MRIvox(mri_seg, x, y, 0) == 0)
        continue;
