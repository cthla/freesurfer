...
HPtL_tErr HPtL_New(mriHeadPointListRef *oList, char *isListName, char *isTransformName, MATRIX *iClientTransform)
{
  ...
  eResult = HPtL_CreateTransform_(this, isTransformName, iClientTransform);
  if (HPtL_tErr_NoErr != eResult)
  {
    goto error;
  }
...
HPtL_tErr HPtL_ReadHeadListFile_(mriHeadPointListRef this, char *isListName)
{
  ...
  /* allocate storage */
  aStorage = (HPtL_tHeadPoint *)calloc(nNumPoints, sizeof(HPtL_tHeadPoint));
  if (NULL == aStorage)
  {
    eResult = HPtL_tErr_AllocationFailed;
    goto error;
  ...
  for (nPoint = 0; nPoint < nNumPoints; nPoint++)
  {
    ...
    xVoxl_SetFloat(&aStorage[nPoint].mPoint, fPointX, fPointY, fPointZ);
  }
...
HPtL_tErr HPtL_ConvertListToClientSpace_(mriHeadPointListRef this)
{
  ...
  while ((eResult = HPtL_NextPoint(this, &pHeadPt)) == HPtL_tErr_NoErr)
  {

    /* first scale the point up */
    DebugNote(("Multiplying point %.2f, %.2f, %.2f by 1000", xVoxl_ExpandFloat(&(pHeadPt->mPoint))));
    ...
    /* run the transform */
    DebugNote(("Transforming point %.2f, %.2f, %.2f to client space", xVoxl_ExpandFloat(&(pHeadPt->mClientPoint))));
    Trns_ConvertBtoA(this->mTransform, &(pHeadPt->mClientPoint), &(pHeadPt->mClientPoint));
  }
...
HPtL_tErr HPtL_WriteHeadPointFile(mriHeadPointListRef this, char *isDest)
{
  ...
  for (nHeadPt = 0; nHeadPt < this->mnNumPoints; nHeadPt++)
  {
    ...
    fprintf(file,
            "%s %d %f %f %f\n",
            this->maPoints[nHeadPt].msLabel,
            this->maPoints[nHeadPt].mnIndex,
...
HPtL_tErr
HPtL_ResetIterator(mriHeadPointListRef this, HPtL_tIterationPlane iPlane, float ifPlaneNumber, float ifPlaneRange)
{
...
HPtL_tErr HPtL_NextPoint(mriHeadPointListRef this, HPtL_tHeadPointRef *opPoint)
{
  ...
  bGoodPoint = FALSE;
  while (!bGoodPoint && this->mnCurPoint < this->mnNumPoints)
  {
    ...
    case HPtL_tIterationPlane_X:
      if (abs(xVoxl_GetFloatX(&(point->mClientPoint)) - this->mfIterPlaneNumber) <= this->mfIterPlaneRange)
      {
        bGoodPoint = TRUE;
      }
      break;
    case HPtL_tIterationPlane_Y:
      if (abs(xVoxl_GetFloatY(&(point->mClientPoint)) - this->mfIterPlaneNumber) <= this->mfIterPlaneRange)
      {
        bGoodPoint = TRUE;
      }
      break;
    case HPtL_tIterationPlane_Z:
      if (abs(xVoxl_GetFloatZ(&(point->mClientPoint)) - this->mfIterPlaneNumber) <= this->mfIterPlaneRange)
      {
        bGoodPoint = TRUE;
      }
      break;
                                ...
                                HPtL_tHeadPointRef *opPoint)
{
  ...
  while ((eResult = HPtL_NextPoint(this, &pPoint)) == HPtL_tErr_NoErr)
  {

    /* calc the distance in client space don't bother square rooting
       because we don't care about the real distance */
    fDistance = pow(xVoxl_GetFloatX(&(pPoint->mClientPoint)) - xVoxl_GetFloatX(iWhere), 2)
                + pow(xVoxl_GetFloatY(&(pPoint->mClientPoint)) - xVoxl_GetFloatY(iWhere), 2)
                + pow(xVoxl_GetFloatZ(&(pPoint->mClientPoint)) - xVoxl_GetFloatZ(iWhere), 2);
                                         ...
                                         HPtL_tHeadPointRef *opPoint)
{
  ...
  for (nCurPoint = 0; nCurPoint < this->mnNumPoints; nCurPoint++)
  {
    ...
    {
    case HPtL_tIterationPlane_X:
      fDistance = pow(xVoxl_GetFloatY(&(point->mClientPoint)) - xVoxl_GetFloatY(iWhere), 2)
                  + pow(xVoxl_GetFloatZ(&(point->mClientPoint)) - xVoxl_GetFloatZ(iWhere), 2);
      break;
    case HPtL_tIterationPlane_Y:
      fDistance = pow(xVoxl_GetFloatX(&(point->mClientPoint)) - xVoxl_GetFloatX(iWhere), 2)
                  + pow(xVoxl_GetFloatZ(&(point->mClientPoint)) - xVoxl_GetFloatZ(iWhere), 2);
      break;
    case HPtL_tIterationPlane_Z:
      fDistance = pow(xVoxl_GetFloatX(&(point->mClientPoint)) - xVoxl_GetFloatX(iWhere), 2)
                  + pow(xVoxl_GetFloatY(&(point->mClientPoint)) - xVoxl_GetFloatY(iWhere), 2);
      break;
    default:
      eResult = HPtL_tErr_InvalidParameter;
      goto error;
...
HPtL_tErr HPtL_AlignPointToClientVoxel(mriHeadPointListRef this, HPtL_tHeadPointRef iPoint, xVoxelRef iClientVox)
{
  ...
  if (NULL == iPoint || NULL == iClientVox)
  {
    eResult = HPtL_tErr_InvalidParameter;
    goto error;
  }
  ...
  DebugPrint(("ana dest: %d %d %d local src %.2f %.2f %.2f\n"
              ...
              xVoxl_ExpandFloat(&destPt),
              xVoxl_ExpandFloat(&srcPt),
              fDeltaX,
              fDeltaY,
              fDeltaZ));
