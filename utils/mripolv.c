...
float ic_x_vertices[NVERTICES]
    = { 0,      0.8944, 0.2764, -0.7236, -0.7236, 0.2764,  -0.4253, -0.8507, -0.4253, 0.1625,  -0.2629,
        0.5257, 0.6882, 0.1625, 0.6882,  -0.2629, -0.5878, 0,       -0.9511, -0.9511, -0.5878, 1.00 };

float ic_y_vertices[NVERTICES]
    = { 0, 0,       0.8507, 0.5257, -0.5257, -0.8507, -0.3090, 0,      0.3090,  -0.5000, -0.8090,
        0, -0.5000, 0.5000, 0.5000, 0.8090,  0.8090,  1.0000,  0.3090, -0.3090, -0.8090, 0.00 };

float ic_z_vertices[NVERTICES]
    = { 1.0000, 0.4472, 0.4472, 0.4472, 0.4472, 0.4472, 0.8507, 0.5257, 0.8507, 0.8507, 0.5257,
        0.8507, 0.5257, 0.8507, 0.5257, 0.5257, 0,      0,      0,      0,      0,      0.00 };
...
MRI *MRIpolvMeanRegion(MRI *mri_src, MRI *mri_dst, MRI *mri_polv, int wsize, MRI_REGION *region)
{
  ...
  if (mri_src->type != MRI_UCHAR)
    ErrorReturn(mri_dst, (ERROR_UNSUPPORTED, "MRIpolvMeanRegion: unsupported src type"));
...
{
  int whalf, xk, yk, xi, yi, zi, *pxi, *pyi, *pzi, count;
  // int  n;
  float e1_x, e1_y, e1_z, e2_x, e2_y, e2_z, xbase, ybase, zbase;
  BUFTYPE val;
...
{
  int width, height, depth, x, y, z, whalf, vertex, xk, yk, zk, pno, mini;
  // int maxi;
...
{
  int width, height, depth, x, y, z, whalf, vertex, xk, yk, mini, xi, yi, zi, *pxi, *pyi, *pzi, x1, y1, z1, x0, y0, z0;
  // int maxi;
  float min_var, max_var, total, total_sq, nv, varv, avgv, val, background_val, fmax;
  // float min_mean;
  BUFTYPE *pdst;
  // BUFTYPE max_val;
...
{
  int width, height, depth, x, y, z, yk, vertex, xi, yi, zi, *pxi, *pyi, *pzi, thin, was_white, black_white, val;
  // int  whalf, n;
...
MRI *MRIresegmentThinWMStrands(MRI *mri_src, MRI *mri_dst, int thickness)
{
  ...
  float nx, ny, nz, nd;
  BUFTYPE *pdst, *psrc;
  double val, xf, yf, zf, up_dist, down_dist;
  // double  max_dist;
  MRI *mri_label;
...
MRI *MRIthickenThinWMStrands(MRI *mri_T1, MRI *mri_src, MRI *mri_dst, int thickness, int nsegments, float wm_hi)
{
  int width, height, depth, x, y, z, thin, i, dont_fill, up_added, down_added, total_filled, nfilled, nseg, nx, ny, nz,
      xv, yv, zv, v;
  float nd;
  double val, xf, yf, zf, up_dist, down_dist /*, xt, yt, zt*/;
  // double max_dist;
  MRI_SEGMENTATION *mriseg;
          ...
          for (nx = 0; nx <= 1; nx++)
          {
            ...
            while (thin > 0)
            {
              ...
              if (up_dist <= down_dist)
              {
                ...
                // bounds-checking
                if (xv >= mri_dst->width || xv < 0 || yv >= mri_dst->height || yv < 0 || zv >= mri_dst->depth || zv < 0)
                {
...
{
  int width, height, depth, x, y, z, whalf, xk, yk, vertex, xi, yi, zi, *pxi, *pyi, *pzi, order;
  // int  n;
  float e1_x, e1_y, e1_z, e2_x, e2_y, e2_z, xbase, ybase, zbase;
  float val;
...
{
  int width, height, depth, x, y, z, vertex, num_on, num_off, num, changed, low_thresh;
  // int  whalf;
  float thresh;
...
{
  int whalf, vertex, xk, yk, min_vertex, xi, yi, zi, *pxi, *pyi, *pzi;
  // int width, height, depth, max_vertex;
  float min_var, max_var, total, total_sq, nv, varv, avgv, val;
  // float min_mean;
  // BUFTYPE max_val;
  float xbase, ybase, zbase, *pe1_x, *pe1_y, *pe1_z, *pe2_x, *pe2_y, *pe2_z, e1_x, e1_y, e1_z, e2_x, e2_y, e2_z;
    ...
    MRI *mri, MRI *mri_labeled, int x0, int y0, int z0, int wsize, float len, float gray_hi, float wm_low)
{
  ...
  float dist, x, y, z, median, nx, ny, nz, white_dist, gray_dist, gray_val, white_val, val, white_grad, gray_grad,
      max_val, min_val;
  // float min_val_dist, max_val_dist;
      ...
      for (y = 0; y < height; y++)
      {
        for (x = 0; x < width; x++)
        {
          ...
          /*
            ...
            being considered.
          */
          if ((MRIvox(mri_dst, x, y, z) > WM_MIN_VAL) || (MRIvox(mri_strand_border, x, y, z) == 0))
            continue;
