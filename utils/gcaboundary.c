...
static int
gcabFindClosestIcoVertex(GCAB *gcab, TRANSFORM *transform, float x0, float y0, float z0, float nx, float ny, float nz);
...
GCAB *GCABalloc(GCA *gca, int spacing, int ico_order, int nintensity_bins, int ngrad_bins, int target_label)
{
  ...
  if (gcab == NULL)
    ErrorExit(ERROR_NOMEMORY, "GCABalloc: could not alloc GCAB");
...
#define PAD 5
  ...
  if (gcab->ico == NULL)
    ErrorExit(ERROR_NOMEMORY, "GCABalloc: could not read ico");
  ...
  for (x = 0; x < width; x++)
  {
    ...
    if (!gcab->bs[x])
      ErrorExit(ERROR_NOMEMORY, "GCABalloc: could not allocate %dth **", x);

    for (y = 0; y < height; y++)
    {
      gcab->bs[x][y] = (GCABS *)calloc(depth, sizeof(GCABS));
      if (!gcab->bs[x][y])
        ErrorExit(ERROR_NOMEMORY, "GCABalloc: could not allocate %d,%dth *", x, y);
      for (z = 0; z < depth; z++)
      {
        if (x == Gx && y == Gy && z == Gz)
          DiagBreak();
...
int GCABwrite(GCAB *gcab, char *fname)
{
  ...
  fp = fopen(fname, "wb");
  if (fp == NULL)
    ErrorReturn(ERROR_BADFILE, (ERROR_BADFILE, "GCABwrite(%s) fopen failed", fname));
    ...
    for (y = 0; y < gcab->height; y++)
    {
      for (z = 0; z < gcab->depth; z++)
      {
        ...
        for (vno = 0; vno < gcab->nvertices; vno++)
        {
          ...
          for (i1 = 0; i1 < gcab->nintensity_bins; i1++)
            for (i2 = 0; i2 < gcab->nintensity_bins; i2++)
              fwriteFloat(gcabs->pdfs[vno][i1][i2], fp);
        }
      }
    }
  }
...
GCAB *GCABread(char *fname, GCA *gca)
{
  ...
  fp = fopen(fname, "rb");
  if (fp == NULL)
    ErrorReturn(NULL, (ERROR_BADFILE, "GCABread(%s) fopen failed", fname));
  ...
  if (version != GCAB_VERSION)
    ErrorReturn(NULL,
                (ERROR_BADFILE,
                 ...
                 version,
                 GCAB_VERSION));
  if (fread(gca_fname, sizeof(char), STRLEN - 1, fp) != STRLEN - 1)
  {
    ErrorReturn(NULL, (ERROR_BADFILE, "GCABread(%s): could not read string", fname));
  }
  if (gca == NULL)
  {
    gca = GCAread(gca_fname);
    if (gca == NULL)
      ErrorReturn(NULL, (ERROR_BADFILE, "GCABread(%s): could not read gca from %s", fname, gca_fname));
  }
  label = freadInt(fp);
  spacing = freadInt(fp);
  freadInt(fp); // nVertices
  ...
  if (fread(gcab->fname, sizeof(char), STRLEN - 1, fp) != STRLEN - 1)
  {
    ErrorReturn(NULL, (ERROR_BADFILE, "GCABread(%s): could not read string", fname));
  }
  strcpy(gcab->fname, fname);
    ...
    for (y = 0; y < gcab->height; y++)
    {
      for (z = 0; z < gcab->depth; z++)
      {
        ...
        for (vno = 0; vno < gcab->nvertices; vno++)
        {
          ...
          for (i1 = 0; i1 < gcab->nintensity_bins; i1++)
            for (i2 = 0; i2 < gcab->nintensity_bins; i2++)
              gcabs->pdfs[vno][i1][i2] = freadFloat(fp);
        }
      }
    }
  }
                                   ...
                                   int target_label)
{
  ...
  if (vno < 0)
    return (NO_ERROR); // no well-defined normal - skip it
...
static int gcabUpdateNodeIout(GCAB *gcab, int xn, int yn, int zn, double val2, int vno, float wt)
{
  ...
  if (xn < 0 || xn >= gcab->width || yn < 0 || yn >= gcab->height || zn < 0 || zn >= gcab->depth)
    return (NO_ERROR);

  gcabValsToBins(gcab, 0, val2, 0, &i1f, &i2f, &gbinf);

  // do linear interpolation
  // i2b   = (int)i2f ; i2u = i2b+1 ;
  // i2d   = i2f - i2b ;
...
static int gcabUpdateNode(GCAB *gcab, int xn, int yn, int zn, double val1, double val2, float grad, int vno, float wt)
{
  ...
  // do linear interpolation
  // gbinb = (int)gbinf ; gbinu = gbinb+1 ;
  // i1b   = (int)i1f ; i1u = i1b+1 ;
  // i2b   = (int)i2f ; i2u = i2b+1 ;
  // i1d   = i1f - i1b ;
  // i2d   = i2f - i2b ;
  // gbind = gbinf - gbinb ;
...
static MRI *gcabWritePDFToMRI(GCAB *gcab, MRI *mri, int x, int y, int z)
{
  ...
  if (mri == NULL)
    mri = MRIalloc(gcab->nvertices, gcab->nintensity_bins, gcab->nintensity_bins, MRI_FLOAT);

  gcabs = &gcab->bs[x][y][z];
  for (vno = 0; vno < gcab->nvertices; vno++)
    for (i1 = 0; i1 < gcab->nintensity_bins; i1++)
      for (i2 = 0; i2 < gcab->nintensity_bins; i2++)
        MRIsetVoxVal(mri, vno, i1, i2, 0, gcabs->pdfs[vno][i1][i2]);
  return (mri);
}
static MRI *gcabWriteMRIToPDF(GCAB *gcab, MRI *mri, int x, int y, int z)
{
  ...
  for (vno = 0; vno < gcab->nvertices; vno++)
    for (i1 = 0; i1 < gcab->nintensity_bins; i1++)
      for (i2 = 0; i2 < gcab->nintensity_bins; i2++)
        gcabs->pdfs[vno][i1][i2] = MRIgetVoxVal(mri, vno, i1, i2, 0);
  return (NO_ERROR);
}
...
static int gcabSmoothPDF(GCAB *gcab, int x, int y, int z, MRI *mri_kernel)
{
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON)
    MRIwrite(mri_pdf, "pdf_before.mgz");
  // width = mri_pdf->width ; height = mri_pdf->height ; depth = mri_pdf->depth ;
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON)
    MRIwrite(mri_pdf, "pdf_after.mgz");
