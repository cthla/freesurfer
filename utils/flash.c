...
MRI *MRIparameterMapsToFlash(MRI *mri_src, MRI *mri_dst, double *TRs, double *TEs, double *FAs, int nflash)
{
  ...
  if (!mri_dst)
    mri_dst = MRIallocSequence(mri_src->width, mri_src->height, mri_src->depth, mri_src->type, nflash);
...
{
  double best_T1, best_PD, norm_im, norm_pred, sse, T1, pred_vals[MAX_FLASH_VOLUMES], error, upper_T1, lower_T1, mid_T1,
...
static int build_lookup_table(double tr, double flip_angle, double te, double min_T1, double max_T1, double step)
{
  ...
  if (!flt->flash)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocated %dth lookup table", Progname, ntables);
