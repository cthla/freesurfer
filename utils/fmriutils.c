...
int MRIglmFitAndTest(MRIGLM *mriglm)
{
  ...
  mriglm->pervoxflag = 0;
  if (mriglm->w != NULL || mriglm->npvr != 0 || mriglm->FrameMask != NULL)
    mriglm->pervoxflag = 1;
...
int MRIglmLoadVox(MRIGLM *mriglm, int c, int r, int s, int LoadBeta)
{
  ...
  if (mriglm->FrameMask != NULL)
  {
    ...
    if (mriglm->glm->X != NULL && nfprev != nf)
      MatrixFree(&(mriglm->glm->X));
    if (mriglm->glm->y != NULL && nfprev != nf)
  ...
  for (f = 1; f <= mriglm->y->nframes; f++)
  {
    if (mriglm->FrameMask != NULL && MRIgetVoxVal(mriglm->FrameMask, c, r, s, f - 1) < 0.5)
      continue;
    nthf++;
