...
static GCA_SAMPLE *
gcaExtractLabelAsSamples(GCA *gca, MRI *mri_labeled, TRANSFORM *transform, int *pnsamples, int label);
#if 0
...
static int
nbr_priors(GCA *gca, int label, int x0, int y0, int z0)
{
  int  xn, yn, zn, xk, yk, zk ;
  float p ;
  ...
  for (xk = -1 ; xk <= 1 ; xk++)
    for (yk = -1 ; yk <= 1 ; yk++)
      for (zk = -1 ; zk <= 1 ; zk++)
      {
        ...
        yn = yk+y0 ;
        zn = zk+z0 ;
        if (xn < 0 || xn >= gca->node_width ||
            yn < 0 || yn >= gca->node_height ||
            zn < 0 || zn >= gca->node_depth)
        {
          continue ;
        }
        p = get_node_prior(gca, label,  xn,  yn,  zn) ;
        if (FZERO(p))
        {
          return(1) ; // should be 0 to enable this code
        }
      }
  return(1) ;
}
    ...
    const GCA *gca, MRI *mri, TRANSFORM *transform, int xv, int yv, int zv, int *pxn, int *pyn, int *pzn)
{
  ...
  if (*pxn < 0 || *pyn < 0 || *pzn < 0 || *pxn >= gca->node_width || *pyn >= gca->node_height
      || *pzn >= gca->node_depth)
  {
    return (ERROR_BADPARM);
  }
...
#if 1
  m_prior2source_voxel = m_tmp; // m_voxsize = NULL ; // for compiler warning
#else
                               ...
                               float *pzv)
{
  ...
  if (transform->type != MORPH_3D_TYPE)
  {
    if (transform->type == LINEAR_VOX_TO_VOX) // from src to talairach volume
    {
      ...
      zv = zc;
#if 0
      // this should already be done in the vox->vox transforrm
      xv = xc/mri->xsize;
      yv = yc/mri->ysize;
      zv = zc/mri->zsize;
#endif
      ...
      for (y = 0; y < gca->node_height; y++)
      {
        ...
        for (z = 0; z < gca->node_depth; z++)
        {
          ...
          if (max_labels > 0)
          {
            ...
            if (!gcan->gcs)
              ErrorExit(ERROR_NOMEMORY, "GCANalloc: couldn't allocate gcs to %d", gcan->max_labels);
            // allocate label storage up to max_labels
            gcan->labels = (unsigned short *)calloc(gcan->max_labels, sizeof(unsigned short));
            if (!gcan->labels)
              ErrorExit(ERROR_NOMEMORY, "GCANalloc: couldn't allocate labels to %d", gcan->max_labels);
          }
        }
      }
    }
      ...
      for (y = 0; y < gca->prior_height; y++)
      {
        ...
        for (z = 0; z < gca->prior_depth; z++)
        {
          ...
          if (max_labels > 0)
          {
            ...
            if (!gcap->labels)
              ErrorExit(ERROR_NOMEMORY, "GCANalloc: couldn't allocate labels to %d", gcap->max_labels);
            // create prior space
            gcap->priors = (float *)calloc(max_labels, sizeof(float));
            if (!gcap->priors)
              ErrorExit(ERROR_NOMEMORY, "GCANalloc: couldn't allocate priors to %d", max_labels);
          }
        }
      }
    }
...
/* allocate new ones */
      ...
      if (!gcan->gcs)
        ErrorExit(ERROR_NOMEMORY, "GCANupdateNode: couldn't expand gcs to %d", gcan->max_labels);
      gcan->labels = (unsigned short *)calloc(gcan->max_labels, sizeof(unsigned short));
      if (!gcan->labels)
          ...
          if ((gca->flags & GCA_NO_MRF) == 0)
          {
            for (i = 0; i < GIBBS_NEIGHBORS; i++)
            {
              for (j = 0; j < gc->nlabels[i]; j++)
              {
                ...
                check_finite("GCAcompleteMeanTraining: "
                             "label_priors",
                             gc->label_priors[i][j]);
              }
            }
...
#define MIN_GC_SAMPLES(gca) ((gca->ninputs * (gca->ninputs + 1)) / 2)
          if (gc->means[0] < 0 || nsamples < MIN_GC_SAMPLES(gca))
          {
            ...
            gc_nbr = GCAfindClosestValidGC(gca, x, y, z,
                                           gcan->labels[n], 1) ;
            if (!gc_nbr)
            {
              ErrorPrintf(ERROR_BADPARM,
    ...
    for (y = 0; y < height; y++)
    {
      for (z = 0; z < depth; z++)
      {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn, &zn))
        {
          ...
          {
#if 0
            p = GCAcomputePosteriorDensity(gcap, gcan, -1, n, vals,
    ...
    for (y = 0; y < height; y += prior_stride)
    {
      ...
      for (z = 0; z < depth; z += prior_stride)
      {
        ...
        if (priors[4] > .5 * min_prior || /* left lat ven */
            priors[5] > .5 * min_prior
            || /* inf left lat ven */
            priors[14] > .5 * min_prior
            || /* 3rd ven */
            priors[15] > .5 * min_prior
            || /* 4th ven */
...
#else
            gcaFindClosestMeanSample(gca, 255, min_prior, x, y, z, 0, prior_stride / 2, &gcas[nfound]);
            if (means[0] > 100)
#endif
            {
              int xv, yv, zv;
...
static int gcaFindBestSample(GCA *gca, int x, int y, int z, int label, int wsize, GCA_SAMPLE *gcas)
{
  int xk, yk, zk, xi, yi, zi, width, height, depth, n,
      // best_n,
      best_x, best_y, best_z, xn, yn, zn, r, c, v;
  GCA_PRIOR *gcap;
  GC1D *gc, *best_gc;
  float max_prior,
      // min_var,
...
static int GCAupdateNodeGibbsPriors(GCA *gca, MRI *mri, int xn, int yn, int zn, int xl, int yl, int zl, int label)
{
  ...
  for (i = 0; i < GIBBS_NEIGHBORHOOD; i++)
  {
    ...
    if (n >= gc->nlabels[i]) /* not there - reallocate stuff */
    {
      ...
      if (!gc->labels[i])
        ErrorExit(ERROR_NOMEMORY, "GCANupdateNode: couldn't expand labels to %d", gc->nlabels[i] + 1);
                              ...
                              int max_iter)
{
  ...
  GCA_NODE *gcan ;
  GC1D     *gc ;
  double   dist, max_p, p, prior, ll, lcma = 0.0, new_posterior, old_posterior ;
...
#if 0
  {
    ...
    if (cp)
    {
      ...
      if (mri_cma)
      {
        ...
        lcma = GCAgibbsImageLogPosterior(gca, mri_cma, mri_inputs, lta) ;
...
MRI *GCAanneal(MRI *mri_inputs, GCA *gca, MRI *mri_dst, TRANSFORM *transform, int max_iter, double prior_factor)
{
  int x, y, z, width, height, depth, label,
      // val,
      iter, xn, yn, zn, n, nchanged, index, nindices, old_label;
  short *x_indices, *y_indices, *z_indices;
  GCA_NODE *gcan;
    ...
    GCA *gca, MRI *mri_labels, MRI *mri_inputs, int x, int y, int z, TRANSFORM *transform, double gibbs_coef)
{
  ...
  for (i = 0 ; i < GIBBS_NEIGHBORS ; i++)
  {
    ...
    log_posterior =
      GCAvoxelGibbsLogPosterior(gca, mri_labels, mri_inputs, xnbr, ynbr,
...
// just check
  ...
  if (tmp > log_posterior)
  {
    printf("GCAvoxelLogPosterior: (%d, %d, %d)\n", x, y, z);
    printf("label %s(%d) log_posterior %.2f is less"
           " than no label found %.2f\n",
           cma_label_to_name(label), label, log_posterior, tmp);
  }
#endif
...
MRI *GCAexpandVentricle(GCA *gca, MRI *mri_inputs, MRI *mri_src, MRI *mri_dst, TRANSFORM *transform, int target_label)
{
  int nchanged, x, y, z,
      //  width, height, depth,
      xn, yn, zn, xk, yk, zk, xi, yi, zi, label, total_changed, i, j, count, found, xmin, xmax, ymin, ymax, zmin, zmax;
  // GCA_NODE *gcan ;
  GC1D *gc;
...
MRI *GCAexpandVentricle(GCA *gca, MRI *mri_inputs, MRI *mri_src, MRI *mri_dst, TRANSFORM *transform, int target_label)
{
  int nchanged, x, y, z, width, height, depth, xn, yn, zn, xi, yi, zi, xk, yk, zk, label, total_changed, i;
  GCA_NODE *gcan;
  GC1D *gc, *gc_vent;
                    ...
                    /* should it be changed? */
                    {
                      if (!GCAsourceVoxelToNode(gca, mri_dst, transform, xi, yi, zi, &xn, &yn, &zn))
                      {
                        ...
                        if (pv > 5 * plabel)
                        {
                          if (xi == Ggca_x && yi == Ggca_y && zi == Ggca_z)
                          {
                            int olabel = nint(MRIgetVoxVal(mri_tmp, xi, yi, zi, 0));
                            printf("GCAexpandVentricle:voxel"
                                   "(%d, %d, %d) changed from %s (%d) "
      ...
      for (y = 0; y < height; y++)
      {
        for (x = 0; x < width; x++)
        {
              ...
              || label == Right_Cerebral_White_Matter)
          {
            ...
            gc_gm = GCAfindSourceGC(
                gca, mri_src, transform, x, y, z, left ? Left_Cerebral_Cortex : Right_Cerebral_Cortex);
            if (gc_gm)
              ...
              /* might change to wm */
              {
                if (wdist < ldist && GCAisPossible(gca, mri_inputs, wm_nbr, transform, x, y, z, 0))
                {
                  if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
                  {
                    ...
                    if (olabel != wm_nbr)
                      printf("GCAexpandCortex:voxel (%d, %d, %d)"
                             " changed from %s (%d) "
          ...
          if (borderVoxel(mri_dst, x, y, z))
          {
                ...
                && (label == Ggca_label || nint(MRIgetVoxVal(mri_tmp, x, y, z, 0)) == Ggca_label || Ggca_label < 0))
            {
              ...
              if (label != nint(MRIgetVoxVal(mri_dst, x, y, z, 0)))
                printf("MLE (%d, %d, %d): old label %s (%d), "
                       "new label %s (%d)\n",
                       x,
                       y,
                       z,
    ...
    GCA *gca, MRI *mri_T1, MRI *mri_PD, MRI *mri_labeled, TRANSFORM *transform, const char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorExit(ERROR_NOFILE, "GCAhistogramTissueStatistics: could not open %s", fname);
      ...
      for (y = 0; y < mri_T1->height; y++)
      {
        ...
        for (x = 0; x < mri_T1->width; x++)
        {
          ...
          if (transform)
          {
            ...
            if ((xp < 0 || xp >= mri_T1->width) || (yp < 0 || yp >= mri_T1->height) || (zp < 0 || zp >= mri_T1->depth))
            {
              continue;
            }
          }
                                                             ...
                                                             float pthresh)
{
  int i,
      // width, height, depth,
      x, y, z, xi, yi, zi, xk, yk, zk, whalf, r, c, v;
  int nsamples = 0;
  GCA_SAMPLE *gcas;
  GCA_PRIOR *gcap;
  GC1D *gc;
...
int GCArenormalizeLabels(MRI *mri_in, MRI *mri_labeled, GCA *gca, TRANSFORM *transform)
{
  ...
  for (label = 1; label <= biggest_label; label++)
  {
    ...
    {
      index = ordered_indices[i];
      if (!GCApriorToSourceVoxel(gca, mri_means, transform, gcas[index].xp, gcas[index].yp, gcas[index].zp, &x, &y, &z))
      {
        val = MRIgetVoxVal(mri_in, gcas[index].x, gcas[index].y, gcas[index].z, 0);
        if (x == 19 && y == 14 && z == 15)
        {
...
int GCArenormalizeIntensities(GCA *gca, int *labels, float *intensities, int num)
{
  ...
  if (scales == NULL)
    ErrorExit(ERROR_NOMEMORY, "GCArenormalizeIntensities: could not alloc %d elt array");
...
GC1D *alloc_gcs(int nlabels, int flags, int ninputs)
{
  ...
  if (gcs == NULL)
    ErrorExit(ERROR_NOMEMORY, "alloc_gcs(%d, %x): could not allocated %d gcs", nlabels, flags, nlabels);
  for (i = 0; i < nlabels; i++)
  {
        ...
        */
        ...
        if (n < 0) // no transform - compute one
        {
          ...
          else
          {
            ...
            {
              if (m_by_label[Left_Cerebral_White_Matter])
                m_L = MatrixCopy(m_by_label[Left_Cerebral_White_Matter], m_L);
            }
        ...
        for (zn = 0; zn < gca->node_depth; zn++)
        {
          ...
          for (i = 0; i < gcan->nlabels; i++)
          {
            ...
            if (ranks_before[i].label != ranks_after[i].label)
            {
              ...
              if ((xn == Gx && yn == Gy && zn == Gz)
                  && (ranks_after[i].label == gcan->labels[i] || ranks_after[j].label == gcan->labels[j]
                      || Ggca_label < 0))
              {
                printf("ordering of labels %s and %s changed, "
                       "modifying means by %2.0f (%2.1f) "
                                                       ...
                                                       int *plabel_computed)
{
  ...
  for (j = 0; j < NALIGN_LABELS; j++)
  {
    ...
    */
    if (transform->type != MORPH_3D_TYPE)
      ErrorExit(ERROR_UNSUPPORTED, "Long renorm must use 3d morph\n");
...
{
  HISTOGRAM *h, *hsmooth;
  int l, nbins, i, x, y, z, max_p_label, xn, yn, zn, num, frame, bin, n,
      // label,
      c, max_label;
...
static int gcaRelabelSegment(GCA *gca, TRANSFORM *transform, MRI *mri_inputs, MRI *mri_dst, MRI_SEGMENT *mseg)
{
  ...
  for (max_posterior = 0.0, old_label = max_label = i = 0; i < mseg->nvoxels; i++)
  {
    ...
    if (i == 0)
      old_label = max_label = MRIgetVoxVal(mri_dst, x, y, z, 0); // current label
    gcap = getGCAP(gca, mri_dst, transform, x, y, z);
    for (n = 0; n < gcap->nlabels; n++)
    {
...
gcaComputeHistogramNormalization(GCA *gca, HISTOGRAM *h_mri, int label)
{
  ...
  h_gca = gcaGetLabelHistogram(gca, label, 0, 0) ;
          ...
          if (label == wm_label && pwmsa > pwm)
          {
            ...
            if ((wm_dist > wmsa_dist) && (wm_mdist > wmsa_mdist))
            {
              ...
              */
              NotWMSA = 0;
              for (n = 0; n < mri_inputs->nframes; n++)
                if (fabs(VECTOR_ELT(v_dif_wmsa, n + 1)) >= fabs(VECTOR_ELT(v_dif_label, n + 1)))
                  NotWMSA = 1;
...
#define WHALF ((WSIZE - 1) / 2)
            ...
            else if (label == wm_label)
            {
              ...
              for (n = 0; n < mri_inputs->nframes; n++)
                if (fabs(VECTOR_ELT(v_dif_wmsa, n + 1)) >= fabs(VECTOR_ELT(v_dif_label, n + 1)))
                  NotWMSA = 1;
              if (!NotWMSA || (wmsa_dist * 3 < wm_dist))
                label = wmsa_label;
            }
...
{
  int x, y, z, width, height, depth, xn, yn, zn, n, label /*, found*/, xp, yp, zp;
  GCA_NODE *gcan;
  GCA_PRIOR *gcap;
  GC1D *gc;
  ...
  */
  ...
  for (total_log_p = 0.0, x = 0; x < width; x++)
  {
    for (y = 0; y < height; y++)
    {
      for (z = 0; z < depth; z++)
      {
        ...
        log_p = gcaComputeLogDensity(gc, vals, gca->ninputs, getPrior(gcap, label), label);
        total_log_p += log_p;
        if (log_p < min_log_p)
        {
          min_log_p = log_p;
  ...
  total_log_p /= (double)(mri_inputs->width * mri_inputs->height * mri_inputs->depth);
  return (total_log_p);
}
#endif
...
#else
static int entropy_labels[] = { Right_Cerebral_White_Matter,
                                Left_Cerebral_White_Matter,
                                Left_Cerebral_Cortex,
                                Right_Cerebral_Cortex,
                                Left_Caudate,
      ...
      for (z = 0; z < gca->node_depth; z++)
      {
        ...
        for (n = 0; n < gcan->nlabels; n++)
        {
          ...
          case LH_CLASS:
            ...
            break ;
          case RH_CLASS:
            same_class = IS_RH(gcan->labels[n]) ;
            break ;
#endif
          case CSF_CLASS:
                                 ...
                                 float sigma)
{
  ...
  int xp, yp, zp, xn, yn, zn, width, height, depth, label, n; // max_n,
            ...
            if (d_old > mah_dist_thresh)
            {
              ...
              if (max_gc)
              {
                ...
                //                if (d_new < mah_dist_thresh/2)
                {
                  ...
                  if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
                    printf("GCAreclassifyUnlikelyVoxels: "
                           "(%d, %d, %d) %s (%d) --> %s (%d)\n",
                           x,
                           y,
                           z,
        ...
        for (z = 0; z < depth; z++)
        {
          ...
          if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn, &zn))
          {
            ...
            if (max_gc && (max_label != label))
            {
              ...
              if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
                printf("GCAreclassifyVoxelsAtOptimalScale: "
  ...
  for (z = 0; z < mri->depth; z++)
  {
    ...
    for (y = 0; y < mri->height; y++)
    {
      for (x = 0; x < mri->width; x++)
      {
        ...
        for (f = 0; f < gca->ninputs; f++)
          MRIsetVoxVal(out, x, y, z, f, 0);
        if (seg)
          MRIsetVoxVal(*seg, x, y, z, 0, 0);
        ...
        if (max_n < 0)
          continue; /* couldn't find any valid label at this location */

        // Set the output
        gc = &gcan->gcs[max_n];
        for (f = 0; f < gca->ninputs; f++)
          MRIsetVoxVal(out, x, y, z, f, gc->means[f]);
        if (seg)
          MRIsetVoxVal(*seg, x, y, z, 0, gcan->labels[max_n]);
      }
    }
  }
...
int MRIwmsaHalo(MRI *mri_inputs, MRI *mri_labeled, int n)
{
  ...
  int h, label,
      // nwm, wm_label,
      nwmsa, ngm, x, y, z, wmsa_label, gm_label, a, b, changed;
        ...
        for (y = 0; y < mri_labeled->height; y++)
        {
          for (z = 0; z < mri_labeled->depth; z++)
          {
            ...
            if (IS_WM(label))
            {
              ...
              if ((VECTOR_ELT(v_vals, 1) < VECTOR_ELT(wm_means, 1)) && (VECTOR_ELT(v_vals, 2) > VECTOR_ELT(wm_means, 2))
