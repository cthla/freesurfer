...
int LPAFreadImageAnswer(LPAF *lpaf, int current)
{
  ...
  fp = fopen(fname, "rb");
  if (!fp)
    ErrorReturn(-1, (ERROR_NO_FILE, "LPAFreadImageAnswer(%d): could not open %s", current, fname));

  ecode = fread_header(fp, &Iheader, fname);
  fclose(fp);
  if (ecode)
    ErrorReturn(-2, (ERROR_BADFILE, "LPAFreadImageAnswer(%s): could not read header", fname));
...
*/
  ...
  if (lpb->xc < 0 || lpb->xc >= Iheader.cols || lpb->yc < 0 || lpb->xc >= Iheader.rows)
    return (0);
...
int LPAFresetImageAnswer(LPAF *lpaf, int current)
{
  ...
  infp = fopen(fname, "rb");
  if (!infp)
    ErrorReturn(-1, (ERROR_NO_FILE, "LPAFwriteImageAnswer(%d): could not open %s", current, fname));
  ecode = fread_header(infp, &Iheader, fname);
  if (ecode)
  {
    fclose(infp);
    ErrorReturn(-2, (ERROR_BADFILE, "LPAFwriteImageAnswer(%d): could not read header", current));
  }
  fclose(infp);
...
LP_ANSWER_FILE *LPAFcreate(char *out_fname, int argc, char *argv[])
{
  ...
  if (out_fname) /* write output to file as well as into image file */
  {
    ...
    if (!lpaf->fp)
      ErrorReturn(NULL, (ERROR_NO_FILE, "LPAFcreate: could not open %s", out_fname));
  }
  else
    lpaf->fp = NULL;
...
int LPAFread(LPAF *lpaf, int current)
{
  ...
  if (LPAFreadImageAnswer(lpaf, current) <= 0) /* not in image file */
  {
    ...
    if (fseek(lpaf->fp, lpb->fpos, SEEK_SET) < 0)
      ErrorReturn(-1, (ERROR_BADFILE, "LPAFread could not seek to %ld", lpb->fpos));
    ...
    if (sscanf(cp,
               ...
               &lpb->yp[3])
        != 11)
      ErrorReturn(-1, (ERROR_BADFILE, "LPAFread: could not scan all parms from %s", cp));
  }
...
static void lpafAllocParms(IMAGE *I)
{
  ...
  if (!params)
    ErrorExit(ERROR_NO_MEMORY, "lpafAllocParms: could not allocate (%d, %d)", nframes, sizeof(*params));
  I->paramdealloc = TRUE;
