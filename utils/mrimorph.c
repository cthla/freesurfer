...
static double
ltaGradientStep(MRI *mri_in, MRI *mri_ref, LTA *lta, double dt, double momentum, NECK_PARMS *np, double tmul);
...
static int
ComputeStepTransform(VOXEL_LIST *vl_source, VOXEL_LIST *vl_target, float cx, float cy, float cz, MATRIX *Minc);
...
int MatrixPrintHires(FILE *fp, MATRIX *mat)
{
  ...
  if (mat == NULL)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "MatrixPrintHires: mat = NULL!"));
...
MRI *MRIfindNeck(MRI *mri_src, MRI *mri_dst, int thresh_low, int thresh_hi, MP *parms, int dir, NECK_PARMS *np)
{
  ...
  if (spinal_label < 0)
    ErrorReturn(NULL, (ERROR_BADPARM, "MRIfindNeck: could not find spinal chord"));
...
static int mriLinearAlignPyramidLevel(MRI *mri_in, MRI *mri_ref, MORPH_PARMS *parms)
{
  ...
  if (parms->mri_in == NULL)
    parms->mri_in = mri_in;
  if (parms->mri_ref == NULL)
...
#define MAX_TRANSLATION_DEL 0.25
      for (r = 1; r <= 3; r++)
      {
        for (c = 1; c <= 3; c++)
        {
          if (fabs(*MATRIX_RELT(lt->m_dL, r, c)) > max_val)
...
static int openLogFile(MORPH_PARMS *parms)
{
  ...
  if ((Gdiag & DIAG_WRITE) && (parms->log_fp == NULL))
  {
    ...
    if (!parms->log_fp)
      ErrorExit(ERROR_NOFILE, "%s: could not open log file %s", Progname, fname);
    ...
    if (!FZERO(parms->l_priors))
      fprintf(parms->log_fp, "l_priors = %2.4f\n", parms->l_priors);
    if (!FZERO(parms->l_dist))
      fprintf(parms->log_fp, "l_dist = %2.4f\n", parms->l_dist);
    if (!FZERO(parms->l_compression))
      fprintf(parms->log_fp, "l_compression = %2.4f\n", parms->l_compression);
    ...
    if (!FZERO(parms->l_nldist))
      fprintf(parms->log_fp, "l_nldist = %2.4f\n", parms->l_nldist);
...
static int
m3dCompressionTerm(MORPH_3D *m3d, double l_compression, int i, int j, int k, double *pdx, double *pdy, double *pdz);
static int
m3dNonlinearDistanceTerm(MORPH_3D *m3d, double l_nldist, int i, int j, int k, double *pdx, double *pdy, double *pdz);
...
static int m3dCorrelationTerm(MRI *mri_in,
                              ...
                              double *pdz);
static int
m3dDistanceTerm(MORPH_3D *m3d, double l_distance, int i, int j, int k, double *pdx, double *pdy, double *pdz);
...
static double
m3dIntegrationStep(MRI *mri_in, MRI *mri_ref, MRI *mri_ref_blur, MORPH_PARMS *parms, MORPH_3D *m3d, double dt);
  ...
  for (sse = 0.0f, x = 0; x < m3d->width; x++)
  {
    ...
    for (y = 0; y < m3d->height; y++)
    {
      ...
      for (z = 0; z < m3d->depth; z++)
      {
        ...
        if (x == 3 * width / 4 && y == 3 * height / 4 && z == 3 * depth / 4)
          DiagBreak();
        if (DZERO(ref_std))
...
static double m3dDistanceSSE(MRI *mri_in, MRI *mri_ref, MORPH_3D *m3d)
{
  ...
  MNP *mnp;
  double dist, xd, yd, zd, delta, sse;
  // double node_spacing;
...
static double m3dNonlinearAreaSSE(MRI *mri_in, MRI *mri_ref, MORPH_3D *m3d, MORPH_PARMS *parms)
{
  double sse, delta, ratio, exponent;
  // double  n3;
...
static int m3dAllocProperties(MORPH_3D *m3d)
{
  ...
  if (!m3d->pnodes)
    ErrorExit(ERROR_NOMEMORY, "m3dAllocProperties: could not allocate node width");
  for (x = 0; x < width; x++)
  {
    m3d->pnodes[x] = (MORPH_NODE_PROPERTIES **)calloc(height, sizeof(MORPH_NODE_PROPERTIES *));
    if (!m3d->pnodes)
      ErrorExit(ERROR_NOMEMORY, "m3dAllocProperties: could not allocate node height %d", x);
    for (y = 0; y < height; y++)
    {
      m3d->pnodes[x][y] = (MORPH_NODE_PROPERTIES *)calloc(depth, sizeof(MORPH_NODE_PROPERTIES));
      if (!m3d->pnodes)
        ErrorExit(ERROR_NOMEMORY, "m3dAllocProperties: could not allocate node depth %d,%d", x, y);
    }
  }
  return (NO_ERROR);
}
...
static MORPH_3D *m3dalloc(int width, int height, int depth, float node_spacing)
{
  ...
  if (!m3d)
    ErrorExit(ERROR_NOMEMORY, "m3dalloc: could not allocate m3d");
  ...
  if (!m3d->nodes)
    ErrorExit(ERROR_NOMEMORY, "m3dalloc: could not allocate node width");
  for (x = 0; x < width; x++)
  {
    m3d->nodes[x] = (MORPH_NODE **)calloc(height, sizeof(MORPH_NODE *));
    if (!m3d->nodes)
      ErrorExit(ERROR_NOMEMORY, "m3dalloc: could not allocate node height %d", x);
    for (y = 0; y < height; y++)
    {
      m3d->nodes[x][y] = (MORPH_NODE *)calloc(depth, sizeof(MORPH_NODE));
      if (!m3d->nodes)
        ErrorExit(ERROR_NOMEMORY, "m3dalloc: could not allocate node depth %d,%d", x, y);
    }
  }
...
static MORPH_3D *m3dInit(MRI *mri_in, MRI *mri_ref, MORPH_PARMS *parms, float node_spacing)
{
  ...
  m3d->nodes = (MORPH_NODE ***)calloc(width, sizeof(MORPH_NODE **));
  if (!m3d->nodes)
    ErrorExit(ERROR_NOMEMORY, "m3dInit: could not allocate node width");
  for (x = 0; x < width; x++)
  {
    m3d->nodes[x] = (MORPH_NODE **)calloc(height, sizeof(MORPH_NODE *));
    if (!m3d->nodes)
      ErrorExit(ERROR_NOMEMORY, "m3dInit: could not allocate node height %d", x);
    for (y = 0; y < height; y++)
    {
      m3d->nodes[x][y] = (MORPH_NODE *)calloc(depth, sizeof(MORPH_NODE));
      if (!m3d->nodes)
        ErrorExit(ERROR_NOMEMORY, "m3dInit: could not allocate node depth %d,%d", x, y);
    }
  }
...
MORPH_3D *MRI3Dmorph(MRI *mri_in, MRI *mri_ref, MORPH_PARMS *parms)
{
  ...
  char base_name[STRLEN];
  double base_sigma, sigma, base_tol, max_thick;
  // double dt, base_intensity;
...
static int m3dAreaTerm(MORPH_3D *m3d, double l_area, int i, int j, int k, double *pdx, double *pdy, double *pdz)
{
  ...
  MNP *mnp;
  // MNP *mnpi, *mnpj, *mnpk;
  float delta, n3 /*, node_spacing, odx, ody, odz*/;
    ...
    MORPH_3D *m3d, double l_nlarea, int i, int j, int k, double *pdx, double *pdy, double *pdz, MORPH_PARMS *parms)
{
  MORPH_NODE *mn, *mni, *mnj, *mnk;
  MNP *mnp;
  float delta, delta_scale, ratio, n3;
  // float node_spacing;
  int n, width, height, depth, num, invert;
  static VECTOR *v_i = NULL, *v_j, *v_k, *v_k_x_j, *v_j_x_i, *v_i_x_k, *v_grad, *v_tmp;
  double exponent;
  ...
  for (n = parms->start_t; n < parms->start_t + parms->niterations; n++)
  {
    ...
    if ((((n + 1) - parms->start_t) % AVG_LEN) == 0)
    {
      ...
      fprintf(stdout,
              ...
              current_avg,
              (last_avg - current_avg) / current_avg);
      if ((((last_avg - current_avg) / current_avg < parms->tol) || FZERO(parms->tol)) && (last_neg <= m3d->neg))
        break;
      last_neg = m3d->neg;
...
int MRI3Dwrite(MORPH_3D *m3d, char *fname)
{
  ...
  fp = fopen(fname, "wb");
  if (!fp)
    ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "mris3Dwrite: could not open file %s", fname));

  if (fwriteInt(M3D_MAGIC, fp) != 1)
    ErrorReturn(ERROR_BADFILE, (ERROR_BADFILE, "m3dwrite: fwrite failed"));
...
MORPH_3D *MRI3DreadSmall(char *fname)
{
  MORPH_3D *m3d;
  FILE *fp;
  int i, j, k, width, height, depth, magic;
  // int version;
  float node_spacing;
  MORPH_NODE *mn;
  char buf[UNUSED_SPACE_SIZE + 1];
  ...
  fp = fopen(fname, "rb");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NOFILE, "MRI3DreadSmall: could not open file %s", fname));
  ...
  /* so stuff can be added to the header in the future */
  if (fread(buf, sizeof(char), UNUSED_SPACE_SIZE, fp) != UNUSED_SPACE_SIZE)
  {
    ErrorReturn(NULL, (ERROR_BADFILE, "Could not read unused space"));
  }
...
MRI_SURFACE *MRISshrinkWrapSkull(MRI *mri, MORPH_PARMS *parms)
{
  ...
  float radius, max_radius, x0, y0, z0, sx, sy, sz, thick;
  // float scale;
  ...
  if (ncalls == 1 && (Gdiag & DIAG_WRITE) && DIAG_VERBOSE_ON)
    MRISwrite(mris, "outer_skin1.geo");
  lparms.l_intensity = 0.015;
  lparms.niterations = 200;
  MRISpositionSurface(mris, mri, mri_smooth, &lparms); /* outer skin */
  if (ncalls == 1 && (Gdiag & DIAG_WRITE) && DIAG_VERBOSE_ON)
    MRISwrite(mris, "outer_skin2.geo");
  ...
  if (ncalls == 1 && (Gdiag & DIAG_WRITE) && DIAG_VERBOSE_ON)
    MRISwrite(mris, "inner_skin_expanded.geo");
  ...
  MRISpositionSurface(mris, mri, mri_smooth, &lparms);
  if (ncalls == 1 && (Gdiag & DIAG_WRITE) && DIAG_VERBOSE_ON)
    MRISwrite(mris, "inner_skin_smoothed.geo");
  Gdiag = diag;
...
{
  int n, num;
  double dx, dy, dz, dist, avg_expansion;
  ...
  for (num = n = 0; n < NEIGHBORS; n++)
  {
    ...
    if (mn_nbr)
    {
      ...
      dist = sqrt(dx * dx + dy * dy + dz * dz);
      num++;
      if (!FZERO(mnp->orig_dist[n]))
        avg_expansion += dist / mnp->orig_dist[n];
    }
  }
...
static double m3dCompressionSSE(MRI *mri_in, MRI *mri_ref, MORPH_3D *m3d)
{
  ...
  MORPH_NODE *mn, *mn_nbr;
  MNP *mnp;
  double dist, xd, yd, zd, delta, sse, expansion;
  // double node_spacing;
...
int MRIeraseNeck(MRI *mri, NECK_PARMS *np)
{
  ...
  // dy = np->neck_dy;
  // dz = np->neck_dz;
  for (y = y0; y < mri->height + 25; y++)
    MRIerasePlane(mri, x0, y, z0, np->neck_dx, np->neck_dy, np->neck_dz, 0);
...
static int m3RecomputeTranslation(MORPH_3D *m3d, MRI *mri_in, MRI *mri_ref, MORPH_PARMS *parms)
{
  ...
  if (Gdiag & DIAG_SHOW)
    fprintf(stdout, "recomputing translation values based on skull morph...\n");
...
static int m3dMorphSkull(MORPH_3D *m3d, MRI_SURFACE *mris_in_skull, MRI_SURFACE *mris_ref_skull, MRI *mri)
{
  ...
  MNP *mnp;
  int i, j, k, width, height, depth, n;
  float ref_x0, ref_y0, ref_z0, scale, nx, ny, nz, ref_v_x0, ref_v_y0, ref_v_z0, thick, ref_dist, in_dist;
  // in_x0, in_y0, in_z0, float dx, dy, dz;
  ...
  for (i = nlevels - 1; i >= 0; i--)
  {
    ...
    if (parms->m_xforms)
    {
      ...
      if (parms->m_xform_covariance)
        MatrixFree(&parms->m_xform_covariance);
      if (parms->m_inv_cov)
        MatrixFree(&parms->m_inv_cov);
      ...
      if (!parms->m_inv_cov)
        ErrorExit(ERROR_BADPARM, "%s: could not invert covariance matrix", Progname);
    }
...
{
#if 1
  int row, col, i, xn, yn, zn, xv, yv, zv, n, ndets = 0;
  // int width, height, depth;
  VECTOR *v_X, *v_means, *v_X_T; /* original and transformed coordinate systems */
  // VECTOR *v_Y;
...
int MRIquasiNewtonAlignVolumes(MRI *mri_in, MRI *mri_ref, MORPH_PARMS *parms, MATRIX *m_L)
{
  ...
  if ((Gdiag & DIAG_WRITE) && parms->log_fp)
    fprintf(parms->log_fp, "aligning input volume to reference.\n");
...
static double compute_label_likelihood(VOXEL_LIST *vl_source, VOXEL_LIST *vl_target, MATRIX *m_L)
{
  int x, y, z, width, height, depth, i;
