...
MRI_SURFACE *GWU_make_surface_from_lists(GWUTILS_VERTEX *vertices, int vertexcount, GWUTILS_FACE *faces, int facecount)
{
  ...
  for (vno = 0; vno < vertexcount; vno++)
  {
    ...
    if (!v->v)
      ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %dth vertex list.", __func__, vno);
    v->vnum = 0;
  }
  ...
  for (vno = 0; vno < vertexcount; vno++)
  {
    ...
    if (!v->f)
      ErrorExit(ERROR_NO_MEMORY, "%s: could not allocate %d faces", __func__, v->num);
    v->n = (uchar *)calloc(v->num, sizeof(uchar));
    if (!v->n)
      ErrorExit(ERROR_NO_MEMORY, "%s: could not allocate %d nbrs", __func__, v->n);
