...
static void XMLCALL cb_start_doctype(void *, const char *, const char *, const char *, int);
...
static int ewrite_text_ele(int, const char *, const char *, int, int, FILE *);
static int ewrite_coordsys(gxml_data *, giiCoordSystem *, FILE *);
static int ewrite_data(gxml_data *, giiDataArray *, FILE *);
static int ewrite_data_line(void *, int, long long, long long, int, FILE *);
...
static char *enames[GXML_MAX_ELEN] = { "Invalid",
                                       "GIFTI",
                                       "MetaData",
                                       "MD",
                                       "Name",
...
gifti_image *gxml_read_image(const char *fname, int read_data, const int *dalist, int dalen)
{
  ...
  if (xd->verb > 1)
  {
    ...
    if (xd->da_list)
      fprintf(stderr, "   (length %d DA list)\n", xd->da_len);
  ...
  if (xd->verb > 1)
  {
    if (xd->gim)
      fprintf(stderr,
              ...
              fname,
              xd->gim->numDA,
              gifti_gim_DA_size(xd->gim, 1));
    else
      fprintf(stderr, "** gifti image '%s', failure\n", fname);
  }
...
int gxml_write_image(gifti_image *gim, const char *fname, int write_data)
{
  ...
  if (GXD.verb > 1)
  {
    fprintf(stderr, "++ writing gifti image (%s data) to '%s'", write_data ? "with" : "no", fname);
    if (write_data)
      fprintf(stderr, " (%d DA elements = %lld MB)", gim->numDA, gifti_gim_DA_size(gim, 1));
    fputc('\n', stderr);
  }

#ifndef HAVE_ZLIB /* if the data is to be compressed, fail */
  if (gim->darray && gim->darray[0] && gim->darray[0]->encoding == GIFTI_ENCODING_B64GZ)
  {
...
{
  if (val)
    GXD.dstore = 1; /* includes -1 as applying the default */
  else
    GXD.dstore = 0;
  return 0;
}
...
{
  if (val == -1)
    GXD.update_ok = 1; /* default is yes */
  else if (val >= 0)
    GXD.update_ok = val;
  else
    return 1; /* failure - no action */
...
static int init_gxml_data(gxml_data *dp, int doall, const int *dalist, int len)
{
  ...
  if (doall)
  { /* user modifiable - init all to defaults */
    dp->verb = 1;
    dp->dstore = 1;
    dp->indent = 3;
  ...
  if (dalist && len > 0)
  {
    if (short_sorted_da_list(dp, dalist, len))
      errs++; /* continue */
  }
  else
  {
...
{
  if (show)
    fprintf(fp, "%*s %02d ", 3 * depth, "", depth);
  else
    fprintf(fp, "%*s    ", 3 * depth, "");
}
...
static int epush(gxml_data *xd, int etype, const char *ename, const char **attr)
{
  ...
  if (xd->verb > 4)
  { /* maybe we want to print something */
...
static int push_gifti(gxml_data *xd, const char **attr)
{
  ...
  if (xd->verb > 1)
    fprintf(stderr, "++ set %d GIFTI attr(s)\n", c / 2);
  if (xd->verb > 3)
    gifti_disp_gifti_image("push:", gim, 0);
...
static int get_label_attrs(gxml_data *xd, const char **attr, int *key, float *rgba)
{
  giiLabelTable *lt = &xd->gim->labeltable;
  const char **aptr;
  ...
  {
    if (found == 1)
      fprintf(stderr, "-- have Label Key %d\n", *key);
    else
      fprintf(stderr, "-- have Label Key %d, RGBA %g %g %g %g\n", *key, rgba[0], rgba[1], rgba[2], rgba[3]);
  }
...
static int push_darray(gxml_data *xd, const char **attr)
{
  ...
  if (xd->da_list)
  {
    if ((xd->da_ind < xd->da_len) && (xd->da_list[xd->da_ind] == xd->eleDA - 1)) /* keeper */
    {
      if (xd->verb > 1)
        fprintf(stderr, "++ keeping DA[%d]\n", xd->eleDA - 1);
      xd->da_ind++;
    }
    else
    {
      if (xd->verb > 1)
        fprintf(stderr, "++ skipping DA[%d]\n", xd->eleDA - 1);
      xd->skip = xd->depth;
      return 1; /* return and skip this element */
    }
  }

  if (gifti_add_empty_darray(xd->gim, 1))
    return 1;

  da = xd->gim->darray[xd->gim->numDA - 1]; /* get new pointer */

  /* fill the struct from the attributes (store ex_atrs) */
  if (gifti_set_DA_atrs(da, attr, 0, 1))
    return 1;
  ...
  if (xd->verb > 4)
    gifti_disp_DataArray("push:", da, 0);
...
static int pop_darray(gxml_data *xd)
{
  ...
  if (xd->b64_errors > 0)
  {
    ...
    else if (xd->b64_check == GIFTI_B64_CHECK_COUNT || xd->b64_check == GIFTI_B64_CHECK_SKIPNCOUNT)
...
static int push_dspace(gxml_data *xd)
{
  ...
  if (CSind < 0)
  {
    fprintf(stderr, "** PD: bad numCS %d in darray %d, skipping...", CSind + 1, xd->gim->numDA - 1);
    xd->skip = xd->depth;
    return 1;
  }
...
static int push_xspace(gxml_data *xd)
{
  ...
  if (CSind < 0)
  {
    fprintf(stderr, "** PX: bad numCS %d in darray %d, skipping...", CSind + 1, xd->gim->numDA - 1);
    xd->skip = xd->depth;
    return 1;
  }
...
static int push_xform(gxml_data *xd)
{
  ...
  if (CSind < 0)
  {
    fprintf(stderr, "** PXform: bad numCS %d in darray %d, skipping...", CSind + 1, xd->gim->numDA - 1);
    xd->skip = xd->depth;
    return 1;
  }
...
static void XMLCALL cb_start_ele(void *udata, const char *ename, const char **attr)
{
...
static void XMLCALL cb_char(void *udata, const char *cdata, int length)
{
  ...
  {
    if (xd->verb > 3)
      fprintf(stderr, "-- skipping char [%d]\n", len);
    return;
  }

  /* act based on the parent type */
  parent = xd->stack[xd->depth - 1];
  if (parent == GXML_ETYPE_CDATA)
    parent = xd->stack[xd->depth - 2];

  if (parent != GXML_ETYPE_DATA)
    wlen = whitespace_len(str, length);
  ...
  {
    show_depth(xd->depth, 1, stderr);
    if (parent == GXML_ETYPE_DATA && len > 40)
      len = 40;
...
static int append_to_data(gxml_data *xd, const char *cdata, int len)
{
  ...
  case GIFTI_ENCODING_B64BIN:
    return append_to_data_b64(xd, (char *)da->data, da->nvals * da->nbyper, cdata, len);
  case GIFTI_ENCODING_B64GZ:
    return append_to_data_b64(xd, xd->zdata, xd->zlen, cdata, len);

  default:
    fprintf(stderr,
            "** A2D: invalid encoding value %d (%s)\n",
            da->encoding,
 ...
 */
static int append_to_data_b64(gxml_data *xd, char *dest, long long tot_bytes, const char *cdata, int cdlen)
{
  const char *cptr;
  ...
  while (rem_bytes_in > 0)
  {
    ...
    if (xd->doff > 0)
    {
      if (xd->verb > 5)
        fprintf(
            stderr, "++ A2Db64: move %d bytes from %d (blen %d)\n", xd->doff, xd->dlen - unused - xd->doff, xd->dlen);
      /* (subtract unused+1, since 1 byte is saved for null */
      memmove(xd->ddata, xd->ddata + xd->dlen - (unused + 1) - xd->doff, xd->doff);
      if (xd->verb > 6)
        fprintf(stderr, "   bytes are '%.*s'\n", xd->doff, (char *)xd->ddata);
    }
...
*/
static int copy_b64_data(gxml_data *xd, const char *src, char *dest, int src_len, int *dest_len)
{
  const unsigned char *usrc = (const unsigned char *)src;
  int c, errs = 0, apply_len;

  if (xd->verb > 1)
  { /* in verbose mode, perform automatic check */
    c = count_bad_b64_chars(src, src_len);
    if (c > 0)
    {
      fprintf(stderr, "CB64D: found %d bad b64 chars\n", c);
      if (xd->verb > 5)
        show_bad_b64_chars(src, src_len);
    }
  }
...
static int append_to_data_ascii(gxml_data *xd, const char *cdata, int len)
{
  ...
  if (whitespace_len(cdata, len) == len)
  {
    xd->doff = 0;
    return 0;
  }
  ...
  while (rem_len > 0)
  {
    ...
    if (xd->dind == 0)
      mod_prev = 0; /* nothing to modify at first */
    ...
    {
      if (xd->verb)
        fprintf(stderr, "** A2D: failed to process buffer\n");
      fprintf(stderr, "** rem = %d\n", xd->doff);
      xd->doff = 0; /* blow away the buffer and continue */
    }
...
static int append_to_xform(gxml_data *xd, const char *cdata, int len)
{
  ...
  while (rem_len > 0)
  {
    ...
    {
      if (xd->verb)
        fprintf(stderr, "** A2X: failed to process buffer\n");
      fprintf(stderr, "** rem = %d\n", xd->doff);
      xd->doff = 0; /* blow away the buffer and continue */
    }
    ...
    if (xd->doff > 0)
    {
      ...
      memmove(xd->xdata, xd->xdata + xd->xlen - (unused + 1) - xd->doff, xd->doff);
      if (xd->verb > 6)
        fprintf(stderr, "   bytes are '%.*s'\n", xd->doff, (char *)xd->ddata);
    }
...
#define GII_B64_decode4v2(w, x, y, z, a, b)                                                                            \
  (a = (b64_decode_table[w] << 2) | (b64_decode_table[x] >> 4),                                                        \
   b = (b64_decode_table[x] << 4) | (b64_decode_table[y] >> 2))
...
*/
static int decode_b64(gxml_data *xd, char *cdata, int cdlen, char *dptr, long long *needed)
{
  ...
  { /* so *needed < 3 */
    ...
    unsigned char a, b;
    GII_B64_decode4v2(din[0], din[1], din[2], din[3], a, b);
    if (*needed >= 1)
      dout[0] = a;
    if (*needed >= 2)
      dout[1] = b;
...
/* given: source pointer, remaining length, nvals desired, dest loc and type
   ...
   read failure happens only when no characters are processed
*/
static int decode_ascii(gxml_data *xd, char *cdata, int cdlen, int type, void *dptr, long long *nvals, int *mod_prev)
{
  ...
  case NIFTI_TYPE_FLOAT32:
  {
    ...
    while ((vals < 0 || vals < *nvals) && p1)
    {
      ...
      if (xd->verb > 6)
        fprintf(stderr, "  v %f (%f)", ptr[vals], dval);
      vals++; /* count new value   */
    }
    if (xd->verb > 6)
      fputc('\n', stderr);
  ...
  case NIFTI_TYPE_COMPLEX64:
  {
    ...
    while ((vals < 0 || vals < *nvals) && p1)
    {
      ...
      if (xd->verb > 6)
        fprintf(stderr, "  v %f (%f)", ptr[2 * vals], dval);
      if (piece == 1)
        vals++; /* have a complete data value */
      piece = 1 - piece;
    }
    if (xd->verb > 6)
      fputc('\n', stderr);
  ...
  case NIFTI_TYPE_INT64:
  {
    ...
    while ((vals < 0 || vals < *nvals) && p1)
    {
      ...
      if (xd->verb > 6)
        fprintf(stderr, "  v %lld (%lld)", ptr[vals], llval);
      vals++; /* count new value   */
    }
    if (xd->verb > 6)
      fputc('\n', stderr);
...
static void XMLCALL cb_xml_dec(void *udata, const char *ver, const char *enc, int standalone)
{
  gxml_data *xd = (gxml_data *)udata;
  if (xd->verb > 2)
  {
...
cb_start_doctype(void *udata, const char *doctype, const char *sysid, const char *pubid, int has_subset)
{
  ...
  if (xd->verb > 2)
  {
    ...
    fprintf(stderr,
            "start_doctype, dt='%s', sid='%s',pid='%s', sub=%d\n",
            doctype,
            sysid ? sysid : "NULL",
            pubid ? pubid : "NULL",
            has_subset);
...
static void XMLCALL cb_elem_dec(void *udata, const char *ename, XML_Content *content)
{
  gxml_data *xd = (gxml_data *)udata;
  if (xd->verb > 2)
  {
    show_depth(xd->depth, 1, stderr);
    fprintf(stderr,
            "%s: type=%d, quant=%d, name=%s, numc=%d, cp=%p\n",
            ename,
            content->type,
            content->quant,
            content->name,
...
static int stack_is_valid(gxml_data *xd)
{
  ...
  case GXML_ETYPE_GIFTI: /* should only be at depth 1 */
    ...
    break;
  case GXML_ETYPE_META:
    if (parent != GXML_ETYPE_GIFTI && parent != GXML_ETYPE_DATAARRAY)
      bad_parent = 1;
    break;
  case GXML_ETYPE_MD:
    if (parent != GXML_ETYPE_META)
      bad_parent = 1;
    break;
  case GXML_ETYPE_NAME:
    if (parent != GXML_ETYPE_MD)
      bad_parent = 1;
    break;
  case GXML_ETYPE_VALUE:
    if (parent != GXML_ETYPE_MD)
  ...
  case GXML_ETYPE_LABELTABLE:
    ...
    break;
  case GXML_ETYPE_LABEL:
    if (parent != GXML_ETYPE_LABELTABLE)
      bad_parent = 1;
    break;
  case GXML_ETYPE_DATAARRAY:
    if (parent != GXML_ETYPE_GIFTI)
    ...
    break;
  case GXML_ETYPE_CSTM:
    if (parent != GXML_ETYPE_DATAARRAY)
      bad_parent = 1;
    break;
  case GXML_ETYPE_DATA:
    if (parent != GXML_ETYPE_DATAARRAY)
  ...
  case GXML_ETYPE_MATRIXDATA:
    ...
    break;
  case GXML_ETYPE_CDATA:
    if (parent != GXML_ETYPE_NAME && parent != GXML_ETYPE_VALUE && parent != GXML_ETYPE_LABEL
        && parent != GXML_ETYPE_DATASPACE
        && parent != GXML_ETYPE_XFORMSPACE
        && parent != GXML_ETYPE_MATRIXDATA)
      bad_parent = 1;
    break;
  }
  ...
  if ((!valid || bad_parent) && GXD.verb > 1)
    show_stack("** invalid ", xd);
...
static int partial_buf_size(long long nbytes)
{
  ...
  if (ibytes <= GXML_MIN_BSIZE)
    return GXML_MIN_BSIZE;
  if (ibytes <= 64 * 1024)
    return ibytes;
...
static int update_partial_buffer(char **buf, int *blen, long long bytes, int full)
{
  int bsize = partial_buf_size(bytes);

  if (full)
    bsize = bytes; /* want entire buffer */
...
static int gxml_write_gifti(gxml_data *xd, FILE *fp)
{
  ...
  if (gim->version)
  {
    fprintf(fp, " Version=\"%s\"", gim->version);
    first = 0;
  }
  ...
  {
    if (xd->verb > 0)
      fprintf(stderr, "** gifti_image, missing darray\n");
  }
  else
  {
...
static int ewrite_darray(gxml_data *xd, giiDataArray *da, FILE *fp)
{
  ...
  if (xd->verb > 3)
    fprintf(stderr, "++ write giiDataArray\n");
  ...
  ewrite_str_attr("ArrayIndexingOrder", gifti_list_index2string(gifti_index_order_list, da->ind_ord), offset, 0, fp);
  ...
  ewrite_str_attr("Encoding", gifti_list_index2string(gifti_encoding_list, da->encoding), offset, 0, fp);
...
static int ewrite_data(gxml_data *xd, giiDataArray *da, FILE *fp)
{
  ...
  if (xd->dstore)
  {
    ...
    else if (da->encoding == GIFTI_ENCODING_EXTBIN)
    {
      ...
      if (gifti_write_extern_DA_data(da))
        errs = 1;
    }
    else
    {
...
static int gxml_disp_b64_data(const char *mesg, const void *data, int len, FILE *fp)
{
...
static int ewrite_coordsys(gxml_data *xd, giiCoordSystem *cs, FILE *fp)
{
  ...
  if (xd->verb > 3)
    fprintf(stderr, "++ write giiCoordSystem\n");
...
static int ewrite_data_line(void *data, int type, long long row, long long cols, int space, FILE *fp)
{
  int c;
  ...
  case NIFTI_TYPE_COMPLEX256:
  {
    ...
    for (c = 0; c < 2 * cols; c += 2)
      fprintf(fp, "%Lf %Lf   ", ptr[c], ptr[c + 1]);
    break;
  }
  }
...
static int ewrite_text_ele(int ele, const char *cdata, const char *attr, int spaces, int in_CDATA, FILE *fp)
{
  int index = ele;

  if (ele < 0 || ele > GXML_MAX_ELEN)
    index = 0; /* be safe */
...
static int ewrite_LT(gxml_data *xd, giiLabelTable *lt, int in_CDATA, FILE *fp)
{
  ...
  if (xd->verb > 3)
    fprintf(stderr, "++ write giiLabelTable\n");
  ...
  for (c = 0; c < lt->length; c++)
  {
    ...
    ewrite_text_ele(GXML_ETYPE_LABEL, lt->label[c], attr, spaces + xd->indent, in_CDATA, fp);
  }
  fprintf(fp, "%*s</LabelTable>\n", spaces, "");
...
static int ewrite_meta(gxml_data *xd, giiMetaData *md, FILE *fp)
{
  ...
  if (xd->verb > 3)
    fprintf(stderr, "++ write giiMetaData\n");
  ...
  {
    if (!md->name[c])
    { /* allow empty value, but not name */
      if (xd->verb > 1)
        fprintf(stderr, "** MD[%d] unset\n", c);
      continue;
    }
...
static int ewrite_ex_atrs(gxml_data *xd, nvpairs *nvp, int offset, int first, FILE *fp)
{
  int c, spaces = xd->indent * xd->depth + offset;

  if (xd->verb > 2)
    fprintf(stderr, "++ write %d ex_atr's\n", nvp->length);
...
static int ewrite_int_attr(const char *name, int value, int spaces, int first, FILE *fp)
{
  fprintf(fp,
          "%s%*s%s=\"%d\"",
          (first) ? "" : "\n", /* maybe a newline   */
...
static int ewrite_long_long_attr(const char *name, long long value, int spaces, int first, FILE *fp)
{
  fprintf(fp,
          "%s%*s%s=\"%lld\"",
          (first) ? "" : "\n", /* maybe a newline   */
...
static int ewrite_str_attr(const char *name, const char *value, int spaces, int first, FILE *fp)
{
  fprintf(fp,
          "%s%*s%s=\"%s\"",
          (first) ? "" : "\n", /* maybe a newline   */
