...
LTA *LTAcopy(LTA *lta, LTA *ltacp)
{
  ...
  if (ltacp == NULL)
    ltacp = LTAalloc(lta->num_xforms, NULL);
...
void readVolGeom(FILE *fp, VOL_GEOM *vg)
{
  ...
  if (p) // we read one more line
  {
    ...
    {
      // fail =
      fseek(fp, pos, SEEK_SET); // restore the position
    }
...
int vg_isNotEqualThresh(const VOL_GEOM *vg1, const VOL_GEOM *vg2, const double thresh)
{
  ...
  if (vg1->depth != vg2->depth)
    return (4);
  if (!FZEROTHR(vg1->xsize - vg2->xsize, thresh))
    return (5);
  if (!FZEROTHR(vg1->ysize - vg2->ysize, thresh))
    return (6);
  if (!FZEROTHR(vg1->zsize - vg2->zsize, thresh))
    return (7);
  if (!FZEROTHR(vg1->x_r - vg2->x_r, thresh))
    return (8);
  if (!FZEROTHR(vg1->x_a - vg2->x_a, thresh))
    return (9);
  if (!FZEROTHR(vg1->x_s - vg2->x_s, thresh))
    return (10);
  if (!FZEROTHR(vg1->y_r - vg2->y_r, thresh))
    return (11);
  if (!FZEROTHR(vg1->y_a - vg2->y_a, thresh))
    return (12);
  if (!FZEROTHR(vg1->y_s - vg2->y_s, thresh))
    return (13);
  if (!FZEROTHR(vg1->z_r - vg2->z_r, thresh))
    return (14);
  if (!FZEROTHR(vg1->z_a - vg2->z_a, thresh))
    return (15);
  if (!FZEROTHR(vg1->z_s - vg2->z_s, thresh))
    return (16);
  if (!FZEROTHR(vg1->c_r - vg2->c_r, thresh))
    return (17);
  if (!FZEROTHR(vg1->c_a - vg2->c_a, thresh))
    return (18);
  if (!FZEROTHR(vg1->c_s - vg2->c_s, thresh))
    return (19);
  return (0);
}
/*-----------------------------------------------------
...
LINEAR_TRANSFORM_ARRAY *LTAalloc(int nxforms, MRI *mri)
{
  ...
  if (!lta->xforms)
    ErrorExit(ERROR_NOMEMORY, "LTAalloc(%d): could not allocate xforms", nxforms);
  lta->inv_xforms = (LINEAR_TRANSFORM *)calloc(nxforms, sizeof(LT));
  if (!lta->inv_xforms)
    ErrorExit(ERROR_NOMEMORY, "LTAalloc(%d): could not allocate inverse xforms", nxforms);
  for (i = 0; i < nxforms; i++)
  {
    lta->xforms[i].x0 = x0;
    lta->xforms[i].y0 = y0;
...
static LTA *ltaReadFile(const char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (fp == NULL)
    ErrorReturn(NULL, (ERROR_BADFILE, "ltaReadFile(%s): can't open file", fname));
...
static int ltaFSLwrite(const LTA *lta, const char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "ltaFSLwrite: could not open file %s", fname));
...
static int ltaMNIwrite(const LTA *lta, const char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "ltaMNIwrite: could not open file %s", fname));
...
static LTA *ltaMNIread(const char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NOFILE, "ltaMNIread: could not open file %s", fname));
...
void mincGetVolInfo(const char *infoline, const char *infoline2, VOL_GEOM *vgSrc, VOL_GEOM *vgDst)
{
  ...
  // retVal =
  mincFindVolume(infoline, infoline2, &psrcVol, &pdstVol);
...
LTA *ltaMNIreadEx(const char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NOFILE, "ltMNIreadEx: could not open file %s", fname));
...
LTA *ltaReadFileEx(const char *fname)
{
  ...
  {
    lt = &lta->xforms[i];
    if (fscanf(fp, "mean      = %f %f %f\n", &lt->x0, &lt->y0, &lt->z0) != 3)
    {
      fprintf(stderr, "Could not read line\n");
      return (NULL);
    }
    if (fscanf(fp, "sigma     = %f\n", &lt->sigma) != 1)
    {
      fprintf(stderr, "Could not read line\n");
      return (NULL);
    }
...
int LTAprint(FILE *fp, const LTA *lta)
{
  ...
  fprintf(fp, "type      = %d ", lta->type);
  if (lta->type == LINEAR_VOX_TO_VOX)
    fprintf(fp, "# LINEAR_VOX_TO_VOX");
  if (lta->type == LINEAR_RAS_TO_RAS)
    fprintf(fp, "# LINEAR_RAS_TO_RAS");
  if (lta->type == REGISTER_DAT)
    fprintf(fp, "# REGISTER_DAT");
  fprintf(fp, "\n");
  fprintf(fp, "nxforms   = %d\n", lta->num_xforms);
  for (i = 0; i < lta->num_xforms; i++)
  {
    ...
    {
      for (c = 1; c <= 4; c++)
        fprintf(fp, "%18.15le ", (double)lt->m_L->rptr[r][c]);
      fprintf(fp, "\n");
    }
  }
    ...
    LTAwriteEx(const LTA *lta, const char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (fp == NULL)
    ErrorReturn(ERROR_BADFILE, (ERROR_BADFILE, "LTAwrite(%s): can't create file", fname));
  user = getenv("USER");
  if (!user)
    user = getenv("LOGNAME");
  if (!user)
...
static LTA *ltaFSLread(const char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NOFILE, "ltFSLread: could not open file %s", fname));
...
LTA *LTAconcat(LTA **ltaArray, int nLTAs, int Reduce)
{
  ...
  {
    lta1 = ltaArray[n];
    if (lta1->type != lta->type)
      LTAchangeType(lta1, lta->type);
    nx = lta1->num_xforms;
  ...
  for (s = 0; s < f->depth; s++)
  {
    for (r = 0; r < f->height; r++)
    {
      ...
      for (c = 0; c < f->width; c++)
      {
        ...
        if (regseg->type == MRI_UCHAR)
          vseg = (*(unsigned char *)pseg);
        if (regseg->type == MRI_INT)
          vseg = (*(int *)pseg);
        if (regseg->type == MRI_FLOAT)
          vseg = (*(float *)pseg);
        if (vseg == 2 || vseg == 41)
        {
          // white matter
