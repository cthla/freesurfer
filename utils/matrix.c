...
MATRIX *MatrixAlloc(const int rows, const int cols, const int type)
{
  ...
  if (!mat)
    ErrorExit(ERROR_NO_MEMORY, "MatrixAlloc(%d, %d, %d): could not allocate mat", rows, cols, type);
...
MATRIX *MatrixReadTxt(const char *fname, MATRIX *mat)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NO_FILE, "MatrixRead(%s) - file open failed\n", fname));
...
MATRIX *MatrixCalculateEigenSystemHelper(MATRIX *m, float *evalues, MATRIX *m_evectors, int isSymmetric)
{
  ...
  {
    if (OpenNonSymmetricEigenSystem(m->data, m->rows, evalues, mTmp->data) != NO_ERROR)
      return (NULL);
  }
...
MATRIX *MatrixCovariance(MATRIX *mInputs, MATRIX *mCov, VECTOR *mMeans)
{
  ...
  if (!mCov)
    ErrorExit(ERROR_NO_MEMORY, "MatrixCovariance: could not allocate %d x %d covariance matrix", nvars, nvars);
...
MATRIX *MatrixUpdateCovariance(MATRIX *mInputs, MATRIX *mCov, MATRIX *mMeans)
{
  ...
  if (!nvars)
    ErrorReturn(NULL, (ERROR_BADPARM, "MatrixUpdateCovariance: zero size input"));
...
int MatrixAsciiWrite(const char *fname, MATRIX *m)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "MatrixAsciiWrite: could not open file %s", fname));
...
MATRIX *MatrixAsciiRead(const char *fname, MATRIX *m)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NO_FILE, "MatrixAsciiRead: could not open file %s", fname));
...
MATRIX *MatrixAsciiReadFrom(FILE *fp, MATRIX *m)
{
  ...
  if (!m)
  {
    ...
    if (!m)
      ErrorReturn(NULL, (ERROR_BADFILE, "MatrixAsciiReadFrom: could not allocate matrix"));
  }
  else
  {
...
MATRIX *MatrixReadFrom(FILE *fp, MATRIX *m)
{
  ...
  if (!m)
  {
    m = MatrixAlloc(rows, cols, type);
    if (!m)
      ErrorReturn(NULL, (ERROR_BADFILE, "MatrixReadFrom: could not allocate matrix"));
  }
  else
  {
    if (m->rows != rows || m->cols != cols || m->type != type)
...
MATRIX *VectorOuterProduct(VECTOR *v1, VECTOR *v2, MATRIX *m)
{
  ...
  if (rows != cols)
    ErrorReturn(NULL, (ERROR_BADPARM, "VectorOuterProduct: v1->rows %d != v2->rows %d", rows, cols));
...
int MatrixWriteTxt(const char *fname, MATRIX *mat)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "MatrixWriteTxt(%s) - file open failed\n", fname));
