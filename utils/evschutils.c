...
int EVSfree(EVENT_SCHEDULE **ppEvSch)
{
  ...
  if (pEvSch->tevent != NULL)
    free(pEvSch->tevent);
  if (pEvSch->eventid != NULL)
    free(pEvSch->eventid);
  if (pEvSch->weight != NULL)
    free(pEvSch->weight);
  if (pEvSch->nEvReps != NULL)
    free(pEvSch->nEvReps);
  if (pEvSch->EvDur != NULL)
    free(pEvSch->EvDur);
...
/*-------------------------------------------------------------
  ...
  -------------------------------------------------------------*/
MATRIX *
EVS2FIRmtx(int EvId, EVSCH *EvSch, float tDelay, float TR, int Ntps, float PSDMin, float PSDMax, float dPSD, MATRIX *X)
{
...
int EVSPrint(FILE *fp, EVENT_SCHEDULE *EvSch)
{
  ...
  for (n = 0; n < EvSch->nevents; n++)
  {
    ...
    if (EvSch->weight != 0)
      fprintf(fp, "  %8.4f", EvSch->weight[n]);
    fprintf(fp, "\n");
  }
  return (0);
}
                         ...
                         float tNullMax)
{
  ...
  {
    printf("ERROR: stimulation time %g (including tNullMin %g) exceeds scan "
           "time %g\n",
           tStimTot,
           nevents * tNullMin,
           tScanTot);
...
{
  if (!strcmp("eff", CostString))
    return (EVS_COST_EFF);
  if (!strcmp("effinv", CostString))
    return (EVS_COST_EFF_INV);
  if (!strcmp("vrfavg", CostString))
    return (EVS_COST_VRFAVG);
  if (!strcmp("vrfstd", CostString))
    return (EVS_COST_VRFSTD);
  if (!strcmp("vrfavgstd", CostString))
    return (EVS_COST_VRFAVGSTD);
  if (!strcmp("idealxtx", CostString))
    return (EVS_COST_IDEALXTX);
...
/*-------------------------------------------------------------
  ...
  -------------------------------------------------------------*/
MATRIX *
EVSfirXtXIdeal(int nEvTypes, int *nEvReps, float *EvDur, float TR, int Ntp, float PSDMin, float PSDMax, float dPSD)
{
  MATRIX *XtX = NULL;
  ...
  for (n = 0; n < nEvTypes; n++)
    EvDur_dPSD[n] = (int)(rint(EvDur[n] / dPSD));
