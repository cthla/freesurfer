...
MRI *MRISvolumeTH3(MRIS *w, MRIS *p, MRI *vol, MRI *mask, double *totvol)
{
  ...
  if (!vol)
    vol = MRIalloc(w->nvertices, 1, 1, MRI_FLOAT);
  ...
  for (vno = 0; vno < w->nvertices; vno++)
  {
    ...
    if (mask && MRIgetVoxVal(mask, vno, 0, 0, 0) < 0.5)
      continue;
    nfacenbrs = w->vertices[vno].num;
    volsum = 0.0;
    for (nthfacenbr = 0; nthfacenbr < nfacenbrs; nthfacenbr++)
    {
      nbrfaceno = w->vertices[vno].f[nthfacenbr];
...
#define ERASE_MM 3
  ...
  for (vno = 0; vno < mris->nvertices; vno++)
  {
    ...
    for (d = 0; d <= 2; d += 0.5)
    {
      ...
      if (label == Left_Lateral_Ventricle || (IS_WM(label) && IS_WM(base_label) && (base_label != label))
          || // crossed hemi staying in wm
          ...
          || label == Left_Thalamus_Proper || label == Brain_Stem || label == Left_VentralDC
...
int MRISfindPath(int *vert_vno, int num_vno, int max_path_length, int *path, int *path_length, MRIS *mris)
{
  ...
  for (cur_vert_vno = 0; cur_vert_vno < num_vno - 1; cur_vert_vno++)
  {
    ...
    if (src_vno < 0 || src_vno >= mris->nvertices || dest_vno < 0 || dest_vno >= mris->nvertices)
      continue;
...
int L2SaddPoint(LABEL2SURF *l2s, double col, double row, double slice, int PointType, int Operation)
{
  ...
  if (PointType != 0)
  {
    ...
    { // Add vertex to label
      ...
      if (l2s->debug)
        printf("Adding surf=%d vtxno=%d  np=%5d \n", nmin, vtxno, label->n_points);
    ...
    { // Operation != 1, Remove vertex from label
      if (pointno == 0)
        return (0); // not already there, cant remove it
  ...
  for (nthhop = 0; nthhop < l2s->nhopsmax; nthhop++)
  {
    ...
    for (nthnbr = 0; nthnbr < nnbrs; nthnbr++)
    {
      ...
      { // Operation != 1, Remove vertex from label
        if (pointno == 0)
          continue; // not already there, cant remove it
        lv = &(label->lv[pointno - 1]);
        // If it gets here, then remove the vertex
        if (l2s->debug)
  ...
  if (nthhoplast == l2s->nhopsmax - 1)
    if (l2s->debug)
      printf("WARNING: hop saturation nthsurf=%d, cvtxno = %d\n", nmin, vtxnominmin);
...
int L2Sinit(LABEL2SURF *l2s)
{
  ...
  if (l2s->nhopsmax == -1)
    l2s->nhopsmax = 3
                    * round(sqrt(pow(l2s->mri_template->xsize, 2.0) + pow(l2s->mri_template->ysize, 2)
...
LABEL2SURF *L2Salloc(int nsurfs, char *subject)
{
  ...
  for (n = 0; n < nsurfs; n++)
    l2s->labels[n] = LabelAlloc(100, subject, NULL);
