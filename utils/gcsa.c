...
static double
gcsaNbhdGibbsLogLikelihood(GCSA *gcsa, MRI_SURFACE *mris, double *v_inputs, int vno, double gibbs_coef, int label);
static double gcsaVertexGibbsLogLikelihood(GCSA *gcsa, MRI_SURFACE *mris, double *v_inputs, int vno, double gibbs_coef);
...
static int GCSAupdateNodeMeans(GCSA_NODE *gcsan, int label, double *v_inputs, int ninputs)
{
  ...
  if (n >= gcsan->nlabels) /* have to allocate a new classifier */
  {
    if (n >= gcsan->max_labels)
    {
      ...
      if (!gcsan->gcs)
        ErrorExit(ERROR_NOMEMORY, "GCANupdateNodeMeans: couldn't expand gcs to %d", gcsan->max_labels);
...
int GCSAwrite(GCSA *gcsa, char *fname)
{
  ...
  fp = fopen(fname, "wb");
  if (!fp)
    ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "GCSAwrite(%s): could not open file", fname));
...
GCSA *GCSAread(char *fname)
{
  ...
  fp = fopen(fname, "rb");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NOFILE, "GCSAread(%s): could not open file", fname));
  ...
  for (i = 0; i < ninputs; i++)
  {
    ...
    {
      ErrorReturn(NULL, (ERROR_BADFILE, "afniRead(): error reading from file %s", gcsa->inputs[i].fname));
    }
  ...
  for (vno = 0; vno < gcsa->mris_classifiers->nvertices; vno++)
  {
    ...
    if (!gcsan->gcs)
      ErrorExit(ERROR_NOMEMORY, "GCSAread(%s): could not allocate gcs %d", fname, vno);
    gcsan->labels = (int *)calloc(gcsan->nlabels, sizeof(int));
    if (!gcsan->labels)
      ErrorExit(ERROR_NOMEMORY, "GCSAread(%s): could not allocate labels %d", fname, vno);
  ...
  for (vno = 0; vno < gcsa->mris_priors->nvertices; vno++)
  {
    ...
    if (!cpn->cps)
      ErrorExit(ERROR_NOMEMORY, "GCSAread(%s): could not allocate cps %d", fname, vno);
    ...
    for (n = 0; n < cpn->nlabels; n++)
    {
      ...
      for (i = 0; i < GIBBS_SURFACE_NEIGHBORS; i++)
      {
        ...
        if (!cp->labels[i])
          ErrorExit(ERROR_NOMEMORY, "GCSAread: couldn't allocate labels to %d", cp->nlabels[i] + 1);
...
int dump_gcsan(GCSA_NODE *gcsan, CP_NODE *cpn, FILE *fp, int verbose)
{
  ...
  for (n = 0; n < cpn->nlabels; n++)
  {
    ...
    gcs = getGC(gcsan, cpn->labels[n], NULL);
    if (!gcs)
      ErrorPrintf(ERROR_BADPARM, "dump_gcsan: could not find GCS for node %d!", n);
...
static int GCSAupdateNodeGibbsPriors(CP_NODE *cpn, int label, MRI_SURFACE *mris, int vno)
{
  ...
  if (n >= cpn->nlabels) /* have to allocate a new prior struct */
  {
    if (n >= cpn->max_labels)
    {
      ...
      if (!cpn->cps)
        ErrorExit(ERROR_NOMEMORY, "GCANupdateNodeGibbsPriors: couldn't expand cps to %d", cpn->max_labels);
...
gcsaNbhdGibbsLogLikelihood(GCSA *gcsa, MRI_SURFACE *mris, double *v_inputs, int vno, double gibbs_coef, int label)
{
  ...
  total_ll = gcsaVertexGibbsLogLikelihood(gcsa, mris, v_inputs, vno, gibbs_coef);

  for (n = 0; n < v->vnum; n++)
  {
    ll = gcsaVertexGibbsLogLikelihood(gcsa, mris, v_inputs, v->v[n], gibbs_coef);
    total_ll += ll;
  }
...
static double gcsaVertexGibbsLogLikelihood(GCSA *gcsa, MRI_SURFACE *mris, double *v_inputs, int vno, double gibbs_coef)
{
  ...
  m_cov_inv = MatrixInverse(gcs->m_cov, m_cov_inv);
  if (!m_cov_inv)
    ErrorExit(ERROR_BADPARM, "GCSAvertexLogLikelihood: could not invert matrix");
  ...
  do
  {
    ...
    for (vno = 0; vno < gcsa->mris_priors->nvertices; vno++)
    {
      ...
      *cpn = *cpn_nbr;
      cpn->cps = (CP *)calloc(cpn->nlabels, sizeof(CP));
      if (!cpn->cps)
        ErrorExit(ERROR_NOMEMORY, "fill_cpn_holes(): could not allocate cps %d", vno);
      cpn->labels = (int *)calloc(cpn->nlabels, sizeof(int));
      if (!cpn->labels)
        ErrorExit(ERROR_NOMEMORY, "fill_cpn_holes(): could not allocate labels %d", vno);
  ...
  do
  {
    ...
    for (vno = 0; vno < gcsa->mris_classifiers->nvertices; vno++)
    {
      ...
      *gcsan = *gcsan_nbr;
      gcsan->gcs = (GCS *)calloc(gcsan->nlabels, sizeof(GCS));
      if (!gcsan->gcs)
        ErrorExit(ERROR_NOMEMORY, "fill_gcsan_holes(): could not allocate gcs %d", vno);
      gcsan->labels = (int *)calloc(gcsan->nlabels, sizeof(int));
      if (!gcsan->labels)
        ErrorExit(ERROR_NOMEMORY, "fill_gcsan_holes(): could not allocate labels %d", vno);
