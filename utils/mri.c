...
MATRIX *MRItkRegMtxFromVox2Vox(MRI *ref, MRI *mov, MATRIX *vox2vox)
{
  ...
  if (vox2vox == NULL)
    return (MRItkRegMtx(ref, mov, NULL));
...
int MRIcheckSize(MRI *mri_src, MRI *mri_check, int width, int height, int depth)
{
  ...
  if (width != mri_check->width || height != mri_check->height || depth != mri_check->depth)
    return (0);
...
int MRItransformRegion(MRI *mri_src, MRI *mri_dst, MRI_REGION *src_region, MRI_REGION *dst_region)
{
  ...
  */
  ...
  default:
    ErrorReturn(
        ERROR_UNSUPPORTED,
        (ERROR_UNSUPPORTED, "MRIregionToTalairachRegion: unsupported slice direction %d", getSliceDirection(mri_src)));
  }
...
MRI *MRIthresholdFrame(MRI *mri_src, MRI *mri_dst, float threshold, int frame)
{
  ...
  if (frame > f + 1)
    ErrorReturn(NULL, (ERROR_BADPARM, "MRIthreshold: invalid frame number"));
...
MRI *MRIallocChunk(int width, int height, int depth, int type, int nframes)
{
  ...
  if (!mri->slices)
    ErrorExit(ERROR_NO_MEMORY, "MRIalloc: could not allocate %d slices\n", mri->depth);
...
MRI *MRIallocSequence(int width, int height, int depth, int type, int nframes)
{
  ...
  mri->slices = (BUFTYPE ***)calloc(depth * nframes, sizeof(BUFTYPE **));
  if (!mri->slices)
    ErrorExit(ERROR_NO_MEMORY, "MRIallocSequence: could not allocate %d slices\n", mri->depth);
...
MRI *MRIallocHeader(int width, int height, int depth, int type, int nframes)
{
  ...
  if (!mri->frames)
    ErrorExit(ERROR_NO_MEMORY, "MRIalloc: could not allocate %d frame\n", nframes);
...
int MRIfree(MRI **pmri)
{
  ...
  if (mri->bvals)
    MatrixFree(&mri->bvals);
  if (mri->bvecs)
    MatrixFree(&mri->bvecs);
...
MRI *MRIrotate_I(MRI *mri_src, MRI *mri_dst, MATRIX *mR, MATRIX *mO)
{
  ...
  mRinv = MatrixInverse(mR, NULL);
  if (!mRinv)
    ErrorReturn(NULL, (ERROR_BADPARM, "MRIrotate_I: rotation matrix is singular"));
  ...
  if (!I)
  {
    I = ImageAlloc(height,
                   ...
                   mri->type == MRI_UCHAR ? PFBYTE
                                          : mri->type == MRI_INT ? PFINT : mri->type == MRI_FLOAT ? PFFLOAT : PFBYTE,
                   1);
  }
...
MRI *MRIresize(MRI *mri, double xsize, double ysize, double zsize, int nframes)
{
  ...
  *MATRIX_RELT(M, 3, 3) = mri->z_s * zsize;
  ...
  if (nframes == 0)
    mri2 = MRIallocHeader(width, height, depth, MRI_FLOAT, 1);
  if (nframes < 0)
    mri2 = MRIallocHeader(width, height, depth, MRI_FLOAT, mri->nframes);
  if (nframes > 0)
    mri2 = MRIallocHeader(width, height, depth, MRI_FLOAT, nframes);
...
double *MRItrilinKernel(MRI *mri, double c, double r, double s, double *kernel)
{
  ...
  if (kernel == NULL)
    kernel = (double *)calloc(8, sizeof(double));
           ...
           jy1++, jy_rel++)
      {
             ...
             jx1++, jx_rel++)
        {
          ...
          case MRI_FLOAT:
            ...
            break;
          default:
            ErrorReturn(ERROR_UNSUPPORTED, (ERROR_UNSUPPORTED, "MRIsincSampleVolume: unsupported type %d", mri->type));
            break;
          }
        }
...
MRI *MRIinverseLinearTransform(MRI *mri_src, MRI *mri_dst, MATRIX *mA)
{
  ...
  if (!m_inv)
    ErrorReturn(NULL, (ERROR_BADPARM, "MRIinverseLinearTransform: xform is singular!"));
  // fprintf
  //(stderr,
...
MRI *MRIsincTransform(MRI *mri_src, MRI *mri_dst, MATRIX *mA, int hw)
{
  ...
  if (!mAinv)
    ErrorReturn(NULL, (ERROR_BADPARM, "MRIsincTransform: xform is singular"));
...
MRI *MRIresampleFill(MRI *src, MRI *template_vol, int resample_type, float fill_val)
{
  ...
  if (!src->ras_good_flag)
    printf("MRIresample(): WARNING: ras_good_flag "
          ...
          if (di % 20 == 0 && dj == di && dk == dj)
          {
            ...
            else
            {
              ...
              if (src->type == MRI_UCHAR)
              {
                ...
                val001 = (!i_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRIseq_vox(
                                                                                    src, si, sj, sk + 1, nframe));
                val010 = (!i_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRIseq_vox(
                                                                                    src, si, sj + 1, sk, nframe));
                val011 = (!i_good_flag || !j1_good_flag || !k1_good_flag ? 0.0 : (float)MRIseq_vox(
                                                                                     src, si, sj + 1, sk + 1, nframe));
                val100 = (!i1_good_flag || !j_good_flag || !k_good_flag ? 0.0 : (float)MRIseq_vox(
                                                                                    src, si + 1, sj, sk, nframe));
                val101 = (!i1_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRIseq_vox(
                                                                                     src, si + 1, sj, sk + 1, nframe));
                val110 = (!i1_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRIseq_vox(
                                                                                     src, si + 1, sj + 1, sk, nframe));
              ...
              if (src->type == MRI_SHORT)
              {
                val000 = (!i_good_flag || !j_good_flag || !k_good_flag ? 0.0
                                                                       : (float)MRISseq_vox(src, si, sj, sk, nframe));
                val001 = (!i_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRISseq_vox(
                                                                                    src, si, sj, sk + 1, nframe));
                val010 = (!i_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRISseq_vox(
                                                                                    src, si, sj + 1, sk, nframe));
                val011 = (!i_good_flag || !j1_good_flag || !k1_good_flag ? 0.0 : (float)MRISseq_vox(
                                                                                     src, si, sj + 1, sk + 1, nframe));
                val100 = (!i1_good_flag || !j_good_flag || !k_good_flag ? 0.0 : (float)MRISseq_vox(
                                                                                    src, si + 1, sj, sk, nframe));
                val101 = (!i1_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRISseq_vox(
                                                                                     src, si + 1, sj, sk + 1, nframe));
                val110 = (!i1_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRISseq_vox(
                                                                                     src, si + 1, sj + 1, sk, nframe));
                val111 = (!i1_good_flag || !j1_good_flag || !k1_good_flag
                              ? 0.0
              ...
              if (src->type == MRI_INT)
              {
                ...
                val001 = (!i_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRIIseq_vox(
                                                                                    src, si, sj, sk + 1, nframe));
                val010 = (!i_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRIIseq_vox(
                                                                                    src, si, sj + 1, sk, nframe));
                val011 = (!i_good_flag || !j1_good_flag || !k1_good_flag ? 0.0 : (float)MRIIseq_vox(
                                                                                     src, si, sj + 1, sk + 1, nframe));
                val100 = (!i1_good_flag || !j_good_flag || !k_good_flag ? 0.0 : (float)MRIIseq_vox(
                                                                                    src, si + 1, sj, sk, nframe));
                val101 = (!i1_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRIIseq_vox(
                                                                                     src, si + 1, sj, sk + 1, nframe));
                val110 = (!i1_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRIIseq_vox(
                                                                                     src, si + 1, sj + 1, sk, nframe));
                val111 = (!i1_good_flag || !j1_good_flag || !k1_good_flag
                              ? 0.0
              ...
              if (src->type == MRI_LONG)
              {
                ...
                val001 = (!i_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRILseq_vox(
                                                                                    src, si, sj, sk + 1, nframe));
                val010 = (!i_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRILseq_vox(
                                                                                    src, si, sj + 1, sk, nframe));
                val011 = (!i_good_flag || !j1_good_flag || !k1_good_flag ? 0.0 : (float)MRILseq_vox(
                                                                                     src, si, sj + 1, sk + 1, nframe));
                val100 = (!i1_good_flag || !j_good_flag || !k_good_flag ? 0.0 : (float)MRILseq_vox(
                                                                                    src, si + 1, sj, sk, nframe));
                val101 = (!i1_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRILseq_vox(
                                                                                     src, si + 1, sj, sk + 1, nframe));
                val110 = (!i1_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRILseq_vox(
                                                                                     src, si + 1, sj + 1, sk, nframe));
                val111 = (!i1_good_flag || !j1_good_flag || !k1_good_flag
                              ? 0.0
              ...
              if (src->type == MRI_FLOAT)
              {
                ...
                val001 = (!i_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRIFseq_vox(
                                                                                    src, si, sj, sk + 1, nframe));
                val010 = (!i_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRIFseq_vox(
                                                                                    src, si, sj + 1, sk, nframe));
                val011 = (!i_good_flag || !j1_good_flag || !k1_good_flag ? 0.0 : (float)MRIFseq_vox(
                                                                                     src, si, sj + 1, sk + 1, nframe));
                val100 = (!i1_good_flag || !j_good_flag || !k_good_flag ? 0.0 : (float)MRIFseq_vox(
                                                                                    src, si + 1, sj, sk, nframe));
                val101 = (!i1_good_flag || !j_good_flag || !k1_good_flag ? 0.0 : (float)MRIFseq_vox(
                                                                                     src, si + 1, sj, sk + 1, nframe));
                val110 = (!i1_good_flag || !j1_good_flag || !k_good_flag ? 0.0 : (float)MRIFseq_vox(
                                                                                     src, si + 1, sj + 1, sk, nframe));
                val111 = (!i1_good_flag || !j1_good_flag || !k1_good_flag
                              ? 0.0
              ...
              {
                val = (1.0 - si_f) * (1.0 - sj_f) * (1.0 - sk_f) * val000 + (1.0 - si_f) * (1.0 - sj_f) * (sk_f)*val001
                      + (1.0 - si_f) * (sj_f) * (1.0 - sk_f) * val010 + (1.0 - si_f) * (sj_f) * (sk_f)*val011
                      + (si_f) * (1.0 - sj_f) * (1.0 - sk_f) * val100 + (si_f) * (1.0 - sj_f) * (sk_f)*val101
                      + (si_f) * (sj_f) * (1.0 - sk_f) * val110 + (si_f) * (sj_f) * (sk_f)*val111;
              }
...
double
MRImeanInLabelInRegion(MRI *mri_src, MRI *mri_labeled, int label, int x0, int y0, int z0, int whalf, double *psigma)
{
  int x, y, z, nvox, l;
  double mean = 0.0;
  double var = 0.0;
        ...
        for (xk = -1; xk <= 1 && !border; xk++)
        {
          ...
          for (yk = -1; yk <= 1 && !border; yk++)
          {
            ...
            for (zk = -1; zk <= 1; zk++)
            {
              ...
              if (((this_label == label) && (that_label != label)) || ((this_label != label) && (that_label == label)))
              {
                border = 1;
                break;
              }
...
MRI *MRIconformSliceOrder(MRI *mri)
{
  ...
  if (fabs(Mv2v[2][0]) > fabs(Mv2v[0][0]) && fabs(Mv2v[2][0]) > fabs(Mv2v[1][0]))
    xd = (Mv2v[2][0] < 0) ? -3 : 3;

  if (fabs(Mv2v[1][1]) > fabs(Mv2v[0][1]))
    yd = (Mv2v[1][1] < 0) ? -2 : 2;

  if (fabs(Mv2v[2][1]) > fabs(Mv2v[0][1]) && fabs(Mv2v[2][1]) > fabs(Mv2v[1][1]))
    yd = (Mv2v[2][1] < 0) ? -3 : 3;

  if (fabs(Mv2v[1][2]) > fabs(Mv2v[0][2]))
    zd = (Mv2v[1][2] < 0) ? -2 : 2;

  if (fabs(Mv2v[2][2]) > fabs(Mv2v[0][2]) && fabs(Mv2v[2][2]) > fabs(Mv2v[1][2]))
    zd = (Mv2v[2][2] < 0) ? -3 : 3;
...
double MRIestimateTIV(char *theLtaFile, double theScaleFactor, double *theAtlasDet)
{
  ...
  if (atlas_lta == NULL)
    ErrorExit(ERROR_NOFILE, "%s: could not open atlas transform file %s", Progname, theLtaFile);
...
{
  if (mri1->width != mri2->width || mri1->height != mri2->height || mri1->depth != mri2->depth)
    return (0);

  if (!FEQUAL(mri1->xsize, mri2->xsize) || !FEQUAL(mri1->ysize, mri2->ysize) || !FEQUAL(mri1->zsize, mri2->zsize))
    return (0);

  if (!FEQUAL(mri1->c_r, mri2->c_r) || !FEQUAL(mri1->c_a, mri2->c_a) || !FEQUAL(mri1->c_s, mri2->c_s))
    return (0);
  return (1);
}
...
int *MRIhistogramLabels(MRI *mri, int *counts, int max_label)
{
  ...
  if (counts == NULL)
    ErrorExit(ERROR_NOMEMORY, "MRIhistogramLabels: could not allocate %d counts", max_label + 1);
  for (x = 0; x < mri->width; x++)
    for (y = 0; y < mri->height; y++)
      for (z = 0; z < mri->depth; z++)
      {
  ...
  for (r = -MAX_FOV; r <= MAX_FOV; r += vsize)
  {
    ...
    for (a = -MAX_FOV; a <= MAX_FOV; a += vsize)
    {
      ...
      for (s = -MAX_FOV; s <= MAX_FOV; s += vsize)
      {
        ...
        y = nint(V3_Y(v_vox));
        z = nint(V3_Z(v_vox));
        if (x < 0 || x >= mri_aseg->width || y < 0 || y >= mri_aseg->height || z < 0 || z >= mri_aseg->depth)
          continue;
