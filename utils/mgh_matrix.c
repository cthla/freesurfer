...
#define PYTHAG(a, b)                                                                                                   \
  ((at = fabs(a)) > (bt = fabs(b)) ? (ct = bt / at, at * sqrt(1.0 + ct * ct))                                          \
...
void mgh_svdcmp(FLOATTYPE **a, FLOATTYPE *w, FLOATTYPE **v, int m, int n)
{
  ...
  if (m < n)
    ErrorExit(ERROR_BADPARM, "Singular value decompostion: input matrix has too few rows");
  rv1 = MGH_vector(n);
  for (i = 0; i < n; i++)
  {
    l = i + 1;
  ...
  for (k = n - 1; k >= 0; k--)
  {
    for (its = 1; its <= 30; its++)
    {
      ...
      for (l = k; l >= 0; l--)
      {
        ...
        if ((FLOATTYPE)(fabs(w[nm]) + anorm) == anorm)
          break;
      }
      if (flag)
      {
        ...
        for (i = l; i <= k; i++)
        {
          f = s * rv1[i];
          rv1[i] = c * rv1[i];
          if ((FLOATTYPE)(fabs(f) + anorm) == anorm)
            break;
          g = w[i];
          h = PYTHAG(f, g);
          w[i] = h;
      ...
      if (its == 30)
        ErrorExit(ERROR_BADPARM, "Singular value decomposition failed to converge");
      x = w[l];
      nm = k - 1;
      y = w[nm];
      g = rv1[nm];
