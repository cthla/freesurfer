...
----------------------------------------------------------------------*/
KINPUT *
KinputAlloc(int rows, int cols, int nscales, int input_size, float sigma, float sigma_scale_factor, int abs_gradient)
{
  KINPUT *kinput;
  int scale;
  ...
  {
    kinput->gImages[scale] = ImageGaussian1d(sigma * sigma_scale_factor * (float)(scale), 0);
...
int KinputInit(KINPUT *kinput, IMAGE *image)
{
  ...
  for (scale = 1; scale < kinput->nscales; scale++)
  {
    ImageConvolveGaussian(kinput->xInputs, kinput->gImages[scale], kinput->xInputs, scale);
    ImageConvolveGaussian(kinput->yInputs, kinput->gImages[scale], kinput->yInputs, scale);
  }
#if 0
...
#define BILINEAR 1
  ...
  if ((x1 < 0) || (y1 < 0) || (x2 >= cols) || (y2 >= pyr->images[level]->rows))
  {
    fprintf(stderr, "kinputGetPoint(%d, %d, %d): point (%d,%d,%d,%d) out of bounds\n", x, y, level, x1, y1, x2, y2);
    return (0.0f);
  }
  ...
  val = (1.0 - dy) * ((1.0 - dx) * *f1p + dx * *f3p) + dy * ((1.0 - dx) * *f2p + dx * *f4p);
