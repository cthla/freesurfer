...
char *FunD_ksaErrorString[FunD_tErr_knNumErrorCodes] = { "No error.",
                   ...
                   tBoolean ibIsLeftHemisphere)
{
  ...
  DebugEnterFunction(("FunD_New( opVolume=%p, isFileName=%s, "
                      ...
                      isFileName,
                      iRegistrationType,
                      isRegistrationFileName,
                      inScalarSize,
                      iAnatomicalVolume));

  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != opVolume && NULL != isFileName), eResult, FunD_tErr_InvalidParameter);
  ...
  DebugAssertThrowX((NULL != this->mpData), eResult, FunD_tErr_CouldntReadMRI);
  ...
  if (!this->mbScalar)
  {
    ...
    eResult = FunD_ParseRegistrationAndInitMatricies_(this, iRegistrationType, iAnatomicalVolume);
...
FunD_tErr FunD_Delete(mriFunctionalDataRef *iopVolume)
{
  ...
  {
    for (column = 0; column < (this->mNumConditions - 1) * this->mNumTimePoints; column++)
    {
...
FunD_tErr
FunD_ReshapeIfScalar_(mriFunctionalDataRef this, int inNumValues, tBoolean *obReshaped, tBoolean ibIsLeftHemisphere)
{
  ...
  if (this->mpData->width * this->mpData->height * this->mpData->depth == 2 * inNumValues)
  {
    ...
    MRIfree(&mri_tmp);
    MRIfree(&this->mpData);
    this->mpData = mri_tmp2;
  }

  if (this->mpData->width * this->mpData->height * this->mpData->depth == inNumValues)
  {

    /* Reshape it so that everything is in the x direction. */
    DebugNote(("Calling mri_reshape"));
    reshapedVolume = mri_reshape(this->mpData, inNumValues, 1, 1, this->mpData->nframes);
    DebugAssertThrowX((NULL != reshapedVolume), eResult, FunD_tErr_CouldntAllocateVolume);
...
FunD_tErr FunD_FindAndParseStemHeader_(mriFunctionalDataRef this)
{
  ...
  while (!feof(pHeader))
  {
    ...
    DebugAssertThrowX((1 == nValuesRead && !feof(pHeader)), eResult, FunD_tErr_UnrecognizedHeaderFormat);
    ...
    else if (strcmp(sKeyword, "hCovMtx") == 0)
    {
      ...
      this->mCovMtx = (float **)calloc(this->mNumTimePoints * (this->mNumConditions - 1), sizeof(float *));
      DebugAssertThrowX((NULL != this->mCovMtx), eResult, FunD_tErr_CouldntAllocateStorage);

      /* The cov mtx doesn't have values for the null condition, so we
         start with 1 instead of 0 in our loops here, and use that-1. */
      for (nCovMtxRow = 0; nCovMtxRow < this->mNumTimePoints * (this->mNumConditions - 1); nCovMtxRow++)
      {

        DebugNote(("Allocating cov mtx col %d", nCovMtxCol));
        this->mCovMtx[nCovMtxRow] = (float *)calloc(this->mNumTimePoints * (this->mNumConditions - 1), sizeof(float *));
        DebugAssertThrowX((NULL != this->mCovMtx), eResult, FunD_tErr_CouldntAllocateStorage);

        for (nCovMtxCol = 0; nCovMtxCol < this->mNumTimePoints * (this->mNumConditions - 1); nCovMtxCol++)
        {

          DebugNote(("Reading cov mtx value %d, %d", nCovMtxCol, nCovMtxRow));
          nValuesRead = fscanf(pHeader, "%f", &this->mCovMtx[nCovMtxRow][nCovMtxCol]);
          bGood = (1 == nValuesRead);
  ...
  DebugAssertThrowX((bSomethingRead), eResult, FunD_tErr_UnrecognizedHeaderFormat);
                                                  ...
                                                  mriVolumeRef iAnatomicalVolume)
{
  ...
  DebugEnterFunction(("FunD_ParseRegistrationAndInitMatricies_( this=%p )", this));

  DebugNote(("Checking this"));
  DebugAssertThrowX((NULL != this), eResult, FunD_tErr_InvalidParameter);
  DebugNote(("Checking registration type"));
  DebugAssertThrowX((iType >= 0 && iType < FunD_knNumRegistrationTypes), eResult, FunD_tErr_InvalidParameter);
  DebugNote(("Checking iAnatomicalVolume"));
  DebugAssertThrowX((NULL != iAnatomicalVolume), eResult, FunD_tErr_InvalidParameter);
  ...
  case FunD_tRegistration_Find:
    ...
    if (FunD_tRegistration_Find == iType)
    {
      ...
      *pBaseEnd = '\0';
      xUtil_snprintf(sFileName, sizeof(sFileName), "%s/%s", sBasePath, FunD_ksRegisterFileName);

      /* Save the file name we found. */
      strncpy(this->msRegistrationFileName, sFileName, sizeof(this->msRegistrationFileName));
    }
    ...
    DebugAssertThrowX((S_ISREG(fileInfo.st_mode)), eResult, FunD_tErr_CouldntReadRegisterFile);
    ...
    DebugAssertThrowX((NULL != theRegInfo), eResult, FunD_tErr_CouldntReadRegisterFile);
        ...
        || !FEQUAL(this->mpData->zsize, theRegInfo->slice_thickness))
    {
      ...
      DebugPrint(("\tWARNING: MRI xsize and zsize are not equal to "
                  ...
                  this->mpData->xsize,
                  this->mpData->zsize,
                  theRegInfo->in_plane_res,
                  theRegInfo->slice_thickness));
    }
  ...
  DebugAssertThrowX((ERROR_NONE == eMRI), eResult, FunD_tErr_CouldntReadRegisterFile);
...
FunD_tErr FunD_RestoreRegistration(mriFunctionalDataRef this)
{
  ...
  DebugAssertThrowX((NULL != this->mOriginalIdxToIdxTransform), eResult, FunD_tErr_ErrorAccessingTransform);

  /* Delete the matrix and clone the backup. */
  Trns_Delete(&this->mIdxToIdxTransform);
  Trns_DeepClone(this->mOriginalIdxToIdxTransform, &this->mIdxToIdxTransform);
    ...
    mriFunctionalDataRef this, int inXMin, int inYMin, int inZMin, int inXMax, int inYMax, int inZMax)
{
  ...
  DebugAssertThrowX((inXMax >= inXMin && inYMax >= inYMin && inZMax >= inZMin), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_SetConversionMethod(mriFunctionalDataRef this, FunD_tConversionMethod iMethod)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this), eResult, FunD_tErr_InvalidParameter);
  DebugAssertThrowX((iMethod >= 0 && iMethod < FunD_knNumConversionMethods), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetSampleType(mriFunctionalDataRef this, FunD_tSampleType *oType)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((oType != NULL), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_SetSampleType(mriFunctionalDataRef this, FunD_tSampleType iType)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iType >= 0 && iType < FunD_knNumSampleTypes), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_ClientSpaceIsTkRegRAS(mriFunctionalDataRef this)
{
  ...
  DebugAssertThrowX((NULL != this->mIdxToIdxTransform), eResult, FunD_tErr_ErrorAccessingTransform);

  DebugNote(("Creating identity matrix"));
  identity = MatrixIdentity(4, NULL);
  DebugAssertThrowX((NULL != identity), eResult, FunD_tErr_CouldntAllocateMatrix);

  DebugNote(("Copying in transform"));
  eTrns = Trns_CopyAtoRAS(this->mIdxToIdxTransform, identity);
  DebugAssertThrowX((Trns_tErr_NoErr == eTrns), eResult, FunD_tErr_ErrorAccessingTransform);
...
FunD_tErr FunD_IsScalar(mriFunctionalDataRef this, tBoolean *obScalar)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((obScalar != NULL), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetData(mriFunctionalDataRef this, xVoxelRef iClientVox, int iCondition, int iTimePoint, float *oValue)
{
  ...
  DebugNote(("Checking parameters"));
#endif
  DebugAssertThrowX((NULL != this && NULL != iClientVox && NULL != oValue), eResult, FunD_tErr_InvalidParameter);
#ifndef BE_SUPA_FAST
  DebugNote(("Checking condition"));
#endif
  DebugAssertThrowX((iCondition >= 0 && iCondition < this->mNumConditions), eResult, FunD_tErr_InvalidParameter);
#ifndef BE_SUPA_FAST
  DebugNote(("Checking time point"));
#endif
  DebugAssertThrowX((iTimePoint >= 0 && iTimePoint < this->mNumTimePoints), eResult, FunD_tErr_InvalidParameter);
...
#endif
  FunD_GetValue_(this, this->mpData, &funcIdx, iCondition, iTimePoint, &fValue);
...
FunD_tErr
FunD_GetSampledData(mriFunctionalDataRef this, xVoxelRef iClientVox, int iCondition, int iTimePoint, float *oValue)
{
  ...
  DebugNote(("Getting value"));
#endif
  FunD_GetSampledValue_(this, this->mpData, &funcIdx, iCondition, iTimePoint, &fValue);
...
FunD_tErr FunD_GetDataForAllTimePoints(mriFunctionalDataRef this, xVoxelRef iClientVox, int iCondition, float *oaValue)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != iClientVox && NULL != oaValue), eResult, FunD_tErr_InvalidParameter);
  DebugNote(("Checking condition"));
  DebugAssertThrowX((iCondition >= 0 && iCondition < this->mNumConditions), eResult, FunD_tErr_InvalidParameter);
  ...
  for (nTimePoint = 0; nTimePoint < this->mNumTimePoints; nTimePoint++)
  {
    ...
    DebugNote(("Getting value"));
    FunD_GetValue_(this, this->mpData, &funcIdx, iCondition, nTimePoint, &fValue);
...
FunD_tErr
FunD_GetDeviation(mriFunctionalDataRef this, xVoxelRef iClientVox, int iCondition, int iTimePoint, float *oValue)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oValue), eResult, FunD_tErr_InvalidParameter);
  DebugNote(("Checking condition"));
  DebugAssertThrowX((iCondition >= 0 && iCondition < this->mNumConditions), eResult, FunD_tErr_InvalidParameter);
  DebugNote(("Checking time point"));
  DebugAssertThrowX((iTimePoint >= 0 && iTimePoint < this->mNumTimePoints), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr
FunD_GetDeviationForAllTimePoints(mriFunctionalDataRef this, xVoxelRef iClientVox, int iCondition, float *oaValue)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oaValue), eResult, FunD_tErr_InvalidParameter);
  DebugNote(("Checking condition"));
  DebugAssertThrowX((iCondition >= 0 && iCondition < this->mNumConditions), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_ConvertTimePointToSecond(mriFunctionalDataRef this, int iTimePoint, float *oSecond)
{
  ...
  DebugEnterFunction(("FunD_ConvertTimePointToSecond( this=%p, "
                      "iTimePoint=%d, oSecond=%p)",
                      this,
                      iTimePoint,
                      oSecond));

  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oSecond), eResult, FunD_tErr_InvalidParameter);
  DebugNote(("Checking time point"));
  DebugAssertThrowX((iTimePoint >= 0 && iTimePoint < this->mNumTimePoints), eResult, FunD_tErr_InvalidParameter);
  ...
  timeAtFirstPoint = -((float)(this->mNumPreStimTimePoints) * this->mTimeResolution);
  DebugNote(("Setting return value"));
...
FunD_tErr FunD_ConvertSecondToTimePoint(mriFunctionalDataRef this, float iSecond, int *oTimePoint)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oTimePoint), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_SetTimeResolution(mriFunctionalDataRef this, float iTimeResolution)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this), eResult, FunD_tErr_InvalidParameter);
  DebugAssertThrowX((iTimeResolution > 0), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_SetNumPreStimTimePoints(mriFunctionalDataRef this, int iNumPoints)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this), eResult, FunD_tErr_InvalidParameter);
  DebugAssertThrowX((iNumPoints >= 0 && iNumPoints < this->mNumTimePoints), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetSubjectName(mriFunctionalDataRef this, char *oSubjectName)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oSubjectName), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetNumTimePoints(mriFunctionalDataRef this, int *oNumTimePoints)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oNumTimePoints), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetNumConditions(mriFunctionalDataRef this, int *oNumConditions)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oNumConditions), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetTimeResolution(mriFunctionalDataRef this, float *oTimeResolution)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oTimeResolution), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetNumPreStimTimePoints(mriFunctionalDataRef this, int *oNumPoints)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oNumPoints), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetValueRange(mriFunctionalDataRef this, float *oMin, float *oMax)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oMin && NULL != oMax), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_IsErrorDataPresent(mriFunctionalDataRef this, tBoolean *oPresent)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oPresent), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_GetBoundsInClientSpace(mriFunctionalDataRef this, xVoxelRef oBeginCorner, xVoxelRef oEndCorner)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != oBeginCorner && NULL != oEndCorner), eResult, FunD_tErr_InvalidParameter);
  ...
  while (xVoxl_IncrementUntilLimits(&curIdx, this->mpData->width, this->mpData->height, this->mpData->depth))
  {
...
FunD_tErr FunD_GetValueAtPercentile(mriFunctionalDataRef this, float iPercentile, float *oValue)
{
  ...
  DebugAssertThrowX((iPercentile >= 0 && iPercentile <= 100), eResult, FunD_tErr_InvalidParameter);
  ...
  numValues
      = this->mpData->width * this->mpData->height * this->mpData->depth * this->mNumConditions * this->mNumTimePoints;
  ...
  value = this->mMinValue + (((this->mMaxValue - this->mMinValue + 1) * nBin) / this->mNumBins);
                                ...
                                float *oThresholdMin)
{
  ...
  if (NULL != iMaskVolume)
  {
    ...
    pLocalMaskVol = MRIalloc(this->mpData->width, this->mpData->height, this->mpData->depth, this->mpData->type);
    DebugAssertThrowX((NULL != pLocalMaskVol), eResult, FunD_tErr_CouldntAllocateVolume);
    ...
    DebugAssertThrowX((NULL != localToClient), eResult, FunD_tErr_CouldntAllocateMatrix);

    DebugNote(("Converting client mask to local mask"));
    eMRI = MRIvol2Vol(iMaskVolume, pLocalMaskVol, localToClient, SAMPLE_NEAREST, 0);
    DebugAssertThrowX((ERROR_NONE == eMRI), eResult, FunD_tErr_CouldntTransformMask);
  }
  ...
  DebugAssertThrowX((ERROR_NONE == eMRI), eResult, FunD_tErr_ErrorPerformingFDR);
...
FunD_tErr FunD_SaveRegistration(mriFunctionalDataRef this)
{
  ...
  DebugAssertThrowX((NULL != regInfo), eResult, FunD_tErr_CouldntAllocateStorage);
  ...
  regInfo->mri2fmri = MatrixCopy(this->mIdxToIdxTransform->mARAStoBRAS, NULL);
...
FunD_tErr FunD_ApplyTransformToRegistration(mriFunctionalDataRef this, MATRIX *iTransform)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != iTransform), eResult, FunD_tErr_InvalidParameter);
  ...
  DebugAssertThrowX((Trns_tErr_NoErr == eTransform), eResult, FunD_tErr_ErrorAccessingTransform);
...
FunD_tErr FunD_TranslateRegistration(mriFunctionalDataRef this, float ifDistance, tAxis iAxis)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this), eResult, FunD_tErr_InvalidParameter);
  DebugAssertThrowX((iAxis >= 0 && iAxis < knNumAxes), eResult, FunD_tErr_InvalidParameter);
...
FunD_tErr FunD_RotateRegistration(mriFunctionalDataRef this, float ifDegrees, tAxis iAxis, xVoxelRef iCenterFuncRAS)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != this && NULL != iCenterFuncRAS), eResult, FunD_tErr_InvalidParameter);
  DebugAssertThrowX((iAxis >= 0 && iAxis < knNumAxes), eResult, FunD_tErr_InvalidParameter);
  ...
  /* do the inverse of this action */
  DebugNote(("Rotating transformation object by %f degrees around %d", -ifDegrees, iAxis));
...
FunD_tErr FunD_ResampleData_(mriFunctionalDataRef this)
{
  ...
  DebugAssertThrowX((this->mbHaveClientBounds), eResult, FunD_tErr_InvalidParameter);
  ...
  for (nCondition = 0; nCondition < this->mNumConditions; nCondition++)
  {
    for (nTimePoint = 0; nTimePoint < this->mNumTimePoints; nTimePoint++)
    {
      ...
      while (xVoxl_IncrementWithMinsUntilLimits(
          &clientIdx, xVoxl_GetX(&begin), xVoxl_GetY(&begin), xVoxl_GetX(&end), xVoxl_GetY(&end), xVoxl_GetZ(&end)))
      {

        DebugPrint(("\rResampling: %d %d %d", nCondition, nTimePoint, xVoxl_GetZ(&clientIdx)));
        ...
        FunD_SetValue_(this, volume, &resampleIdx, nCondition, nTimePoint, value);
      }
    }
  }
...
FunD_tErr FunD_CalcFrequencies_(mriFunctionalDataRef this, int iNumBins)
{
  ...
  DebugEnterFunction(("FunD_CalcFrequencies_( this=%p, iNumBins=%d )", this, iNumBins));
  ...
  DebugAssertThrowX((NULL != aFrequencies), eResult, FunD_tErr_CouldntAllocateStorage);
  ...
  for (nCondition = 0; nCondition < this->mNumConditions; nCondition++)
  {
    for (nTimePoint = 0; nTimePoint < this->mNumTimePoints; nTimePoint++)
    {
      ...
      while (xVoxl_IncrementUntilLimits(&curIdx, this->mpData->width, this->mpData->height, this->mpData->depth))
      {
        ...
        FunD_GetValue_(this, this->mpData, &curIdx, nCondition, nTimePoint, &value);

        /* Calculate the bin in which it should be. */
        nBin = (value - this->mMinValue) / (((this->mMaxValue - this->mMinValue) + 1.0) / (float)iNumBins);
...
void FunD_ConvertClientToFuncIdx_(mriFunctionalDataRef this, xVoxelRef iClientVox, xVoxelRef oFuncIdx)
{
  ...
  DebugNote(("Converting client to func idx"));
  eTransform = Trns_ConvertAtoB(this->mIdxToIdxTransform, &this->mTmpVoxel1, &this->mTmpVoxel2);
...
void FunD_ConvertClientToFloatFuncIdx_(mriFunctionalDataRef this, xVoxelRef iClientVox, xVoxelRef oFuncIdx)
{
  ...
  DebugNote(("Converting client to func idx"));
  eTransform = Trns_ConvertAtoB(this->mIdxToIdxTransform, &this->mTmpVoxel1, oFuncIdx);
  DebugAssertThrow((Trns_tErr_NoErr == eTransform));
...
void FunD_ConvertClientToFuncRAS_(mriFunctionalDataRef this, xVoxelRef iClientVox, xVoxelRef oFuncRAS)
{
  ...
  eTransform = Trns_ConvertBtoRAS(this->mIdxToIdxTransform, &this->mTmpVoxel2, &this->mTmpVoxel1);
  DebugAssertThrow((Trns_tErr_NoErr == eTransform));
...
void FunD_ConvertFuncIdxToClient_(mriFunctionalDataRef this, xVoxelRef iFuncIdx, xVoxelRef oClientVox)
{
  ...
  eTransform = Trns_ConvertBtoA(this->mIdxToIdxTransform, &this->mTmpVoxel1, &this->mTmpVoxel2);
  DebugAssertThrow((Trns_tErr_NoErr == eTransform));
...
void FunD_ConvertRASToFuncIdx_(mriFunctionalDataRef this, xVoxelRef iRAS, xVoxelRef oFuncIdx)
{
  ...
  eTransform = Trns_ConvertBRAStoB(this->mIdxToIdxTransform, &this->mTmpVoxel1, &this->mTmpVoxel2);
  DebugAssertThrow((Trns_tErr_NoErr == eTransform));
...
void FunD_ConvertFuncIdxToFuncRAS_(mriFunctionalDataRef this, xVoxelRef iFuncIdx, xVoxelRef oFuncRAS)
{
  ...
  eTransform = Trns_ConvertBtoRAS(this->mIdxToIdxTransform, &this->mTmpVoxel1, &this->mTmpVoxel2);
  DebugAssertThrow((Trns_tErr_NoErr == eTransform));
    ...
    mriFunctionalDataRef this, MRI *iData, xVoxelRef iIdx, int inCondition, int inTimePoint, float iValue)
{
  ...
  {
  case MRI_UCHAR:
    MRIseq_vox(iData, xVoxl_GetX(iIdx), xVoxl_GetY(iIdx), xVoxl_GetZ(iIdx), nFrame) = iValue;
    break;
  case MRI_INT:
    MRIIseq_vox(iData, xVoxl_GetX(iIdx), xVoxl_GetY(iIdx), xVoxl_GetZ(iIdx), nFrame) = iValue;
    break;
  case MRI_LONG:
    MRILseq_vox(iData, xVoxl_GetX(iIdx), xVoxl_GetY(iIdx), xVoxl_GetZ(iIdx), nFrame) = iValue;
    break;
  case MRI_FLOAT:
    MRIFseq_vox(iData, xVoxl_GetX(iIdx), xVoxl_GetY(iIdx), xVoxl_GetZ(iIdx), nFrame) = iValue;
    break;
  case MRI_SHORT:
    MRISseq_vox(iData, xVoxl_GetX(iIdx), xVoxl_GetY(iIdx), xVoxl_GetZ(iIdx), nFrame) = iValue;
    break;
  default:
    break;
  }
...
void FunD_GetSigma_(mriFunctionalDataRef this, xVoxelRef iFuncIdx, int inTimePoint, float *oSigma)
{
  ...
  {
  case MRI_UCHAR:
    *oSigma = MRIseq_vox(this->mpData, xVoxl_GetX(iFuncIdx), xVoxl_GetY(iFuncIdx), xVoxl_GetZ(iFuncIdx), nFrame);
    break;
  case MRI_INT:
    *oSigma = MRIIseq_vox(this->mpData, xVoxl_GetX(iFuncIdx), xVoxl_GetY(iFuncIdx), xVoxl_GetZ(iFuncIdx), nFrame);
    break;
  case MRI_LONG:
    *oSigma = MRILseq_vox(this->mpData, xVoxl_GetX(iFuncIdx), xVoxl_GetY(iFuncIdx), xVoxl_GetZ(iFuncIdx), nFrame);
    break;
  case MRI_FLOAT:
    *oSigma = MRIFseq_vox(this->mpData, xVoxl_GetX(iFuncIdx), xVoxl_GetY(iFuncIdx), xVoxl_GetZ(iFuncIdx), nFrame);
    break;
  case MRI_SHORT:
    *oSigma = MRISseq_vox(this->mpData, xVoxl_GetX(iFuncIdx), xVoxl_GetY(iFuncIdx), xVoxl_GetZ(iFuncIdx), nFrame);
    break;
  default:
    *oSigma = 0;
    break;
...
FunD_tErr FunD_Verify(mriFunctionalDataRef this)
{
  ...
  DebugAssertThrowX((NULL != this), eResult, FunD_tErr_InvalidParameter);
  DebugAssertThrowX((FunD_kSignature == this->mSignature), eResult, FunD_tErr_InvalidSignature);
...
FunD_tErr FunD_VerifyFuncIdx_(mriFunctionalDataRef this, xVoxelRef iFuncIdx)
{
  ...
  DebugAssertThrowX((xVoxl_GetFloatX(iFuncIdx) >= 0 && xVoxl_GetFloatX(iFuncIdx) < this->mpData->width
                     && xVoxl_GetFloatY(iFuncIdx) >= 0 && xVoxl_GetFloatY(iFuncIdx) < this->mpData->height
                     && xVoxl_GetFloatZ(iFuncIdx) >= 0 && xVoxl_GetFloatZ(iFuncIdx) < this->mpData->depth),
                    eResult,
...
FunD_tErr FunD_VerifyTimePoint(mriFunctionalDataRef this, int iTimePoint)
{
  ...
  DebugAssertThrowX((iTimePoint >= 0 && iTimePoint < this->mNumTimePoints), eResult, FunD_tErr_InvalidTimePoint);
...
FunD_tErr FunD_VerifyCondition(mriFunctionalDataRef this, int iCondition)
{
  ...
  DebugAssertThrowX((iCondition >= 0 && iCondition < this->mNumConditions), eResult, FunD_tErr_InvalidConditionIndex);
