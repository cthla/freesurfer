...
CLUSTER_SET *CSinit(int max_clusters, int ninputs, int normalize)
{
  ...
  if (!cs)
    ErrorExit(ERROR_NO_MEMORY, "ClusterInit(%d, %d); could not allocate struct", max_clusters, ninputs);
  ...
  if (!cs->m_sw)
    ErrorExit(ERROR_NO_MEMORY, "ClusterInit(%d, %d); could not allocate Sw", max_clusters, ninputs);
  cs->m_sb = MatrixAlloc(ninputs, ninputs, MATRIX_REAL);
  if (!cs->m_sb)
    ErrorExit(ERROR_NO_MEMORY, "ClusterInit(%d, %d); could not allocate Sb", max_clusters, ninputs);
  cs->nobs = 0;
  cs->means = (float *)calloc(ninputs, sizeof(float));
  if (!cs->means)
    ErrorExit(ERROR_NO_MEMORY, "ClusterInit(%d, %d); could not allocate means", max_clusters, ninputs);
  cs->stds = (float *)calloc(ninputs, sizeof(float));
  if (!cs->stds)
    ErrorExit(ERROR_NO_MEMORY, "ClusterInit(%d, %d); could not allocate std", max_clusters, ninputs);
...
static int clusterInit(CLUSTER *cluster, int ninputs)
{
  ...
  if (!cluster->m_scatter)
    ErrorExit(ERROR_NO_MEMORY, "clusterInit(%d): could not allocate scatter matrix", ninputs);
  cluster->v_means = VectorAlloc(ninputs, MATRIX_REAL);
  if (!cluster->v_means)
    ErrorExit(ERROR_NO_MEMORY, "clusterInit(%d): could not allocate mean vector", ninputs);
  cluster->v_seed = VectorAlloc(ninputs, MATRIX_REAL);
  if (!cluster->v_seed)
    ErrorExit(ERROR_NO_MEMORY, "clusterInit(%d): could not allocate centroid vector", ninputs);
  cluster->evalues = (float *)calloc(ninputs + 1, sizeof(float));
  if (!cluster->evalues)
    ErrorExit(ERROR_NO_MEMORY, "clusterInit(%d): could not allocate eigen values", ninputs);
