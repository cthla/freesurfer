...
BACKPROP *
BackpropAlloc(int ninputs, int noutputs, int nhidden, float trate, float momentum, float *mean_out, float *std_out)
{
  BACKPROP *backprop;
...
{
  if (bp_dst
      && ((bp_dst->ninputs != bp_src->ninputs) || (bp_dst->noutputs != bp_src->noutputs)
...
int BackpropWrite(BACKPROP *backprop, char *fname, int argc, char *argv[], char *comments, int mode)
{
  ...
  case BP_APPEND:
    fp = fopen(fname, "r+");
    if (!fp)
      ErrorReturn(-3, (ERROR_BAD_FILE, "BackpropWrite(%s): could not create file for appending\n", fname));
