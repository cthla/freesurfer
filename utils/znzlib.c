...
#ifdef HAVE_ZLIB
  if (file->zfptr != NULL)
    return gzgets(file->zfptr, str, size);
#endif
...
#ifdef HAVE_ZLIB
  if (file->zfptr != NULL)
    return gzflush(file->zfptr, Z_SYNC_FLUSH);
#endif
