...
MRI *MRImeanByte(MRI *mri_src, MRI *mri_dst, int wsize)
{
  ...
  if (mri_dst->type != MRI_UCHAR)
    ErrorReturn(mri_dst, (ERROR_UNSUPPORTED, "MRImeanByte: dst must be MRI_UCHAR"));
...
MRI *MRIstdInMask(MRI *mri_src, MRI *mri_dst, MRI *mri_mean, MRI *mri_mask, int wsize)
{
  ...
  if (mri_dst->type != MRI_FLOAT)
    ErrorReturn(mri_dst, (ERROR_UNSUPPORTED, "MRIstd: dst must be MRI_FLOAT"));
...
MRI *MRIorder(MRI *mri_src, MRI *mri_dst, int wsize, float pct)
{
  ...
  if (mri_dst->type != MRI_UCHAR)
    ErrorReturn(mri_dst, (ERROR_UNSUPPORTED, "MRIorder: dst must be MRI_UCHAR"));
    ...
    for (r = 0; r < src->height; r++)
    {
      for (s = 0; s < src->depth; s++)
      {
        ...
        for (f = 0; f < src->nframes; f++)
          MRIsetVoxVal(srcmasked, c, r, s, f, 0.0);
      }
    }
  }
...
MRI *MRIconvolve1d(MRI *mri_src, MRI *mri_dst, float *k, int len, int axis, int src_frame, int dst_frame)
{
  ...
  case MRI_UCHAR:
    ...
    case MRI_WIDTH:
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(y, x, inBase, foutPix, ki, i, total) shared(                                     \
    depth, height, width, len, halflen, mri_src, mri_dst, src_frame, dst_frame, xi, yi, zi) schedule(static, 1)
#endif
      for (z = 0; z < depth; z++)
      {
        for (y = 0; y < height; y++)
    ...
    case MRI_HEIGHT:
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(y, x, foutPix, ki, i, total) shared(                                             \
    depth, height, width, len, halflen, mri_dst, src_frame, dst_frame, xi, yi, zi) schedule(static, 1)
#endif
      for (z = 0; z < depth; z++)
      {
        for (y = 0; y < height; y++)
...
#pragma omp parallel for firstprivate(y, x, foutPix, ki, i, total) shared(                                             \
  ...
  case MRI_FLOAT:
    ...
    case MRI_WIDTH:
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(y, x, inBase_f, foutPix, ki, i, total) shared(                                   \
    depth, height, width, len, halflen, mri_dst, src_frame, dst_frame, xi, yi, zi) schedule(static, 1)
#endif
      for (z = 0; z < depth; z++)
      {
...
#pragma omp parallel for firstprivate(y, x, foutPix, ki, i, total) shared(                                             \
  ...
  default:
    ...
    case MRI_WIDTH:
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(y, x, foutPix, ki, i, val, total) shared(                                        \
    depth, height, width, len, halflen, mri_dst, src_frame, dst_frame, xi, yi, zi) schedule(static, 1)
#endif
      for (z = 0; z < depth; z++)
      {
...
MB2D *MB2Dcopy(MB2D *src, int CopyMRI, MB2D *copy)
{
  ...
  if (copy == NULL)
    copy = (MB2D *)calloc(sizeof(MB2D), 1);
  copy->type = src->type;
