...
MATRIX *MatlabRead(const char *fname)
{
  ...
  int nrows, ncols, row, col; // file_type,
  float *fptr = NULL;
    ...
    for (col = 0; col < mf->ncols; col++)
    {
      for (row = 0; row < mf->mrows; row++)
      {
        ...
        case MAT_DOUBLE:
          ...
          if (nitems != sizeof(double))
          {
            ErrorPrintf(ERROR_BADFILE, "%s: could not read val[%d, %d] from .mat file - imag\n", MatProgname, row, col);
            /*exit(4) ;*/
            return (-1);
          }
          break;
...
char *MatReadHeader0(FILE *fp, MATFILE *mf)
{
  ...
  if (!name)
    ErrorExit(ERROR_NO_MEMORY, "matReadHeader: couldn't allocate %d bytes for name", mf->namlen + 1);
...
MLFC *ReadMatlabFileContents(const char *fname)
{
  ...
  char *name, c;
  double **real_matrix, **imag_matrix;
  int nrows, ncols, row, col, len; // file_type,
  float *fptr = NULL;
...
char *MatReadHeader(FILE *fp, MATFILE *mf, long32 *compressed)
{
  ...
  if (mf->version == 5)
  {
    ...
    fseek(fp, 1, SEEK_CUR);
    if (fread(&dt, fourbytes, 1, fp) != 1)
    {
      ErrorPrintf(ERROR_BADFILE, "%s: could not read file\n", MatProgname);
    }
    ...
    if (fread(&tmp, fourbytes, 1, fp) != 1)
    {
      ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
    }
    if (DIFFERENT_ENDIAN(mf))
      tmp = swapLong32(tmp);
    ...
    fseek(fp, 12, SEEK_CUR);
    if (fread(&(mf->mrows), fourbytes, 1, fp) != 1)
    {
      ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
    }
    if (fread(&(mf->ncols), fourbytes, 1, fp))
    {
      ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
    }
    if (DIFFERENT_ENDIAN(mf))
    ...
    { /*normal element*/
      fseek(fp, 4, SEEK_CUR);
      if (fread(&(mf->namlen), 1, fourbytes, fp) != fourbytes)
      {
        ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
      }
      if (DIFFERENT_ENDIAN(mf))
      ...
      if (fread(name, (int)mf->namlen, sizeof(char), fp) != sizeof(char))
      {
        ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
      }
    ...
    if (fread(&(mf->type), 1, fourbytes, fp) != fourbytes)
    {
      ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
    }
    if (DIFFERENT_ENDIAN(mf))
...
char *znzMatReadHeader(FILE *fp, MATFILE *mf, char **data)
{
  ...
  // cthla: dt is typed as long32, which is an int. This fails at least on my 64 bit machine because sizeof(long) is
  // bigger than long32.
  //  if (fread(&dt, sizeof(long), 1, fp) != 1)
  if (fread(&dt, sizeof(long32), 1, fp) != 1)
  {
    ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
  }
  if (DIFFERENT_ENDIAN(mf))
  ...
  // if (fread(&size, sizeof(long), 1, fp) != 1)
  if (fread(&size, sizeof(long32), 1, fp) != 1)
  {
    ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
  }
  if (DIFFERENT_ENDIAN(mf))
  ...
  if (fread(buff, sizeof(char), size, fp) != size)
  {
    ErrorPrintf(ERROR_BADFILE, "%s: could not read variable\n", MatProgname);
  }
