...
char Volm_ksaErrorStrings[Volm_knNumErrorCodes][Volm_knErrStringLen]
    = { "No error.",
        ...
        "While trying to find a suitable destination volume for the display transform, couldn't find FREESURFER_HOME "
...
Volm_tErr Volm_New(mriVolumeRef *opVolume)
{
  ...
  DebugAssertThrowX((opVolume != NULL), eResult, Volm_tErr_InvalidParamater);
  ...
  stuff_four_by_four(tmpMatrix, 0.99, 0, 0, 0, 0.00, 0.9688, 0.042, 0, 0.00, -0.0485, 0.839, 0, 0.00, 0, 0, 1);
  ...
  MatrixIdentity(4, tmpMatrix);
  stuff_four_by_four(tmpMatrix, 0.99, 0, 0, 0, 0.00, 0.9688, 0.046, 0, 0.00, -0.0485, 0.9189, 0, 0.00, 0, 0, 1);
...
Volm_tErr Volm_Delete(mriVolumeRef *iopVolume)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != iopVolume), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_DeepClone(mriVolumeRef this, mriVolumeRef *opVolume)
{
  ...
  DebugEnterFunction(("Volm_DeepClone( this=%p, opVolume=%p )", this, opVolume));
  ...
  DebugAssertThrowX((NULL != clone->mpMriValues), eResult, Volm_tErr_CouldntCopyVolume);

  /* copy the transforms */
  if (NULL != this->mMRIIdxToAnaIdxTransform)
  {
    DebugNote(("Copying MRI index to ana idx transform"));
    eTransform = Trns_DeepClone(this->mMRIIdxToAnaIdxTransform, &(clone->mMRIIdxToAnaIdxTransform));
    DebugAssertThrowX((Trns_tErr_NoErr == eTransform), eResult, Volm_tErr_CouldntCopyTransform);
  }
  if (NULL != this->mDisplayTransform)
  {
    DebugNote(("Copying display transform"));
    clone->mDisplayTransform = TransformCopy(this->mDisplayTransform, NULL);
    DebugAssertThrowX((NULL != clone->mDisplayTransform), eResult, Volm_tErr_CouldntCopyTransform);
  }
  if (NULL != this->mMNITalLtzToRealTalTransform)
  {
    DebugNote(("Copying real tal ltz transform"));
    eTransform = Trns_DeepClone(this->mMNITalLtzToRealTalTransform, &(clone->mMNITalLtzToRealTalTransform));
    DebugAssertThrowX((Trns_tErr_NoErr == eTransform), eResult, Volm_tErr_CouldntCopyTransform);
  }
  if (NULL != this->mMNITalGtzToRealTalTransform)
  {
    DebugNote(("Copying real tal gtz transform"));
    eTransform = Trns_DeepClone(this->mMNITalGtzToRealTalTransform, &(clone->mMNITalGtzToRealTalTransform));
    DebugAssertThrowX((Trns_tErr_NoErr == eTransform), eResult, Volm_tErr_CouldntCopyTransform);
  }
  if (NULL != this->mScannerTransform)
  {
    DebugNote(("Copying scanner transform"));
    eTransform = Trns_DeepClone(this->mScannerTransform, &(clone->mScannerTransform));
    DebugAssertThrowX((Trns_tErr_NoErr == eTransform), eResult, Volm_tErr_CouldntCopyTransform);
  }

  /* copy the strings */
  xUtil_strncpy(clone->msSubjectName, this->msSubjectName, sizeof(clone->msSubjectName));
  xUtil_strncpy(clone->msVolumeName, this->msVolumeName, sizeof(clone->msVolumeName));
  xUtil_strncpy(clone->msOriginalPath, this->msOriginalPath, sizeof(clone->msOriginalPath));

  /* allocate and copy color tables */
  memmove(clone->mafColorTable, this->mafColorTable, sizeof(this->manColorTable));
  memmove(clone->manColorTable, this->manColorTable, sizeof(this->mafColorTable));
  ...
  clone->mResampleToRAS = MatrixCopy(this->mResampleToRAS, NULL);
  clone->mResampleToRASOrig = MatrixCopy(this->mResampleToRASOrig, NULL);
  clone->mResampleToRASInverse = MatrixCopy(this->mResampleToRASInverse, NULL);
  clone->mResampleToSlice = MatrixCopy(this->mResampleToSlice, NULL);
  clone->mResampleToSliceOrig = MatrixCopy(this->mResampleToSliceOrig, NULL);
  clone->mResampleToSliceInverse = MatrixCopy(this->mResampleToSliceInverse, NULL);
...
Volm_tErr Volm_CreateFromVolume(mriVolumeRef this, mriVolumeRef iVolume)
{
  ...
  DebugEnterFunction(("Volm_CreateFromVolume( this=%p, iVolume=%p )", this, iVolume));
  ...
  eResult = Volm_GetDimensions(iVolume, &nDimensionX, &nDimensionY, &nDimensionZ);
  DebugAssertThrow((eResult == Volm_tErr_NoErr));
  ...
  DebugNote(("Creating the mri with dimensions %d,%d,%d type %d nframes %d",
             ...
             nType,
             nDimensionFrames));
  mriVolume = MRIallocSequence(nDimensionX, nDimensionY, nDimensionZ, nType, nDimensionFrames);
  DebugAssertThrowX((NULL != mriVolume), eResult, Volm_tErr_CouldntReadVolume);
  ...
  Volm_CopySubjectName(iVolume, this->msSubjectName, sizeof(this->msSubjectName));
...
Volm_tErr Volm_ImportData(mriVolumeRef this, char *isSource)
{
  ...
  DebugEnterFunction(("Volm_ImportData( this=%p, isSource=%s )", this, isSource));
  ...
  xUtil_strncpy(this->msOriginalPath, isSource, sizeof(this->msOriginalPath));
...
Volm_tErr Volm_SetFromMRI_(mriVolumeRef this, MRI *iMRI)
{
  ...
  if (NULL != this->mpMriValues && this->mpMriValues != iMRI)
  {
    MRIfree(&this->mpMriValues);
  }
  ...
  DebugAssertThrowX((NULL != m_resample), eResult, Volm_tErr_CouldntNormalizeVolume);
  ...
  stuff_four_by_four(this->mResampleToSlice,
                     1.0 / this->mpMriValues->xsize,
                     0,
                     0,
                     0,
                     0,
                     1.0 / this->mpMriValues->ysize,
                     0,
                     0,
                     0,
                     0,
                     1.0 / this->mpMriValues->zsize,
                     0,
                     0,
                     0,
                     0,
  ...
  DebugAssertThrowX((NULL != scannerTransform), eResult, Volm_tErr_AllocationFailed);
...
Volm_tErr Volm_CalculateMRIIdxToAnaIdx_(mriVolumeRef this)
{
  ...
  */
  ...
  DebugAssertThrowX((NULL != idxToRASTransform), eResult, Volm_tErr_AllocationFailed);
...
Volm_tErr Volm_ExportNormToCOR(mriVolumeRef this, char *isPath)
{
  ...
  DebugEnterFunction(("Volm_ExportNormToCOR( this=%p, isPath=%s )", this, isPath));
...
#endif
  DebugAssertThrowX((NO_ERROR == eMRI), eResult, Volm_tErr_CouldntExportVolumeToCOR);
...
Volm_tErr Volm_Save(mriVolumeRef this, char *isFileName, tBoolean ibSaveFileName)
{
  ...
  DebugEnterFunction(("Volm_Save( this=%p, isFileName=%s, ibSaveFileName )", this, isFileName, ibSaveFileName));
  ...
  if (NULL != isFileName && ibSaveFileName)
  {
    ...
    xUtil_strncpy(this->msOriginalPath, isFileName, sizeof(this->msOriginalPath));
  }
...
Volm_tErr Volm_LoadDisplayTransform(mriVolumeRef this, char *isFileName)
{
  ...
  DebugEnterFunction(("Volm_LoadDisplayTransform( this=%p, isFileName=%s )", this, isFileName));
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != isFileName), eResult, Volm_tErr_InvalidParamater);
  ...
  DebugAssertThrowX((NULL != transform), eResult, Trns_tErr_LTAImportFailed);
  ...
  if (transform->type == LINEAR_RAS_TO_RAS)
  {
    ...
    if (!lta->xforms[0].dst.valid && NULL != strstr(isFileName, "talairach.xfm"))
    {
      ...
      /* Get FREESURFER_HOME. */
      fsenv = FSENVgetenv();
      DebugAssertThrowX((NULL != fsenv), eResult, Volm_tErr_FsenvNotFoundWhileSearchingForTransformDest);
      DebugAssertThrowX(
          (NULL != fsenv->FREESURFER_HOME), eResult, Volm_tErr_FsenvNotFoundWhileSearchingForTransformDest);

      /* Build the file name. */
      snprintf(sMNI305FileName, sizeof(sMNI305FileName), "%s/average/mni305.cor.mgz", fsenv->FREESURFER_HOME);

      /* Make sure the file exists. */
      eStat = stat(sMNI305FileName, &statMNI305);
      DebugAssertThrowX((0 == eStat), eResult, Volm_tErr_MNI305NotFoundWhilePopulatingTramsformDest);

      /* Load it and read the header. */
      MNI305volume = MRIreadHeader(sMNI305FileName, MRI_VOLUME_TYPE_UNKNOWN);
      DebugAssertThrowX((NULL != MNI305volume), eResult, Volm_tErr_ErrorReadingMNI305WhilePopulatingTramsformDest);
...
Volm_tErr Volm_CopyGeometryInformation(mriVolumeRef this, VOL_GEOM *ioVolumeGeometry)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != ioVolumeGeometry), eResult, Volm_tErr_InvalidParamater);
...
void Volm_GetIntColorAtIdx(mriVolumeRef this, xVoxelRef iIdx, xColor3nRef oColor)
{
  ...
  if (NULL != this->mDisplayTransform)
  {
    ...
    TransformSampleInverse(this->mDisplayTransform, xVoxl_ExpandFloat(iIdx), &x, &y, &z);
...
void Volm_GetMaxIntColorAtIdx(mriVolumeRef this, xVoxelRef iIdx, mri_tOrientation iOrientation, xColor3nRef oColor)
{
  ...
  colorIdx = (int)(255.0 * (value - this->mfMinValue) / (this->mfMaxValue - this->mfMinValue));
  *oColor = this->manColorTable[colorIdx];
}
...
Volm_tErr Volm_GetDimensions(mriVolumeRef this, int *onDimensionX, int *onDimensionY, int *onDimensionZ)
{
  ...
  DebugEnterFunction(
      ("Volm_GetDimensions( this=%p, onDimension=%p,%p,%p )", this, onDimensionX, onDimensionY, onDimensionZ));
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX(
      (onDimensionX != NULL) && (onDimensionY != NULL) && (onDimensionZ != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_GetNumberOfFrames(mriVolumeRef this, int *onDimensionFrames)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((onDimensionFrames != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_GetType(mriVolumeRef this, int *onType)
{
  ...
  DebugAssertThrowX((NULL != this->mpMriValues), eResult, Volm_tErr_MRIVolumeNotPresent);
...
Volm_tErr Volm_GetResampleMethod(mriVolumeRef this, Volm_tResampleMethod *oMethod)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((oMethod != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_SetResampleMethod(mriVolumeRef this, Volm_tResampleMethod iMethod)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iMethod >= 0 && iMethod < Volm_knNumResampleMethods), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_GetSampleType(mriVolumeRef this, Volm_tSampleType *oType)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((oType != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_SetSampleType(mriVolumeRef this, Volm_tSampleType iType)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iType >= 0 && iType < Volm_knNumSampleTypes), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_GetValueAtIdx(mriVolumeRef this, xVoxelRef iIdx, float *oValue)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iIdx != NULL && oValue != NULL), eResult, Volm_tErr_InvalidParamater);
  eResult = Volm_VerifyIdx_(this, iIdx);
  DebugAssertThrow((eResult == Volm_tErr_NoErr));

  /* return the value */
  DebugNote(("Fetching the value at (%d,%d,%d) and returning it", xVoxl_ExpandInt(iIdx)));
...
Volm_tErr Volm_GetValueAtIdxFrame(mriVolumeRef this, xVoxelRef iIdx, int iFrame, float *oValue)
{
  ...
  /* return the value */
  DebugNote(("Fetching the value at (%d,%d,%d) frame %d and returning it", xVoxl_ExpandInt(iIdx), iFrame));
...
Volm_tErr Volm_GetValueAtMRIIdx(mriVolumeRef this, xVoxelRef iMRIIdx, float *oValue)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iMRIIdx != NULL && oValue != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_ConvertIdxToRAS(mriVolumeRef this, xVoxelRef iIdx, xVoxelRef oRAS)
{
  ...
  DebugEnterFunction(("Volm_ConvertIdxToRAS( this=%p, iIdx=%p, oRAS=%p )", this, iIdx, oRAS));
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iIdx != NULL && oRAS != NULL), eResult, Volm_tErr_InvalidParamater);
  eResult = Volm_VerifyIdx_(this, iIdx);
  DebugAssertThrow((eResult == Volm_tErr_NoErr));
  ...
  /* convert idx to ras */
  DebugNote(("Converting idx (%.2f, %.2f, %.2f) to RAS with MRIvoxelToWorld", xVoxl_ExpandFloat(&mriIdx)));
  MRIvoxelToWorld(this->mpMriValues, xVoxl_ExpandFloat(&mriIdx), &rasX, &rasY, &rasZ);
...
Volm_tErr Volm_ConvertRASToIdx(mriVolumeRef this, xVoxelRef iRAS, xVoxelRef oIdx)
{
  ...
  DebugEnterFunction(("Volm_ConvertRASToIdx( this=%p, iRAS=%p, oIdx=%p )", this, iRAS, oIdx));
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iRAS != NULL && oIdx != NULL), eResult, Volm_tErr_InvalidParamater);

  /* convert ras to MRI idx */
  DebugNote(("Converting RAS (%.2f, %.2f, %.2f) to idx with MRIworldToVoxel", xVoxl_ExpandFloat(iRAS)));
  MRIworldToVoxel(this->mpMriValues, xVoxl_ExpandFloat(iRAS), &idxX, &idxY, &idxZ);
...
Volm_tErr Volm_ConvertIdxToMNITal(mriVolumeRef this, xVoxelRef iIdx, xVoxelRef oMNITal)
{
  ...
  transform_point(this->mpMriValues->linear_transform, xVoxl_ExpandFloat(&scannerRAS), &talX, &talY, &talZ);
...
Volm_tErr Volm_ConvertTalToIdx(mriVolumeRef this, xVoxelRef iTal, xVoxelRef oIdx)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iTal != NULL && oIdx != NULL), eResult, Volm_tErr_InvalidParamater);
  ...
  DebugNote(("Converting tal (%.2f, %.2f, %.2f) to idx with "
             "MRItalairachToVoxel",
             xVoxl_ExpandFloat(&mniTal)));
  eMRI = MRItalairachToVoxel(this->mpMriValues, xVoxl_ExpandFloat(&mniTal), &idxX, &idxY, &idxZ);
...
Volm_tErr Volm_ConvertIdxToScanner(mriVolumeRef this, xVoxelRef iIdx, xVoxelRef oScanner)
{
  ...
  DebugAssertThrowX((NULL != this->mScannerTransform), eResult, Volm_tErr_ScannerTransformNotPresent);
  ...
  DebugNote(("Converting idx (%.2f, %.2f, %.2f) to scanner", xVoxl_ExpandFloat(&mriIdx)));
...
Volm_tErr Volm_ConvertMRIIdxToScanner(mriVolumeRef this, xVoxelRef iMRIIdx, xVoxelRef oScanner)
{
  ...
  /* convert idx to scanner */
  DebugNote(("Converting idx (%.2f, %.2f, %.2f) to scanner", xVoxl_ExpandFloat(iMRIIdx)));
...
Volm_tErr Volm_ConvertMRIIdxToRAS(mriVolumeRef this, xVoxelRef iMRIIdx, xVoxelRef oRAS)
{
  ...
  DebugEnterFunction(("Volm_ConvertIdxToRAS( this=%p, iMRIIdx=%p, oRAS=%p )", this, iMRIIdx, oRAS));
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iMRIIdx != NULL && oRAS != NULL), eResult, Volm_tErr_InvalidParamater);
  eResult = Volm_VerifyMRIIdx_(this, iMRIIdx);
  DebugAssertThrow((eResult == Volm_tErr_NoErr));

  /* Convert idx to ras */
  DebugNote(("Converting idx to RAS with MRIvoxelToWorld"));
  MRIvoxelToWorld(this->mpMriValues, xVoxl_ExpandFloat(iMRIIdx), &rasX, &rasY, &rasZ);
...
Volm_tErr Volm_ConvertRASToMRIIdx(mriVolumeRef this, xVoxelRef iRAS, xVoxelRef oMRIIdx)
{
  ...
  DebugEnterFunction(("Volm_ConvertRASToIdx( this=%p, iRAS=%p, oMRIIdx=%p )", this, iRAS, oMRIIdx));
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iRAS != NULL && oMRIIdx != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_ConvertMRIIdxToSurfaceRAS(mriVolumeRef this, xVoxelRef iMRIIdx, xVoxelRef oSurfaceRAS)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iMRIIdx != NULL && oSurfaceRAS != NULL), eResult, Volm_tErr_InvalidParamater);
  eResult = Volm_VerifyMRIIdx_(this, iMRIIdx);
  DebugAssertThrow((eResult == Volm_tErr_NoErr));

  /* Convert idx to ras */
  DebugNote(("Converting idx to surface RAS with MRIvoxelToSurfaceRAS"));
  MRIvoxelToSurfaceRAS(this->mpMriValues, xVoxl_ExpandFloat(iMRIIdx), &rasX, &rasY, &rasZ);
...
Volm_tErr Volm_ConvertSurfaceRASToMRIIdx(mriVolumeRef this, xVoxelRef iSurfaceRAS, xVoxelRef oMRIIdx)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iSurfaceRAS != NULL && oMRIIdx != NULL), eResult, Volm_tErr_InvalidParamater);

  /* convert ras to MRI idx */
  DebugNote(("Converting surface RAS to mri idx with MRIsurfaceRASToVoxel"));
  MRIsurfaceRASToVoxel(this->mpMriValues, xVoxl_ExpandFloat(iSurfaceRAS), &idxX, &idxY, &idxZ);
...
Volm_tErr Volm_GetMRIIdxToAnaIdxTransform(mriVolumeRef this, mriTransformRef *opTransform)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((NULL != opTransform), eResult, Volm_tErr_InvalidParamater);

  /* make sure we have the transform */
  DebugAssertThrowX((NULL != this->mMRIIdxToAnaIdxTransform), eResult, Volm_tErr_IdxToRASTransformNotPresent);
...
Volm_tErr Volm_Flood(mriVolumeRef this, Volm_tFloodParams *iParams)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((iParams->mfSourceValue >= this->mfMinValue && iParams->mfSourceValue <= this->mfMaxValue),
                    eResult,
  ...
  {
    DebugAssertThrowX((iParams->mOrientation > mri_tOrientation_None && iParams->mOrientation < mri_knNumOrientations),
                      eResult,
                      Volm_tErr_InvalidParamater);
  }
  DebugAssertThrowX((iParams->mpFunction != NULL), eResult, Volm_tErr_InvalidParamater);
  ...
  while (xList_tErr_NoErr == eList)
  {
    ...
    if (eList == xList_tErr_NoErr && NULL != pCurVoxel)
    {
      ...
      volume and continue. */
      nVisitedIndex = xVoxl_ExpandToIndex(curVoxel, this->mnDimensionZ, this->mnDimensionY);
      ...
      {
        fDistance = sqrt(((xVoxl_GetX(curVoxel) - xVoxl_GetX(&(iParams->mSourceIdx)))
      ...
      eVisit = iParams->mpFunction(curVoxel, fValue, iParams->mpFunctionData);
      switch (eVisit)
      {
...
Volm_tErr Volm_VisitAllVoxels(mriVolumeRef this, Volm_tVisitFunction iFunc, void *ipData)
{
  ...
  DebugEnterFunction(("Volm_tVisitFunction( this=%p, iFunc=%p, ipData=%p )", this, iFunc, ipData));
  ...
  while (bGo)
  {
    ...
    case Volm_tVisitComm_Continue:
      bGo = xVoxl_IncrementUntilLimits(&idx, this->mnDimensionX - 1, this->mnDimensionY - 1, this->mnDimensionZ - 1);
      continue;
      break;
    case Volm_tVisitComm_SkipRestOfRow:
...
Volm_tErr Volm_FindMaxValues(mriVolumeRef this)
{
  ...
  DebugAssertThrowX((NULL != this->mpMaxValuesX), eResult, Volm_tErr_AllocationFailed);
  for (nY = 0; nY < this->mnDimensionY; nY++)
  {
    this->mpMaxValuesX[nY] = (float *)calloc(this->mnDimensionZ, sizeof(float *));
    DebugAssertThrowX((NULL != this->mpMaxValuesX[nY]), eResult, Volm_tErr_AllocationFailed);
  }

  this->mpMaxValuesY = (float **)calloc(this->mnDimensionX, sizeof(float *));
  DebugAssertThrowX((NULL != this->mpMaxValuesY), eResult, Volm_tErr_AllocationFailed);
  for (nX = 0; nX < this->mnDimensionX; nX++)
  {
    this->mpMaxValuesY[nX] = (float *)calloc(this->mnDimensionZ, sizeof(float *));
    DebugAssertThrowX((NULL != this->mpMaxValuesY[nX]), eResult, Volm_tErr_AllocationFailed);
  }

  this->mpMaxValuesZ = (float **)calloc(this->mnDimensionX, sizeof(float *));
  DebugAssertThrowX((NULL != this->mpMaxValuesZ), eResult, Volm_tErr_AllocationFailed);
  for (nX = 0; nX < this->mnDimensionX; nX++)
  {
    this->mpMaxValuesZ[nX] = (float *)calloc(this->mnDimensionY, sizeof(float *));
    DebugAssertThrowX((NULL != this->mpMaxValuesZ[nX]), eResult, Volm_tErr_AllocationFailed);
  }
...
Volm_tErr Volm_GetBrightnessAndContrast(mriVolumeRef this, float *ofBrightness, float *ofContrast)
{
  ...
  eResult = Volm_Verify(this);
  DebugAssertThrow((eResult == Volm_tErr_NoErr));
  DebugAssertThrowX((NULL != ofBrightness && NULL != ofContrast), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_SaveToSnapshot(mriVolumeRef this)
{
  ...
  DebugAssertThrowX((NULL != this->mpSnapshot), eResult, Volm_tErr_AllocationFailed);
...
Volm_tErr Volm_Rotate(mriVolumeRef this, mri_tOrientation iAxis, float ifDegrees)
{
  ...
  DebugEnterFunction(("Volm_Rotate( this=%p, iAxis=%d, ifDegrees=%.2f )", this, iAxis, ifDegrees));
...
Volm_tErr Volm_Threshold(mriVolumeRef this, float iThreshold, tBoolean ibAbove, float iNewValue)
{
  ...
  while (xVoxl_IncrementUntilLimits(&idx, this->mnDimensionX - 1, this->mnDimensionY - 1, this->mnDimensionZ - 1))
  {
    ...
    if ((ibAbove && value > iThreshold) || (!ibAbove && value < iThreshold))
    {
...
Volm_tErr Volm_Flip(mriVolumeRef this, mri_tOrientation iAxis)
{
  ...
  while (xVoxl_IncrementUntilLimits(&idx, nXLimit, nYLimit, nZLimit))
  {
    ...
    {
    case mri_tOrientation_Coronal:
      xVoxl_Set(&flippedIdx, xVoxl_GetX(&idx), xVoxl_GetY(&idx), this->mnDimensionZ - 1 - xVoxl_GetZ(&idx));
      break;
    case mri_tOrientation_Sagittal:
      xVoxl_Set(&flippedIdx, this->mnDimensionX - 1 - xVoxl_GetX(&idx), xVoxl_GetY(&idx), xVoxl_GetZ(&idx));
      break;
    case mri_tOrientation_Horizontal:
      xVoxl_Set(&flippedIdx, xVoxl_GetX(&idx), this->mnDimensionY - 1 - xVoxl_GetY(&idx), xVoxl_GetZ(&idx));
      break;
    default:
      DebugAssertThrowX((1), eResult, Volm_tErr_InvalidParamater);
      break;
...
Volm_tErr Volm_CopySubjectName(mriVolumeRef this, char *oSubjectName, int inDestLen)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((oSubjectName != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_CopyVolumeName(mriVolumeRef this, char *oVolumeName, int inDestLen)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((oVolumeName != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_CopySourceDir(mriVolumeRef this, char *oSourceDir, int inDestLen)
{
  ...
  DebugNote(("Checking parameters"));
  DebugAssertThrowX((oSourceDir != NULL), eResult, Volm_tErr_InvalidParamater);
...
Volm_tErr Volm_ExtractAndSetSubjectName(mriVolumeRef this, char *isSource)
{
  ...
  DebugEnterFunction(("Volm_ExtractAndSetSubjectName( this=%p, isSource=%s)", this, isSource));
  ...
  else
  {
    ...
    if (NULL != sWord)
    {
      ...
      nChar = 0;
      while (sWord[nChar] != '/' && sWord[nChar] != '\0')
      {
        nChar++;
      }
...
Volm_tErr Volm_ExtractAndSetVolumeName(mriVolumeRef this, char *isSource)
{
  ...
  DebugEnterFunction(("Volm_ExtractAndSetVolumeName( this=%p, isSource=%s )", this, isSource));
...
Volm_tErr Volm_Conform(mriVolumeRef this)
{
  ...
  DebugAssertThrowX((NULL != conformed), eResult, Volm_tErr_CouldntReadVolume);
...
void Volm_ConvertScreenIdxToMRIIdx_(mriVolumeRef this, xVoxelRef iScreenIdx, xVoxelRef oMRIIdx)
{
  ...
  *MATRIX_RELT(this->mpTmpScreenIdx, 4, 1) = 1.0;
  ...
  if ((xVoxl_GetX(oMRIIdx) < 0 || xVoxl_GetX(oMRIIdx) >= this->mnDimensionX || xVoxl_GetY(oMRIIdx) < 0
       || xVoxl_GetY(oMRIIdx) >= this->mnDimensionY || xVoxl_GetZ(oMRIIdx) < 0
...
void Volm_ConvertMRIIdxToScreenIdx_(mriVolumeRef this, xVoxelRef iMRIIdx, xVoxelRef oScreenIdx)
{
  ...
  *MATRIX_RELT(this->mpTmpMRIIdx, 4, 1) = 1.0;
  ...
  MatrixMultiply(this->m_resample_inv, this->mpTmpMRIIdx, this->mpTmpScreenIdx);
  ...
  if ((xVoxl_GetX(oScreenIdx) < 0 || xVoxl_GetX(oScreenIdx) >= 256 || xVoxl_GetY(oScreenIdx) < 0
       || xVoxl_GetY(oScreenIdx) >= 256 || xVoxl_GetZ(oScreenIdx) < 0 || xVoxl_GetZ(oScreenIdx) >= 256))
  {
...
void Volm_GetValueAtIdx_(mriVolumeRef this, xVoxelRef iIdx, float *oValue)
{
  ...
  case MRI_UCHAR:
    *oValue = MRIvox(
        this->mpMriValues, xVoxl_GetX(&this->mTmpVoxel), xVoxl_GetY(&this->mTmpVoxel), xVoxl_GetZ(&this->mTmpVoxel));
    break;
  case MRI_INT:
    *oValue = MRIIvox(
        this->mpMriValues, xVoxl_GetX(&this->mTmpVoxel), xVoxl_GetY(&this->mTmpVoxel), xVoxl_GetZ(&this->mTmpVoxel));
...
void Volm_GetValueAtMRIIdx_(mriVolumeRef this, xVoxelRef iMRIIdx, float *oValue)
{
  ...
  case MRI_UCHAR:
    *oValue = MRIvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx));
    break;
  case MRI_INT:
    *oValue = MRIIvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx));
    break;
  case MRI_LONG:
    *oValue = MRILvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx));
    break;
  case MRI_FLOAT:
    *oValue = MRIFvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx));
    break;
  case MRI_SHORT:
    *oValue = MRISvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx));
    break;
  default:
    *oValue = 0;
    break;
...
void Volm_GetValueAtIdxFrame_(mriVolumeRef this, xVoxelRef iIdx, int iFrame, float *oValue)
{
  ...
  {
  case MRI_UCHAR:
    *oValue = MRIseq_vox(this->mpMriValues,
                         xVoxl_GetX(&this->mTmpVoxel),
                         xVoxl_GetY(&this->mTmpVoxel),
                         xVoxl_GetZ(&this->mTmpVoxel),
                         iFrame);
    break;
  case MRI_INT:
    *oValue = MRIIseq_vox(this->mpMriValues,
                          xVoxl_GetX(&this->mTmpVoxel),
                          xVoxl_GetY(&this->mTmpVoxel),
                          xVoxl_GetZ(&this->mTmpVoxel),
                          iFrame);
    break;
  case MRI_LONG:
    *oValue = MRILseq_vox(this->mpMriValues,
                          xVoxl_GetX(&this->mTmpVoxel),
                          xVoxl_GetY(&this->mTmpVoxel),
                          xVoxl_GetZ(&this->mTmpVoxel),
                          iFrame);
    break;
  case MRI_FLOAT:
    *oValue = MRIFseq_vox(this->mpMriValues,
                          xVoxl_GetX(&this->mTmpVoxel),
                          xVoxl_GetY(&this->mTmpVoxel),
                          xVoxl_GetZ(&this->mTmpVoxel),
                          iFrame);
    break;
  case MRI_SHORT:
    *oValue = MRISseq_vox(this->mpMriValues,
                          xVoxl_GetX(&this->mTmpVoxel),
                          xVoxl_GetY(&this->mTmpVoxel),
                          xVoxl_GetZ(&this->mTmpVoxel),
                          iFrame);
...
void Volm_SetValueAtIdx_(mriVolumeRef this, xVoxelRef iIdx, float iValue)
{
  ...
  default:
    break;
  case MRI_UCHAR:
    MRIvox(this->mpMriValues, xVoxl_GetX(&this->mTmpVoxel), xVoxl_GetY(&this->mTmpVoxel), xVoxl_GetZ(&this->mTmpVoxel))
        = (BUFTYPE)iValue;
    break;
  case MRI_SHORT:
    MRISvox(this->mpMriValues, xVoxl_GetX(&this->mTmpVoxel), xVoxl_GetY(&this->mTmpVoxel), xVoxl_GetZ(&this->mTmpVoxel))
        = (short)iValue;
    break;
  case MRI_FLOAT:
    MRIFvox(this->mpMriValues, xVoxl_GetX(&this->mTmpVoxel), xVoxl_GetY(&this->mTmpVoxel), xVoxl_GetZ(&this->mTmpVoxel))
        = (float)iValue;
    break;
  case MRI_LONG:
    MRILvox(this->mpMriValues, xVoxl_GetX(&this->mTmpVoxel), xVoxl_GetY(&this->mTmpVoxel), xVoxl_GetZ(&this->mTmpVoxel))
        = (long)iValue;
    break;
  case MRI_INT:
    MRIIvox(this->mpMriValues, xVoxl_GetX(&this->mTmpVoxel), xVoxl_GetY(&this->mTmpVoxel), xVoxl_GetZ(&this->mTmpVoxel))
        = (int)iValue;
    break;
  }
}
...
void Volm_SetValueAtMRIIdx_(mriVolumeRef this, xVoxelRef iMRIIdx, float iValue)
{
  ...
  default:
    break;
  case MRI_UCHAR:
    MRIvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx)) = (BUFTYPE)iValue;
    break;
  case MRI_SHORT:
    MRISvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx)) = (short)iValue;
    break;
  case MRI_FLOAT:
    MRIFvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx)) = (float)iValue;
    break;
  case MRI_LONG:
    MRILvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx)) = (long)iValue;
    break;
  case MRI_INT:
    MRIIvox(this->mpMriValues, xVoxl_GetX(iMRIIdx), xVoxl_GetY(iMRIIdx), xVoxl_GetZ(iMRIIdx)) = (int)iValue;
    break;
  }
}
...
Volm_tErr Volm_Verify(mriVolumeRef this)
{
  ...
  DebugAssertThrowX((Volm_kSignature == this->mSignature), eResult, Volm_tErr_InvalidSignature);
...
Volm_tErr Volm_VerifyIdx_(mriVolumeRef this, xVoxelRef iIdx)
{
  ...
  DebugAssertThrowX((xVoxl_GetX(&mriIdx) >= 0 && xVoxl_GetRoundX(&mriIdx) < this->mnDimensionX
                     && xVoxl_GetY(&mriIdx) >= 0 && xVoxl_GetRoundY(&mriIdx) < this->mnDimensionY
                     && xVoxl_GetZ(&mriIdx) >= 0 && xVoxl_GetRoundZ(&mriIdx) < this->mnDimensionZ),
                    eResult,
...
Volm_tErr Volm_VerifyMRIIdx_(mriVolumeRef this, xVoxelRef iMRIIdx)
{
  ...
  DebugAssertThrowX((xVoxl_GetX(iMRIIdx) >= 0 && xVoxl_GetRoundX(iMRIIdx) < this->mnDimensionX
                     && xVoxl_GetY(iMRIIdx) >= 0 && xVoxl_GetRoundY(iMRIIdx) < this->mnDimensionY
                     && xVoxl_GetZ(iMRIIdx) >= 0 && xVoxl_GetRoundZ(iMRIIdx) < this->mnDimensionZ),
                    eResult,
                    Volm_tErr_InvalidIdx);
...
Volm_tErr Volm_VerifyIdxInMRIBounds(mriVolumeRef this, xVoxelRef iScreenIdx)
{
  ...
  *MATRIX_RELT(this->mpTmpScreenIdx, 4, 1) = 1.0;
  ...
  DebugNote(("Checking mri idx"));
  DebugAssertThrowX((xVoxl_GetX(&this->mTmpVoxel) >= 0 && xVoxl_GetRoundX(&this->mTmpVoxel) < this->mnDimensionX
                     && xVoxl_GetY(&this->mTmpVoxel) >= 0 && xVoxl_GetRoundY(&this->mTmpVoxel) < this->mnDimensionY
                     && xVoxl_GetZ(&this->mTmpVoxel) >= 0 && xVoxl_GetRoundZ(&this->mTmpVoxel) < this->mnDimensionZ),
                    eResult,
                    Volm_tErr_InvalidIdx);
