...
int StcWrite(char *fname, MATRIX *m_data, float epoch_begin_lat, float sample_period, int *vertices, int nvertices)
{
  ...
  fp = fopen(fname, "w");
  if (fp == NULL)
    ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "MRISwriteStc: can't open file %s\n", fname));
...
STC_FILE *StcOpen(char *fstem)
{
  ...
  if (stc_file->file_handle == NULL)
    ErrorReturn(NULL, (ERROR_NOFILE, "StcRead: could not open %s", fname));
...
STC *StcRead(char *fstem)
{
  ...
  if (!stc->vertices)
    ErrorExit(ERROR_NOMEMORY, "StcRead(%s): could not allocated %d vector", fname, stc->nvertices);
