...
#endif
static MRI *
mriSplineNormalizeShort(MRI *mri_src, MRI *mri_dst, MRI **pmri_field, float *inputs, float *outputs, int npoints);
...
MRI *MRIsplineNormalize(MRI *mri_src, MRI *mri_dst, MRI **pmri_field, float *inputs, float *outputs, int npoints)
{
  ...
  if (pmri_field)
  {
    ...
    if (!mri_field)
      *pmri_field = mri_field = MRIalloc(BIAS_IMAGE_WIDTH, mri_src->height, 1, MRI_UCHAR);
  }
    ...
    for (z = 0; z < depth; z++)
    {
      for (y = 0; y < height; y++)
      {
        for (x = 0; x < width; x++)
        {
          ...
          {
            if (Gx >= 0)
              printf("bias at (%d, %d, %d) = %2.1f, T1 %2.0f --> %d\n", Gx, Gy, Gz, bias, src, nint(norm));
            DiagBreak();
          }
        }
      }
    ...
    for (z = 0; z < depth; z++)
    {
      for (y = 0; y < height; y++)
      {
        ...
        for (x = 0; x < width; x++)
        {
          ...
          if (num > 0)
            MRISseq_vox(mri_dst, x, y, z, f) = (short)nint((float)mean / (float)num);
        }
      }
    }
...
MRI *MRInormalizeHighSignalLowStd(MRI *mri_src, MRI *mri_norm, float bias_sigma, float wm_target)
{
  ...
  // nctrl =
  MRInormAddFileControlPoints(mri_ctrl, 255);
  MRIbinarize(mri_ctrl, mri_ctrl, 1, CONTROL_NONE, CONTROL_MARKED);
  mri_bias = MRIbuildBiasImage(mri_src, mri_ctrl, NULL, bias_sigma);
  MRIfree(&mri_ctrl);
  for (z = 0; z < depth; z++)
