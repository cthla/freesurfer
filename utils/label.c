...
#include "minc_volume_io.h" // order necessary to compile
...
#include "mrishash.h" // order necessary to compile
...
LABEL *LabelReadFrom(const char *subject_name, FILE *fp)
{
  ...
  if (!cp)
    ErrorReturn(NULL, (ERROR_BADFILE, "%s: empty label", Progname));
...
LABEL *LabelRead(const char *subject_name, const char *label_name)
{
  ...
  if (!fp)
    ErrorReturn(NULL, (ERROR_NOFILE, "%s: could not open label file %s", Progname, fname));
...
int LabelFree(LABEL **parea)
{
  ...
  area = *parea;
  *parea = NULL;
  if (area->vertex_label_ind)
    free(area->vertex_label_ind);
...
int LabelWrite(LABEL *area, const char *label_name)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_NOFILE, (ERROR_NO_FILE, "%s: could not open label file %s", Progname, fname));
...
LABEL *LabelCompact(LABEL *lsrc, LABEL *ldst)
{
  ...
  if (ldst->vertex_label_ind != NULL)
    for (i = 0; i < ldst->max_points; i++)
      ldst->vertex_label_ind[i] = -1;
  ...
  for (i = n = 0; i < lsrc->n_points; i++)
    if (lsrc->lv[i].deleted == 0)
    {
      ...
      if (lsrc->lv[i].vno >= 0 && lsrc->lv[i].vno < ldst->max_points)
        ldst->vertex_label_ind[lsrc->lv[i].vno] = n;
      n++;
    }
  ldst->n_points = n;
  return (ldst);
...
/*-------------------------------------------------------------
  ...
  -------------------------------------------------------------*/
LABEL *MaskSurfLabel(LABEL *lbl, MRI *SurfMask, float thresh, char *masksign, int frame)
{
  LABEL *msklbl;
    ...
    for (vno = 0; vno < mris->nvertices; vno++)
    {
      ...
      if (found)
      {
        ...
        if (area->n_points >= area->max_points)
          LabelRealloc(area, nint(area->max_points * 1.5));
...
LABEL *LabelBaryFill(MRIS *mris, LABEL *srclabel, double delta)
{
  ...
  for (k = 0; k < mris->nfaces; k++)
  {
    ...
    for (n = 0; n < 3; n++)
    {
      ...
      if (MRIgetVoxVal(mri, vtxno, 0, 0, 0) < 0.5)
        continue;
      ok = 1;
      break;
    }
  ...
  for (n = 0; n < area->n_points; n++)
  {
    ...
    if (bucket) // some vertices map to this voxel
    {
      ...
      for (bin = bucket->bins, i = 0; i < bucket->nused; i++, bin++) // find min dist vertex
      {
        ...
        // check to see if this vertex maps to the voxel in question and if so add it
        MRISgetCoords(v, coords, &x, &y, &z);
        MRISsurfaceRASToVoxel(mris, mri_template, x, y, z, &xv, &yv, &zv);
        if (nint(xv) == lv->xv && nint(yv) == lv->yv && nint(zv) == lv->zv)
...
static int update_vertex_indices(LABEL *area)
{
  ...
  if (area->vertex_label_ind == NULL || mris == NULL)
    return (NO_ERROR);
