...
double
cvector_compute_snr_F(float *c1_mean, float *c2_mean, float *vvar, float *snr, int num, int *pi, float bonferroni)
{
  int i, max_i;
  float f, max_snr;
...
int *cvector_sort(float *vbest_snr, int nvertices)
{
  ...
  if (!se_table)
    ErrorExit(ERROR_NOMEMORY, "cvector_sort: could not allocated %d vector", nvertices);
