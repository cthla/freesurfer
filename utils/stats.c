...
STAT_TABLE *LoadStatTable(const char *statfile)
{
  ...
  if (fgets(tmpstr, 100000, fp) == NULL)
  {
...
SV *StatReadVolume(const char *prefix)
{
  ...
  if (!sv)
    ErrorExit(ERROR_NOMEMORY, "StatReadVolume(%s): could not allocate sv", prefix);
  ...
  if (fp) /* means there are time points and means and sigmas */
  {
    ...
    {
      if (fscanf(fp, "%*s %f", &TER) != 1)
      {
        printf("ERROR: could not read file\n");
        return (NULL);
      }
      if (fscanf(fp, "%*s %d", &DOF) != 1)
      {
        printf("ERROR: could not read file\n");
        return (NULL);
      }
  ...
  if (fscanf(fp, "%d %d %d", &width, &height, &nframes) != 3)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "StatReadVolume: could not read file"));
  }
...
int StatAccumulateSurfaceVolume(SV *sv_surf, SV *sv, MRI_SURFACE *mris)
{
  ...
  float mean, surf_mean, std, surf_std, surf_dof, dof, xs, ys, zs;
  // float xoff, yoff, zoff, sxoff, syoff, szoff;
  ...
  for (event = 0; event < sv_surf->nevents; event++)
  {
    ...
    if (Gdiag & DIAG_SHOW)
      fprintf(stderr, "\rprocessing event %d of %d....", event + 1, sv_surf->nevents);
...
int StatAccumulateTalairachVolume(SV *sv_tal, SV *sv)
{
  ...
  // double   xf, yf, zf ;
  float mean, tal_mean, std, tal_std, tal_dof, dof;
  // float xoff, yoff, zoff, sxoff, syoff, szoff;
  VECTOR *v_struct, *v_func;
  MRI *mri_avg;
  // MRI *mri_std;
...
int StatWriteVolume(SV *sv, const char *prefix)
{
  ...
  FILE *fp;
  int event_number, width, height, t, event, nitems, x, y, z, nframes;
  // int nslices;
  float *buf, fval;
  ...
  if (sv->voltype != 0) /* not a raw stats file (sel averaged) */
  {
    ...
    fp = fopen(fname, "w");
    if (!fp)
      ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "StatWriteVolume: could not open dat file %s", fname));
    fprintf(fp, "tr %f\n", sv->tr);
...
int StatWriteRegistration(fMRI_REG *reg, const char *fname)
{
  ...
  fp = fopen(fname, "w");
  if (!fp)
    ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "StatWriteRegistration: could not open %s", fname));
  fprintf(fp, "%s\n", reg->name);
  fprintf(fp, "%f\n", reg->in_plane_res);
  fprintf(fp, "%f\n", reg->slice_thickness);
...
int PrintSegStat(FILE *fp, SEGSTAT *segstat)
{
  ...
  for (n = 0; n < c - 1; n++)
    fprintf(fp, "%s ", segstat->ColHeaders[n]);
  fprintf(fp, " \n");
  for (n = 0; n < segstat->nentries; n++)
  {
    ...
    if (segstat->DoIntensity)
    {
      ...
      if (segstat->DoSNR)
        fprintf(fp, "%10.4f ", segstat->entry[n].snr);
    }
    fprintf(fp, "\n");
  }
