...
GCLASSIFY *GCalloc(int nclasses, int nvars, char *class_names[])
{
  ...
  if (!gc)
    ErrorReturn(NULL, (ERROR_NO_MEMORY, "GCalloc(%d): could not allocate GC", nclasses));

  gc->nclasses = nclasses;
  gc->nvars = nvars;
  gc->classes = (GCLASS *)calloc(nclasses, sizeof(GCLASS));
  if (!gc->classes)
    ErrorReturn(NULL, (ERROR_NO_MEMORY, "GFalloc(%d): could not allocated class table", nclasses));
...
GCLASSIFY *GCasciiReadFrom(FILE *fp, GCLASSIFY *gc)
{
  ...
  if (!gc)
  {
    ...
    if (!gc)
      ErrorReturn(NULL, (ERROR_BADFILE, "GCasciiReadFrom: GCalloc failed"));
  }
