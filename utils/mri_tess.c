...
static int mrisFindNeighbors2(MRI_SURFACE *mris)
{
  ...
  for (k = 0; k < mris->nvertices; k++)
  {
    ...
    if (!mris->vertices[k].v)
      ErrorExit(ERROR_NOMEMORY, "mrisFindNeighbors2: could not allocate nbr array");
...
static void reallocateVertices(tesselation_parms *parms)
{
  ...
  if (!tmp)
    ErrorExit(ERROR_NOMEMORY, "%s: max vertices %d exceeded", Progname, parms->maxvertices);
...
static void reallocateFaces(tesselation_parms *parms)
{
  ...
  if (!tmp)
    ErrorExit(ERROR_NOMEMORY, "%s: max faces %d exceeded", Progname, parms->maxfaces);
