...
static NODE *rfAllocateNode(int ntraining, int depth, int nclasses)
{
  ...
  if (node == NULL)
    ErrorExit(ERROR_NOMEMORY, "rfAllocateNode: could not allocate node");
  node->class_counts = (int *)calloc(nclasses, sizeof(node->class_counts[0]));
  if (node->class_counts == NULL)
    ErrorExit(ERROR_NOMEMORY, "rfAllocateNode: could not allocate node class counts");
  node->training_set = (int *)calloc(ntraining, sizeof(node->training_set[0]));
  if (node->training_set == NULL)
    ErrorExit(ERROR_NOMEMORY, "rfAllocateNode: could not allocate node training_set");
  node->depth = depth;
  return (node);
}
...
static int
rfTrainNode(RANDOM_FOREST *rf, TREE *tree, NODE *node, int *training_classes, double **training_data, int ntraining)
{
  if (node->left == NULL) // not trained yet
  {
    ...
    if (find_optimal_feature_and_threshold(
            rf, tree, node, node->left, node->right, training_classes, training_data, rf->ntraining)
        == 0)
    {
...
static int rfFindNodeLeaves(TREE *tree, NODE *node)
{
  if (node->left == NULL)
  {
    if (tree->leaves)
    {
      if (tree->nleaves >= tree->max_leaves && tree->max_leaves > 0)
        DiagBreak();
            ...
            int ntraining)
{
  ...
  n = 0;
  ii = 0;
#pragma omp parallel for firstprivate(tree, start_no, end_no, ii, index) shared(                                       \
    rf, nfeatures_per_tree, Gdiag, training_classes, training_data) schedule(static, 1)
#endif
...
int RFcomputeOutOfBagCorrect(RANDOM_FOREST *rf, int *training_classes, double **training_data, int ntraining)
{
  ...
  for (n = 0; n < rf->ntrees; n++)
  {
    ...
    for (i = 0; i < tree->root.total_counts; i++)
      tree_training[n][tree->root.training_set[i]] = 1;
  }
