...
static AE *AEalloc(AE *prev, int ninputs, int nhidden, int noutputs)
{
  ...
  ae->v_input = VectorAlloc(ninputs, MATRIX_REAL); // will be a copy of previous
                                                   // layer to allow calculation
                                                   // of errors
  if (ae->v_input == NULL)
  ...
  if (ae->average_act == NULL)
    ErrorExit(ERROR_NOMEMORY, "AEalloc: could not allocate average_act");
...
static void AEfree(AE **pae)
{
  ...
  *pae = NULL;
  ...
  if (ae->zero_indices)
    free(ae->zero_indices);
  if (ae->saved_inputs)
    free(ae->saved_inputs);
...
AE *SAEaddLayer(SAE *sae, float scale)
{
  ...
  //  noutputs = sae->type == FOCUSED_AUTOENCODER ? 1 :
  //  last->v_hidden_bias->rows ;
...
static int AEactivateLayer(AE *ae, VECTOR *v_input)
{
  ...
  if (ae->noise_fraction > 0)
  {
    ...
    if (ae->saved_inputs == NULL)
      ae->saved_inputs = (double *)calloc(max_i, sizeof(double));
  ...
  if (ae->next)
  {
    ...
    MatrixCopy(ae->next->v_output, ae->v_hidden); // output of next layer is
                                                  // hidden activation of this
                                                  // one since it autoencodes
  }
...
// NOTE: input MRI must be type float and scaled to be 0->1 (that is, scale down
// MRI_UCHAR vols by 255)
  ...
  do
  {
    ...
    for (running_last_rms = total_rms = 0.0, visited = ind = 0; ind < nvox; ind++)
    {
      ...
      if (x < sae->whalf || y < sae->whalf || z < sae->whalf || x >= mri->width - sae->whalf
          || y >= mri->height - sae->whalf
          || z >= mri->depth - sae->whalf)
        continue;
  ...
  do
  {
    ...
    for (running_last_rms = total_rms = 0.0, visited = ind = 0; ind < vl->nvox; ind++)
    {
      ...
      if (x < sae->whalf || y < sae->whalf || (z < sae->whalf && mri->depth > 1) || x >= mri->width - sae->whalf
          || y >= mri->height - sae->whalf
          || (z >= mri->depth - sae->whalf && mri->depth > 1))
        continue;
                               ...
                               SAE_INTEGRATION_PARMS *parms)
{
  ...
  for (i = start_index; i <= end_index; i++)
  {
    ...
    if (x < whalf || y < whalf || (z < whalf && mri->depth > 1) || x >= mri->width - whalf || y >= mri->height - whalf
        || (z >= mri->depth - whalf && mri->depth > 1))
      continue;
...
static int AEclearGradients(AE *ae)
{
  ...
  if (ae->m_grad_input_to_hidden == NULL)
    return (NO_ERROR);
  for (j = 0; j < ae->v_hidden->rows; j++)
    ae->average_act[j] = 0;
...
// NOTE: compute all GRADIENTS first. Will multiply by -dt at the end to get
// right direction of update
...
#pragma omp parallel for firstprivate(noutputs, nhidden) shared(ae) schedule(static, 1)
  ...
  if (parms->class_label >= 0)
  {
    ...
    for (j = 1; j <= nhidden; j++)
    {
      ...
      if (ae->hidden_class_labels[j - 1] <= 0)
        continue; // no class label specified for this node
  ...
  for (j = 0; j < nhidden; j++) // keep track of average hidden node activation
                                // for use in sparsity gradient calculated later
...
static double aeApplyAccumulatedGradient(AE *ae, SAE_INTEGRATION_PARMS *parms)
{
  ...
  if (parms->integration_type != INTEGRATE_CONJUGATE_GRADIENT) // don't scale
                                                               // grads by -dt
                                                               // for conjugate
                                                               // gradient
  {
  ...
  else if (parms->integration_type == INTEGRATE_CONJUGATE_GRADIENT)
  {
    ...
    //    MatrixAdd(parms->v_dir_hidden_bias, ae->v_hidden_bias,
    //    ae->v_hidden_bias) ;
...
#pragma omp parallel for firstprivate(noutputs, nhidden, dt) shared(ae) schedule(static, 1)
  ...
  for (j = 1; j <= nhidden; j++)
  {
    ...
    hidden = VECTOR_ELT(ae->v_hidden, j);
    fprime = D_SIGMOID(hidden);
    if (!devFinite(fprime) || !devFinite(hidden))
      DiagBreak();
    for (dE_dbj = 0.0, k = 1; k <= noutputs; k++)
    {
      error = VECTOR_ELT(ae->v_error, k);
...
#pragma omp parallel for firstprivate(noutputs, nhidden, dt) shared(ae) schedule(static, 1)
  ...
  if (parms->class_label >= 0)
  {
    ...
    for (j = 1; j <= nhidden; j++)
    {
      ...
      hidden = VECTOR_ELT(ae->v_hidden, j);
      fprimej = D_SIGMOID(hidden);
      if (!devFinite(fprimej) || !devFinite(hidden))
        DiagBreak();
...
int SAEwrite(SAE *sae, char *fname)
{
  ...
  written = fprintf(fp, "%d %d %lf %d %d\n", sae->whalf, sae->nencoders, sae->scale, sae->type, sae->nlevels);
  if (written < 0)
  {
    fprintf(stderr, "Warning: nothing written\n");
  }
...
{
  FILE *fp;
  int whalf, nencoders, i, type, nlevels;
  SAE *sae;
  AE *ae;
  double scale;
  ...
  if (fscanf(fp, "%d %d %lf %d %d\n", &whalf, &nencoders, &scale, &type, &nlevels) != 5)
  {
    ErrorReturn(NULL, (ERROR_BAD_FILE, "SAEread(%s): error reading parameters", fname));
  }
...
void AEdump(AE *ae)
{
  ...
  {
    printf("f(I=%2.2f * wij=%2.2f + bj=%2.2f = %2.2f) = %2.2f * wjk=%2.2f + "
           "bk=%2.2f = %2.2f, E=%2.2f\n",
           ae->v_input->rptr[1][1],
...
double CSAEtrainLayerFromVoxlist(CSAE *csae, int layer, VOXEL_LIST *vl, MRI **mri_pyramid, SAE_INTEGRATION_PARMS *parms)
{
  ...
  if (mri_pyramid[0]->type != MRI_FLOAT)
    ErrorExit(ERROR_BADPARM,
              "CSAEtrainLayerFromVoxlist: input type must be "
              "MRI_FLOAT scaled to [0->1]");
  ...
  do
  {
    ...
    if (iter == 0)
    {
      //      min_rms = last_total_rms = CSAEcomputeVoxlistRMS(csae, parms,
      //      layer, mri_pyramid, vl, indices, end_index+1, vl->nvox, &always,
      //      &never) ;
      min_rms = last_total_rms
          = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, 0, end_index, &always, &never);
      printf("%3.3d: rms = %2.4f\n", iter, last_total_rms);
    }
    if (nbad < parms->max_no_progress - 2)
      reset_constant_nodes(ae_train, 1000.0);
...
#endif
    //    total_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl,
    //    indices, end_index+1, vl->nvox, &always, &never) ;
    total_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, 0, end_index, &always, &never);
    if (always || never)
      printf("\thidden nodes always (%d) or never (%d) active\n", always, never);
    pct_decrease = 100 * (last_total_rms - total_rms) / (last_total_rms + total_rms);
    last_total_rms = total_rms;
    printf("%3.3d: rms = %2.4f (%2.3f%%)\n", ++iter, total_rms, pct_decrease);
    if (out_fname)
                             ...
                             int *never)
{
  ...
  for (i = start_index; i <= end_index; i++)
  {
    ...
    AEactivateLayer(ae, ae->v_input);
    for (h = 0; h < nhidden; h++)
      if ((VECTOR_ELT(ae->v_hidden, h + 1)) > .5)
        histo[h]++;
    rms = AEcomputeRMS(ae);
    total_rms += rms;
    ...
    if (rms > 1000)
      DiagBreak();
    nvox++;
    if (last_class_rms[iz] > 0 && class_rms > last_class_rms[iz])
      DiagBreak();
    if (last_rms[iz] > 0 && rms > last_rms[iz])
      DiagBreak();
    last_rms[iz] = rms;
...
CSAE *CSAEread(char *fname)
{
  ...
  {
    ae = csae->aes[layer];
    csae->mri_outputs[layer] = MRIallocSequence(
        csae->sae->vg.width, csae->sae->vg.height, csae->sae->vg.depth, MRI_FLOAT, ae->v_hidden->rows);
