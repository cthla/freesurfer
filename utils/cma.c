...
int CMAclaimPoints(CMAoutlineField *field, short label, short *points, int n_points, short seed_x, short seed_y)
{
  ...
  if (label < 0 || label > MAX_CMA_LABEL)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM,
                 "CMAclaimPoints(): label out of range (label "
                 "is %d, MAX_CMA_LABEL is %d)",
                 label,
                 MAX_CMA_LABEL));
  ...
  if (seed_x < 0 || seed_x >= field->width)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM,
                 "CMAclaimPoints(): seed point out of range "
                 "(seed_x = %d, field width = %d)",
                 seed_x,
                 field->width));
  if (seed_y < 0 || seed_y >= field->height)
    ErrorReturn(ERROR_BADPARM,
...
{
  if (SegId == Left_Thalamus)
    return (1);
  if (SegId == Right_Thalamus)
    return (1);
  if (SegId == Left_Thalamus_Proper)
    return (1);
  if (SegId == Right_Thalamus_Proper)
    return (1);
  if (SegId == Left_Caudate)
    return (1);
  if (SegId == Right_Caudate)
    return (1);
  if (SegId == Left_Putamen)
    return (1);
  if (SegId == Right_Putamen)
    return (1);
  if (SegId == Left_Pallidum)
    return (1);
  if (SegId == Right_Pallidum)
    return (1);
  // if(SegId == Brain_Stem) return(1);
  if (SegId == Left_Hippocampus)
    return (1);
  if (SegId == Right_Hippocampus)
    return (1);
  if (SegId == Left_Amygdala)
    return (1);
  if (SegId == Right_Amygdala)
    return (1);
  if (SegId == Left_Accumbens_area)
    return (1);
  if (SegId == Right_Accumbens_area)
    return (1);
  if (SegId == Left_VentralDC)
    return (1);
  if (SegId == Right_VentralDC)
    return (1);
  if (SegId == Left_Substancia_Nigra)
    return (1);
  if (SegId == Right_Substancia_Nigra)
    return (1);
  ...
  for (c = 0; c < aseg->width; c++)
  {
    for (r = 0; r < aseg->height; r++)
    {
      for (s = 0; s < aseg->depth; s++)
      {
        ...
        if (SegId == Left_Lateral_Ventricle)
          vol += VoxSize;
        if (SegId == Right_Lateral_Ventricle)
          vol += VoxSize;
        if (SegId == Left_choroid_plexus)
          vol += VoxSize;
        if (SegId == Right_choroid_plexus)
          vol += VoxSize;
        if (SegId == Left_Inf_Lat_Vent)
          vol += VoxSize;
        if (SegId == Right_Inf_Lat_Vent)
          vol += VoxSize;
        if (SegId == WM_hypointensities)
          vol += VoxSize;
        if (SegId == Left_WM_hypointensities)
          vol += VoxSize;
        if (SegId == Right_WM_hypointensities)
          vol += VoxSize;
        if (SegId == Left_Thalamus_Proper)
          vol += VoxSize;
        if (SegId == Right_Thalamus_Proper)
        ...
        if (SegId == Right_Thalamus)
          vol += VoxSize;
        if (SegId == CC_Posterior)
          vol += VoxSize;
        if (SegId == CC_Mid_Posterior)
          vol += VoxSize;
        if (SegId == CC_Central)
          vol += VoxSize;
        if (SegId == CC_Mid_Anterior)
          vol += VoxSize;
        if (SegId == CC_Anterior)
          vol += VoxSize;
        if (SegId == Left_VentralDC)
          vol += VoxSize;
        if (SegId == Right_VentralDC)
  ...
  for (c = 0; c < aseg->width; c++)
  {
    for (r = 0; r < aseg->height; r++)
    {
      for (s = 0; s < aseg->depth; s++)
      {
        ...
        RibbonVal = MRIgetVoxVal(ribbon, c, r, s, 0);
        if (hemi == 1 && RibbonVal != 3)
          continue;
        if (hemi == 2 && RibbonVal != 42)
          continue;
        ...
        // 2
        if (SegId != 3 && SegId != 42 && SegId != 2 && SegId != 41 && SegId != 0)
          vol2 += VoxSize;
        ...
        if (SegId == Right_Accumbens_area)
          vol += VoxSize;
        if (SegId == Left_Cerebellum_Cortex)
          vol += VoxSize;
        if (SegId == Right_Cerebellum_Cortex)
          vol += VoxSize;
        if (SegId == Left_vessel)
          vol += VoxSize;
      }
    }
  }
  ...
  for (c = 0; c < aseg->width; c++)
  {
    for (r = 0; r < aseg->height; r++)
    {
      for (s = 0; s < aseg->depth; s++)
      {
        ...
        if (id > 999)
        {
          // This should handle the cortical and wmparc labels in aparc+aseg,
          // etc
          if (id >= 1000 && id < 1999)
            id2 = id + 1000;
          if (id >= 2000 && id < 2999)
          ...
          if (id >= 4000 && id < 4999)
            id2 = id - 1000;
          if (id >= 11100 && id < 11999)
            id2 = id + 1000;
          if (id >= 12100 && id < 12999)
            id2 = id - 1000;
          if (id >= 13100 && id < 13999)
            id2 = id + 1000;
          if (id >= 14100 && id < 14999)
            id2 = id - 1000;
          if (id == 5001)
            id2 = 5002;
          if (id == 5002)
...
double *ComputeBrainVolumeStats(char *subject, char *suffix, char *sdir)
{
  ...
  {
    printf("WARNING: %s does not exist, ribbon based measurements will be "
           "inaccurate\n",
           tmpstr);
    ribbon = aseg;
    asegfixed = aseg;
  ...
  for (c = 0; c < aseg->width; c++)
  {
    for (r = 0; r < aseg->height; r++)
    {
      for (s = 0; s < aseg->depth; s++)
      {
        ...
        ribbonid = MRIgetVoxVal(ribbon, c, r, s, 0);
        // Corpus Callosum
        if (asegid == 251 || asegid == 252 || asegid == 253 || asegid == 254 || asegid == 255)
          CCVol++;
        ...
        // structures.
        // Does not use PVC for subcort GM. Make sure to include hypointensities
        // (77)
        ...
        if (IsSubCorticalGray(asegfixedid))
          SubCortGMVol++;
        ...
        if (asegid == Left_choroid_plexus || asegid == Right_choroid_plexus || asegid == Left_Lateral_Ventricle
            || asegid == Right_Lateral_Ventricle
            || asegid == Left_Inf_Lat_Vent
        ...
        if (asegfixedid != 0 && asegfixedid != Brain_Stem)
          BrainSegVol++;
        ...
        // a check)
        if (asegfixedid == 2 || asegfixedid == 78 || (asegfixedid == 77 && c < 128))
          lhCtxWMCount++;
        if (asegfixedid == 41 || asegfixedid == 79 || (asegfixedid == 77 && c >= 128))
          rhCtxWMCount++;
      } // c
    }   // r
  }     // s
        ...
        for (cB = cBmin; cB < cBmax; cB++)
        {
          ...
          for (rB = rBmin; rB < rBmax; rB++)
          {
            ...
            for (sB = sBmin; sB < sBmax; sB++)
            {
              ...
              if (ctab->entries[segnoB]->TissueType <= 3)
                continue;
              // Skip if this voxel has already been counted for this segno
              if (MRIgetVoxVal(hitmap, cB, rB, sB, nthseg) > 0.5)
                continue;
