...
MRIC *MRICalloc(int nrounds, int *types, int *features, void *parms)
{
  ...
  if (!mric)
    ErrorExit(ERROR_NO_MEMORY, "MRICalloc(%d): could not allocate struct", nrounds);
...
int MRICretrain(MRIC *mric, char *file_name)
{
  ...
  if (!fp)
    ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "MRICtrain(%s): could not open file", file_name));
...
int MRICtrain(MRIC *mric, char *file_name, char *prior_fname)
{
  ...
  for (round = 0; round < mric->nrounds; round++)
  {
    ...
    {
    case CLASSIFIER_RBF:
      if (mricTrainRBF(mric, fp, nfiles, round) != NO_ERROR)
        return (Gerror);
      break;
    case CLASSIFIER_GAUSSIAN:
...
MRIC *MRICread(char *fname)
{
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NO_FILE, "MRICread(%s): could not open file", fname));
...
int MRICwrite(MRIC *mric, char *fname)
{
  ...
  fp = fopen(fname, "wb");
  if (!fp)
    ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "MRICwrite(%s): could not open file", fname));
...
int MRICcomputeInputs(MRIC *mric, MRI *mri, int x, int y, int z, VECTOR *v_inputs, int features)
{
  ...
  /*
     ...
     */
  if (!mri_prev || mri_prev != mri || (old_features != features) || (REGIONinside(&region, x, y, z) == REGION_OUTSIDE))
  {
    ...
    mri_prev = mri;
    if (mri_cpolv_curv5)
      MRIfree(&mri_cpolv_curv5);
    if (mri_cpolv_curv7)
      MRIfree(&mri_cpolv_curv7);
    if (mri_cpolv_zscore5)
      MRIfree(&mri_cpolv_zscore5);
    if (mri_cpolv_order)
      MRIfree(&mri_cpolv_order);
    if (mri_cpolv_zscore7)
      MRIfree(&mri_cpolv_zscore7);
...
#define ORDER_THRESH 2
  ...
  if (features & FEATURE_CPOLV_ZSCORE5)
    VECTOR_ELT(v_inputs, index++) = MRIFvox(mri_cpolv_zscore5, x0, y0, z0);
  if (features & FEATURE_CPOLV_ZSCORE7)
    VECTOR_ELT(v_inputs, index++) = MRIFvox(mri_cpolv_zscore7, x0, y0, z0);
  if (features & FEATURE_CPOLV_CURV5)
    VECTOR_ELT(v_inputs, index++) = MRIFvox(mri_cpolv_curv5, x0, y0, z0);
  if (features & FEATURE_CPOLV_CURV7)
    VECTOR_ELT(v_inputs, index++) = MRIFvox(mri_cpolv_curv7, x0, y0, z0);
  return (NO_ERROR);
}
/*-----------------------------------------------------
        Parameters:
...
char *MRICfeatureName(MRIC *mric, int round, int feature_number)
{
  ...
  if (f & FEATURE_ZSCORE5)
    return ("ZSCORE5");
  if (f & FEATURE_CPOLV_ZSCORE5)
    return ("CPOLV ZSCORE5");
  if (f & FEATURE_CPOLV_ZSCORE7)
    return ("CPOLV ZSCORE7");
  if (f & FEATURE_CPOLV_CURV5)
    return ("CPOLV NORMAL CURVATURE 5");
  if (f & FEATURE_CPOLV_CURV7)
    return ("CPOLV NORMAL CURVATURE 7");
...
static int mricTrainRBF(MRIC *mric, FILE *fp, int nfiles, int round)
{
  ...
  if (RBFtrain(mric->classifier[round].rbf, mricGetClassifierInput, &parms, 0.0f) != NO_ERROR)
    return (Gerror);
...
static int mricRetrainRBF(MRIC *mric, FILE *fp, int nfiles, int round)
{
  ...
  if (RBFretrain(mric->classifier[round].rbf, mricGetClassifierInput, &parms, 0.0f) != NO_ERROR)
    return (Gerror);
...
int MRICexamineTrainingSet(MRIC *mric, char *file_name, int round)
{
  ...
  fp = fopen(file_name, "r");
  if (!fp)
    ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "MRICexamineTrainingSet(%s): could not open file", file_name));
  parms.fp = fp;
  parms.mric = mric;
  parms.round = round;
