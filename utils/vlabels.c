...
VOXEL_LABELS_IMAGE *VLalloc(int width, int height, int depth, float resolution)
{
  ...
  for (x = 0; x < width; x++)
  {
    ...
    if (!vl[x])
      ErrorExit(ERROR_NOMEMORY, "%s: could not allocate  VL ** %d", Progname, x);
    for (y = 0; y < height; y++)
    {
      vl[x][y] = (VL *)calloc(depth, sizeof(VL));
      if (!vl[x][y])
        ErrorExit(ERROR_NOMEMORY, "%s: could not allocate  VL * %d,%d", Progname, x, y);
    }
  }
...
int VLwrite(VLI *vli, char *fname)
{
  ...
  fp = fopen(fname, "wb");
  if (!fp)
    ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "VLwrite: could not open %s", fname));
...
VLI *VLread(char *fname)
{
  ...
  fp = fopen(fname, "rb");
  if (!fp)
    ErrorReturn(NULL, (ERROR_NOFILE, "VLwrite: could not open %s", fname));
  magic = freadInt(fp);
  if (magic != VL_MAGIC)
