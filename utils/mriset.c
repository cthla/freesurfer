          ...
          for (dr = -1; dr <= 1; dr++)
          {
            for (ds = -1; ds <= 1; ds++)
            {
              ...
              if (NNDef == NEAREST_NEIGHBOR_FACE)
                if (abs(dc) + abs(dr) + abs(ds) > 1)
                  continue;
              if (NNDef == NEAREST_NEIGHBOR_EDGE)
                if (abs(dc) + abs(dr) + abs(ds) > 2)
                  continue;
...
MRI *MRIerodeSegmentation(MRI *seg, MRI *out, int nErodes, int nDiffThresh)
{
  ...
  if (!out)
    out = MRIallocSequence(seg->width, seg->height, seg->depth, seg->type, 1);
...
MRI *MRImask(MRI *mri_src, MRI *mri_mask, MRI *mri_dst, int mask, float out_val)
{
  ...
  if (mri_src->width != mri_mask->width || mri_src->height != mri_mask->height || mri_src->depth != mri_mask->depth
      || mri_src->xsize != mri_mask->xsize
      || mri_src->ysize != mri_mask->ysize
      || mri_src->zsize != mri_mask->zsize
      || mri_src->c_r != mri_mask->c_r
      || mri_src->c_a != mri_mask->c_a
  ...
  if (mri_src->type != mri_dst->type)
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "MRImask: src and dst must be same type"));
...
MRI *MRImaskThreshold(MRI *mri_src, MRI *mri_mask, MRI *mri_dst, float threshold, int out_label)
{
  ...
  if (mri_mask->type != MRI_UCHAR)
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "MRI3Dthreshold: mask must be MRI_FLOAT"));
...
MRI *MRIsetBoundingBox(MRI *template, MRI_REGION *region, double InVal, double OutVal)
{
  ...
  if (region->dx < 0)
    region->dx = template->width;
  if (region->dy < 0)
    region->dy = template->height;
  if (region->dz < 0)
    region->dz = template->depth;
