...
static MRI *weights_to_mri(RBM *rbm)
{
  ...
  if (rbm->input_type == RBM_INPUT_IMAGE)
  {
    ...
    for (h = 0; h < rbm->nhidden; h++)
    {
      for (v = k1 = 0; k1 < rbm->ksize; k1++)
        for (k2 = 0; k2 < rbm->ksize; k2++, v++)
          MRIsetVoxVal(mri, k1, k2, 0, h, rbm->weights[v][h]);
    }
  ...
  else
  {
    ...
    for (v = 0; v < rbm->nvisible; v++)
      for (h = 0; h < rbm->nhidden; h++)
        MRIsetVoxVal(mri, v, h, 0, 0, rbm->weights[v][h]);
  }
  return (mri);
}
...
static MRI *layer_weights_to_mri(DBN *dbn, int layer)
{
  ...
  if (layer == 0)
    return (weights_to_mri(rbm_first));
    ...
    RBM *rbm, double *dvisible_bias, double *dvariance, double *dhidden_bias, double **dw, RBM_PARMS *parms, int step)
{
  ...
  for (v = 0; v < rbm->nvisible; v++)
    printf("%2.0f ", rbm->visible[v]);
  printf("\n");
  printf("hidden state:   ");
  for (h = 0; h < rbm->nhidden; h++)
    printf("%d ", (int)rbm->hidden_state[h]);
  printf("\n");

  printf("visible bias:   ");
  for (v = 0; v < rbm->nvisible; v++)
    printf("%+2.5f ", rbm->visible_bias[v]);
  printf("\n");
  printf("variance:       ");
  for (v = 0; v < rbm->nvisible; v++)
    printf("%+2.5f ", exp(rbm->variance[v]));
  printf("\n");
  printf("hidden bias:    ");
  for (h = 0; h < rbm->nhidden; h++)
    printf("%+2.5f ", rbm->hidden_bias[h]);
  printf("\n");
  ...
  printf("visible bias:    ");
  for (v = 0; v < rbm->nvisible; v++)
    printf("%+2.5f ", dvisible_bias[v]);
  printf("\n");
  printf("variance:        ");
  for (v = 0; v < rbm->nvisible; v++)
    printf("%+2.5f ", dvariance[v]);
  printf("\n");
  printf("hidden bias:     ");
  for (h = 0; h < rbm->nhidden; h++)
    printf("%+2.5f ", dhidden_bias[h]);
  printf("\n");
...
double RBMfreeEnergy(RBM *rbm, double *visible)
{
  ...
  if (rbm->nlabels > 0)
  {
    for (total = 0.0, l = 0; l < rbm->nlabels; l++)
    {
      ...
      {
        for (v = 0; v < rbm->nvisible; v++)
          free_energy[l] -= visible[v] * rbm->visible_bias[v];
        for (h = 0; h < rbm->nhidden; h++)
        {
      ...
      else // Gaussian inputs
      {
        ...
        for (h = 0; h < rbm->nhidden; h++)
          free_energy[l] -= log(1 + exp(rbm->act[h]));
      }
    ...
    if (DZERO(total))
      total = 0.00000001;
    for (l = 0; l < rbm->nlabels; l++)
      rbm->labels[l] = exp(-free_energy[l]) / total;
  }
  else
  {
    free_energy[0] = 0;
    for (v = 0; v < rbm->nvisible; v++)
      free_energy[0] -= visible[v] * rbm->visible_bias[v];
    for (h = 0; h < rbm->nhidden; h++)
    {
      double pj = rbm->hidden[h];
      if (DZERO(pj))
...
RBM *RBMalloc(int type, int nvisible, int nhidden, int nlabels, int input_type)
{
  ...
  if (nlabels > 0)
  {
    ...
    if (!rbm->label_bias)
      ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate label array");

    rbm->lact = (double *)calloc(nlabels, sizeof(rbm->labels[0]));
    if (!rbm->lact)
      ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate label act array");
    ...
    if (!rbm->label_weights)
      ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate label weight array");

    rbm->label_states = (int *)calloc(nlabels, sizeof(rbm->label_states[0]));
    if (!rbm->label_states)
      ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate label state array");

    rbm->label_pvals = (LABEL_PVAL *)calloc(nlabels, sizeof(rbm->label_pvals[0]));
    if (!rbm->label_pvals)
      ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate label pval array");
  ...
  if (!rbm->weights)
    ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate weight array");

  rbm->hidden_bias = (double *)calloc(nhidden, sizeof(double));
  if (!rbm->hidden_bias)
    ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate hidden bias array");

  rbm->visible_bias = (double *)calloc(nvisible, sizeof(double));
  if (!rbm->visible_bias)
    ErrorExit(ERROR_NOMEMORY, "RBMalloc: could not allocate visible bias array");
...
int RBMactivateForward(RBM *rbm, double *visible)
{
  ...
  if (visible)
    memcpy(rbm->visible, visible, rbm->nvisible * sizeof(visible[0]));
...
#pragma omp parallel for shared(rbm) schedule(static, 1)
    ...
    for (total = 0.0, v = 0; v < rbm->nlabels; v++)
      total += rbm->labels[v];
...
int RBMfree(RBM **prbm)
{
  ...
  *prbm = NULL;
  ...
  if (rbm->label_states)
    free(rbm->label_states);
  if (rbm->label_pvals)
    free(rbm->label_pvals);

  if (rbm->active)
    free(rbm->active);
  if (rbm->label_weights)
    for (v = 0; v < rbm->nlabels; v++)
      free(rbm->label_weights[v]);
  if (rbm->labels)
    free(rbm->labels);
  if (rbm->lact)
    free(rbm->lact);
...
int RBMfillVisible(RBM *rbm, MRI *mri_inputs, double *visible, int x0, int y0, int z0, int f0, int ksize)
{
  ...
  else
  {
    ...
    for (f = 0; f < mri_inputs->nframes; f++)
      visible[f] = MRIgetVoxVal(mri_inputs, x0, y0, z0, f);
  }
...
double RBMvoxlistRMS(RBM *rbm, VOXLIST *vl, RBM_PARMS *parms, int *indices, int index, int num)
{
  ...
  if (indices)
  {
    ...
    for (rms = 0.0, nvox = 0, ind = index; ind < index + num; ind++)
    {
      ...
      for (h = 0; h < rbm->nhidden; h++)
        rbm->active[h] += rbm->hidden_state[h];
...
int RBMtrainFromVoxlistImage(RBM *rbm, VOXLIST *vl, RBM_PARMS *parms)
{
  ...
  for (nbad = step = 0; step < parms->nsteps; step++)
  {
    ...
    if (!((step + 1) % parms->write_iterations))
      RBMwriteNetwork(rbm, step + 1, parms, -1);
          ...
          for (z = 0; z < mri_inputs->depth; z++)
          {
            ...
            */
            ...
            if (rbm->nlabels > 0)
            {
              ...
              if (rbm->labels[label] > threshold)
                MRIsetVoxVal(mri_labeled, x, y, z, 2, label);

              for (h = 0; h < rbm->nhidden; h++)
                if (rbm->hidden_state[h])
                  HISTO2DaddSample(histo_labels, h, label, -1, -1, -1, -1);
            }
...
int DBNactivateForward(DBN *dbn, double *visible, int nlayers)
{
  ...
  for (l = 1; l < nlayers; l++)
    RBMactivateForward(dbn->rbms[l], dbn->rbms[l - 1]->hidden_state);
...
int DBNtrainFromVoxlistImage(DBN *dbn, VOXLIST *vl, RBM_PARMS *parms)
{
  ...
  for (v = 0; v < dbn->rbms[0]->nvisible; v++)
    dbn->rbms[0]->variance[v] = log(var);
  ...
  for (l = 0; l < dbn->nlayers; l++)
  {
    ...
    for (nbad = step = 0; step < parms->nsteps; step++)
    {
      ...
      if (!((step + 1) % parms->write_iterations))
        RBMwriteNetwork(rbm, step + 1, parms, l);
      pct_diff = 100 * (last_rms - rms) / (.5 * (last_rms + rms));
      printf("iter %3.3d: rms = %2.5f (%2.5f%%)\n", step + 1, rms, pct_diff);
      if (step == Gdiag_no)
        DiagBreak();
        ...
        for (y = whalf; y < mri_inputs->height - whalf; y++)
          for (z = 0; z < mri_inputs->depth; z++)
          {
            ...
            if (rbm_last->nlabels > 0)
            {
              ...
              if (rbm_last->labels[label] > threshold)
                MRIsetVoxVal(mri_labeled, x, y, z, 2, out_label);
              for (h = 0; h < rbm_last->nhidden; h++)
                if (rbm_last->hidden_state[h])
                  HISTO2DaddSample(histo_labels, h, label, -1, -1, -1, -1);
            }
            for (n = 0; n < dbn->nlayers; n++)
            {
...
CDBN *CDBNalloc(int type, int nlayers, int *ksizes, int *ngroups, int nlabels, MRI *mri_inputs)
{
  ...
  if (cdbn->mri_outputs == NULL)
    ErrorExit(ERROR_NOMEMORY, "CDBNalloc: could not allocate CDBN mri_outputs array");
...
int CDBNtrainFromVoxlistImage(CDBN *cdbn, VOXLIST *vl, RBM_PARMS *parms, MRI *mri_inputs, MRI *mri_labels)
{
  ...
  for (v = 0; v < cdbn->rbms[0]->nvisible; v++)
    cdbn->rbms[0]->variance[v] = log(var);
  ...
  for (layer = 0; layer < cdbn->nlayers; layer++)
  {
    ...
    printf("************* TRAINING LAYER %d, sparsity = %2.2f wt %2.2f, momentum = %2.1f, dt = %2.4f, weight decay "
           "%2.5f ************\n",
           layer,
           sparsity,
    ...
    CDBN *cdbn, int layer, VOXLIST *vl, RBM_PARMS *parms, int *indices, int index, int num, double *plabel_rms)
{
  ...
  if (indices)
  {
    ...
    for (label_rms = rms = 0.0, nvox = 0, ind = index; ind < index + num; ind++)
    {
      ...
      for (l = 0; l < rbm->nlabels; l++, nvox++)
        rms += SQR(l == current_label ? 1 : 0 - rbm->labels[l]);
      if (rbm->nlabels > 0)
        label_rms += SQR(1 - rbm->labels[current_label]);
    }
    for (h = 0; h < rbm->nhidden; h++)
      rbm->active[h] /= (double)num;
    if (rbm->nlabels > 0)
  ...
  else
  {
    for (label_rms = rms = 0.0, nvox = i = 0; i < vl->nvox; i++)
    {
      ...
      for (v = 0; v < rbm->nvisible; v++, nvox++)
        rms += SQR(visible[v] - rbm->visible[v]);
      if (rbm->nlabels > 0)
        label_rms += SQR(1 - rbm->labels[nint(vl->vsrc[i])]);
    }
    if (rbm->nlabels > 0)
    {
      label_rms = sqrt(label_rms / num);
                         ...
                         int index)
{
  ...
  if (rbm->nlabels > 0)
  {
    for (v = 0; v < rbm->nlabels; v++)
    {
      ...
      for (h = 0; h < rbm->nhidden; h++)
        dlabel_weights[v][h] *= (scale * 1); // was .01
    }
  }
                                       ...
                                       int index)
{
    ...
    for (l = 0; l < rbm->nlabels; l++)
    {
      ...
      for (h = 0; h < rbm->nhidden; h++)
      {
        ...
        if (compute_label_weights)
          dlabel_weights[l][h] -= rbm->hidden[h] * rbm->labels[l];
        if (l == current_label) // add in positive term
        {
          ...
          if (compute_label_weights)
            dlabel_weights[l][h] += rbm->hidden[h];
        }
      }
    }
  }
  ...
  for (l = 0; l < rbm->nlabels; l++)
  {
    ...
    for (h = 0; h < rbm->nhidden; h++)
      dlabel_weights[l][h] *= (scale * 1); // was .01
  }
                              ...
                              int index)
{
  ...
  for (ind = index; ind < index + parms->mini_batch_size; ind++)
  {
    ...
    for (l = 0; l < rbm->nlabels; l++)
    {
      ...
      if (compute_label_bias)
        dlabel_bias[l] += (l0 - rbm->labels[l]) * softmax_deriv[l];
      if (!devFinite(dlabel_bias[h]))
        DiagBreak();
      if (compute_label_weights)
      {
        for (h = 0; h < rbm->nhidden; h++)
...
int CDBNactivateForward(CDBN *cdbn, double *visible, int nlayers)
{
  ...
  if (nlayers <= 0)
    nlayers = cdbn->nlayers;

  RBMactivateForward(cdbn->rbms[0], visible);
  for (l = 1; l < nlayers; l++)
    RBMactivateForward(cdbn->rbms[l], cdbn->rbms[l - 1]->hidden_state);
