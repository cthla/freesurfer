                            ...
                            int which_inputs)
{
  ...
  {
    if (ca_classes)
      ca_classes[i] = ((int)vl_total->vdst[i] == target_label) ? 1 : -1;
                      ...
                      float *sigmas)
{
  ...
  for (i = 0; i < nscales; i++)
  {
    ...
    if (which_inputs & CA_INPUT_D2I_S)
    {
      ...
      if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON)
        MRIwrite(mri_2nd_deriv_s[i], "deriv.mgz");
    }
    MRIfree(&mri_kernel);
  }
  ...
  for (i = 0; i < nscales; i++)
  {
    ...
    if (which_inputs & CA_INPUT_GRADIENT)
      MRIfree(&mri_grad[i]);
    if (which_inputs & CA_INPUT_LAPLACIAN)
      MRIfree(&mri_laplacian[i]);
    if (which_inputs & CA_INPUT_D2I_S)
      MRIfree(&mri_2nd_deriv_s[i]);
  }
  if (which_inputs & (CA_INPUT_DTRANS | CA_INPUT_DTRANS_GRAD))
    MRIfree(&mri_dtrans);
  return (svm_inputs);
            ...
            float *sigmas)
{
  ...
  if (ca->classifiers == NULL)
    ErrorExit(ERROR_NOMEMORY, "CAalloc: could not allocate %d classifers", width);
  for (x = 0; x < width; x++)
  {
    ca->classifiers[x] = (CLASSIFIER ***)calloc(height, sizeof(CLASSIFIER **));
    if (ca->classifiers[x] == NULL)
      ErrorExit(ERROR_NOMEMORY, "CAalloc: could not allocate X %d classifers", height);
    for (y = 0; y < height; y++)
    {
        ...
        for (o = 0; o < ICO0_NVERTICES; o++)
        {
          ...
          case CA_GAUSSIAN:
            ...
            if (ca->classifiers[x][y][z][o].c1_means == NULL || ca->classifiers[x][y][z][o].c1_vars == NULL
                || ca->classifiers[x][y][z][o].c2_means == NULL
                || ca->classifiers[x][y][z][o].c2_vars == NULL)
...
int CAtrain(CA *ca, VOXEL_LIST *vl, MRI *mri_norm, MRI *mri_aseg, int source_label, int target_label)
{
  ...
  if (classes == NULL)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %d class vector", Progname, vl->nvox);
  for (i = 0; i < vl->nvox; i++)
    classes[i] = ((int)vl->vdst[i] == target_label) ? 1 : -1;
  ...
  for (i = 0; i < vl->nvox; i++)
  {
    ...
    if (i == Gdiag_no)
      DiagBreak();
    if (xi < 0 || yi < 0 || zi < 0 || xi >= ca->width || yi >= ca->height || zi >= ca->depth)
      continue;
...
int CAwrite(CA *ca, char *fname)
{
  ...
  fp = fopen(fname, "wb");
  if (fp == NULL)
    ErrorExit(ERROR_NOFILE, "CAwrite(%s): could not open file for writing", fname);
  ...
  for (i = 0; i < ca->nscales; i++)
    fwriteDouble(ca->sigmas[i], fp);
...
float CAclassifyVoxel(CA *ca, MRI *mri_normals, int x, int y, int z, float *inputs)
{
  ...
  if (xi < 0 || xi >= ca->width || yi < 0 || yi >= ca->height || zi < 0 || zi >= ca->depth)
    return (0.0);

  if (x == Gx && y == Gy && z == Gz && ((Gdiag_no >= 0 && Gdiag_no == o) || Gdiag_no < 0))
    DiagBreak();
  cl = &ca->classifiers[xi][yi][zi][o];
  if (xi == Gx && yi == Gy && zi == Gz)
  {
  ...
  if (cl->c1_ntraining == 0 || cl->c2_ntraining == 0)
    return (0.0);
...
static MATRIX *
compute_ras_basis_vectors(MRI *mri_aseg_orig, MRI *mri_aseg_edit, int label, int width, int height, int depth, int pad)
{
  ...
  *MATRIX_RELT(m_trans, 3, 4) = means[2];
  ...
  // now compute a transform that takes the bounding box to the desired
  // width/height/depth
