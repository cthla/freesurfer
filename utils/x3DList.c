...
char *x3Lst_ksaErrorString[x3Lst_knNumErrorCodes] = { "No error.",
                                                      "Invalid space ptr.",
                                                      "Invalid signature.",
...
x3Lst_tErr x3Lst_New(x3DListRef *op3DList, int inListSize)
{
  ...
  {
    this->mPlane[plane] = (xListRef *)calloc(sizeof(xList), this->mnPlaneSize);
...
x3Lst_tErr x3Lst_AddItem(x3DListRef this, xVoxelRef iWhere, void *ipItem)
{
  ...
  eList = xList_PushItem(this->mPlane[x3Lst_tPlane_X][xVoxl_GetX(iWhere)], ipItem);
  if (xList_tErr_NoErr != eList)
  {
  ...
  eList = xList_PushItem(this->mPlane[x3Lst_tPlane_Y][xVoxl_GetY(iWhere)], ipItem);
  if (xList_tErr_NoErr != eList)
  {
    eResult = x3Lst_tErr_ErrorAccessingList;
    goto error;
  ...
  eList = xList_PushItem(this->mPlane[x3Lst_tPlane_Z][xVoxl_GetZ(iWhere)], ipItem);
  if (xList_tErr_NoErr != eList)
  {
    eResult = x3Lst_tErr_ErrorAccessingList;
    goto error;
...
x3Lst_tErr x3Lst_RemoveItem(x3DListRef this, xVoxelRef iWhere, void **iopItemToRemove)
{
  ...
  eList = xList_RemoveItem(this->mPlane[x3Lst_tPlane_X][xVoxl_GetX(iWhere)], &pItemToRemove);
  if (xList_tErr_NoErr != eList)
  {
    goto error;
  }
  ...
  pItemToRemove = *iopItemToRemove;
  eList = xList_RemoveItem(this->mPlane[x3Lst_tPlane_Y][xVoxl_GetY(iWhere)], &pItemToRemove);
  if (xList_tErr_NoErr != eList)
  {
    goto error;
  }
  ...
  pItemToRemove = *iopItemToRemove;
  eList = xList_RemoveItem(this->mPlane[x3Lst_tPlane_Z][xVoxl_GetZ(iWhere)], &pItemToRemove);
  if (xList_tErr_NoErr != eList)
  {
    goto error;
  }
...
error:
  ...
  if (x3Lst_tErr_NoErr != eResult && x3Lst_tErr_ItemNotInSpace != eResult)
  {
...
x3Lst_tErr x3Lst_IsInList(x3DListRef this, void *ipData, tBoolean *outIsInList)
{
  ...
  for (plane = 0; plane < x3Lst_knNumPlanes; plane++)
  {
    ...
    for (list = 0; list < this->mnPlaneSize; list++)
    {
      eList = xList_IsInList(this->mPlane[plane][list], ipData, &isInList);
      if (xList_tErr_NoErr != eList)
      {
        eResult = x3Lst_tErr_ErrorAccessingList;
        goto error;
...
x3Lst_tErr x3Lst_GetItemsInXPlane(x3DListRef this, int inXPlane, xListRef *opList)
{
  ...
  eResult = x3Lst_GetItemsInPlane_(this, x3Lst_tPlane_X, inXPlane, opList);
  if (x3Lst_tErr_NoErr != eResult)
  {
    goto error;
  }
...
x3Lst_tErr x3Lst_GetItemsInYPlane(x3DListRef this, int inYPlane, xListRef *opList)
{
  ...
  eResult = x3Lst_GetItemsInPlane_(this, x3Lst_tPlane_Y, inYPlane, opList);
  if (x3Lst_tErr_NoErr != eResult)
  {
    goto error;
  }
...
x3Lst_tErr x3Lst_GetItemsInZPlane(x3DListRef this, int inZPlane, xListRef *opList)
{
  ...
  eResult = x3Lst_GetItemsInPlane_(this, x3Lst_tPlane_Z, inZPlane, opList);
  if (x3Lst_tErr_NoErr != eResult)
  {
    goto error;
  }
...
x3Lst_tErr x3Lst_GetItemsInPlane_(x3DListRef this, x3Lst_tPlane iPlane, int inPlaneIndex, xListRef *opList)
{
  ...
  if (inPlaneIndex < 0 || inPlaneIndex >= this->mnPlaneSize)
  {
...
x3Lst_tErr x3Lst_VerifyLocation(x3DListRef this, xVoxelRef iWhere)
{
  ...
  if (xVoxl_GetX(iWhere) < 0 || xVoxl_GetX(iWhere) >= this->mnPlaneSize || xVoxl_GetY(iWhere) < 0
      || xVoxl_GetY(iWhere) >= this->mnPlaneSize || xVoxl_GetZ(iWhere) < 0 || xVoxl_GetZ(iWhere) >= this->mnPlaneSize)
  {
