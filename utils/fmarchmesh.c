...
float ReCompute(int vIDc, int vIDa, int vIDb, MRI_SURFACE *mesh, float *T, unsigned char *label)
{
  ...
  a = sqrt((Vb->x - Vc->x) * (Vb->x - Vc->x) + (Vb->y - Vc->y) * (Vb->y - Vc->y) + (Vb->z - Vc->z) * (Vb->z - Vc->z));
  b = sqrt((Va->x - Vc->x) * (Va->x - Vc->x) + (Va->y - Vc->y) * (Va->y - Vc->y) + (Va->z - Vc->z) * (Va->z - Vc->z));
  ...
  c = sqrt((Va->x - Vb->x) * (Va->x - Vb->x) + (Va->y - Vb->y) * (Va->y - Vb->y) + (Va->z - Vb->z) * (Va->z - Vb->z));
