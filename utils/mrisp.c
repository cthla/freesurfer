...
MRI_SP *MRIScoordsToParameterization(MRI_SURFACE *mris, MRI_SP *mrisp, float scale, int which_vertices)
{
  ...
  for (vno = 0; vno < mris->nvertices; vno++)
  {
    ...
    if ((u == DEBUG_U) && (v == DEBUG_V))
      fprintf(stderr, "v = %6.6d, x = (%2.3f,%2.3f,%2.3f)\n", vno, x, y, z);
  }
...
{
  float a, b, c, phi, theta, z, uf, vf, du, dv, curv, d;
  // float x, y;
  int vno, u0, v0, u1, v1;
  VERTEX *vertex;
  ...
  for (vno = 0; vno < mris->nvertices; vno++)
  {
    ...
    curv = du * dv * *IMAGEFseq_pix(mrisp->Ip, u1, v1, fno) + (1.0f - du) * dv * *IMAGEFseq_pix(mrisp->Ip, u0, v1, fno)
...
{
  float a, b, c, phi, theta, z, uf, vf, du, dv, origx, origy, origz, d;
  // float x, y, val1, val2, val3, val4;
  int vno, u0, v0, u1, v1;
  VERTEX *vertex;
...
{
  float a, b, c, phi, theta, z, uf, vf, du, dv, curv, d, var;
  // float x, y;
  int vno, u0, v0, u1, v1;
  VERTEX *vertex;
...
{
  float a, b, c, phi, theta, z, uf, vf, du, dv, d;
  // float  x, y;
  int vno, u0, v0, u1, v1;
  VERTEX *vertex;
...
MRI_SP *MRISPalloc(float scale, int nfuncs)
{
  ...
  if (!mrisp)
    ErrorExit(ERROR_NOMEMORY, "MRISPalloc(%d, %d): allocation failed", u_dim, v_dim);
...
MRI_SP *MRISPread(char *fname)
{
  ...
  if (!mrisp->Ip)
    ErrorReturn(NULL, (ERROR_NOFILE, "MRISPread(%s): could not open file", fname));
...
float MRISPsample(MRI_SP *mrisp, float x, float y, float z, int fno)
{
  ...
  retval = du * dv * *IMAGEFseq_pix(mrisp->Ip, u1, v1, fno) + (1.0f - du) * dv * *IMAGEFseq_pix(mrisp->Ip, u0, v1, fno)
           + (1.0f - du) * (1.0f - dv) * *IMAGEFseq_pix(mrisp->Ip, u0, v0, fno)
           + du * (1.0f - dv) * *IMAGEFseq_pix(mrisp->Ip, u1, v0, fno);
