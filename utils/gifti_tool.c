...
int main(int argc, char *argv[])
{
  ...
  if (rv < 0)
    return 1; /* non-zero means terminate */
  else if (rv > 0)
    return 0;
  ...
  if (opts.gt_display)
    rv = gt_display(&opts);
  else if (opts.gt_compare)
    rv = gt_compare(&opts);
  else if (opts.gt_copy)
    rv = gt_copy(&opts);
  else if (opts.gt_write)
    rv = gt_write(&opts);
  else
    rv = gt_test(&opts);
...
static int process_opts(int argc, char *argv[], gt_opts *opts)
{
  ...
  for (ac = 1; ac < argc; ac++)
  {
    ...
    else if (!strcmp(argv[ac], "-mod_DA_atr"))
    {
      ...
      if (add_to_str_list(&opts->DA_atrs, argv[ac]) || add_to_str_list(&opts->DA_atrs, argv[ac + 1]))
        return -1;
    ...
    else if (!strcmp(argv[ac], "-mod_DA_meta"))
    {
      ...
      if (add_to_str_list(&opts->DA_meta, argv[ac]) || add_to_str_list(&opts->DA_meta, argv[ac + 1]))
        return -1;
      ac++; /* and consume last arg */
    }
    ...
    else if (!strcmp(argv[ac], "-mod_gim_atr"))
    {
      ...
      if (add_to_str_list(&opts->gim_atrs, argv[ac]) || add_to_str_list(&opts->gim_atrs, argv[ac + 1]))
        return -1;
      ac++; /* and consume last arg */
    }
    else if (!strcmp(argv[ac], "-mod_gim_meta"))
    {
      ...
      if (add_to_str_list(&opts->gim_meta, argv[ac]) || add_to_str_list(&opts->gim_meta, argv[ac + 1]))
        return -1;
      ac++; /* and consume last arg */
    }
...
static int free_gt_opts(gt_opts *opts)
{
  ...
  opts->DAlistr.len = 0;
  opts->DAlistr.list = NULL;
  opts->DAmodlist.len = 0;
  opts->DAmodlist.list = NULL;

  opts->gim_atrs.len = 0;
  opts->gim_atrs.list = NULL;
  opts->gim_meta.len = 0;
  opts->gim_meta.list = NULL;
  opts->DA_atrs.len = 0;
  opts->DA_atrs.list = NULL;
  opts->DA_meta.len = 0;
  opts->DA_meta.list = NULL;

  opts->infiles.len = 0;
  opts->infiles.list = NULL;
...
int gt_copy(gt_opts *opts)
{
  ...
  if (opts->copy_DA_meta)
  {
    ...
    if (opts->DA_meta.len == 1 && !strcmp(opts->DA_meta.list[0], "ALL"))
    {
      ...
      { /* all seems well, copy all meta from src to dest */
...
int gt_write_dataset(gt_opts *opts, gifti_image *gim)
{
  ...
  if (opts->ofile_gifti)
  {
    ...
    if (opts->set_extern)
    {
      ...
      if (gifti_set_extern_filelist(gim, opts->ext_files.len, opts->ext_files.list))
        return 1;
    }
...
int write_1D_file(giiDataArray **dlist, int len, char *prefix, int add_suf)
{
  ...
  { /* write da->nvals lines of 'num values */
...
int write_surf_file(giiDataArray *dc, giiDataArray *dt, char *prefix, int add_suf)
{
  ...
  if (add_suf && !strstr(prefix, ".asc"))
  { /* maybe create a new name */
