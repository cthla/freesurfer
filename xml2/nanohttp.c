 ...
 */
static void xmlHTTPErrMemory(const char *extra)
{
...
static int xmlNanoHTTPSend(xmlNanoHTTPCtxtPtr ctxt, const char *xmt_ptr, int outlen)
{
  int total_sent = 0;
#ifdef HAVE_POLL_H
  struct pollfd p;
  ...
  if ((ctxt->state & XML_NANO_HTTP_WRITE) && (xmt_ptr != NULL))
  {
    ...
    {
      int nsent = send(ctxt->fd, xmt_ptr + total_sent, outlen - total_sent, 0);
...
static char *xmlNanoHTTPReadLine(xmlNanoHTTPCtxtPtr ctxt)
{
  char buf[4096];
  char *bp = buf;
  int rc;
...
static void xmlNanoHTTPScanAnswer(xmlNanoHTTPCtxtPtr ctxt, const char *line)
{
  ...
  else if (!xmlStrncasecmp(BAD_CAST line, BAD_CAST "Location:", 9))
  {
    ...
    if (*cur == '/')
    {
      ...
      xmlChar *tmp_loc = xmlStrcat(tmp_http, (const xmlChar *)ctxt->hostname);
      ctxt->location = (char *)xmlStrcat(tmp_loc, (const xmlChar *)cur);
    }
    else
    {
...
static int xmlNanoHTTPConnectAttempt(struct sockaddr *addr)
{
#ifndef HAVE_POLL_H
  fd_set wfd;
#ifdef _WINSOCKAPI_
...
static int xmlNanoHTTPConnectHost(const char *host, int port)
{
  struct hostent *h;
...
#ifdef DEBUG_HTTP
  xmlGenericError(xmlGenericErrorContext, "xmlNanoHTTPConnectHost:  unable to connect to '%s'.\n", host);
#endif
  return (-1);
}
...
int xmlNanoHTTPRead(void *ctx, void *dest, int len)
{
  ...
  if (ctxt->usesGzip == 1)
  {
    ...
    while (ctxt->strm->avail_out > 0 && (ctxt->strm->avail_in > 0 || xmlNanoHTTPRecv(ctxt) > 0))
    {
...
retry:
  ...
  if (ctxt->hostname == NULL)
  {
    __xmlIOErr(XML_FROM_HTTP, XML_HTTP_UNKNOWN_HOST, "Failed to identify host in URI");
    xmlNanoHTTPFreeCtxt(ctxt);
    if (redirURL != NULL)
      xmlFree(redirURL);
    return (NULL);
  ...
  if (proxy)
  {
    if (ctxt->port != 80)
    {
      p += snprintf(p, blen - (p - bp), "%s http://%s:%d%s", method, ctxt->hostname, ctxt->port, ctxt->path);
    }
    else
      p += snprintf(p, blen - (p - bp), "%s http://%s%s", method, ctxt->hostname, ctxt->path);
  }
  else
...
#ifdef DEBUG_HTTP
  xmlGenericError(xmlGenericErrorContext, "-> %s%s", proxy ? "(Proxy) " : "", bp);
  if ((blen -= strlen(bp) + 1) < 0)
    xmlGenericError(xmlGenericErrorContext, "ERROR: overflowed buffer by %d bytes\n", -blen);
#endif
  ...
  if ((ctxt->location != NULL) && (ctxt->returnValue >= 300) && (ctxt->returnValue < 400))
  {
    ...
    if (redirURL != NULL)
      xmlFree(redirURL);
#ifdef DEBUG_HTTP
    xmlGenericError(xmlGenericErrorContext, "xmlNanoHTTPMethodRedir: Too many redirects, aborting ...\n");
#endif
    return (NULL);
  }
...
void *xmlNanoHTTPMethod(
    const char *URL, const char *method, const char *input, char **contentType, const char *headers, int ilen)
{
  return (xmlNanoHTTPMethodRedir(URL, method, input, contentType, NULL, headers, ilen));
}
