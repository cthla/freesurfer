 ...
 */
static void xmlXPtrErrMemory(const char *extra)
{
 ...
 * Handle a redefinition of attribute error
 */
static void xmlXPtrErr(xmlXPathParserContextPtr ctxt, int error, const char *msg, const xmlChar *extra)
{
  if (ctxt != NULL)
    ctxt->error = error;
 ...
 */
static int xmlXPtrGetArity(xmlNodePtr cur)
{
  int i;
  if (cur == NULL)
    return (-1);
 ...
 *         in case of error
 */
static int xmlXPtrGetIndex(xmlNodePtr cur)
{
  int i;
  if (cur == NULL)
    return (-1);
 ...
 */
static int xmlXPtrCmpPoints(xmlNodePtr node1, int index1, xmlNodePtr node2, int index2)
{
...
static void xmlXPtrRangeCheckOrder(xmlXPathObjectPtr range)
{
  ...
  if (range->user2 == NULL)
    return;
  tmp = xmlXPtrCmpPoints(range->user, range->index, range->user2, range->index2);
  if (tmp == -1)
  {
    tmp2 = range->user;
...
xmlXPathObjectPtr xmlXPtrNewRangeNodePoint(xmlNodePtr start, xmlXPathObjectPtr end)
{
  ...
  if (end == NULL)
    return (NULL);
  if (start->type != (xmlElementType)XPATH_POINT)
    return (NULL);
  if (end->type != XPATH_POINT)
    return (NULL);
...
xmlLocationSetPtr xmlXPtrLocationSetCreate(xmlXPathObjectPtr val)
{
  ...
  if (val != NULL)
  {
    ...
    memset(ret->locTab, 0, XML_RANGESET_DEFAULT * (size_t)sizeof(xmlXPathObjectPtr));
...
void xmlXPtrLocationSetAdd(xmlLocationSetPtr cur, xmlXPathObjectPtr val)
{
  ...
  if (cur->locMax == 0)
  {
    ...
    memset(cur->locTab, 0, XML_RANGESET_DEFAULT * (size_t)sizeof(xmlXPathObjectPtr));
...
void xmlXPtrLocationSetDel(xmlLocationSetPtr cur, xmlXPathObjectPtr val)
{
  ...
  {
#ifdef DEBUG
    xmlGenericError(xmlGenericErrorContext, "xmlXPtrLocationSetDel: Range wasn't found in RangeList\n");
#endif
    return;
  }
  cur->locNr--;
...
xmlXPathObjectPtr xmlXPtrEval(const xmlChar *str, xmlXPathContextPtr ctx)
{
  ...
  if ((ctxt->value != NULL) && (ctxt->value->type != XPATH_NODESET) && (ctxt->value->type != XPATH_LOCATIONSET))
  {
    ...
    if (tmp != NULL)
    {
      if (tmp != init)
      {
        if (tmp->type == XPATH_NODESET)
        {
          ...
          xmlNodeSetPtr set;
          set = tmp->nodesetval;
          if ((set->nodeNr != 1) || (set->nodeTab[0] != (xmlNodePtr)ctx->doc))
            stack++;
        }
        else
          stack++;
 ...
 */
static xmlNodePtr xmlXPtrBuildRangeNodeList(xmlXPathObjectPtr range)
{
...
static xmlXPathObjectPtr xmlXPtrCoveringRange(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr loc)
{
  if (loc == NULL)
    return (NULL);
  if ((ctxt == NULL) || (ctxt->context == NULL) || (ctxt->context->doc == NULL))
    return (NULL);
  switch (loc->type)
  {
  case XPATH_POINT:
    return (xmlXPtrNewRange(loc->user, loc->index, loc->user, loc->index));
  case XPATH_RANGE:
    if (loc->user2 != NULL)
    {
      return (xmlXPtrNewRange(loc->user, loc->index, loc->user2, loc->index2));
    }
    else
    {
      ...
      {
        return (xmlXPtrNewRange(node, 0, node, xmlXPtrGetArity(node)));
      }
      else
      {
        switch (node->type)
        ...
        case XML_HTML_DOCUMENT_NODE:
        {
          ...
          node = node->parent;
          return (xmlXPtrNewRange(node, indx - 1, node, indx + 1));
        }
        default:
          return (NULL);
        }
...
static xmlXPathObjectPtr xmlXPtrInsideRange(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr loc)
{
  ...
  case XPATH_RANGE:
  {
    ...
    if (loc->user2 != NULL)
    {
      return (xmlXPtrNewRange(node, loc->index, loc->user2, loc->index2));
    }
    else
    {
      switch (node->type)
...
found:
  if ((cur->type != XML_ELEMENT_NODE) && (cur->type != XML_TEXT_NODE) && (cur->type != XML_DOCUMENT_NODE)
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrAdvanceChar(xmlNodePtr *node, int *indx, int bytes)
{
  xmlNodePtr cur;
  int pos;
  int len;
  ...
  while (bytes >= 0)
  {
     ...
     */
    while ((cur != NULL) && ((cur->type == XML_ELEMENT_NODE) || (cur->type == XML_DOCUMENT_NODE)
                             || (cur->type == XML_HTML_DOCUMENT_NODE)))
    {
      if (pos > 0)
      {
 ...
 *            of the range
 */
static int xmlXPtrMatchString(const xmlChar *string, xmlNodePtr start, int startindex, xmlNodePtr *end, int *endindex)
{
  xmlNodePtr cur;
  int pos;       /* 0 based */
  int len;       /* in bytes */
  ...
  while (stringlen > 0)
  {
    ...
    if ((cur->type != XML_ELEMENT_NODE) && (cur->content != NULL))
    {
      ...
      if (len >= pos + stringlen)
      {
        ...
        {
#ifdef DEBUG_RANGES
          xmlGenericError(xmlGenericErrorContext, "found range %d bytes at index %d of ->", stringlen, pos + 1);
      ...
      else
      {
        ...
        {
#ifdef DEBUG_RANGES
          xmlGenericError(xmlGenericErrorContext, "found subrange %d bytes at index %d of ->", sub, pos + 1);
          xmlDebugDumpString(stdout, cur->content);
          xmlGenericError(xmlGenericErrorContext, "\n");
#endif
          string = &string[sub];
 ...
 */
static int
xmlXPtrSearchString(const xmlChar *string, xmlNodePtr *start, int *startindex, xmlNodePtr *end, int *endindex)
{
  xmlNodePtr cur;
  const xmlChar *str;
  int pos; /* 0 based */
...
#ifdef DEBUG_RANGES
            xmlGenericError(xmlGenericErrorContext, "found '%c' at index %d of ->", first, pos + 1);
            xmlDebugDumpString(stdout, cur->content);
            xmlGenericError(xmlGenericErrorContext, "\n");
#endif
 ...
 */
#ifdef DEBUG_RANGES
          xmlGenericError(xmlGenericErrorContext, "found '' at index %d of ->", pos + 1);
          xmlDebugDumpString(stdout, cur->content);
          xmlGenericError(xmlGenericErrorContext, "\n");
#endif
          *start = cur;
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrGetLastChar(xmlNodePtr *node, int *indx)
{
  xmlNodePtr cur;
  int pos, len = 0;
  ...
  while (cur != NULL)
  {
    if (cur->last != NULL)
      cur = cur->last;
    else if ((cur->type != XML_ELEMENT_NODE) && (cur->content != NULL))
    {
      len = xmlStrlen(cur->content);
      break;
    }
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrGetStartPoint(xmlXPathObjectPtr obj, xmlNodePtr *node, int *indx)
{
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrGetEndPoint(xmlXPathObjectPtr obj, xmlNodePtr *node, int *indx)
{
  if ((obj == NULL) || (node == NULL) || (indx == NULL))
    return (-1);
...
#ifdef DEBUG_RANGES
    xmlGenericError(xmlGenericErrorContext, "from index %d of ->", startindex);
    xmlDebugDumpString(stdout, start->content);
    xmlGenericError(xmlGenericErrorContext, "\n");
    xmlGenericError(xmlGenericErrorContext, "to index %d of ->", endindex);
