...
#define MAX_TITLE_CHARS(d) (MIN(STR_LEN - 1, (nint((float)d->dispImage->cols / (float)CHAR_WIDTH))))
...
static void xv_dimage_repaint(Canvas canvas, Xv_Window window, Rectlist *repaint_area);
...
static void xvCreateImage(XV_FRAME *xvf, DIMAGE *dimage, int x, int y, int which);
...
static IMAGE *GtmpFloatImage = NULL, *GtmpByteImage = NULL, *GtmpByteImage2 = NULL, *GtmpScaledFloatImage = NULL;
...
----------------------------------------------------------------------*/
XV_FRAME *
XValloc(int rows, int cols, int button_rows, int display_rows, int display_cols, char *name, Notify_value (*poll)(void))
{
  int row;
...
static void xvCreateFrame(XV_FRAME *xvf, char *name)
{
  ...
  height = PANEL_HEIGHT + xvf->rows * xvf->display_rows + (xvf->rows - 1) * CHAR_HEIGHT;
  if (width < MIN_FRAME_WIDTH)
    width = MIN_FRAME_WIDTH;

  xvf->frame = (Frame)xv_create((Xv_opaque)NULL,
                                FRAME,
                                FRAME_LABEL,
                                name,
                                XV_X,
  ...
  debug_frame = (Frame)xv_create(xvf->frame,
                                 FRAME_CMD,
                                 XV_HEIGHT,
                                 DEBUG_ROWS,
                                 XV_WIDTH,
                                 ...
                                 FALSE,
                                 NULL);
  debug_panel = (Panel)xv_create((Xv_opaque)debug_frame, PANEL, XV_X, 0, XV_Y, 0, NULL);
  debug_menu = (Menu)xv_create((Xv_opaque)NULL,
                               MENU_TOGGLE_MENU,
                               MENU_NOTIFY_PROC,
                               debugMenuItem,
                               MENU_STRINGS,
  ...
  debug_panel_item = (Panel_item)xv_create(
      debug_panel, PANEL_BUTTON, PANEL_LABEL_STRING, debug_str, PANEL_ITEM_MENU, debug_menu, NULL);
  show_diags(Gdiag);

  /* create a region for messages */
  xvf->msg_item[0] = (Panel_item)xv_create((Xv_opaque)xvf->panel,
                                           PANEL_MESSAGE,
                                           PANEL_LABEL_BOLD,
                                           TRUE,
                                           XV_X,
                                           ...
                                           xvf->msg_str[0],
                                           NULL);
  xvf->msg_item[1] = (Panel_item)xv_create((Xv_opaque)xvf->panel,
                                           PANEL_MESSAGE,
                                           PANEL_LABEL_BOLD,
                                           TRUE,
                                           PANEL_LABEL_STRING,
...
static void xvInitColors(XV_FRAME *xvf)
{
  ...
  shift = MAX_COLORS == 32 ? 11 : MAX_COLORS == 64 ? 10 : MAX_COLORS == 128 ? 9 : 8;
  ...
  xvf->cms = (Cms)xv_create(xvf->screen,
                            CMS,
                            CMS_NAME,
                            "xvutil",
                            CMS_SIZE,
                            ...
                            xcolors,
                            CMS_TYPE,
                            MAX_COLORS == 64 ? XV_STATIC_CMS : XV_DYNAMIC_CMS,
                            NULL);
...
void XVrepaintImage(XV_FRAME *xvf, int which)
{
  ...
  if (dimage->used == DIMAGE_IMAGE)
  {
    ...
    XPutImage(xvf->display, (Drawable)dimage->window, xvf->gc, dimage->ximage, 0, 0, 0, 0, image->cols, image->rows);
  }
...
void XVshowImage(XV_FRAME *xvf, int which, IMAGE *image, int frame)
{
  ...
  if (((image != dimage->sourceImage) && (image != dimage->oSourceImage)) || (dimage->dx == 0))
  {
  ...
  {
    /*    dimage->oSourceImage = image ;*/
    dimage->zoomImage = ImageExtract(image, NULL, dimage->x0, dimage->y0, dimage->dx, dimage->dy);
    if (dimage->sourceImage && dimage->sourceImage != dimage->oSourceImage && (dimage->sourceImage != image))
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVshowImageRange(XV_FRAME *xvf, int which, IMAGE *image, int frame, float fmin, float fmax)
{
  float xscale, yscale;
  // float scale;
  DIMAGE *dimage;
...
static void xv_dimage_event_handler(Xv_Window xv_window, Event *event)
{
  int x, y, yprint, i;
  double val = 0.0;
  // Window window;
  ...
  */
  ...
  {
    if (y >= dimage->sourceImage->rows)
      y = dimage->sourceImage->rows - 1;
    if (x >= dimage->sourceImage->cols)
      x = dimage->sourceImage->cols - 1;
  }
  ...
  {
    if (y >= dimage->sourceImage->rows)
      y = dimage->sourceImage->rows;
    if (x >= dimage->sourceImage->cols)
      x = dimage->sourceImage->cols;
    switch (event_id(event))
    {
    case MS_RIGHT: /* zoom out to normal view */
      if (event_is_up(event))
      {
        dimage->dx1 = dimage->dy1 = dimage->dx = dimage->dy = dimage->x0 = dimage->y0 = dimage->x1 = dimage->y1 = 0;
        rows = xvf->display_rows;
        cols = xvf->display_cols;
        aspect = (float)dimage->sourceImage->rows / (float)dimage->sourceImage->cols;
        if (rows > cols)
    ...
    case MS_LEFT:
      ...
      else if (event_is_up(event)) /* erase box and show zoomed image */
      {
        int dir = xvf->ydir;
        xvf->ydir = -1;
        XVdrawBox(xvf, which, dimage->x1, dimage->y1, dimage->dx1, dimage->dy1, XXOR);
        xvf->ydir = dir;
        if (dimage->dx1 < 0)
        {
...
/* enforce appropriate aspect ratio */
    ...
    switch (event_id(event)) /* ctrl is not down */
    {
      ...
      if (dimage->sync && !xvf->noprint)
      {
        for (i = 0; i < xvf->rows * xvf->cols; i++)
        {
          ...
          if (dimage2 && (dimage2->sync == dimage->sync))
          {
            ...
            {
            case PFDOUBLE:
              sprintf(buf, "%2.2f", *IMAGEDseq_pix(dimage2->sourceImage, x, y, dimage2->frame));
              break;
            case PFFLOAT:
              sprintf(buf, "%2.2f", *IMAGEFseq_pix(dimage2->sourceImage, x, y, dimage2->frame));
              break;
            case PFBYTE:
              sprintf(buf, "%d", (int)*IMAGEseq_pix(dimage2->sourceImage, x, y, dimage2->frame));
              break;
            case PFINT:
              sprintf(buf, "%d", *IMAGEIseq_pix(dimage2->sourceImage, x, y, dimage2->frame));
              break;
            }
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVsetParms(void (*event_handler)(Event *event, DIMAGE *dimage)) { XVevent_handler = event_handler; }
/*----------------------------------------------------------------------
            Parameters:
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVsetQuitFunc(void (*quit_func)(void)) { XVquit_func = quit_func; }
/*----------------------------------------------------------------------
            Parameters:
...
static XImage *xvCreateXimage(XV_FRAME *xvf, IMAGE *image)
{
  ...
  {
    fprintf(stderr, "xvCreateXimage: unsupported pixel format %d, must be BYTE\n", image->pixel_format);
    exit(4);
  }
  rows = image->rows;
  cols = image->cols;
  ximage = XCreateImage(xvf->display,
                        DefaultVisual(xvf->display, DefaultScreen(xvf->display)),
                        8,       /* 8 bits per pixel */
                        ZPixmap, /* not a bitmap */
                        0,
                        (char *)image->image, // byte *
                        cols,
                        rows,
                        8, /* 8 bits per pixel */
                        cols);
...
static Panel_setting xvHipsCommand(Panel_item item, Event *event)
{
  ...
  else
  {
    ...
    if (system(hips_cmd_str) != 0)
      fprintf(stderr, "non zero return from system call\n");
...
static void xvHipsCmdFrameInit(XV_FRAME *xvf)
{
  xvf->fname_frame = (Frame)xv_create(xvf->frame,
                                      FRAME_CMD,
                                      XV_HEIGHT,
                                      HIPS_CMD_ROWS,
                                      XV_WIDTH,
                                      ...
                                      FALSE,
                                      NULL);
  xvf->fname_panel = (Panel)xv_create((Xv_opaque)xvf->fname_frame, PANEL, XV_X, 0, XV_Y, 0, NULL);
  xvf->fname_panel_item = (Panel_item)xv_create((Xv_opaque)xvf->fname_panel,
                                                PANEL_TEXT,
  ...
  hips_cmd_frame = (Frame)xv_create(xvf->frame,
                                    FRAME_CMD,
                                    XV_HEIGHT,
                                    HIPS_CMD_ROWS,
                                    XV_WIDTH,
  ...
  hips_cmd_panel = (Panel)xv_create((Xv_opaque)hips_cmd_frame, PANEL, XV_X, 0, XV_Y, 0, NULL);
  hips_cmd_panel_item = (Panel_item)xv_create((Xv_opaque)hips_cmd_panel,
                                              PANEL_TEXT,
                                              PANEL_VALUE_STORED_LENGTH,
                                              300,
                                              PANEL_VALUE_DISPLAY_LENGTH,
...
void XVdrawPoint(XV_FRAME *xvf, int which, int x, int y, int color)
{
  ...
  if (xvf->ydir == -1)
    y = nint((float)(((dimage->sourceImage->rows - 1) - (y - dimage->y0)) + 0.5f) * yscale);
  else
  {
    if (dimage->dy)
      y = nint(((float)(dimage->y0 + y + dimage->dy - dimage->sourceImage->rows) + 0.5f) * yscale);
    else
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVdrawBox(XV_FRAME *xvf, int which, int x, int y, int dx, int dy, int color)
{
  GC gc;
  Display *display;
  Window window;
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVdrawLinef(XV_FRAME *xvf, int which, float x, float y, float dx, float dy, int color)
{
  GC gc;
  Display *display;
  Window window;
  ...
  if (xvf->ydir < 0)
    yi = nint((float)(((dimage->zoomImage->rows - 1) - (y - dimage->y0)) + 0.5f) * yscale);
  else
  {
    if (dimage->dy)
      yi = nint(((float)(dimage->y0 + y + dimage->dy - dimage->sourceImage->rows) + 0.5f) * yscale);
    else
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVdrawLine(XV_FRAME *xvf, int which, int x, int y, int dx, int dy, int color)
{
  GC gc;
  Display *display;
  Window window;
  ...
  x = nint(((float)(x - dimage->x0) + 0.5f) * xscale);
  if (xvf->ydir < 0)
    y = nint((float)(((dimage->zoomImage->rows - 1) - (y - dimage->y0)) + 0.5f) * yscale);
  else
  {
    if (dimage->dy)
      y = nint(((float)(dimage->y0 + y + dimage->dy - dimage->sourceImage->rows) + 0.5f) * yscale);
...
void XVdrawArrow(XV_FRAME *xvf, int which, int x, int y, float dx, float dy, int color)
{
  GC gc;
  Display *display;
  Window window;
...
----------------------------------------------------------------------*/
void XVshowVectorImage(XV_FRAME *xvf, int which, int xo, int yo, int width, int height, int color, IMAGE *image)
{
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVsetKBhandler(void (*kb_handler)(Event *event, DIMAGE *dimage)) { XVkb_handler = kb_handler; }
/*----------------------------------------------------------------------
            Parameters:
...
{
  if (which < 0 || which > 1)
    ErrorReturn(-1, (ERROR_BADPARM, "XVsetMessagePosition(%d): out of range", which));
...
int XVsetImageSize(XV_FRAME *xvf, int which, int rows, int cols)
{
  ...
  do
  {
    ...
    xv_set(canvas_paint_window(dimage->canvas),
           ...
           NULL,
           NULL);
    dimage->window = (Window)xv_get(canvas_paint_window(dimage->canvas), XV_XID);
...
int XVresize(XV_FRAME *xvf)
{
  ...
  for (row = 0; row < xvf->rows; row++)
  {
    ...
    for (col = 0; col < xvf->cols; col++)
    {
      ...
      xv_set(dimage->canvas, XV_X, x, XV_Y, y, NULL);

      xv_set(dimage->title_item, XV_X, x, XV_Y, y - CHAR_HEIGHT + CHAR_PAD, NULL);
...
static void xvCreateImage(XV_FRAME *xvf, DIMAGE *dimage, int x, int y, int which)
{
  ...
  dimage->canvas = (Canvas)xv_create((Xv_opaque)xvf->frame,
                                     CANVAS,
                                     XV_X,
                                     x,
                                     XV_Y,
  ...
  dimage->title_item = (Panel_item)xv_create((Xv_opaque)xvf->panel,
                                             PANEL_MESSAGE,
                                             PANEL_LABEL_BOLD,
                                             TRUE,
                                             XV_X,
                                             x,
                                             XV_Y,
                                             y - CHAR_HEIGHT + CHAR_PAD,
                                             PANEL_LABEL_STRING,
                                             dimage->title_string,
                                             NULL);

  dimage->dispImage = ImageAlloc(xvf->display_rows, xvf->display_cols, PFBYTE, 1);
  dimage->ximage = xvCreateXimage(xvf, dimage->dispImage);
  xv_set(canvas_paint_window(dimage->canvas),
         WIN_EVENT_PROC,
         xv_dimage_event_handler,
  ...
  dimage->clearGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  XSetFunction(xvf->display, dimage->clearGC, GXclear);
  dimage->xorGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  XSetFunction(xvf->display, dimage->xorGC, GXinvert);

  dimage->greenGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  ...
  dimage->blueGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  ...
  dimage->purpleGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  ...
  dimage->yellowGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  ...
  dimage->cyanGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  ...
  dimage->redGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
  ...
  dimage->whiteGC = XCreateGC(xvf->display, dimage->window, (unsigned long)0, &GCvalues);
...
int XVzoom(XV_FRAME *xvf, int which, float zoom)
{
  ...
  max_dim = (float)MIN(dimage->sourceImage->cols, dimage->sourceImage->rows);
  dimage->zoom *= zoom;
...
int XVsyncAll(XV_FRAME *xvf, int which)
{
...
#endif
  {
    ...
    if (dimage && (dimage->sync == 0))
    {
      ...
      {
        dimage2 = XVgetDimage(xvf, which2, DIMAGE_IMAGE);
        if (dimage2 && (dimage2->sourceImage->rows == rows) && (dimage2->sourceImage->cols == cols))
          dimage2->sync = sync;
      }
      XVdoSync(xvf, which);
    }
...
int XVshowAllSyncedImages(XV_FRAME *xvf, int which)
{
  ...
  if (dimage->sync)
    for (which2 = 0; which2 < xvf->rows * xvf->cols; which2++)
    {
      ...
      if (dimage2 && (dimage2->sync == dimage->sync))
      {
        XVsetImageSize(xvf, which2, dimage->dispImage->rows, dimage->dispImage->cols);
        XVshowImage(xvf, which2, dimage2->sourceImage, dimage2->frame);
      }
    }
  return (NO_ERROR);
...
int XVgetFileName(XV_FRAME *xvf, char *default_fname, int (*fname_func)(char *fname), ...)
{
  va_list args;
  char *fmt;
  int len;
...
static Panel_setting xvFileNameCommand(Panel_item item, Event *event)
{
  strcpy(xvf_fname->file_name, (char *)xv_get(xvf_fname->fname_panel_item, PANEL_VALUE));
...
int XVsetDepthFunc(XV_FRAME *xvf, IMAGE *(*get_image)(IMAGE *Iold, int which, int dir))
{
...
int XVshowHistogram(XV_FRAME *xvf, int which, HISTOGRAM *histo)
{
  GC gcred, gcgreen;
  Display *display;
  DIMAGE *dimage;
  ...
  for (binno = 1; binno < nbins; binno++)
  {
    ...
    if (!(binno % 50)) /* draw tick */
      XDrawLine(display, window, gcgreen, x, height - (PAD / 2), x, height - PAD / 2 - TICK_HEIGHT);
  }
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
int XVsetWriteFunc(XV_FRAME *xvf,
                   char *frame_name,
                   char *prompt_str,
