...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
int mri_event_handler(XV_FRAME *xvf, Event *event, DIMAGE *dimage, int *px, int *py, int *pz)
{
    ...
    case MS_LEFT:
      ...
      case MRI_UCHAR:
        if (talairach)
          ...
          XVprintf(xvf, 0, "(%d,%d,%d) --> %d", x, y, z, MRIseq_vox(mri, x, y, z, frame));
        break;
      case MRI_FLOAT:
        if (talairach)
          XVprintf(xvf,
          ...
          XVprintf(xvf, 0, "(%d,%d,%d) --> %2.3f", x, y, z, MRIFseq_vox(mri, x, y, z, frame));
        break;
      }
      break;
    default:
      if (event_is_up(event))
        ...
        case 'w':
          ...
          fp = fopen(fname, "w");
          MRIvoxelToWorld(mri, (Real)x_click, (Real)y_click, (Real)z_click, &xr, &yr, &zr);
          MRIvoxelToTalairach(mri, (Real)x_click, (Real)y_click, (Real)z_click, &xt, &yt, &zt);
...
#endif
  {
    ...
    if (which_click != which) /* erase old point */
    {
      ...
      sprintf(view_str, "view: %s", view == MRI_CORONAL ? "CORONAL" : view == MRI_SAGITTAL ? "SAGITTAL" : "HORIZONTAL");
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVMRIdrawPoint(XV_FRAME *xvf, int which, int view, int depth, MRI *mri, int x, int y, int z, int color)
{
  int xi, yi;
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
void XVMRIdrawRegion(XV_FRAME *xvf, int which, int view, int depth, MRI *mri, MRI_REGION *reg, int color)
{
  int xi, yi, dx, dy;
...
IMAGE *XVMRIshowFrame(XV_FRAME *xvf, MRI *mri, int which, int slice, int frame)
{
  ...
  mag = MIN((float)xvf->orig_disp_rows / (float)I->rows, (float)xvf->orig_disp_cols / (float)I->cols);

  XVsetImageSize(xvf, which, nint((float)I->rows * mag), nint((float)I->cols * mag));
  /*  XVresize(xvf) ;*/
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *XVMRIshowRange(XV_FRAME *xvf, MRI *mri, int which, int slice, float fmin, float fmax)
{
  IMAGE *I;
  float mag;
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *XVMRIshow(XV_FRAME *xvf, MRI *mri, int which, int slice) { return (XVMRIshowFrame(xvf, mri, which, slice, 0)); }
/*----------------------------------------------------------------------
            Parameters:

           Description:
----------------------------------------------------------------------*/
int XVMRIinit(XV_FRAME *xvf_init, int view_row, int view_col)
{
  int i;
  ...
  view_menu = (Menu)xv_create((Xv_opaque)NULL,
                              MENU,
                              MENU_NOTIFY_PROC,
                              viewMenuItem,
                              MENU_STRINGS,
  ...
  view_panel = (Panel_item)xv_create(xvf->panel,
                                     PANEL_BUTTON,
                                     PANEL_LABEL_STRING,
                                     view_str,
                                     XV_X,
...
static IMAGE *get_next_slice(IMAGE *Iold, int which, int dir)
{
  ...
  if (mri)
  {
    ...
    I = MRItoImageView(mri, Idisplay[which], depth - offset, mri_views[which], mri_frames[which]);
...
static void repaint_handler(XV_FRAME *xvf, DIMAGE *dimage)
{
  ...
  if (dimage->sync || (which == which_click))
    XVMRIdrawPoint(xvf, which, mri_views[which], 0, mri, x_click, y_click, z_click, XRED);
}
/*----------------------------------------------------------------------
            Parameters:
  ...
  if (dimage->sync)
  {
    for (which2 = 0; which2 < xvf->rows * xvf->cols; which2++)
    {
      ...
      if (dimage2 /* && (which2 != which) */ && mri2)
      {
        ...
        y2 = nint(yr);
        z2 = nint(zr);
        if (x2 < 0 || x2 >= mri2->width || y2 < 0 || y2 >= mri2->height || z2 < 0 || z2 >= mri2->depth)
          continue;
#if 0
        x2 = MAX(0, x2) ;
        y2 = MAX(0, y2) ;
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *XVMRIredisplayFrame(XV_FRAME *xvf, MRI *mri, int which, int slice, int frame)
{
  IMAGE *I;
  float fmin, fmax;
