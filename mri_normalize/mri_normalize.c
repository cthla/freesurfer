...
static int remove_surface_outliers(MRI *mri_ctrl_src, MRI *mri_dist, MRI *mri_src, MRI *mri_ctrl_dst, float min_dist);
static MRI *remove_surface_outliers_T2(MRI *mri_ctrl_src, MRI *mri_dist, MRI *mri_src, MRI *mri_ctrl_dst);
static MRI *
MRIremoveWMOutliersAndRetainMedialSurface(MRI *mri_src, MRI *mri_src_ctrl, MRI *mri_dst_ctrl, int intensity_below);
static MRI *MRIremoveWMOutliers(MRI *mri_src, MRI *mri_src_ctrl, MRI *mri_dst_ctrl, int intensity_below);
int main(int argc, char *argv[]);
static int remove_outliers_near_surface(
    MRI *mri_ctrl, MRI *mri_dist, MRI *mri_dst, MRI *mri_ctrl_out, float min_dist, float nsigma);
static int get_option(int argc, char *argv[]);
static void usage_exit(int code);
static MRI *compute_bias(MRI *mri_src, MRI *mri_dst, MRI *mri_bias);
static MRI *add_interior_points(MRI *mri_src,
                                MRI *mri_vals,
...
int main(int argc, char *argv[])
{
  ...
  make_cmd_version_string(
      argc, argv, "$Id: mri_normalize.c,v 1.92 2017/01/27 22:31:34 ah221 Exp $", "$Name:  $", cmdline);

  /* rkt: check for and handle version tag */
  nargs = handle_version_option(argc, argv, "$Id: mri_normalize.c,v 1.92 2017/01/27 22:31:34 ah221 Exp $", "$Name:  $");
  ...
  if (!mri_src)
    ErrorExit(ERROR_NO_FILE, "%s: could not open source file %s", Progname, in_fname);
  ...
  if (nsurfs > 0)
  {
    ...
    if (control_point_fname || control_point_label) // do one pass with only file control points first
    {
      ...
      mri_dst = MRI3dGentleNormalize(mri_src,
                                     NULL,
    ...
    for (i = 0; i < nsurfs; i++)
    {
      ...
      if (surface_xform->type == MNI_TRANSFORM_TYPE || surface_xform->type == TRANSFORM_ARRAY_TYPE
  ...
  if (long_flag)
  {
    ...
    if (!mri_bias)
      ErrorExit(ERROR_BADPARM, "%s: could not read bias volume %s", Progname, long_bias_volume_fname);
    mri_ctrl = MRIread(long_control_volume_fname);
    if (!mri_ctrl)
      ErrorExit(ERROR_BADPARM, "%s: could not read control volume %s", Progname, long_control_volume_fname);
    ...
    MRIapplyBiasCorrectionSameGeometry(mri_dst, mri_bias, mri_dst, DEFAULT_DESIRED_WHITE_MATTER_VALUE);
    //    MRIwrite(mri_dst, out_fname) ;
    //    exit(0) ;
  } // end if(long_flag)
  ...
  if (mri_aseg)
  {
    ...
    if (interior_fname1)
    {
      ...
      if (mris_interior1 == NULL)
        ErrorExit(ERROR_NOFILE, "%s: could not read white matter surface from %s\n", Progname, interior_fname1);
      ...
      else
      {
        if (nosnr)
        {
          if (interior_fname1)
          {
            ...
            mri_dst
                = MRIapplyBiasCorrectionSameGeometry(mri_src, mri_bias, mri_dst, DEFAULT_DESIRED_WHITE_MATTER_VALUE);
            MRIfree(&mri_ctrl);
          }
...
static int get_option(int argc, char *argv[])
{
  ...
  else if (!stricmp(option, "MASK_SIGMA"))
  {
    ...
    nargs = 1;
    printf(
        "smoothing input volume with sigma = %2.3f mm and thresholding at %2.0f for mask\n", mask_sigma, mask_thresh);
    nargs = 2;
  }
  ...
  else if (!stricmp(option, "MIN_DIST"))
  {
    ...
    printf("retaining %s points that are at least %2.3fmm from the boundary\n",
           nonmax_suppress ? " nonmaximum suppressed" : "",
           min_dist);
  }
  ...
  else if (!stricmp(option, "lonly") || !stricmp(option, "label_only") || !stricmp(option, "labelonly"))
  {
  ...
  else if (!stricmp(option, "file_only") || !stricmp(option, "fonly") || !stricmp(option, "fileonly"))
  {
    ...
    case 'A':
      ...
      printf("using control point with intensity %2.1f above target.\n", intensity_above);
      nargs = 1;
      break;
    case 'B':
      intensity_below = atof(argv[2]);
      printf("using control point with intensity %2.1f below target.\n", intensity_below);
      nargs = 1;
      break;
    case 'G':
      ...
      for (z = 0; z < mri_ctrl_src->depth; z++)
      {
        ...
        default:
          if ((MRIlabelsInNbhd(mri_aseg, xa, ya, za, 1, Left_Lateral_Ventricle) > 0)
              || (MRIlabelsInNbhd(mri_aseg, xa, ya, za, 1, Right_Lateral_Ventricle)
                  > 0)) // don't use voxels bordering the ventricles
            MRIsetVoxVal(mri_ctrl_dst, x, y, z, 0, CONTROL_NONE);
          else
  ...
  for (nremoved = x = 0; x < mri_ctrl->width; x++)
    for (y = 0; y < mri_ctrl->height; y++)
      for (z = 0; z < mri_ctrl->depth; z++)
      {
        ...
        if ((MRIgetVoxVal(mri_dist, x, y, z, 0) < min_dist)
            && (c > 0)) // it is a control point and it's close to the edge
        {
