...
public:
                   ...
                   int nrhs, const mxArray *prhs[])
  {
    ...
    const std::string usageString = "Usage: calculator = kvlGetCostAndGradientCalculator( typeName, image(s), boundaryCondition, [ transform ], [ means ], [ precisions ], [ weights ], [gaussiansPerClass] )\n where typeName = {'AtlasMeshToIntensityImage','ConditionalGaussianEntropy','MutualInformation'}\n and boundaryCondition = {'Sliding', 'Affine', 'Translation', 'None'}";
    if ((nrhs < 3) ||
        !mxIsChar(prhs[0]) ||
        !mxIsInt64(prhs[1]) ||
        !mxIsChar(prhs[2]))
    ...
    // Retrieve weights if they are provided
    std::vector<vnl_vector<float>> weights;
    if (nrhs > 6)
    {
      // Sanity check
      ...
      //
      const int numberOfClasses = mxGetDimensions(prhs[6])[0];
      const int numberOfContrasts = mxGetDimensions(prhs[6])[1];
      //mexPrintf("numberOfClasses = %d\n",numberOfClasses);
      //mexPrintf("numberOfContrasts = %d\n",numberOfContrasts);
      for (int classNumber = 0; classNumber < numberOfClasses; classNumber++)
      {
        vnl_vector<float> weight(numberOfContrasts, 0.0f);
        for (int imageNumber = 0; imageNumber < numberOfContrasts; imageNumber++)
        {
          weight[imageNumber] = (mxGetPr(prhs[6]))[classNumber + numberOfClasses * imageNumber];
        }
      ...
      // for (unsigned int classNumber = 0; classNumber < numberOfClasses; classNumber++)
      // {
      //   vnl_vector<float> miini = weights[classNumber];
      //   for (unsigned int nima = 0; nima < numberOfContrasts; nima++)
      //   {
    ...
    } // End test if weights are provided
    std::vector<int> classIndex; 
    if (nrhs > 7)
    {
      for (int g = 0; g < mxGetDimensions(prhs[7])[0]; g++){
        classIndex.push_back((int)(mxGetPr(prhs[7]))[g]);
      }

      // for (int g = 0; g < mxGetDimensions(prhs[7])[0]; g++){
      //    mexPrintf("classIndex[%d] = %d\n",g, classIndex[g]);
      // }
    }
    else
    {
      classIndex = {0, 0, 0, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 5, 5, 6, 6};
    }
     
    int numberOfClasses = 7;
    if (nrhs > 8)
    {
      numberOfClasses = (int)*mxGetPr(prhs[8]);
    }
    ...
    case 'A':
    {
      ...
      AtlasMeshToIntensityImageCostAndGradientCalculator::Pointer myCalculator = AtlasMeshToIntensityImageCostAndGradientCalculator::New();
      myCalculator->SetImages(images);
      myCalculator->SetParameters(means, precisions, weights, classIndex, numberOfClasses);
    ...
    case 'C':
    {
      ...
      ConditionalGaussianEntropyCostAndGradientCalculator::Pointer myCalculator = ConditionalGaussianEntropyCostAndGradientCalculator::New();
    ...
    case 'M':
    {
      ...
      MutualInformationCostAndGradientCalculator::Pointer myCalculator = MutualInformationCostAndGradientCalculator::New();
      myCalculator->SetImage(images[0]);
      calculator = myCalculator;
      break;
    }
