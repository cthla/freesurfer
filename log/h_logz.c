...
static int lmConvolve1d(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Igaussian, IMAGE *Idst, int which);
static void mapCalculateLogmap(LOGMAP_INFO *mi);
static void mapCalculateLogmapQuadrant(LOGMAP_INFO *mi, CMAP *mTvToSpoke, CMAP *mTvToRing);
...
static int logFilterNbd(
    LOGMAP_INFO *lmi, int filter[NBD_SIZE], IMAGE *Iin, IMAGE *Iout, int doweight, int start_ring, int end_ring);
static IMAGE *logSobelX(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, int doweight, int start_ring, int end_ring);
static IMAGE *logSobelY(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, int doweight, int start_ring, int end_ring);
...
static void fnormalize(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, float low, float hi);
static void bnormalize(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, byte low, byte hi);
...
LOGMAP_INFO *LogMapInit(double alpha, int cols, int rows, int nrings, int nspokes)
{
  ...
  logMapInfo->logPix = (LOGPIX *)calloc(logsize, sizeof(*(logMapInfo->logPix)));
  logMapInfo->runNoToRing = (int *)calloc(tvsize, sizeof(*(logMapInfo->runNoToRing)));
  logMapInfo->runNoToSpoke = (int *)calloc(tvsize, sizeof(*(logMapInfo->runNoToSpoke)));
  logMapInfo->runNoToLen = (char *)calloc(tvsize, sizeof(*(logMapInfo->runNoToLen)));

  logMapInfo->tvToRing = (UCHAR *)calloc(tvsize, sizeof(*(logMapInfo->tvToRing)));
  logMapInfo->tvToSpoke = (UCHAR *)calloc(tvsize, sizeof(*(logMapInfo->tvToSpoke)));
    ...
    if (crow != UNDEFINED && ccol != UNDEFINED)
    {
      ...
      case PFBYTE:
        ...
        *IMAGEpix(Iout, ring, spoke) = bval;
        break;
      default:
        ErrorReturn(Idst, (ERROR_UNSUPPORTED, "LogMapSample: unsupported pixel format %d", Iout->pixel_format));
        break;
      }
    }
  }
...
IMAGE *LogMapInverseSample(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst)
{
  ...
  case PFFLOAT:
    ...
    for (row = 0; row < rows; row++)
    {
      for (col = 0; col < cols; col++)
      {
        ...
        if (ring != UNDEFINED && spoke != UNDEFINED && (LOG_PIX_AREA(lmi, ring, spoke) > 0))
        {
  ...
  case PFBYTE:
    ...
    break;
  default:
    ErrorReturn(Idst, (ERROR_UNSUPPORTED, "LogMapInverseSample: unsupported pixel format %d", Iout->pixel_format));
    break;
  }
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
int LogMapForward(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst)
{
  int j;
  ...
  for (j = 0; j < Isrc->rows; j++)
  {
    ...
    do
    {
      ...
      if (sring < 0 || sspoke < 0 || sring >= nrings || sspoke >= nspokes)
        ...
        *IMAGEFpix(Iout, sring, sspoke) += (float)(*forward_func_array[(int)(*runl++)])(&tvPtr);
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
int LogMapInverse(LOGMAP_INFO *lmi, IMAGE *Ilog, IMAGE *Idst)
{
...
----------------------------------------------------------------------*/
static void mapCalculateLogmap(LOGMAP_INFO *lmi)
{
...
/*----------------------------------------------------------------------
           ...
           Description:
             ...
             will be accounted for in the building of the full log map.
----------------------------------------------------------------------*/
void mapCalculateLogmapQuadrant(LOGMAP_INFO *lmi, CMAP *mQuadTvToSpoke, CMAP *mQuadTvToRing)
{
...
#else
  r = MIN(hypot((double)halfrows + 0.5, alpha + 0.5), hypot((double)0.5, alpha + 0.5 + halfcols));
#endif
    ...
    for (j = 0; j < halfrows; j++)
    {
      ...
      if (r > 1.0)
      {
        /*
          ...
          */
        ringnum = (int)((double)(nrings / 2) * (log(r) - minlog) / (maxlog - minlog));
        ...
        /*
          ...
          */
        spokenum = (int)(((double)(nspokes / 2) * (theta - minangle)) / (PI / 2.0 - minangle));
...
----------------------------------------------------------------------*/
static void mapQuadToFull(LOGMAP_INFO *lmi, CMAP *mQuadSpoke, CMAP *mQuadRing)
{
...
----------------------------------------------------------------------*/
static void mapInitRuns(LOGMAP_INFO *lmi, int max_run_len)
{
...
static void mapInitLogPix(LOGMAP_INFO *lmi)
{
  int i, j, ring, spoke, area, rows, cols;
  // int nrings, rval;
  double weight;
  // double ring_step, wscale, nrings_m1, log_range, max_ring, min_ring;
...
int LogMapGradient(LOGMAP_INFO *lmi, IMAGE *Iin, IMAGE *Igrad, int doweight, int start_ring, int end_ring)
{
  ...
  LogMapSobel(lmi, Iin, Igrad, xImage, yImage, doweight, start_ring, end_ring);
  return (0);
}
/*----------------------------------------------------------------------
            Parameters:
...
----------------------------------------------------------------------*/
int LogMapSobel(
    LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Igrad, IMAGE *Ix, IMAGE *Iy, int doweight, int start_ring, int end_ring)
{
  ...
  if (Ix->pixel_format != PFFLOAT || Iy->pixel_format != PFFLOAT)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "LogMapSobel: dst must be PFFLOAT"));
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
int LogMapCurvature(LOGMAP_INFO *lmi, IMAGE *Iin, IMAGE *Igrad, float A, int doweight, int start_ring, int end_ring)
{
    ...
    LOGMAP_INFO *lmi, int filter[NBD_SIZE], IMAGE *Iin, IMAGE *Iout, int doweight, int start_ring, int end_ring)
{
  ...
  if (Iout->pixel_format != Iin->pixel_format)
    ErrorReturn(-1, (ERROR_UNSUPPORTED, "logFilterNbd: input and output format must be the same\n"));
  ...
  case PFINT:
    ...
    if (doweight)
      for_each_ring(lmi, ring, spoke, start_ring, end_ring) * IMAGEIpix(Iout, ring, spoke)
          = nint((float)*IMAGEIpix(Iout, ring, spoke) * LOG_PIX_WEIGHT(lmi, ring, spoke));
    break;
  case PFFLOAT:
    for_each_ring(lmi, ring, spoke, start_ring, end_ring)
    {
    ...
    if (doweight)
      for_each_ring(lmi, ring, spoke, start_ring, end_ring) * IMAGEFpix(Iout, ring, spoke)
          *= LOG_PIX_WEIGHT(lmi, ring, spoke);
    break;
  default:
    ErrorReturn(-2, (ERROR_UNSUPPORTED, "logFilterNbd: unsupported input image format %d\n", Iin->pixel_format));
    break; /* never used */
  }
  return (0);
}
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
double LogMapDiffuse(
    LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Iout, double k, int niterations, int doweight, int which, int time_type)
{
  ...
  {
  case DIFFUSE_PERONA:
    k = LogMapDiffusePerona(lmi, fSrcImage, fOut, k, niterations, doweight, time_type);
    break;
  case DIFFUSE_CURVATURE:
    k = LogMapDiffuseCurvature(lmi, fSrcImage, fOut, k, niterations, doweight, time_type);
    break;
  default:
...
----------------------------------------------------------------------*/
double
LogMapDiffusePerona(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Iout, double k, int niterations, int doweight, int time_type)
{
  int ring, spoke = 0, i, n_ring, n_spoke, ci, j, end_ring, start_ring, new_start_ring, new_end_ring, start_spoke,
            end_spoke;
  // int rows, cols, nspokes;
  float c[NBD_SIZE], fvals[NBD_SIZE], dst_val, rho, time, end_time;
  // float weight;
  FILE *fp;
  ...
  if ((Iout->pixel_format != Isrc->pixel_format) || (Iout->pixel_format != PFFLOAT))
  {
    fprintf(stderr,
  ...
  case DIFFUSION_TIME_PERIPHERAL:
    ...
    end_time = ((float)niterations + 9.9f) / 10.0f * exp(2.0f * lmi->max_rho) * KERNEL_MUL;
    ...
    if (time_type != DIFFUSION_TIME_LOG)
    {
      ...
      if ((Gdiag & DIAG_LOGDIFF) && (start_ring <= end_ring))
      {
        ...
        if (Gdiag & DIAG_WRITE)
        {
          ...
          fprintf(fp, "%d %d %d %d %2.3f %2.3f\n", i, npixels, start_ring, end_ring, stime, etime);
        }
      }
    }
#if 0
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
double LogMapDiffuseCurvature(
    LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Iout, double A, int niterations, int doweight, int time_type)
{
  /*  LOGPIX    *npix ;*/
  ...
  if (Isrc->pixel_format != PFFLOAT)
  {
    fprintf(stderr, "LogMapDiffuseCurvature: unsupported input image format %d\n", Isrc->pixel_format);
    exit(1);
  }

  if (Iout->pixel_format != PFFLOAT)
  {
    fprintf(stderr, "LogMapDiffuseCurvature: unsupported output image format %d\n", Iout->pixel_format);
    exit(1);
  }
...
static void mapCalculateParms(LOGMAP_INFO *lmi)
{
  ...
  if (lmi->nrings == 0)
  {
    ...
    fprintf(
        stderr, "nspokes=%d, setting nrings = %d (%2.3f)\n", lmi->nspokes, lmi->nrings, (float)((logrange * k) / PI));
#endif
  }
}
...
static double diffusionCalculateK(LOGMAP_INFO *lmi, IMAGE *image, double percentile)
{
  int i, ring, spoke, npix, ppix, hist_pix[HIST_BINS], oval, val;
  LOGPIX *opix;
  double hist_vals[HIST_BINS], bin_size, k, rval, lval, deltaW, deltaN, fval, min_val, max_val;
  IMAGE *Itmp;
  ...
  for_each_log_pixel(lmi, ring, spoke)
  {
    ...
    if (crow != UNDEFINED && ccol != UNDEFINED)
    {
      ...
      case PFBYTE:
        ...
        break;
      default:
        ErrorSet(ERROR_UNSUPPORTED, "LogMapPatchHoles: unsupported pixel format %d", Itv->pixel_format);
        return;
      }
      *IMAGEFpix(Iout, ring, spoke) = fval;
    }
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *LogMapNormalize(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, float low, float hi)
{
  if (!Idst)
...
IMAGE *Icalculated = NULL, *ImapOffset = NULL, *ImapOrient = NULL, *Ix = NULL, *Iy = NULL;
IMAGE *LogMapNonlocal(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Ismooth, IMAGE *Idst)
{
  int msec;
  struct timeb then;
  int rows, cols, x, y, ax, ay, sx, sy, x1, y1, dx, dy, odx, ody, d, xn, yn, steps, dir, dot, wsize = 3, maxsteps = 6;
...
/* build array of neighboring pixels - x and y are reused */
        ...
        *IMAGEpix(Iout, ring, spoke) = *IMAGEpix(Ismooth, x1 + dx, y1 + dy);
        ...
        break;
      default:
        ErrorReturn(Idst, (ERROR_UNSUPPORTED, "LogMapNonlocal: unsupported pixel format %d", Iout->pixel_format));
        break;
      }
    }
  }
...
{
  int rows, cols, row, col, npix, ring, spoke, nrings, nspokes, ccol, crow;
  ...
  if (!cmi->pix)
    ErrorReturn(ERROR_NO_MEMORY, (ERROR_NO_MEMORY, "LogMapInitForwardFilter: could not allocate LUT"));
...
IMAGE *LogMapInverseBilinear(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst)
{
  ...
  ErrorReturn(NULL, (ERROR_UNSUPPORTED, "LogMapInverseBilinear: not yet implemented"));
  ...
  case PFBYTE:
    ...
    break;
  default:
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "LogMapInverseBilinear: unsupported dst type %d", Idst->pixel_format));
    break;
  }
...
IMAGE *LogMapInverseFilterGaussian(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst)
{
  ...
  CP *cp;
  CMI *cmi;
  float *dpix, dval, dsq, row_cent, col_cent, dx, dy, weight, total_weight, norm;
  byte *bdpix;
  ...
  case PFBYTE:
    ...
    break;
  default:
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "LogMapInverseFilter: unsupported dst type %d", Idst->pixel_format));
    break;
  }
...
IMAGE *LogMapForwardFilter(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst)
{
  ...
  case PFBYTE:
    ...
    break;
  default:
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "LogMapForwardFilter: unsupported src type %d", Isrc->pixel_format));
    break;
  }
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *LogMapFilter(LOGMAP_INFO *lmi, int which, int wsize, IMAGE *Isrc, IMAGE *Idst)
{
  IMAGE *Iout;

  if (wsize != 3)
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "LogMapFilter: unsupported window size %d", wsize));
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *LogMapOffsetOrientation(LOGMAP_INFO *lmi, int wsize, IMAGE *Isrc, IMAGE *Iorient)
{
  static IMAGE *Itmp;
  IMAGE *Iout, *Ix, *Iy;
  int rows, cols;
...
IMAGE *LogMapOffsetDirection(LOGMAP_INFO *lmi, IMAGE *Iorient, IMAGE *Ioffset)
{
  ...
  float *xpix, *ypix, ldx, ldy, *or_xpix, *or_ypix, *oxpix, *oypix, dir, lox, loy, dot;
  LOGPIX *npix, **nbd;
...
IMAGE *LogMapOffsetDirection(LOGMAP_INFO *lmi, IMAGE *Iorient, IMAGE *Ioffset)
{
  ...
  int x0, y0, rows, cols, x, y, whalf, k, ring, spoke;
  float *xpix, *ypix, ldx, ldy, *or_xpix, *or_ypix, *oxpix, *oypix, dir, lox, loy, nrho, nphi = 0.0f, sin_nphi,
                                                                                         cos_nphi, e_to_the_nrho, dot;
  LOGPIX *npix;
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *LogMapOffsetMagnitude(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, int maxsteps)
{
  int rows, cols, x, y, ax, ay, sx, sy, x1, y1, dx, dy, odx, ody, d, steps, dot = 0, xoff, yoff, k, xold, yold;
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *LogMapApplyOffset(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Ioffset, IMAGE *Idst)
{
...
/*----------------------------------------------------------------------
           ...
           Description:
----------------------------------------------------------------------*/
IMAGE *LogMapMedianFilter(LOGMAP_INFO *lmi, IMAGE *Isrc, int wsize, IMAGE *Ioffset, IMAGE *Idst)
{
#if 1
  int rows, cols;
  IMAGE *Iin, *Iout;
  ...
  int x0, y0, rows, cols, whalf, dx, dy, frame, wsq, median_index, k, nring, nspoke;
  float *sptr, *outPix;
  LOGPIX *npix, **nbd;
#define OVERLAPPING_FAST_MEDIAN 0
  ...
  if (!Ioffset)
  {
    ...
    for (frame = 0; frame < Isrc->num_frame; frame++)
    {
      ...
      if (ecode != HIPS_OK)
        ErrorReturn(NULL, (ecode, "LogMapMedianFilter: h_median failed (%d)", ecode));
...
----------------------------------------------------------------------*/
IMAGE *LogMapGaussianFilter(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Igaussian, IMAGE *Idst)
{
  int rows, cols;
  IMAGE *Iout, *Itmp;
...
static IMAGE *logSobelX(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, int doweight, int start_ring, int end_ring)
{
  ...
  if (Idst->pixel_format != Isrc->pixel_format)
    ErrorReturn(Idst, (ERROR_UNSUPPORTED, "logSobelX: input and output format must be the same\n"));
  ...
  case PFBYTE:
    ...
    if (doweight)
      for_each_ring(lmi, ring, spoke, start_ring, end_ring) * IMAGEFpix(Idst, ring, spoke)
          = nint((float)*IMAGEFpix(Idst, ring, spoke) * LOG_PIX_WEIGHT(lmi, ring, spoke));
    break;
  case PFINT:
    for_each_ring(lmi, ring, spoke, start_ring, end_ring)
    {
    ...
    if (doweight)
      for_each_ring(lmi, ring, spoke, start_ring, end_ring) * IMAGEIpix(Idst, ring, spoke)
          = nint((float)*IMAGEIpix(Idst, ring, spoke) * LOG_PIX_WEIGHT(lmi, ring, spoke));
    break;
  case PFFLOAT:
    for_each_ring(lmi, ring, spoke, start_ring, end_ring)
    {
    ...
    break;
  default:
    ErrorReturn(Idst, (ERROR_UNSUPPORTED, "logSobelX: unsupported input image format %d\n", Isrc->pixel_format));
    break; /* never used */
  }
...
static IMAGE *logSobelY(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, int doweight, int start_ring, int end_ring)
{
  ...
  if (Idst->pixel_format != Isrc->pixel_format)
    ErrorReturn(Idst, (ERROR_UNSUPPORTED, "logsobelY: input and output format must be the same\n"));
  ...
  case PFFLOAT:
    ...
    break;
  default:
    ErrorReturn(Idst, (ERROR_UNSUPPORTED, "logSobelY: unsupported input image format %d\n", Isrc->pixel_format));
    break; /* never used */
  }
...
IMAGE *LogMapAddSaltNoise(LOGMAP_INFO *lmi, IMAGE *Isrc, IMAGE *Idst, float density)
{
  float noise, in;
  byte bin;
  int ring, spoke;
  ...
  if (Isrc->pixel_format != Idst->pixel_format)
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "ImageAddSaltNoise: unsupported output format %d\n", Idst->pixel_format));
  ...
  case PFBYTE:
    ...
    break;
  default:
    ErrorReturn(NULL, (ERROR_UNSUPPORTED, "ImageAddSaltNoise: unsupported input format %d\n", Isrc->pixel_format));
    break;
  }
  return (Idst);
}
