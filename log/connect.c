...
static COBJ_TABLE *cTableAlloc(int max_objects, int max_rows, IMAGE *image, int xo, int yo);
...
----------------------------------------------------------------------*/
COBJ_TABLE *ConnectedComponents(LOGMAP_INFO *lmi, IMAGE *image, int white, int thresh, int xo, int yo)
{
  int ring, nobjects, spoke, objno, freeno, tsign;
  // int nrings, nspokes, color;
  COBJ_TABLE *ctable;
  ...
  if (image->pixel_format != PFINT)
    ErrorExit(ERROR_BADPARM, "ConnectedComponents: bad pixel format %d (must by int)\n", image->pixel_format);
...
/*----------------------------------------------------------------------
           ...
           Description:
             ...
             belonging to 'co2'.
----------------------------------------------------------------------*/
void ConnTableFree(COBJ_TABLE **pctable)
{
  COBJ_TABLE *ctable;
...
static void CobjAdd(COBJ *cobj, int ring, int spoke)
{
  ...
  {
    fprintf(stderr,
            "CobjAdd(%d, %d) --> row out of bounds <%d,%d>\n",
            ring,
            spoke,
            cobj->starty,
            cobj->starty + cobj->nrows - 1);
    return;
  }
  // first =
  rle = cobj->rows[rowno]; /* DEBUG */
  // if (first)
  //   sfirst = *first; /* DEBUG */
...
static void dumpTable(COBJ_TABLE *ctable)
{
  ...
  printf("ctable at %d,%d, with %d objects\n", ctable->xo, ctable->yo, ctable->nobjects);
  for (i = 0; i < ctable->nobjects; i++)
  {
    object = ctable->objects + i;
...
static void mergeObjects(COBJ *src, COBJ *dst)
{
  ...
  if (src->parent)
    src->parent->child = src->child;
  if (src->child)
    src->child->parent = src->parent;
  src->child = src->parent = NULL;
  isFree(src);
}
void ClipCopy(CLIP *cSrc, CLIP *cDst) { *cDst = *cSrc; }
/*----------------------------------------------------------------------
            Parameters:
...
{
  int objno, row, x, endx, weight, total_weight, pix_val, white, amount_on, thresh, all_on, y, xo, yo;
  LOGMAP_INFO *lmi;
  COBJ *cobj;
  RLE *rle;
  LOGPIX *logpix;
  ...
  for (cobj = ctable->objects, objno = 0; objno < ctable->nobjects; objno++, cobj++)
  {
    total_weight = 0;
...
int ConnTableMatch(
    LOGMAP_INFO *lmi, COBJ_TABLE *ct1, COBJ_TABLE *ct2, int *px, int *py, int dx_old, int dy_old, int max_change)
{
  int objno, dx, dy, dist, total_dist, min_error, min_anchor1, min_anchor2, i, c2index, cx2, cy2, npoints, xdiff, ydiff,
      avg_dist, min_area, max_area, error, dxTotal, dyTotal, anchorNo, var_x, var_y, nbad, ring;
  // int  std_dev_x, std_dev_y;
...
*/
  ...
  for (anchorNo = 0; anchorNo < ct1->nobjects; anchorNo++)
  {
    ...
    for (objno = 0; objno < ct2->nobjects; objno++)
    {
      ...
      if ((max_change != NO_CONTINUITY) && ((abs(dx_old - dx) > max_change) || (abs(dy_old - dy) > max_change)))
        continue;
      ...
      */
      ...
      for (i = anchorNo; i < ct1->nobjects; i++)
      {
        ...
        cobj2 = CobjFindMatch(
            lmi, ct2, cobj1->col_cent - dx, cobj1->row_cent - dy, 1, &dist, min_area, max_area, MAX_DIST);
  ...
  for (i = 0; i < ct1->nobjects; i++)
  {
    ...
    min_area = nint((double)cobj1->tv_area * MIN_AREA_RATIO);
    max_area = MIN(nint((double)cobj1->tv_area * MAX_AREA_RATIO), MAX_AREA);
    cobj2 = CobjFindMatch(lmi, ct2, cobj1->col_cent - dx, cobj1->row_cent - dy, 1, &dist, min_area, max_area, MAX_DIST);
    if (cobj2) /* found an object */
    {
      c2index = cobj2 - ct2->objects;
      ct2->mapped[c2index] = i;
...
#define DISCARD_POINTS 0
  ...
  for (i = 0; i < ct2->nobjects; i++)
  {
    if (ct2->mapped[i] >= 0)
    {
      ...
      if ((npoints > MIN_POINTS) && ((xdiff > 2 * std_dev_x) || (ydiff > 2 * std_dev_y)))
      {
        if (Gdiag & DIAG_MATCH)
  ...
  */
  ...
  for (i = 0; i < ct1->nobjects; i++)
  {
    if (ct1->mapped[i] < 0)
    {
      ...
      if ((cx2 >= 0) && (cx2 < lmi->ncols) && (cy2 >= 0) && (cy2 < lmi->nrows))
      {
...
/*----------------------------------------------------------------------
           ...
           Description:
              ...
              mapped field.
----------------------------------------------------------------------*/
COBJ *CobjFindMatch(LOGMAP_INFO *lmi,
                    COBJ_TABLE *ctable,
                    int x,
                    int y,
                    int use_map,
...
#define USE_AREA_IN_MATCH 0
    ...
    if ((dist < min_dist)
        ...
        && (cobj->tv_area >= min_area)
        && (cobj->tv_area <= max_area)
#endif
            )
    {
      min_dist = dist;
...
----------------------------------------------------------------------*/
COBJ_TABLE *ConnTableMerge(COBJ_TABLE *ctSrc, COBJ_TABLE *ctDst, int dx, int dy)
{
  int cSrcIndex, srcRadius, dstRadius, dist, copy, dstRing, srcRing, ringFovea;
  ...
  for (cSrcIndex = 0; cSrcIndex < ctSrc->nobjects; cSrcIndex++)
  {
    ...
    cDstObj = CobjFindMatch(
        ctSrc->lmi, ctDst, cSrcObj->col_cent + dx, cSrcObj->row_cent + dy, 0, &dist, 0, MAX_AREA * 10, srcRadius * 20);
...
{
  int o1, o2, merged, radius1, radius2, dist, xdist, ydist, ring1, ring2, ringFovea, nobjects, freeno, objno;
...
int CobjOverlap(COBJ *cobj1, COBJ_TABLE *ct, int dx, int dy)
{
...
int ClipIntersect(CLIP *c1, CLIP *c2)
{
  ...
  if (((x2start < x1start) || (x2start > x1end)) && ((x1start < x2start) || (x1start > x2end)))
    return (0);

  if (((y2start < y1start) || (y2start > y1end)) && ((y1start < y2start) || (y1start > y2end)))
    return (0);
